...
Plan version: 2.41.0-native-kernel-interrupt-time
Roadmap cycle: PooleOS Cycle 135
...
| Native PKLOAD6 / PBLIVE4 / PKMAP2 / PBEXIT1 integration | Exact live PBC1, PSM1, real 323,584-byte canonical PKELF1, six PBART1, PBTP1, and PBTS1 reads; 99 kernel host tests and 184/184 aggregate Rust tests; 2/2 exact PooleBoot, kernel, twelve-file media, and QEMU runs; 25 markers; 155/155 controls; exact 97-entry final PBP1, 89-page kernel and nine-file retained mapping, 32-page guarded stack, five-page guarded manager plus alternate guarded ledger windows, firmware RSDP record, firmware-boundary agreement, live six-format and PBTRUST1 parsing, and independent oracle agreement | Partial N5.1/N5.4-N5.9 | The default path proves retained pages, successful `ExitBootServices`, unsigned denial, and permanent stop. PKXFER1 separately proves the development transfer; signature trust, authenticated state I/O, capability/action enforcement, final framebuffer remap, production transfer, and N5 exit remain open |
| Native PKENTRY1 PooleKernel | Real 323,584-byte canonical PKELF1 product in a 364,544-byte image; fixed 0x9000 entry; 812 relative relocations; 99 host tests; 2/2 exact clean builds; 43/43 hostile controls; exact Rust/Python loaded bytes; matching manifest/live build ID; canonical SHA-256 `2ACD4A5EF30CA1A4A22711FD31E2A259A5C87D97BCE7FB1BF49A3488B3FC02B2` | Partial N6.4-N6.6 | PKXFER1 through PKIRQ1 separately prove opt-in virtualized entry and bounded BSP trap/CPU/xstate/MSR/PMM/ACPI-container/sparse-direct-map/interrupt-time slices, while PKERR1 freezes a pure exact-target policy. Authentication, production PBP1, final framebuffer remap, complete per-CPU exception/xstate/runtime policy, target firmware, and N6 exit remain open |
| Native PKPMM7 / PKACPI1 physical memory | Exact live 97-entry PBLIVE4/PBP1 intake; stable manager in five guarded pages; external generation-owned active ledgers; checked automatic growth and retirement; required RSDP/XSDT/APIC/FACP/HPET/MCFG validation; retained one-page ACPI snapshot; independent Boot Services and ACPI reclaim receipts; generation-bound sparse direct-map manifest; 117,888 source-usable pages; 129,148 final managed pages; 11,250 Boot Services plus 11 ACPI pages admitted; 856 loader pages protected; 11,473 pages and 46,993,408 bytes scrubbed and verified; 2/2 exact 45-marker QEMU runs; 191/191 hostile controls; 91 kernel host tests | Partial N9.1-N9.2 | One BSP and bounded ledger windows only. No AML, complete ACPI resource graph, interrupt-context/concurrent/SMP allocation, heaps, general MMIO/cache policy, general pressure/OOM, target, or N9 exit |
| Native PKVM3 virtual memory | PMM-derived generation-bound sparse write-back direct map with 243 contiguous DMA32 table pages, 117,887 mapped pages across eleven coalesced ranges, and 12,878 unmapped gap pages; exact inherited kernel/entry/32-page guarded-stack/handoff mappings; retained and release-excluded ownership omitted; transactional candidate CR3 activation and exact restoration; PWT/PCD alias rejection; three local invalidation receipts; exact generation-retirement receipt; 2/2 exact 40-marker QEMU runs; 46/46 hostile controls; 367,474 physical table writes; 950,234 bootstrap PTE writes and invalidations | Partial N9.3-N9.4 | One BSP only; remote shootdown is deliberately rejected, so AP startup, SMP generation retirement, incremental concurrent replacement, huge pages, PCID, COW, user faults, pager, heap, broad MMIO/PAT/MTRR qualification, target, N9 exit, and production remain open |
| Native PKREVAL1 PooleKernel revalidation | Allocation-free `no_std` verifier over exact retained PSM1, six PBART1 files, PBTP1, and PBTS1; 70/70 Rust tests; 8/8 Python tests; both target builds; 36/36 hostile controls; 32,768/32,768 role-complete mutation rejects; exact Rust/Python denial receipt; zero authority, actions, and state writes | Partial N5.8/N6.4-N6.6 | The standalone receipt makes no live-entry claim; PKXFER1 separately proves live execution only for the unsigned QEMU development envelope. Cryptographic trust, persistent state selection, capabilities, actions, writes, N5/N6 exit, and production remain open |
...
| Test suite | Cycle 135: 796 Python tests pass with one Windows symlink-permission skip; 99 Rust kernel host tests include PKIRQ1 MADT/HPET/vector/time arithmetic and rollback controls plus retained PMM/PKVM3 coverage | Partial N36 | Predominantly host/reference, bounded-model, emulator, and artifact tests; no authenticated authority, target-firmware, or physical-hardware result exists |
| Release gate | Cycle 135: 94/94 canonical consistency checks over 89 artifacts; Doctor 596/596; 20 native gaps | Partial N37 | `production_ready=false`; not a release acceptance gate |
...
| Native kernel | Real reproducible 89-page PooleKernel PKELF1 source and image with host-qualified PKREVAL1 and opt-in two-run QEMU live-entry/revalidation, trap, CPU, xstate, MSR, PKPMM7/PKACPI1 physical-memory, PKVM3 virtual-memory, and PKIRQ1 interrupt/time traces | Partial N6.4-N8.3/N9.1-N9.4 | No authenticated production entry, complete per-CPU runtime, AP startup, I/O APIC/MSI routing, AML/complete platform graph, ring 3, IPC, capability, or driver-domain execution |
...
| `FLAG-N8-IRQ-001` | REQUIRED | Partial evidence; open in Cycle 135 | PKIRQ1 closes the bounded one-BSP retained-table, local-xAPIC, vector-ledger, guarded-MMIO, calibrated-HPET/local-timer, exact-delivery/EOI, and normal-rollback slice. First-AP startup, per-CPU state, I/O APIC/MSI, IPIs, complete time services, panic-path rollback, target qualification, and real SMP shootdown remain required before closure |
...
Cycle 135 completes only the bounded one-BSP interrupt/time portion of `FLAG-N8-IRQ-001`. PKIRQ1 parses retained MADT/HPET state, reserves 51 vectors, uses guarded uncacheable LAPIC/HPET mappings, masks/restores the legacy PIC, calibrates one-shot local-APIC time against HPET, and records exactly eight deliveries and eight EOIs with zero error/spurious/ISR residue. Ninety-nine kernel host tests, two exact 36-marker qemu64 runs, and 58 hostile controls pass with exact normal-path restoration and zero AP starts, signatures, authority, actions, or production claims. The canonical kernel is 323,584 bytes in a 364,544-byte, 89-page image with 812 relocations and SHA-256 `2ACD4A5EF30CA1A4A22711FD31E2A259A5C87D97BCE7FB1BF49A3488B3FC02B2`. N8.1 and N8.3 are partial; I/O APIC/MSI routing, complete time services, panic-path rollback, first-AP startup and per-CPU state, IPIs, SMP shootdown, target qualification, and N8 exit remain open. N9.1-N9.4 remain partial and PKVM3 continues to reject multi-processor retirement. `FLAG-N7-ERRATA-SOURCE-001` and `FLAG-N7-MICROCODE-FLOOR-001` remain stop-ship. The selected `hardware_fido2_ed25519_sk` device remains physically unavailable, so `N0-HW-KEY-ACQUIRE-001` is still the immediate blocked external move. The owner's broad operation-category authorization remains recorded, but no key, signature, public-key publication, secret use, privileged host probe, driver load, firmware change, physical-media write, tag/release publication, or production promotion occurred in Cycle 135. The next owner-independent engineering move is `N8-SMP-FIRST-AP-001`; PooleGlyph Phase 66 may advance in parallel without outranking N0-N9.
...
| 41 | `N8-SMP-FIRST-AP-001` | Next owner-independent move: derive guarded per-CPU state, install AP-local descriptor/xstate/interrupt state, start exactly one AP through a bounded trampoline/mailbox transaction, prove timeout and rollback, and leave zero unacknowledged interrupts or stale mappings without claiming complete SMP or N8 exit |
| 42 | `N12-SCHED-001` | Neutral scheduler and context switch under SMP stress |
| 43 | `N13-RING3-001` | First user task, syscall, capability object, exception, and clean exit |
| 44 | `N14-IPC-001` | Capability-mediated call/reply, cancellation, quota, and hostile message tests |
| 45 | `N16-VIRTIO-001` | Isolated virtio console/RNG driver domain with revocation and restart |
