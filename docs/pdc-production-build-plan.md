...
Plan version: 2.5.0-native-boot-handoff
Roadmap cycle: PooleOS Cycle 98
...
| Test suite | Cycle 98: 531 tests pass with one Windows symlink-permission skip, including PBP1 layout/golden/version/staleness/claim controls | Partial N36 | Predominantly host/reference, bounded-model, emulator, and artifact tests; no PooleKernel implementation trace exists |
| Release gate | Cycle 98: 71/71 consistency checks over 66 artifacts; 20 native gaps | Partial N37 | `production_ready=false`; not a release acceptance gate |
...
Cycle 98 control-plane validation passes 531 tests with one Windows symlink-permission skip, Doctor passes its complete check set against live PooleGlyph Phase 65, and the complete consistency release gate passes 71/71 checks over 66 artifacts while retaining 20 explicit gaps and `production_ready=false`. Exact public-index, release-gate, and handoff hashes are recorded in the Cycle 98 log after staging.
...
Cycle 98 completes the bounded `N5-BOOTPROTO-001` qualification and closes only `FLAG-N5-BOOTPROTO-001`; N5.8 remains partial until live firmware records are normalized, boot services exit successfully, and PooleKernel consumes the immutable handoff. `FLAG-NATIVE-BOOT-001` remains STOP_SHIP because PooleBoot still lacks its boot grammar, signed-artifact and ELF loading, PBP1 producer integration, `ExitBootServices` transfer, hostile loader corpus, second-host reproduction, target-firmware proof, and physical-media qualification. The selected `hardware_fido2_ed25519_sk` device remains unavailable, so `N0-HW-KEY-ACQUIRE-001` remains the immediate blocked owner move and `FLAG-N0-GOVERNANCE-KEY-001` stays open. No key generation, private-key use, signing, merge, tag, public-key registration, or publication is authorized. `FLAG-N4-MODELS-001` also remains open because all six native implementation-trace cross-checks and separately scoped liveness/refinement/conformance work are unfinished. The next owner-independent engineering move is `N5-BOOTCFG-001`, while PooleGlyph Phase 66 may advance in parallel without outranking N0-N5.
