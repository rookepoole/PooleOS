...
Plan version: 2.19.0-native-policy-semantics
Roadmap cycle: PooleOS Cycle 113
...
Cycle 80 reset architecture claims without discarding validated component evidence. Cycles 81-96 established the native constitution, source boundary, pinned one-host toolchain, owner-controlled governance state, hardware and Tier 0 profiles, bounded formal models, and the PooleGlyph co-development lane. Cycles 97-107 qualified the bounded native boot path through exact profile retention and the permanent pre-transfer stop. Cycles 108-112 froze PINIT1, PREC1, PSYM1, synthetic-only PMCU1, and synthetic-only PFWM1 as the first five independently validated inner formats. Cycle 113 freezes PPOL1 as the sixth: a deterministic qualification-only 1,984-byte role-7 policy with six exact boot modes, eleven PINIT1-cross-bound capability rules, default-deny authority intersection, parent-monotonic attenuation, immutable safe/recovery floors, firmware physical-presence and separate-authority requirements, and durable decision-receipt validation. Independent Python and allocation-free `no_std` Rust implementations agree across three golden bundles, 116 controls, 8,192 parser cases, 4,096 cross-binding cases, 12,288 activation cases, and 8,192 receipt cases. PKLOAD6 validates exact PINIT1, PREC1, PSYM1, PMCU1, PFWM1, and PPOL1 bytes through prohibited-from-production host oracles and raises the aggregate corpus to 130 controls. This remains format and dry-run policy-model evidence only: no target interpreter, signature or persistent-state verification, authority creation, policy application, PooleGlyph executable authority, state mutation, or physical-media write exists.
...
| Native PooleBoot proof | `POOLEOS-N5-POOLEBOOT-7`; 8/8 host tests; 2/2 exact 113,664-byte PE32+ builds; 2/2 exact ten-file GPT/FAT32 images; 2/2 pinned OVMF runs; twenty-three ordered markers; 2/2 dual-channel and GOP-frame matches; 130/130 hostile controls | Partial N5.1-N5.9 | Unsigned seven-artifact load, retained PKMAP2/PBLIVE3 storage, host-only PINIT1/PREC1/PSYM1/PMCU1/PFWM1/PPOL1 validation, successful PBEXIT1, and permanent stop before transfer; no signature trust, PooleBoot inner enforcement, PooleKernel activation/recovery/symbol/policy/update application, authenticated state persistence, kernel-entry profile, final CR3/RSP, target firmware, physical media, N5 exit, or production claim |
...
| Native PBART1 profile artifacts | Fixed 96-byte envelope; exact roles 2-7; independent Rust/Python validation; six whole-file and payload digest bindings; distinct zero-padded loader pages; final-map and PBP1 retention | Partial N5.6 | Envelopes are unsigned; PINIT1, PREC1, PSYM1, PMCU1, PFWM1, and PPOL1 have frozen host-validated inner formats, but no payload is interpreted or applied by PooleBoot or PooleKernel |
| Native PINIT1 initial-system bundle | Canonical 1,764-byte typed declaration bundle; 3 components, 3 services, 3 dependencies, 4 resources, 11 attenuated routes; 3/3 Rust tests; 2/2 `no_std` targets; 3/3 vectors; 120/120 controls; 16,384 Rust/Python differential cases with zero mismatches | Partial N5.6/N19/N21 | Host/reference parsing only; declarations confer no authority, unsigned activation is denied, and PooleBoot/PooleKernel parsing, loading, capability creation, process launch, lifecycle execution, and rollback remain open |
...
| Native PKLOAD6 / PBLIVE3 / PKMAP2 / PBEXIT1 integration | Exact live PBC1, PSM1, real 139,264-byte PKELF1, and six PBART1 reads; 76/76 Rust host tests; 2/2 exact PooleBoot, kernel, ten-file media, and QEMU runs; 23 markers; 130/130 controls; exact final PBP1, retained mapping, firmware-boundary agreement, and host-only PINIT1/PREC1/PSYM1/PMCU1/PFWM1/PPOL1 oracle agreement | Partial N5.1/N5.4-N5.9 | Kernel/artifact/table/stack/handoff pages survive successful `ExitBootServices`; no signature trust, PooleBoot inner enforcement or state I/O, kernel capability creation/activation/recovery/symbol/policy/microcode/firmware application, final CR3/RSP, transferable handoff, kernel entry, final cache policy, or N5 exit |
...
| Test suite | Cycle 113: 691 tests pass with one Windows symlink-permission skip, including PPOL1 mode/precedence/default-deny/attenuation/cross-binding/receipt controls, cross-language differential agreement, integrated-oracle, claim-boundary, and Doctor external-report nonmutation controls | Partial N36 | Predominantly host/reference, bounded-model, emulator, and artifact tests; no live policy enforcement, capability creation, privileged probe, or PooleKernel entry trace exists |
| Release gate | Cycle 113: 82/82 consistency checks over 77 artifacts; 20 native gaps | Partial N37 | `production_ready=false`; not a release acceptance gate |
...
Inherited sections: `013-015`. Added: `ADD-BOOT-001`, `ADD-BOOT-004`, `ADD-BOOT-005`, `ADD-BOOT-006`, `ADD-BOOT-007`, `ADD-BOOT-008`.
...
N5.1-N5.9 are partial. PooleBoot now performs bounded live PBC1/PSM1/PKELF1/PBART1 reads, unsigned digest and envelope binding, transactional firmware-page loading, retained PKMAP2 construction, final PBLIVE3 production, bounded `ExitBootServices`, and firmware-free stop before transfer. Host/reference oracles independently validate PINIT1, PREC1, PSYM1, synthetic-only PMCU1, synthetic-only PFWM1, and qualification-only PPOL1, but PooleBoot does not enforce any inner contract, persist authenticated state, consume symbols, apply policy, observe privileged revisions or live firmware inventory, or apply microcode or firmware. PooleKernel does not create capabilities or execute lifecycle/recovery/policy actions. The path still does not authenticate a signer, enforce authenticated rollback, validate a real vendor payload or redistribution right, establish the final active CR3/RSP or framebuffer cache policy, produce a kernel-entry-valid PBP1, transfer to or execute PooleKernel, enforce Secure Boot, measure through TCG2, use TPM state, test target firmware, write physical media, build an ISO, or satisfy the N5 exit gate. The static identity is not the final animated PooleGlass boot experience or an accessibility result.
...
| `FLAG-N5-INNER-ENFORCEMENT-001` | REQUIRED | Open in Cycle 113 | Integrate all six validators into live PooleBoot, authenticate and persist their required state, bind decisions to retained bytes, and make PooleKernel create only attenuated capabilities and execute only authorized lifecycle, recovery, diagnostic, policy, microcode, and firmware actions with durable audit and rollback evidence |
...
Cycle 113 closes `FLAG-N5-POLICY-BUNDLE-001` and the all-formats `FLAG-N5-INIT-SEMANTICS-001`: PPOL1 exact modes, precedence, PINIT1 route identity, attenuation, safe/recovery floors, firmware authority separation, activation prerequisites, and durable receipt rules are frozen and reproducible, completing independent semantics for all six inner formats. The canonical policy is synthetic qualification data, creates no authority, and denies development activation at the outer-signature gate. N5 remains partial: PSM1 and all seven artifacts are unsigned and untrusted, PBDIGEST1 review and promotion remain open, no real vendor payload or redistribution right is validated, no privileged per-processor revision or live firmware inventory is observed, PooleBoot does not enforce PINIT1, PREC1, PSYM1, PMCU1, PFWM1, or PPOL1, and PooleKernel does not create capabilities, execute lifecycle or recovery, consume symbols, apply policy, update a CPU, or apply firmware. Authenticated persistent state I/O is absent and retained CR3/RSP are never installed. N6.4-N6.6 remain partial because no PooleKernel instruction executes under the live caller. `FLAG-N5-INNER-ENFORCEMENT-001`, `FLAG-N6-BOOT-DIGEST-001`, `FLAG-N6-FRAMEBUFFER-MAP-001`, and `FLAG-NATIVE-BOOT-001` remain open until enforcement, trust, final transfer state, kernel consumption, and target profiles are proved. The selected `hardware_fido2_ed25519_sk` device remains unavailable, so `N0-HW-KEY-ACQUIRE-001` remains the immediate blocked owner move and `FLAG-N0-GOVERNANCE-KEY-001` stays open. Codex has no authorization to generate or use keys, sign, merge, tag, register a public key, or publish a release. `FLAG-N4-MODELS-001` also remains open. The next owner-independent engineering move is `N5-INNER-ENFORCEMENT-001`: integrate the six frozen validators into live PooleBoot, authenticate and persist their required state, bind all decisions to exact retained bytes, and expose only authorized attenuated inputs to PooleKernel without yet claiming full kernel transfer or production trust. PooleGlyph Phase 66 may advance in parallel without outranking N0-N6.
...
| 19 | `N5-INNER-ENFORCEMENT-001` | Next: integrate all six frozen parsers into live PooleBoot, authenticate required state, enforce cross-bindings and fail-closed decisions before exit, and pass only exact authorized inputs toward PooleKernel without claiming capability creation or execution until separately observed |
| 20 | `N5-TRANSFER-001` | After artifact-enforcement and trust prerequisites: validate kernel-entry PBP1, install retained CR3/RSP with interrupts disabled, call the frozen entry ABI, and prove kernel consumption or fail-closed halt |
| 21 | `N7-TRAP-001` | GDT/TSS/IDT and deliberate breakpoint/page-fault evidence after bounded transfer exists |
| 22 | `N9-PMM-001` | Normalized map, bootstrap allocator, physical allocator, guarded mappings |
| 23 | `N8-IRQ-001` | Local APIC, timer, monotonic clock, and first SMP application processor |
| 24 | `N12-SCHED-001` | Neutral scheduler and context switch under SMP stress |
| 25 | `N13-RING3-001` | First user task, syscall, capability object, exception, and clean exit |
| 26 | `N14-IPC-001` | Capability-mediated call/reply, cancellation, quota, and hostile message tests |
| 27 | `N16-VIRTIO-001` | Isolated virtio console/RNG driver domain with revocation and restart |
