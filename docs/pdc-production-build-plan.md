...
Plan version: 2.27.0-native-kernel-errata-policy
Roadmap cycle: PooleOS Cycle 121
...
| Native PKCPU1 read-only CPU policy | Required CPUID identity/features/topology/address-widths plus CR0/CR4/EFER, XCR0, APIC/PAT/MTRR observation; 24/24 kernel tests; 2 exact qemu64 QEMU/OVMF runs; 35 markers; 41/41 controls; exact Rust/Python agreement; 5 MSR reads; zero MSR/control writes, authority, or actions | Partial N7.1/N7.3 | BSP-only qemu64 development evidence. Exact Tier 1 family/stepping, errata/microcode policy, AP-local state, syscall/GS/TSC_AUX/MCE/performance MSRs, xstate ownership, target hardware, N7 exit, and production remain open |
...
| Test suite | Cycle 121: 755 tests pass with one Windows symlink-permission skip, including PKERR1 source-applicability, cross-language, and zero-effect controls plus all PKCPU1, PKTRAP1, PKXFER1, PKREVAL1, and producer controls | Partial N36 | Predominantly host/reference, bounded-model, emulator, and artifact tests; no authenticated authority, qualified privileged mechanism, target-firmware, or physical-hardware result exists |
| Release gate | Cycle 121: 88/88 consistency checks over 83 artifacts; 20 native gaps | Partial N37 | `production_ready=false`; not a release acceptance gate |
...
- live PooleBoot reads exact `TRUST.PBT` and `TRUSTST.PBS` development candidates from the twelve-file medium, cross-binds fourteen facts, emits one `TRUST_STATE DENY` marker, grants zero authority, and writes zero state. The Cycle 120-refreshed PKLOAD6 passes 109/109 Rust tests, 25 markers, and the current integrated hostile corpus across two exact QEMU/OVMF runs;
...
- 24/24 Rust tests, 8/8 Python tests, both `x86_64-unknown-none` and `x86_64-unknown-uefi` builds, 36/36 hostile controls, and 32,768/32,768 deterministic mutation rejects pass. After the Cycle 120 PSYM1 identity refresh, the golden nine-file retained-set SHA-256 is `C46B9030922FB095F5321CD62CD4930B812B2D8D8EA58B93B7C51956B6EEC8B8`;
- the expanded verifier requires a 262,144-byte PooleKernel image with a 180,224-byte canonical file, entry offset `0x8000`, and 333 relative relocations. PKMAP2 now covers 64 kernel pages with 12 read-only, 32 executable, and 20 read-write leaves, then places low guard/stack/high guard at pages 64/65-72/73 and the handoff allocation at page 80;
- PKLOAD6 passes 109/109 Rust tests, two exact PooleBoot and PooleKernel builds, two exact media generations, two QEMU/OVMF producer runs, 25 markers, 155/155 hostile controls, and two independent 5,008-byte PBP1 reconstructions with 96 memory descriptors and ten artifact descriptors;
...
- two exact kernel builds, two feature-enabled PooleBoot builds, one default isolation build, two byte-identical media generations, and two fresh-vars QEMU/OVMF runs reproduce all 30 markers, exact PBP1 bytes, the 180,224-byte PooleKernel SHA-256 `21DCC577077DEC8CF26764CE83BAAB89DFD43B750DA75F19E12E8F3A58E13B40`, and exact guest/host denial. All 58 hostile controls pass;
...
- the Cycle 120-refreshed `runs/native_kernel_entry_readiness.json` records 24/24 host tests, two byte-identical clean linked and canonical builds, 43/43 hostile controls, 333 relative relocations, exact independent Rust/Python loaded bytes, and canonical SHA-256 `21DCC577077DEC8CF26764CE83BAAB89DFD43B750DA75F19E12E8F3A58E13B40`;
...
- N6.4-N6.6 are only partial. PKXFER1 proves opt-in QEMU CR3/RSP installation, live COM1/debugcon/framebuffer diagnostics, and terminal unsigned denial; PKTRAP1 separately proves only a bounded BSP descriptor/exception slice; PKCPU1 proves only a bounded qemu64 read-only CPU-policy slice; and PKERR1 freezes only a pure exact-target rejection policy. KASLR, final debug/unwind policy, framebuffer remap/revocation, direct target errata and microcode-floor authority, xstate ownership, complete per-CPU GDT/IDT/TSS, early self-tests, all exception/NMI/machine-check paths, retained next-boot panic state, authenticated production transfer, target firmware, and boot trust remain open.
...
Inherited sections: `020-022`. Added: `ADD-N7-ERRATA-SOURCE-001`.
...
- 24/24 kernel host tests and 41/41 contract, parser, marker, feature, control-state, MSR, topology, address-width, no-write, no-authority, stale-binding, and profile-isolation controls pass with exact Rust/Python agreement;
...
Cycle 121 closes only `FLAG-N7-ERRATA-POLICY-001`. PKERR1 now freezes the exact Ryzen 7 9800X3D identity, mandatory features, board-lineage BIOS floors, AMD bulletin AGESA floor, homogeneous microcode-evidence shape, direct-source applicability, and RDSEED policy. Independent `no_std` Rust and Python evaluators pass 128 vectors and 24 hostile controls, while the current target remains denied for six exact reasons with zero privileged reads or effects. `FLAG-N7-ERRATA-SOURCE-001` and `FLAG-N7-MICROCODE-FLOOR-001` stay open stop-ship flags; revision guide 58251 is not applicable to Model 44h, and Windows revision `0x0B404023` is not a vendor floor. N7.2 and N15.1 are partial, not complete. N7.4 xstate ownership is still unstarted, and N7.5-N7.6 still lack per-CPU and guarded stacks, complete vectors, user transitions, asynchronous state, NMI/machine check, recursion recovery, retained crash evidence, and target qualification. The selected `hardware_fido2_ed25519_sk` device remains physically unavailable, so `N0-HW-KEY-ACQUIRE-001` is still the immediate blocked external move. No key, signing, privileged or mutating operation, publication, release, or promotion occurred in Cycle 121. The next owner-independent engineering move is `N7-XSTATE-POLICY-001`: freeze x87/SSE/XSAVE ownership, XCR0/XSS dependencies, per-thread image bounds, initialization, exception, context-switch, sensitive-state clearing, and kernel-SIMD rules without claiming scheduler or SMP integration. PooleGlyph Phase 66 may advance in parallel without outranking N0-N7.
...
| 27 | `N7-XSTATE-POLICY-001` | Next owner-independent move: freeze x87/SSE/XSAVE/XCR0/XSS ownership, bounded per-thread images, initialization and exception rules, sensitive-state clearing, context-switch preconditions, and kernel-SIMD restrictions without scheduler/SMP promotion |
| 28 | `N9-PMM-001` | Normalized map, bootstrap allocator, physical allocator, guarded mappings |
| 29 | `N8-IRQ-001` | Local APIC, timer, monotonic clock, and first SMP application processor |
| 30 | `N12-SCHED-001` | Neutral scheduler and context switch under SMP stress |
| 31 | `N13-RING3-001` | First user task, syscall, capability object, exception, and clean exit |
| 32 | `N14-IPC-001` | Capability-mediated call/reply, cancellation, quota, and hostile message tests |
| 33 | `N16-VIRTIO-001` | Isolated virtio console/RNG driver domain with revocation and restart |
