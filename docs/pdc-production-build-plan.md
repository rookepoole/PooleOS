...
The selected move starts at N9-PMM-ACPI-CONSUMER-001 (N9.1-N9.2), continues
through N9.3-N9.5, N8.1/N8.3/N8.5-N8.6 and the existing N12 prerequisite
profiles, and closes this replay before the next N12.3 implementation.
The scoped date repair is tracked under ADD-N36-RECEIPT-COVERAGE-001 and
FLAG-N36-RECEIPT-COVERAGE-001; no new requirement or waiver is introduced.
The measured gate pins now match 219 host tests, 1,305 relocations and kernel
SHA-256 `18EDADA10E141DBADA8C95C1C0B3454696122C5E96C528F45E0AECE6ADD2F07D`.
The stale-pin precheck passed only 15/26 selected checks; all 26 pass after
updating the exact measured bindings. Fourteen forced production-overclaim
cases still reject with the outer schema loader withheld.

[Cycle 161 checkpoint](checkpoints/cycle161-qualified-checkpoints.md) records
the evidence and merge boundary. Private logs, toolchains, signing material
and ISO build products remain outside the public source checkpoint.

Historical Cycle 160 completes the N7-TRAP-001 dependency replay at N7.5/N7.6, with
...
retire-before-reclaim ordering, exact-once payload transfer, nonwrapping
generation limits, pressure retention and shutdown sealing. Nineteen tests
pass in debug and optimized host profiles, one compile-fail borrow test and
206 existing kernel regressions pass, and host/freestanding Clippy passes.
Rebuilding the canonical linked kernel reproduces the Cycle 149 bytes exactly;
the new core is not wired into a live selector. The next step remains
`N12-CONCURRENCY-RECLAMATION-001`: scheduler/address-space lifecycle binding,
acknowledged cross-CPU quiescence and live failure/rollback evidence. N12.3,
...
donation, FIFO notification, writer-preferred reader-writer lock, seqlock,
try/timed paths, five lock ranks with cycle rejection, ownership and recursion
checks, owner-death teardown, and exact failure rollback. Nine exact host
receipts include 8,192 FIFO ticket acquisitions and mutex, notification,
reader-writer, and seqlock contention. Two exact 35-marker four-vCPU
QEMU/OVMF runs exercise one shared live ticket lock across the BSP and three
APs, then revoke all three shared aliases before the existing shootdown,
park, scrub, and release path. Thirty hostile-control categories covering 103
mutations pass. N12.2 is complete for this frozen PKLOCK1 scope only. Deferred
reclamation and ABA-safe object lifetime, general SMP, target qualification,
N12 exit, and production remain open. The next chronological owner-independent
move is `N12-CONCURRENCY-RECLAMATION-001`.

The Cycle 149 image uses a 585,728-byte, 143-page memory envelope. PKENTRY1
records a 513,680-byte canonical kernel, 1,295 relocations, entry `0xA000`,
text end `0x70000`, RELRO end and data start `0x7D000`, and SHA-256
`9029AEE51A4D557EF5B29945985E4A1F07C67DDE9C8C367C80BD1B9EDD9D409E`.
Every dependent native profile from PooleBoot and PKLOAD6 through PMM/VM,
PKSMP5, PKSCHED1-PKSCHED6, PKATOM1, and PKLOCK1 remains bounded by the current
entry contract. Historical receipts retain their original bytes and claims.
...
Cycle 126 adds PKVM1 as selector `9`, an opt-in one-BSP qemu64 continuation of PKPMM1. The fixed-capacity `no_std` core freezes initial 48-bit kernel/user/direct/temporary/image bands; allocates four contiguous generation-bound DMA32 table pages and one data frame; materializes an inactive four-level 4 KiB hierarchy; and proves bounded map, protect, begin-unmap, inactive acknowledgement, completion, W^X rejection, mixed-cache-alias rejection, exact leaf rollback, and deferred frame reuse. The first live attempt failed closed because newly allocated conventional pages are not inherited identity mappings. The corrected privileged adapter validates live BSP CPUID leaf `0x80000008`, exclusively installs supervisor RW/NX mappings in previously absent PKMAP2 leaf 336, performs `INVLPG` after every active-leaf transition, verifies each translation, and revokes the final alias before freeing tables. Two exact fresh-vars QEMU/OVMF runs emit 40 markers and pass 39 hostile controls with 4,104 inactive-table writes, 40 bootstrap-leaf writes, 40 local invalidations, zero PKVM1 CR3 writes, zero shootdowns, exact release of all five pages, and zero signatures, authority grants, or actions. This closes only `FLAG-N9-VM-FOUNDATION-001`; N9.3 and N9.4 become partial. Kernel-complete active roots, direct-map policy, active-address-space invalidation and SMP shootdown, huge pages, PCID, COW, user faults, stack growth, pager IPC, heaps/object caches, MMIO/cache qualification, pressure/OOM behavior, target hardware, and N9 exit remain open.

Cycle 127 adds PKVM2 as selector `10`, a mutually exclusive one-BSP qemu64 continuation of the PKVM1 foundation. The fixed-capacity `no_std` core allocates eight contiguous generation-bound DMA32 table pages plus one data frame, copies and audits the exact current kernel mappings, preserves the RX entry, fourteen-page guarded stack, and read-only/NX handoff, and builds fresh user and direct-map branches. Its bounded direct map covers exactly those nine owned pages as supervisor RW/NX 4 KiB leaves. Activation requires BSP 0, IF clear, and the exact expected original root; it writes and reads back candidate CR3, restores the exact original root on both success and injected readback failure, and host-tests active-leaf write rollback. The live path handles architectural Accessed/Dirty updates, writes and reads probe byte `0xA5`, binds protect, user unmap, and direct unmap to three root/generation/address/CPU/sequence receipts, rejects premature reuse, scrubs and frees the data page, then scrubs and frees all table pages after restoration and bootstrap-alias revocation. Two exact fresh-vars QEMU/OVMF runs emit 40 markers and pass 46 hostile controls with 8,720 physical table writes, 5,336 bootstrap temporary-PTE writes and invalidations, two CR3 writes, three active local invalidations, zero shootdowns, and zero signatures, authority grants, or actions. This closes only `FLAG-N9-VM-ACTIVE-001`; N9.1-N9.4 remain partial. Page scrubbing policy, held-class reclaim, a complete generation-owned direct map, SMP shootdown and deferred reclaim, huge pages, PCID, COW, user faults, pager IPC, heaps/object caches, MMIO/cache qualification, pressure/OOM behavior, target hardware, and N9 exit remain open.

Cycle 128 upgrades selector `8` from PKPMM1 to PKPMM2 and closes `FLAG-N9-PMM-SCRUB-001`. Allocation is planned before ownership commit, then every 64-bit word in the candidate range is zeroed and read back through one temporary supervisor RW/NX alias. Release first validates the exact generation-bound handle and preflights extent insertion; ownership remains live until the same full scrub verifies. Successful transactions emit immutable sequence/operation/range/generation/owner/byte-count receipts. Host fault injection proves partial allocation scrub and release-verification failures preserve ownership, while exact reuse proves a nonzero stale pattern is absent. Two fresh-vars qemu64 QEMU/OVMF runs reproduce 40 markers and pass 63 hostile controls plus 70 kernel host tests. They scrub and verify eight pages and 32,768 bytes, emit four receipts, perform 5,120 physical word writes, 6,144 reads, 28 temporary PTE writes and invalidations, revoke the alias, and end with zero allocated pages, reclaim operations, signatures, authority grants, actions, or production claims. The larger code path rebinds the canonical kernel to 229,376 file bytes, 270,336 image bytes, 66 pages, 542 relocations, and SHA-256 `06D36FAF5531509097CD03819390AC744DCFD2BD9FB64D1E6F6710DD7796776E`; all dependent PKENTRY1 through PKVM2 identities are regenerated, including PKVM2's 5,368 bootstrap temporary-PTE writes and invalidations. A durable mapped allocator-metadata arena, held-class reclaim, complete mapping and SMP TLB policy, heaps, MMIO/cache qualification, concurrency, pressure/OOM policy, target hardware, and N9 exit remain open.

Cycle 129 upgrades selector `8` to PKPMM3 and closes `FLAG-N9-PMM-METADATA-001`. The complete 14,616-byte physical-memory manager moves from bootstrap stack storage into five contiguous DMA32 pages reserved before handoff, scrubbed through the privileged adapter, persistently mapped supervisor RW/NX between absent low/high guards, generation/owner bound, and protected by a logical FNV-1a integrity seal. Migration copies all 97 source records, 11 free extents, the live metadata allocation record, and its scrub receipt; stack bootstrap storage is retired only after exact mapped validation. Mapping and handoff faults roll back all pages and leaves, mapped corruption is rejected before the next public mutation, and every release path excludes metadata. Two exact fresh-vars qemu64 runs emit 41 markers and pass 87 hostile controls plus 74 kernel host tests. They manage 117,918 of 117,919 source-usable pages, protect 825 loader pages, scrub and verify 13 pages and 53,248 bytes, perform 7,680 physical word writes, 8,704 reads, and 43 temporary-PTE writes and invalidations, retain five metadata leaves plus two guards, and finish with exact handoff/final integrity and zero reclaim, signatures, authority, actions, or production claims. The canonical kernel is rebound to 245,760 bytes in a 286,720-byte, 70-page image with 569 relocations and SHA-256 `F81F4B21F67A4A490AAE049C92B0E890A1D5B286DC708E41557DFCEBB37282DC`; PKVM2 correspondingly records 5,432 bootstrap writes and invalidations. Held-class reclaim, scalable metadata growth, concurrent/SMP allocation, complete direct mapping and shootdown policy, heaps, MMIO/cache qualification, pressure/OOM policy, target hardware, and N9 exit remain open.

Cycle 130 upgrades selector `8` to PKPMM4 and closes `FLAG-N9-PMM-RECLAIM-001`. A monotonic lifecycle admits Boot Services only after `PostExitBootServices` and reserves ACPI admission for `AcpiTablesReleased`. Reclaim streams the immutable PBP1 source ledger through complete capacity/arithmetic/retained-overlap preflight, full zero/readback, and infallible metadata commit; no full extent-array plan is held on the bootstrap stack. Seventy source records coalesce into twelve admitted ranges and 11,250 pages, raising managed pages from 117,918 to 129,168 while eleven ACPI pages remain held. Exact repeat calls return the same immutable receipt without physical access. Content faults can leave held bytes partially scrubbed but preserve ownership, free/managed ledgers, receipts, and sequence. Two exact fresh-vars qemu64 runs emit 42 markers and pass 109 hostile controls plus 79 kernel host tests. They scrub and verify 11,263 pages and 46,133,248 bytes, perform 5,767,680 physical word writes, 5,768,704 reads, and 22,543 temporary-PTE writes and invalidations; range checksum `0x5A485D4A5725EED8` and receipt checksum `0x4D3EBF743B7F2CCC` bind the admission. The canonical kernel remains 245,760 bytes in a 286,720-byte, 70-page image with 593 relocations and SHA-256 `20BCE1C7501BC2344A6D7505BCDA749D9B2738A8435255D0EC2EF5A3E177CC4C`. Scalable metadata growth, complete ACPI consumer integration, concurrency/SMP, complete direct mapping and shootdown policy, heaps, MMIO/cache qualification, pressure/OOM policy, target hardware, and N9 exit remain open.

Cycle 131 upgrades selector `8` to PKPMM5 and closes the bounded scope of `FLAG-N9-PMM-GROWTH-001`. The retained five-page guarded manager now contains stable control state only; source, free-extent, allocation, scrub-receipt, and reclaim-receipt ledgers live in a guarded active generation. Two alternate 32-page virtual windows support full reserve/map/copy/seal/switch/revoke/scrub-retire transactions. Generation 2 begins in four pages with capacities 256/32/256/16/2 and explicitly grows to generation 3 in eight pages with capacities 512/64/512/32/4. All precommit failures roll back; postcommit old-generation retirement is retryable; stale generations are revoked before all four old pages are zeroed, verified, and released. Host mapping, copy, integrity, retirement, corruption, and stale-handle faults pass, and an independent Python oracle models the stable manager, retired initial hole, and active grown generation. Two exact fresh-vars qemu64 runs emit 43 markers and pass 137 hostile controls plus 82 kernel host tests. They manage 117,913 source-usable and 129,162 final pages, protect 831 loader pages, retain five manager and eight active-ledger pages plus guards, admit 11,250 Boot Services pages while eleven ACPI pages remain held, scrub and verify 11,279 pages and 46,198,784 bytes, and perform 5,775,872 physical writes, 5,776,896 reads, and 22,591 temporary-PTE writes and invalidations. Growth checksum `0xFA339A347E3A3CAF` binds the transaction. The larger retained layout exposed a shared finalizer assumption; VM-only profiles now correctly require zero active PMM ledgers, and PKVM2 is rebound to 5,528 temporary writes and invalidations. The canonical kernel is 270,336 bytes in a 311,296-byte, 76-page image with 662 relocations and SHA-256 `30E414826D7DF588C07A66032423DCD6AA8C47B0E6CCFC3D6686BC9224AFB947`. Automatic pressure-triggered and repeated growth, both guarded-window exhaustion/fallback paths, complete ACPI consumer integration, concurrency/SMP, complete direct mapping and shootdown policy, heaps, MMIO/cache qualification, pressure/OOM policy, target hardware, and N9 exit remain open under `FLAG-N9-PMM-GROWTH-AUTOMATION-001` and existing N9 gaps.

Cycle 133 upgrades the live handoff to PBLIVE4 and selector `8` to PKPMM7/PKACPI1, closing `FLAG-N9-PMM-ACPI-CONSUMER-001`. PooleBoot now records the UEFI ACPI 2.0 RSDP address. PooleKernel validates RSDP and XSDT signatures, lengths, ranges, and checksums; requires exactly one APIC, FACP, HPET, and MCFG table; allocates and scrubs one retained page; copies 600 table bytes into a 616-byte snapshot; verifies readback; and advances the PMM lifecycle through an opaque evidence token. Boot Services reclaim admits 11,250 pages under receipt `5DEA9A3BC9E10C18`; the separately gated ACPI reclaim admits eleven pages under receipt `60DAA52A8A05ABD6`. Missing, duplicate, malformed, checksum, range, allocation, copy, readback, rollback, early-reclaim, and repeat-call paths fail closed. Two exact fresh-vars qemu64 runs emit 45 markers while 191 hostile controls and 88 kernel host tests pass. They manage 117,888 source-usable and 129,148 final pages, protect 856 loader pages, retain the snapshot, scrub and verify 11,473 pages and 46,993,408 bytes, and perform 5,875,277 physical writes, 5,879,957 reads, and 23,172 temporary-PTE writes and invalidations. Growth checksum `0xCC30B722527718AE` binds the retained ledger sequence. PKVM2 is rebound to 5,640 temporary writes and invalidations. The canonical kernel is 299,008 bytes in a 339,968-byte, 83-page image with a 32-page guarded stack, 693 relocations, and SHA-256 `D9EF9B10B56BF779B155BD18DE55853874CCC032D2A3E5E7841B918F08CDE1F2`. AML execution, a complete ACPI resource graph, interrupt-context/concurrent/SMP allocation, complete direct mapping and shootdown policy, heaps, MMIO/cache qualification, general pressure/OOM policy, target hardware, and N9 exit remain open. `FLAG-N9-VM-DIRECT-MAP-001` is the next owner-independent move.

Cycle 134 upgrades selector `10` to PKVM3 and closes the bounded scope of `FLAG-N9-VM-DIRECT-MAP-001`. PKPMM7 now reconstructs a generation-bound ownership manifest from every free extent and active non-release-excluded allocation, coalesces exact sparse ranges, excludes retained ownership, and binds range/page/gap/exclusion counts, write-back policy, and checksum before and after table allocation. PKVM3 derives its table topology from that manifest, allocates 243 contiguous DMA32 table pages, installs 117,887 supervisor RW/NX write-back leaves across eleven ranges, and leaves 12,878 gap pages unmapped. It audits every admitted leaf plus inherited kernel, 32-page guarded-stack, handoff, sparse-edge, and cache-policy invariants before activation. Forged manifests, retained holes, PWT/PCD drift, write rollback, CR3 rollback, premature reuse, absent/stale retirement receipts, and multi-processor retirement fail closed. Three local INVLPG receipts gate the data frame; exact old-root restoration creates one root/generation/BSP/local-context-flush receipt; table release remains deferred until that exact receipt is supplied. Ninety-one kernel host tests and two exact 40-marker qemu64 runs pass 46 hostile controls with coverage checksum `0x341CF729ADB26B52`, 367,474 physical table writes, and 950,234 temporary-PTE writes and invalidations. The kernel stays 299,008 canonical bytes in a 339,968-byte, 83-page image, now with 729 relocations and SHA-256 `5B581CC1D1ABEB163D0984D12144CA5016C44B46A28B190A6DFCBDCDA689A255`. N9.1-N9.4 remain partial because AP startup, inter-processor shootdown execution, generation-safe remote deferred reclaim, incremental concurrent map replacement, huge pages, PCID, COW, user faults, pager IPC, heaps, MMIO/PAT/MTRR qualification, pressure/OOM, target hardware, and N9 exit remain open. `N8-IRQ-001` is the next owner-independent move.

Cycle 135 adds selector `11` and PKIRQ1 as the first native N8 execution slice. PooleKernel walks the retained, validated PKACPI1 MADT and HPET descriptions; validates one-BSP local-APIC identity; reserves 51 exception, timer, future-IPI, error, and spurious vectors; installs guarded supervisor RW/NX uncacheable LAPIC and HPET mappings; masks and later restores the legacy PIC; and calibrates a checked one-shot local-APIC timer over a bounded ten-millisecond HPET interval. Two exact 36-marker qemu64 runs deliver exactly eight timer interrupts and eight EOIs with zero error, spurious, or remaining ISR bits. The normal path restores APIC, HPET, PIC, IA32_APIC_BASE, and MMIO state before halting. Ninety-nine kernel host tests and 58 hostile controls pass. The current kernel is 323,584 canonical bytes in a 364,544-byte, 89-page image with 812 relocations and SHA-256 `2ACD4A5EF30CA1A4A22711FD31E2A259A5C87D97BCE7FB1BF49A3488B3FC02B2`. N8.1 and N8.3 are partial. `FLAG-N8-IRQ-001` remains open because I/O APIC routing, MSI/MSI-X, complete time services, panic-path rollback, first-AP startup, per-CPU state, IPIs, SMP shootdown, target hardware, and N8 exit remain open. `N8-SMP-FIRST-AP-001` is the next owner-independent move.

Cycle 136 adds selector `12` and PKSMP1 as the first live AP lifecycle. PooleKernel selects APIC ID 1 from the retained validated MADT, allocates fourteen contiguous pages below 1 MiB, builds a W^X identity hierarchy with four absent guards, and installs an RX 16-to-32-to-64-bit trampoline plus guarded RW/NX stack and mailbox. One INIT-SIPI-SIPI sequence observes the AP online in long mode with exact APIC, CPUID, CR0, CR3, CR4, EFER, and TSC evidence; the BSP commands stop, validates quiescence and the dynamic mailbox checksum, issues a final INIT park, revokes aliases, scrubs and verifies 57,344 bytes, and releases all fourteen pages. Two exact fresh-vars two-vCPU qemu64 runs emit 38 ordered markers while 72 hostile controls and 111 kernel host tests pass. Live diagnosis of an initial triple fault found that loading an unaccessed descriptor from the RX trampoline GDT caused a hardware write and page fault; all four descriptors now have the accessed bit pre-set and this regression is source/model tested. The canonical kernel is 335,872 bytes in a 376,832-byte, 92-page image with 855 relocations and SHA-256 `6596CB332EB24813089F95A00AC979C892C47235943CB0E73E3979ED9901B725`. `FLAG-N8-SMP-FIRST-AP-001` closes only this bounded transaction and N8.5 becomes partial. General SMP, AP-local GDT/TSS/IDT/RSP0/IST/xstate/interrupt state, multi-AP and live partial-start fault handling, IPIs, shootdown, target hardware, N8 exit, release, and production remain open. `N8-SMP-PERCPU-RUNTIME-001` is the next owner-independent move.

Cycle 137 adds selector `13` and PKSMP2 as the first processor-local AP runtime. In the frozen two-vCPU `SandyBridge,-avx` TCG profile, APIC ID 1 starts through 16-bit, 32-bit, and 64-bit trampoline stages and then loads one AP-owned GDT/TSS/IDT, a guarded four-page RSP0 stack, two guarded two-page IST stacks, x87/SSE xstate ownership, eight exception gates, and nineteen owned interrupt gates. The 32-page below-1-MiB transaction maps thirteen runtime leaves, leaves fourteen guards and one reserved page absent, verifies the hardware-set busy TSS descriptor and all 27 gates from memory, performs one XSAVE/XRSTOR round trip, commands quiescence, final-INIT parks the AP, revokes the aliases, and zeroes, read-verifies, and releases all 131,072 bytes. Two exact fresh-vars runs emit 42 ordered markers while 19 hostile-control categories cover 159 independently rejected cases and 122 kernel host tests pass. The canonical kernel is 409,600 bytes in a 458,752-byte, 112-page image with 919 relocations and SHA-256 `214F32214494E632063238337551C355BFED150B9B49846DB8A927584B8E47F0`. `FLAG-N8-SMP-PERCPU-RUNTIME-001` closes only this one-AP runtime transaction. General SMP, capability-gated IPI delivery, TLB shootdown, scheduler CPU ownership, multi-AP and live partial-start fault injection, target hardware, N8 exit, release, and production remain open. `N8-SMP-IPI-001` is the next owner-independent move.

Cycle 138 adds selector `14` and PKSMP3 as the first live fixed-vector IPI transport. In the frozen two-vCPU `SandyBridge,-avx` TCG profile, APIC ID 1 installs six owned vectors and acknowledges six allowlisted operation classes behind a checksum-bound development capability. Request and response records bind target APIC ID, vector, operation, sequence, attempt, capability, arguments, status, EOI count, and checksums; stale sequence, bad capability, wrong vector/operation, duplicate attempt, offline target, post-revocation, and malformed state fail closed. Two exact fresh-vars runs emit 39 ordered markers and observe six accepted plus four denied deliveries, ten EOIs, one bounded offline-APIC timeout, panic latching, stop quiescence, final-INIT parking, post-execution descriptor/xstate/APIC-table validation, capability and alias revocation, and exact zero/readback release of all 32 pages or 131,072 bytes. Eighteen hostile-control categories cover 120 independently rejected cases and 130 kernel host tests pass. The canonical kernel remains 409,600 bytes in a 458,752-byte, 112-page image with 959 relocations and SHA-256 `6B8A9C2C3EAC559E1D9CB5965800A1671DB8F487F149861300D4EDCB279B3A11`. `FLAG-N8-SMP-IPI-001` closes only this development transport. Its capability is not production authority; the shootdown operation records transport but performs zero TLB invalidations; call-function is one no-op token and exposes no arbitrary callback. Real generation-bound remote invalidation and deferred reclaim, scheduler CPU ownership, multi-AP and live partial-start fault injection, target hardware, N8/N9 exit, release, and production remain open. `N9-SMP-SHOOTDOWN-001` is the next owner-independent move.

Cycle 140 upgrades selector `14` to PKSMP4 and closes only `FLAG-N9-SMP-SHOOTDOWN-001`. One AP fills a translation for virtual page `0x001FF000` from its own root at physical `0x2000`, then validates a checksum-bound generation-2 request targeting CPU mask `0x2`. The AP executes exactly one linked-image-audited `INVLPG`, observes the replacement frame, and acknowledges the exact root, page, generation, target mask, sequence, and attempt. The BSP deliberately times out offline target mask `0x4`, retries the same attempt against APIC ID 1, rejects premature reclaim, and releases the retired frame only after the exact acknowledgement. Stale generation, duplicate acknowledgement, forged mask, checksum, timeout, retry, and reclaim-state controls fail closed. Two exact fresh-vars two-vCPU `SandyBridge,-avx` TCG runs emit 40 ordered markers; 25 hostile-control categories cover 169 independently rejected cases; and 132 kernel host tests pass. All 32 runtime pages plus old and replacement frames, 34 pages or 139,264 bytes, are scrubbed, read-verified, and released. The canonical kernel remains 409,600 bytes in a 458,752-byte, 112-page image with 969 relocations and SHA-256 `95DDA27784DA944A9C0F5B04029255EDE4DE1BB0684A8EA10DCFC07E686B59A2`. General multi-AP operation, address-space-wide invalidation, multiple concurrent generations, scheduler ownership, production capability authority, target execution, N8/N9 exit, release, and production remain open. `N8-SMP-MULTI-AP-001` is next.
...
| N8 | Partial | `023-025` | 204 | 1 |
| N9 | Partial | `026-029,151` | 410 | 0 |
...
- the retained-set identity is `SHA256("POOLEOS/INNER-LIVE-SET/V1\0" || role_le32 || file_len_le64 || exact_file_bytes)` for roles 2 through 7. The canonical result is `E15FF547B310FC526C5880D83FD88D6B0ED3A5B2E2849F8EEE0FE9D15A7EB463` over 8,761 file bytes and 8,185 payload bytes;
...
- live PooleBoot reads exact `TRUST.PBT` and `TRUSTST.PBS` development candidates from the twelve-file medium, cross-binds fourteen facts, emits one `TRUST_STATE DENY` marker, grants zero authority, and writes zero state. The Cycle 133-refreshed PKLOAD6 passes its dependent Rust host suites, 25 markers, and the current 155-control hostile corpus across two exact QEMU/OVMF runs;
...
- 74/74 Rust tests, 8/8 Python tests, both `x86_64-unknown-none` and `x86_64-unknown-uefi` builds, 36/36 hostile controls, and 32,768/32,768 deterministic mutation rejects pass. The live six-PBART1 retained set is content-bound by the current PKLOAD6 receipt, with PSM1/PBTP1/PBTS1 bound separately;
- the expanded verifier requires a 339,968-byte PooleKernel image with a 299,008-byte canonical file, entry offset `0x9000`, and 693 relative relocations. PKMAP2 covers 83 kernel pages; places low guard/stack/high guard at pages 83/84-115/116 and the handoff allocation at page 117; reserves temporary page 373; places manager guards/pages at 374/375-379/380; and reserves two guarded 32-page ledger windows at 381/382-413/414 and 415/416-447/448;
- PKLOAD6 passes 167/167 Rust tests, two exact PooleBoot and PooleKernel builds, two exact media generations, two QEMU/OVMF producer runs, 25 markers, 155/155 hostile controls, and two independent 5,048-byte PBP1 reconstructions with 97 memory descriptors and ten artifact descriptors;
...
- two exact kernel builds, two feature-enabled PooleBoot builds, one default isolation build, two byte-identical media generations, and two fresh-vars QEMU/OVMF runs reproduce all 30 markers, exact PBP1 bytes, and exact guest/host denial. The Cycle 133-refreshed 299,008-byte PooleKernel has SHA-256 `D9EF9B10B56BF779B155BD18DE55853874CCC032D2A3E5E7841B918F08CDE1F2`; all 58 hostile controls still pass;
...
N5.1-N5.9 remain partial. PooleBoot now performs bounded live intake, exact retention, parsing and cross-binding, final PBLIVE4 production with a firmware RSDP locator, successful `ExitBootServices`, and either the ordinary firmware-free stop or the explicit QEMU-only PKXFER1 transfer. PBSTATE1 still models but does not perform authenticated persistent state selection or recovery. PooleBoot does not authenticate files, verify a policy signature or revocation store, use a real monotonic provider, perform persistent trust-state I/O, consume symbols, apply policy, observe Secure Boot state, privileged revisions, or live firmware inventory, or apply microcode or firmware. PooleKernel live-executes PKREVAL1 only to deny and halt; PKACPI1 separately validates and snapshots its bounded required ACPI table set without AML. It creates no capability and executes no lifecycle, recovery, or policy action. The path still lacks authenticated rollback, a licensed real vendor payload, TCG2/TPM evidence, final framebuffer remap/revocation, production transfer, second-host reproduction, target firmware, physical media, ISO construction, and the N5 exit gate. The static identity is not the final animated PooleGlass boot experience or an accessibility result.
...
- the Cycle 133-refreshed `runs/native_kernel_entry_readiness.json` records 88 Rust host tests, two byte-identical clean linked and canonical builds, 43/43 hostile controls, 693 relative relocations, exact independent Rust/Python loaded bytes, an exact matching PKMID1/live build ID, and canonical SHA-256 `D9EF9B10B56BF779B155BD18DE55853874CCC032D2A3E5E7841B918F08CDE1F2`;
...
Cycle 135 PKIRQ1 one-BSP interrupt/time evidence:

- `specs/native-kernel-interrupt-time-contract.json` freezes retained MADT/HPET parsing, exact qemu64 xAPIC identity, 51-vector ownership, three MMIO guards, uncacheable LAPIC/HPET leaves, checked HPET and APIC calibration arithmetic, eight local timer deliveries and EOIs, and normal-path controller/clock/PIC/MSR/MMIO restoration;
- `native/kernel/src/interrupt_time.rs` provides allocation-free bounded ACPI structure discovery, vector ownership, HPET wrap/regression handling, checked femtosecond-to-nanosecond conversion, and saturated local-timer count derivation. The typed x86_64 adapter and selector-11 transaction execute with IF clear except for eight exact `sti; hlt; cli` delivery windows;
- two exact fresh-vars qemu64 QEMU/OVMF runs emit 36 ordered markers and pass 58 hostile controls plus 99 kernel host tests. Exactly eight local APIC timer interrupts and eight EOIs occur, with zero APIC-error, spurious, or remaining ISR bits. Both MMIO leaves are revoked, all three guards remain absent, both PIC masks and all saved APIC/HPET state are restored, and signatures, authority, actions, and production claims remain zero;
- The N8.1 and N8.3 statuses are partial only. No I/O APIC route, external IRQ, MSI/MSI-X, invariant-TSC calibration, watchdog, timer queue, public time API, AP startup, per-CPU area, IPI, SMP shootdown, panic-time complete rollback, target execution, or N8 exit is claimed. `FLAG-N8-IRQ-001` remains open and `N8-SMP-FIRST-AP-001` is next.

Cycle 136 PKSMP1 first-AP evidence:

- `specs/native-kernel-smp-first-ap-contract.json` freezes exactly one selected xAPIC AP, fourteen pages below 1 MiB, an RX trampoline with pre-accessed GDT descriptors, four absent guards, RW/NX stack/mailbox, one INIT-SIPI-SIPI sequence, observed long-mode state, stop/quiesce/final-INIT park, exact alias revocation and scrub/release, and explicit no-general-SMP/no-IPI/no-shootdown/no-target/nonproduction boundaries;
- `native/kernel/src/smp.rs` provides allocation-free topology selection, guarded resource geometry, mailbox/checksum state, and transaction/rollback modeling. The x86_64 adapter supplies the 16-to-32-to-64-bit trampoline and the selector-12 path couples retained ACPI, PMM, HPET timing, xAPIC delivery, temporary mappings, and fail-closed cleanup;
- two exact fresh-vars two-vCPU qemu64 QEMU/OVMF runs emit 38 ordered markers and pass 72 hostile controls plus 111 kernel host tests. Exactly one AP reaches online and quiesced state, is final-INIT parked before ownership release, and leaves all fourteen pages and 57,344 bytes scrubbed, verified, released, and unmapped;
- live diagnosis corrected a real AP triple fault: loading an unaccessed descriptor from the RX GDT caused a hardware accessed-bit write and page fault. All four descriptors are now pre-accessed and the source/model regression test rejects any recurrence;
- The N8.5 status is partial only. `FLAG-N8-SMP-FIRST-AP-001` is closed, while `FLAG-N8-IRQ-001` remains open. AP-local GDT/TSS/IDT/RSP0/IST/xstate/interrupt state, multi-AP startup, IPIs, shootdown, live partial-start fault injection, target execution, and N8 exit remain open. `N8-SMP-PERCPU-RUNTIME-001` is next.

Cycle 137 PKSMP2 processor-local runtime evidence:

- `specs/native-kernel-smp-percpu-runtime-contract.json` freezes one AP-local 32-page below-1-MiB transaction: one RX trampoline, four page tables, a guarded four-page RSP0 stack, guarded two-page IST1 and IST2 stacks, a guarded mailbox, one GDT/TSS page, one RO/NX IDT page, one xstate page, fourteen absent guard pages, and one absent reserved page;
- `native/kernel/src/smp_runtime.rs` provides allocation-free layout, topology, descriptor, gate, guarded-stack, xstate, mailbox, lifecycle, checksum, teardown, and rollback validation. The x86_64 trampoline loads AP-local `GDTR`, `TR`, and `IDTR`, verifies the hardware-set busy-TSS state, configures eager x87/SSE state, performs one `XSAVE64`/`XRSTOR64` round trip, and keeps IF clear while installing eight exception and nineteen owned interrupt gates;
- two exact fresh-vars, two-vCPU `SandyBridge,-avx` TCG QEMU/OVMF runs emit 42 ordered markers and pass 19 hostile-control categories covering 159 independently rejected cases plus 122 kernel host tests. Exactly one AP reaches runtime-online and quiesced state, is final-INIT parked, and leaves all 32 pages or 131,072 bytes zeroed, read-verified, released, and unmapped;
- `FLAG-N8-SMP-PERCPU-RUNTIME-001` is closed only for that one-AP transaction. The interrupt gates reserve vectors but provide no IPI service. General SMP, multi-AP startup, capability-gated IPI acknowledgement/timeouts, TLB-generation shootdown, scheduler ownership, live partial-start fault injection, target execution, and N8 exit remain open. `N8-SMP-IPI-001` is next.

Cycle 138 PKSMP3 fixed-vector IPI evidence:

- `specs/native-kernel-smp-ipi-contract.json` freezes six fixed development-only vectors and operation classes, checksum-bound request/response records, sequence and attempt replay controls, bounded delivery/acknowledgement/EOI and timeout behavior, panic/stop/park/release ordering, and explicit no-real-shootdown/no-arbitrary-callback/no-scheduler/no-target/nonproduction boundaries;
- `native/kernel/src/smp_ipi.rs` provides the allocation-free transport model and `native/kernel/src/arch/x86_64.rs` installs the live AP handlers, sends xAPIC fixed deliveries, records acknowledgements and EOIs, and validates teardown. Selector 14 retains the proven PKSMP2 runtime and maps one guarded uncacheable LAPIC leaf through a dedicated page-table leaf before revocation;
- two exact fresh-vars, two-vCPU `SandyBridge,-avx` TCG QEMU/OVMF runs emit 39 ordered markers and observe six accepted and four denied deliveries, ten EOIs, one bounded offline-APIC timeout, panic latching, stop quiescence, final-INIT parking, capability and alias revocation, and exact release. Eighteen hostile-control categories cover 120 independently rejected cases and 130 kernel host tests pass;
- `FLAG-N8-SMP-IPI-001` closes only this bounded transport. Its fixed development capability is not production authority; the shootdown class performs zero TLB invalidations; call-function exposes no arbitrary callback. Real generation-bound remote invalidation and deferred reclaim, scheduler ownership, multi-AP startup, live partial-start fault injection, target execution, and N8/N9 exit remain open. `N9-SMP-SHOOTDOWN-001` is next.
...
Cycle 126 PKVM1 evidence:

- `specs/native-kernel-virtual-memory-contract.json` freezes canonical 48-bit address bands, a one-page PKMAP2 bootstrap temporary slot, fixed capacities, PMM generation ownership, 4 KiB leaves, W^X and cache policy, inactive-root receipts, exact qualification controls, and a non-promoting claim boundary;
- `native/kernel/src/virtual_memory.rs` is allocation-free `no_std` code with fixed mapping, frame, and pending-invalidation ledgers. It materializes PML4/PDPT/PD/PT pages from one four-page DMA32 allocation, validates every handle before mutation, commits only after write/readback, rolls back exactly on injected failure, and withholds frame reuse until every alias and receipt is resolved;
- the first selector-9 run failed closed with `table_access`, revealing that new conventional PMM pages are not inherited identity mappings and that the development PBP1 profile does not promise a CPU-bootstrap record. The corrected architecture helper reads BSP CPUID leaf `0x80000008`, accepts only 36-52 physical bits, and the privileged adapter verifies and exclusively uses absent PKMAP2 leaf 336 as a supervisor RW/NX bootstrap mapping;
- two exact fresh-vars qemu64 runs emit 40 ordered markers and pass 39 hostile controls. Each run performs exactly 4,104 writes to inactive tables, 40 writes to the active bootstrap leaf, 40 corresponding local `INVLPG` operations, two map/unmap cycles, one protect, two inactive receipts, exact release of table and data allocations, zero PKVM1 CR3 writes, zero shootdowns, and zero signatures, authority grants, or actions;
- selector 9 never installs the PKVM1 root and the bootstrap alias is revoked and translation-checked before the table allocation is freed. N9.3 and N9.4 are partial only; kernel-complete active roots, direct map, active-address-space invalidation/SMP shootdown, huge pages, PCID, COW, user faults, stack growth, pager IPC, heaps, MMIO/cache qualification, target hardware, and N9 exit remain open.

Cycle 127 PKVM2 evidence:

- `specs/native-kernel-virtual-memory-contract.json` freezes the selector-10 one-BSP threat model, eight-table/one-data-page candidate layout, exact inherited kernel/entry/guarded-stack/handoff requirements, bounded direct-map policy, activation and rollback transaction, Accessed/Dirty allowance, invalidation-receipt identity, and release ordering;
- `native/kernel/src/active_virtual_memory.rs` is allocation-free `no_std` code. It builds the candidate from generation-bound physical-allocation handles, audits it through the bootstrap mapper, requires BSP 0 with IF clear and exact original CR3, rolls back candidate or active-leaf write failures, and refuses data or table reuse until the required unmap, restoration, scrub, and alias-revocation boundaries are complete;
- `runtime/native_kernel_virtual_memory.py` independently binds the candidate root to the PBP1 DMA32 first-fit transcript, reconstructs all 40 markers, and rejects 46 omission, ordering, layout, ownership, activation, readback, receipt, accounting, overclaim, and first-fit mutants;
- two exact fresh-vars qemu64 runs record eight table pages, one data page, exact inherited mappings, a bounded nine-page direct map, exact candidate readback and original-root restoration, probe byte `0xA5`, three active local invalidations, 8,720 physical table writes, 5,640 bootstrap temporary-PTE writes and invalidations after the Cycle 133 retained-layout rebind, exact allocation release, and zero shootdowns, signatures, authority grants, actions, or production claims;
- selector 10 remains one-BSP development evidence. A complete generation-owned direct map, AP startup, SMP shootdown and deferred reclaim, ring 3, huge pages, PCID, COW, user faults, stack growth, pager IPC, heaps, MMIO/cache qualification, target hardware, and N9 exit remain open.

Cycle 128 PKPMM2 scrubbing evidence:

- `specs/native-kernel-physical-memory-contract.json` upgrades selector 8 to PKPMM2 and freezes pre-commit allocation scrubbing, pre-release extent preflight, full-page zero/readback, exact ownership retention on access or verification failure, sequence/generation/owner-bound receipts, exact-reuse stale-data rejection, and the predecessor raw-API nonclaim;
- `native/kernel/src/physical_memory.rs` plans allocations without mutating ownership, uses a `PhysicalPageAccess` boundary for complete 64-bit zero and readback passes, commits only after success, and keeps a release handle live until verified scrub completion. Host tests inject partial allocation and release-verification failures and assert exact ledger rollback;
- the selector-8 adapter validates one absent PKMAP2 leaf, maps one supervisor RW/NX page at a time, performs volatile access and local `INVLPG`, and revokes the alias before return. Two exact fresh-vars qemu64 runs emit 40 markers and agree on 117,923 source-usable pages, 117,922 managed pages, 821 protected loader pages, eight scrubbed and verified pages, 32,768 scrubbed and verified bytes, four receipts, 5,120 writes, 6,144 reads, and 28 temporary mappings and invalidations;
- 70 kernel host tests and 63 hostile controls cover lifecycle, residue, receipts, counts, ownership, source-map substitution, fault rollback, alias revocation, and all nonclaims. Allocated residue, reclaim, concurrency, SMP, signatures, authority, actions, and production claims remain zero;
- `FLAG-N9-PMM-SCRUB-001` is closed. `FLAG-N9-PMM-METADATA-001` remains open for moving the fixed-capacity map, extent, allocation, and receipt ledgers into explicitly reserved, mapped, guarded, generation-owned pages before held-class reclaim.

Cycle 129 PKPMM3 metadata-arena evidence:

- `specs/native-kernel-physical-memory-contract.json` upgrades selector 8 to PKPMM3 and freezes five contiguous DMA32 metadata pages, absent low/high guards, generation/owner binding, complete-manager migration, handoff and final integrity checks, rollback, and exact exclusion from release and ordinary allocation;
- `native/kernel/src/physical_memory.rs` uses fixed source, extent, allocation, and receipt ledgers only during bootstrap, reserves and scrubs the arena, copies the complete 14,616-byte manager into mapped storage, validates and seals it before retiring stack state, and verifies integrity before each public mutation. Host tests fill ledgers, inject mapping and handoff failures, corrupt mapped state, and prove exact rollback or fail-closed rejection;
- `native/kernel/src/main.rs` installs five persistent supervisor RW/NX leaves between two absent guards, rolls back partial installs, revokes the temporary alias, and re-walks every retained metadata leaf and both guards before final success. Two exact fresh-vars qemu64 runs emit 41 markers and agree on 117,919 source-usable pages, 117,918 managed pages, 825 protected loader pages, five metadata pages, two guards, 13 scrubbed pages, 53,248 scrubbed and verified bytes, 7,680 physical word writes, 8,704 reads, and 43 temporary mappings and invalidations;
- 74 kernel host tests and 87 hostile controls cover complete migration, record counts, generation/owner identity, handoff and final seals, mapping rollback, corrupted handoff, mapped corruption, release exclusion, source-map substitution, all accounting, and every nonclaim. Reclaim, scalable growth, concurrency, SMP, signatures, authority, actions, and production claims remain zero;
- `FLAG-N9-PMM-METADATA-001` is closed. `FLAG-N9-PMM-RECLAIM-001` is open for exact held-class admission, lifecycle timing, retained-range exclusion, scrub-before-admission, atomic transition, rollback, idempotence, and receipts.

Cycle 130 PKPMM4 held-memory-reclaim evidence:

- `specs/native-kernel-physical-memory-contract.json` freezes `PreExitBootServices`, `PostExitBootServices`, and `AcpiTablesReleased` as monotonic non-skippable stages; Boot Services and ACPI remain distinct reclaim classes, and every ineligible or repeated transition is deterministic;
- `native/kernel/src/physical_memory.rs` streams the immutable PBP1 source ledger with a scalar cursor through complete preflight, full zero/readback, and infallible commit. It excludes retained source and live non-target allocations, proves free-ledger and receipt capacity before content access, preserves metadata ownership on content faults, and returns the exact prior receipt idempotently without physical access;
- `runtime/native_kernel_physical_memory.py` independently reconstructs 70 Boot Services records, 12 coalesced ranges, 11,250 admitted pages, zone counts 2,018/9,232/0, 13 final free extents, range checksum `0x5A485D4A5725EED8`, receipt checksum `0x4D3EBF743B7F2CCC`, and 129,168 final managed pages directly from PBP1;
- 79 kernel host tests and 109 hostile controls cover positive Boot Services and staged ACPI admission, early/skip/regression rejection, active and retained overlap, capacity and arithmetic exhaustion, idempotence, partial write/readback faults, no full reclaim-extent snapshot, source-map substitution, exact accounting, and every nonclaim. Two exact 42-marker live runs retain eleven ACPI pages and execute zero signatures, authority grants, authorized actions, or production promotion;
- `FLAG-N9-PMM-RECLAIM-001` is closed. `FLAG-N9-PMM-GROWTH-001` is open for generation-owned scalable source, extent, allocation, scrub-receipt, and reclaim-receipt storage with guarded growth, retirement safety, failure atomicity, integrity, deterministic accounting, and rollback.

Cycle 131 PKPMM5 generation-owned ledger-growth evidence:

- `specs/native-kernel-physical-memory-contract.json` separates the stable five-page manager from external active-ledger generations and freezes two alternate 32-page guarded windows, generation/owner identity, logical seals, complete growth preflight, active-pointer switch ordering, old-generation revocation, scrubbed retirement, and retryable postcommit retirement;
- `native/kernel/src/physical_memory.rs` migrates all source, free-extent, allocation, scrub-receipt, and reclaim-receipt storage into mapped ledger pages. Generation 2 uses four pages at capacities 256/32/256/16/2; one explicit transaction grows to generation 3 in eight pages at capacities 512/64/512/32/4. Host tests inject reserve, map, copy, seal, switch, unmap, scrub, and release failures and prove precommit rollback or postcommit retirement retry without reverting the committed active generation;
- `native/kernel/src/main.rs` maps each generation supervisor RW/NX between absent guards, verifies every leaf and guard, revokes old leaves before full zero/readback and release, and emits the exact growth receipt. The independent Python oracle reconstructs the stable manager, retired initial hole, active grown generation, capacities, ownership, checksums, reclaim result, and physical access counts directly from PBP1 and the transcript;
- 82 kernel host tests and 137 hostile controls pass. Two exact 43-marker qemu64 runs manage 117,913 source-usable and 129,162 final pages, protect 831 loader pages, retire four old-generation pages, retain eight active generation pages, keep eleven ACPI pages held, scrub and verify 11,279 pages and 46,198,784 bytes, perform 5,775,872 writes, 5,776,896 reads, and 22,591 temporary writes and invalidations, and execute zero signatures, authority grants, authorized actions, or production promotion;
- `FLAG-N9-PMM-GROWTH-001` is closed for the demonstrated bounded explicit-growth transaction. `FLAG-N9-PMM-GROWTH-AUTOMATION-001` remains open for checked pressure-triggered and repeated growth, both guarded-window exhaustion/fallback paths, and the same rollback, retirement, integrity, accounting, concurrency/SMP, target, and production boundaries.

Cycle 132 PKPMM6 checked automatic-growth evidence:

- `specs/native-kernel-physical-memory-contract.json` freezes exact post-operation ledger demand, one-allocation/four-scrub-receipt rollback and retry headroom, the `4/8/15/29`-page generation sequence, final capacities `2048/256/2048/128/16`, and a bounded 32-page window whose next aligned doubled layout is 58 pages;
- `native/kernel/src/physical_memory.rs` checks pressure before every automatic scrubbed allocate/free, repeats growth through three thresholds, and preserves the complete PKPMM5 reserve/map/copy/seal/switch/revoke/scrub-retire transaction. When 58 pages cannot fit, an operation that fits the active generation records a soft fallback; unsatisfied demand rejects before page reads, page writes, ownership changes, or receipt commitment. Host tests also prove failed growth preserves rollback/retry headroom and the requested ownership state;
- `native/kernel/src/main.rs` emits one exact expanded growth marker. The independent Python oracle reconstructs first-fit placement of all four generations, alternating guarded windows, three retired holes totaling 27 pages, final ownership, reclaim coalescing, exact pressure counters, and all physical access counts rather than accepting the marker as authority;
- 84 kernel host tests and 147 hostile controls pass. Two exact 43-marker qemu64 runs record 121 pressure checks, eight triggers, three automatic growths, sixty successful cycles, four soft fallbacks, one host-proved hard pre-effect rejection, 117,911 source-usable and 129,160 final pages, 833 protected loader pages, eleven ACPI pages held, 11,462 scrubbed and verified pages, 46,948,352 scrubbed and verified bytes, 5,869,568 writes, 5,870,592 reads, and 22,798 temporary writes and invalidations. Growth checksum `0xF7AD111CA266071D` binds the sequence; signatures, authority grants, authorized actions, and production promotion remain zero;
- `FLAG-N9-PMM-GROWTH-AUTOMATION-001` is closed for the demonstrated serial bounded-window behavior. `FLAG-N9-PMM-ACPI-CONSUMER-001` remains open for a real native table consumer, exact copy/validation/release ordering, ACPI reclaim admission, malformed-table rejection, and explicit no-AML/no-SMP/no-target/no-production boundaries.

Cycle 133 PKPMM7 / PKACPI1 required-table snapshot and ACPI-reclaim evidence:

- PBLIVE4 adds a canonical ACPI 2.0 RSDP firmware record while preserving the final-map memory descriptors and retained loader ownership. The 83-page kernel, low guard, 32-page stack, high guard, 256-page handoff, bootstrap temporary page, guarded manager, and both guarded ledger windows have one cross-stage exact layout;
- `native/kernel/src/acpi.rs` validates the RSDP and XSDT signatures, revision, lengths, ranges, and checksums; walks six XSDT entries; requires exactly one APIC, FACP, HPET, and MCFG table; rejects missing, duplicate, malformed, out-of-range, and checksum-failing structures; computes an exact copy plan; and exposes an opaque evidence value only after retained snapshot copy and readback succeed;
- PKPMM7 allocates and fully scrubs one retained snapshot page, copies 600 table bytes into a 616-byte container, verifies every copied byte, and retains the snapshot while separately admitting the eleven ACPI reclaimable pages. Allocation, copy, readback, lifecycle, and reclaim failures preserve ownership and permit exact retry. Repeated successful calls return the immutable receipts without new physical effects;
- two exact 45-marker qemu64 runs and 191 hostile controls plus 88 kernel host tests pass. The runs manage 117,888 source-usable and 129,148 final pages; protect 856 loader pages; bind source checksum `078583AEEFDD6581`, snapshot checksum `4089A5CFEC81CB41`, boot-reclaim receipt `5DEA9A3BC9E10C18`, ACPI-reclaim receipt `60DAA52A8A05ABD6`, and growth checksum `0xCC30B722527718AE`; scrub and verify 11,473 pages and 46,993,408 bytes; and perform 5,875,277 physical writes, 5,879,957 reads, and 23,172 temporary PTE writes and invalidations;
- `FLAG-N9-PMM-ACPI-CONSUMER-001` is closed only for this bounded required-table container and serial reclaim transaction. No AML is parsed or executed, no complete ACPI namespace/resource graph is built, and no SMP, target-hardware, N9-exit, release, or production claim follows;
- `ADD-MEM-002` and `FLAG-N9-VM-DIRECT-MAP-001` are open for the next move: replace PKVM2's bounded nine-page map with a complete generation-owned sparse physical direct map, exact hole/forbidden-range/cache-alias controls, transactional generation switching, local invalidation receipts, future SMP-shootdown dependencies, and deferred old-generation reclamation.

Cycle 134 PKVM3 generation-owned sparse-direct-map evidence:

- PKPMM7 reconstructs a `DirectMapManifest` from every free extent and active non-release-excluded allocation, rejects overlap or accounting drift, excludes retained ownership, coalesces exact ranges, and binds generation, range/page/gap/exclusion counts, write-back policy, and checksum before and after page-table allocation;
- `native/kernel/src/active_virtual_memory.rs` derives its topology from the manifest, allocates 243 contiguous DMA32 table pages, installs 117,887 supervisor RW/NX write-back leaves across eleven ranges, leaves 12,878 gap pages absent, and audits every admitted leaf plus the inherited kernel, entry, 32-page guarded stack, handoff, sparse-edge, and cache-policy invariants before CR3 activation;
- forged or stale manifests, retained-hole admission, PWT/PCD cache drift, partial table-write failure, activation rollback, premature data-frame reuse, and absent, stale, or mismatched retirement receipts fail closed. Three local `INVLPG` receipts gate the data frame, while exact restoration of the old root creates one root/generation/BSP/local-context-flush receipt before table scrub and release;
- two exact selector-10 fresh-vars qemu64 runs emit 40 markers and pass 46 hostile controls. They agree on coverage checksum `0x341CF729ADB26B52`, 367,474 physical table writes, 950,234 temporary-PTE writes and invalidations, one retirement receipt, and zero remote shootdowns, signatures, authority grants, authorized actions, or production claims. Ninety-one kernel host tests include PMM retained-hole reconstruction and PKVM3 transaction, cache, rollback, and retirement controls;
- `ADD-MEM-002` and `FLAG-N9-VM-DIRECT-MAP-001` are closed only for the demonstrated one-BSP sparse direct map. PKVM3 rejects `active_processor_count != 1` with `ShootdownRequired`; AP startup, bounded vector ownership, local APIC/timer policy, inter-processor shootdown execution, generation-safe remote deferred reclaim, incremental concurrent replacement, huge pages, PCID, COW, user faults, pager IPC, heaps, general MMIO/PAT/MTRR qualification, target hardware, and N9 exit remain open. `N8-IRQ-001` is the next owner-independent move.

Cycle 141 PKSMP5 exact multi-AP and aggregate-shootdown evidence:

- selector `14` now binds one exact four-vCPU `SandyBridge,-avx` TCG topology: BSP APIC ID 0 plus legacy-xAPIC APIC IDs 1, 2, and 3. Every AP owns a private 32-page below-1-MiB runtime, page-table root, guarded stacks, descriptor/IDT/xstate state, APIC mapping table, checksummed mailbox, and two data frames. Local masks are derived as `1 << APIC ID`, producing `0x2`, `0x4`, and `0x8`; the aggregate target and acknowledgement mask is `0xE`;
- a first attempt starts APs 1 and 2, deliberately times out absent APIC ID 4, final-INIT parks both started APs, revokes every alias, scrubs and read-verifies all 96 runtime plus six frame pages, releases all three resource sets, and requires fresh allocation before retry. The second attempt brings all three APs online simultaneously and revalidates each private runtime;
- each AP accepts one diagnostic, one generation-bound shootdown, and one stop request while denying one forged capability. The trampoline validates each local root/page/generation/frame/mask/sequence/attempt request before executing the one linked-image-audited `INVLPG (%rax)` instruction once per AP. Three independently validated acknowledgements form mask `0xE`; reclaim is rejected twice before all unique acknowledgements arrive, then generation 1 retires;
- two exact 40-marker runs pass 30 hostile-control categories covering 243 rejected cases plus 137 kernel host tests. Each run records nine accepted and three denied deliveries, twelve EOIs, three remote invalidations, one partial rollback, one fresh retry, three quiesced and final-INIT-parked APs, and exact release of 102 pages or 417,792 verified bytes. The readiness receipt SHA-256 is `750FD41CD4DF00626657AC49509768FE5B01F259636A2734D53712FA22C72AAB`;
- the canonical kernel is 409,600 bytes in a 458,752-byte, 112-page image with 985 relocations and SHA-256 `8118ED5F7761B9D36A4A65EFF1BC1856C5182D5733CE95A8BEEB24D1C2435F8D`. `FLAG-N8-SMP-MULTI-AP-001` closes only for this exact topology and bounded one-page-per-root generation transition. General topology/x2APIC, hotplug/NUMA/multi-socket, address-space-wide invalidation, concurrent generations, scheduler ownership, production capability authority, additional failure interleavings, target hardware, N8/N9 exit, release, and production remain open. `N12-SCHED-001` is the next chronological owner-independent move.

Cycle 140 PKSMP4 generation-bound remote-shootdown evidence:

- selector `14` retains the proven PKSMP3 transport but expands the checksummed mailbox to bind retired and active generations, AP-owned root, one page, target mask, acknowledgement mask, old/new observations, and deferred-reclaim state. Stale generation, wrong root/page/mask/sequence/attempt, duplicate acknowledgement, malformed checksum, and reclaim-before-ack fail closed;
- the AP fills the old translation before service. After the BSP replaces the leaf and restores its temporary physical-page alias, the AP validates the request, executes exactly one `INVLPG` for virtual page `0x001FF000`, observes the replacement-frame sentinel, acknowledges target mask `0x2`, and permits generation-1 retirement. A linked LLVM disassembly audit proves exactly one `invlpg (%rax)` inside the complete AP trampoline and none elsewhere in that scope;
- the BSP deliberately times out offline APIC ID 2 with target mask `0x4`, then retries the same attempt against APIC ID 1. It rejects release before the exact acknowledgement, releases the old frame only after retirement, later releases the replacement frame, final-INIT parks the AP, and scrubs/read-verifies all 32 runtime pages plus both data frames;
- two exact fresh-vars two-vCPU `SandyBridge,-avx` TCG runs emit 40 markers and pass 25 hostile-control categories covering 169 independently rejected cases plus 132 kernel host tests. Each run records one remote invalidation, one retirement, one premature-reclaim rejection, 34 released resources, and 139,264 verified bytes. The canonical kernel is 409,600 bytes in a 458,752-byte, 112-page image with 969 relocations and SHA-256 `95DDA27784DA944A9C0F5B04029255EDE4DE1BB0684A8EA10DCFC07E686B59A2`;
- `FLAG-N9-SMP-SHOOTDOWN-001` is closed only for one AP-owned root, one page, one target AP, and one generation. General multi-AP masks, address-space-wide invalidation, multiple concurrent generations, incremental concurrent replacement, scheduler ownership, production capability authority, target hardware, N8/N9 exit, release, and production remain open. `FLAG-N8-SMP-MULTI-AP-001` is the next chronological owner-independent move.
...
`ADD-N12-SCHED-SMP-001`, `ADD-N12-SCHED-AP-WORKERS-001`, and
`ADD-N12-SCHED-SMP-PREEMPT-001`. Added in Cycle 147:
`ADD-N12-CONCURRENCY-ATOMICS-001`. Added in Cycle 148:
`ADD-N12-CONCURRENCY-LOCKS-001`. Added in Cycle 149:
`ADD-N12-CONCURRENCY-RECLAMATION-001`.
...
Cycle 142 PKSCHED1 through Cycle 147 PKSCHED6 evidence
partially advance N12.1 through N12.7. The fixed-capacity scheduler core provides
generation-safe task IDs, explicit CPU ownership and affinity, four run queues,
bounded priority bypass, lifecycle transitions, accounting, one
direct-inheritance mutex, bounded references, and a raw spinlock. The host
probe and independent Python oracle agree across 4,096 steps. Two PKSCHED1
qemu64 BSP runs execute the exact 18-instruction cooperative switch and clear
32,768 stack bytes. Two PKSCHED2 runs then execute six real local-APIC timer
windows over four isolated task stacks, reproduce task trace `0,1,2,0,3,3`,
balance six EOIs, save six and restore four interrupt frames, perform four
switches, drain the bounded event queue, retire all four tasks, and clear
65,536 stack bytes. PKSCHED3 then dispatches six typed jobs from one real timer
top half only after EOI, alternates two workers over slots `0,2,4,1,5,6`, makes
twelve exact context transitions, completes five jobs, cancels three, retires
all eight slots, restores interrupt-controller state and mappings, and clears
both retained 16-KiB worker stacks. Seven focused deferred-work tests within 173
kernel tests, five exact independent host-probe receipts, two exact 37-marker
qemu64 runs, and 30 hostile-control categories covering 208 rejected cases
pass. PKSCHED4 then runs four queues on one exact BSP-0/AP-1,2,3 topology. One
cross-CPU wake and two migrations commit after exact remote acknowledgements;
each AP dispatches two local tasks while the BSP dispatches two. One APIC-4
timeout preserves source ownership, rejects its late acknowledgement, and a
stale generation is rejected independently. All eight tasks retire, four idle
owners are observed, and PKSMP5 parks and releases all 102 pages after verifying
417,792 cleared bytes. Eight focused tests within 189 kernel tests, five exact
independent host receipts, two exact 37-marker four-vCPU runs, and 32 hostile
control categories covering 209 rejected cases pass. This closes only
`FLAG-N12-SCHED-SMP-001` for the bounded topology. PKSCHED5 then installs three
AP-local workers and queues, enqueues thirteen fixed work items after one timer
top half, and executes nine typed timer-driver plus three typed
generation-reclaim calls only after EOI. One queued cancellation, one remote
in-flight cancellation, one APIC-4 timeout rollback, stale/late rejection,
flush-before-reclaim, maximum bypass 2, three worker retirements, and exact
102-page cleanup pass. Ten focused tests within 189 kernel tests, six exact
independent host receipts, two exact 37-marker four-vCPU runs, and 34 hostile
control categories covering 226 rejected cases close only
`FLAG-N12-SCHED-AP-WORKERS-001`; no N12 subphase completes and N12.8 remains
unstarted. PKSCHED6 then composes the bounded BSP timer semantics with the
exact-topology scheduler and AP-worker ownership boundary. Four timer, event,
frame, and run-queue lanes deterministically apply cancel, wake, and migration;
eight live reschedule IPIs and five modeled acknowledgements gate ownership;
three quantum switches, one offline rollback, maximum bypass two, watchdog age
two, eight task retirements, and exact 102-page cleanup pass. Five focused
tests within 189 kernel tests, seven exact Rust/Python host receipts, two exact
38-marker four-vCPU runs, and 34 hostile-control categories covering 232
rejected cases close only `FLAG-N12-SCHED-SMP-PREEMPT-001`. AP-local timer
interrupt delivery, general SMP preemption, general topology, arbitrary
callbacks or a general driver/service framework, complete atomics, lock, and
reclamation families, ring 3, address-space and full architectural switching,
target qualification, and N12 exit remain open at the Cycle 147 boundary.

Cycle 148 PKATOM1 completes N12.1 only for the frozen typed x86-64 atomic
contract. Allocation-free `AtomicU32`, `AtomicU64`, `AtomicUsize`, and
`AtomicPtr` wrappers expose operation-specific order types for load, store,
exchange, compare-exchange, weak compare-exchange, integer RMW bit operations,
and fences. The contract admits nine compare-exchange success/failure pairs and
rejects eleven invalid pairs before reaching a platform intrinsic. It adds
overflow-safe reference counting, 4,096-round release/acquire publication,
20,480 contended RMW/CAS operations, and 2,048 sequential-consistency rounds.
Seven linked audit symbols prove the expected `mov`, `xchg`, locked
`cmpxchg`/`xadd`/`or`, and fence mappings without helper calls. Selector 21
proves one live BSP process-publication to interrupt-acquire path, one interrupt
RMW publication, ordering before EOI, exact rollback, and zero leaked atomic
state. Seven focused tests within 196 kernel tests, eight exact independent
Rust/Python host receipts, two exact 41-marker qemu64 runs, and 29
hostile-control categories pass. `FLAG-N12-CONCURRENCY-ATOMICS-001` is closed.
This does not prove a general lock family, live multi-AP contention, deferred
reclamation or ABA safety, non-x86 portability, target qualification, N12 exit,
or production. `ADD-N12-CONCURRENCY-LOCKS-001` and
`FLAG-N12-CONCURRENCY-LOCKS-001` bind the next chronological N12.2 move.

Cycle 149 PKLOCK1 completes N12.2 only for the frozen bounded x86-64 lock
contract. Six allocation-free primitives use PKATOM1: FIFO ticket and IRQ-save
spinlocks, a sleeping mutex with direct one-level donation and seven-bypass
fairness, FIFO notification, a writer-preferred reader-writer lock, and a
seqlock. Try and bounded paths, exact owner checks, recursion and IRQ/preemption
nesting rejection, five ranked order classes, cycle precheck, foreign-guard
rejection, owner death, timeout/cancellation, and rollback are unit and
host-probe covered. Four host threads complete 8,192 FIFO ticket acquisitions;
the other host receipts cover eight-waiter mutex fairness, notification,
three-reader/one-writer contention, and seqlock contention. Selector 22 maps
one shared page into three AP roots, obtains tickets `0,1,2,3` across BSP and
APIC IDs 1, 2, and 3, records four acquisitions and CPU mask `0xF`, drains the
queue, clears ownership, and revokes all three aliases before existing PKSMP5
cleanup. Ten focused tests within 206 kernel tests, two exact 35-marker runs,
and 30 hostile-control categories covering 103 rejected mutations pass.
`FLAG-N12-CONCURRENCY-LOCKS-001` is closed. This does not prove deferred
reclamation or ABA safety, transitive donation, NMI safety, arbitrary topology,
hotplug, non-x86 portability, target qualification, N12 exit, release, or
production. `ADD-N12-CONCURRENCY-RECLAMATION-001` and
`FLAG-N12-CONCURRENCY-RECLAMATION-001` bind the next chronological N12.3 move.

Cycle 150 adds the host-qualified PKRECLAIM1 scoped-object core described
above. Existing pins survive retirement; new readers, stale/foreign handles,
premature reclaim, exhausted generations and pressure-driven reuse are rejected.
The pool returns rejected publications intact, runs no destructor under its
metadata gate, and never forcibly clears pins during shutdown or owner loss.
Generation labels are not scheduler registration, capabilities or CPU-stop
evidence. Remaining substeps, in order: actual task/address-space ownership;
acknowledged remote quiescence and alias/shootdown coupling; timeout, cancellation,
...
| `FLAG-N9-VM-FOUNDATION-001` | REQUIRED | Closed in Cycle 126 | PKVM1 freezes the initial 48-bit layout and proves generation-bound inactive four-level 4 KiB tables, map/protect/unmap rollback, W^X and mixed-cache-alias rejection, inactive-root receipts, deferred reuse, and one exactly revoked PKMAP2 bootstrap alias through host fault tests, two exact 40-marker qemu64 runs, and 39 controls without active-root, SMP-shootdown, heap, pager, N9-exit, or production promotion |
| `FLAG-N9-VM-ACTIVE-001` | REQUIRED | Closed in Cycle 127 | PKVM2 proves a kernel-complete one-BSP candidate root, exact inherited kernel/entry/guarded-stack/handoff mappings, a bounded nine-page direct map, transactional CR3 activation and exact restoration, Accessed/Dirty handling, three local invalidation receipts, release ordering, host fault injection, two exact 40-marker qemu64 runs, and 46 controls without SMP shootdown, ring 3, heap, pager, target, N9-exit, or production promotion |
| `FLAG-N9-PMM-SCRUB-001` | REQUIRED | Closed in Cycle 128 | PKPMM2 proves full-page scrub-before-allocation and scrub-before-reuse with readback, immutable sequence/generation/owner receipts, exact-reuse stale-data rejection, preflighted release, ownership-preserving fault rollback, one revoked temporary alias, 70 host tests, two exact 40-marker qemu64 runs, and 63 controls without predecessor raw-API, reclaim, concurrency, target, N9-exit, or production promotion |
| `FLAG-N9-PMM-METADATA-001` | REQUIRED | Closed in Cycle 129 | PKPMM3 moves the complete 14,616-byte manager into five explicitly reserved, scrubbed, supervisor RW/NX metadata pages with two absent guards, generation/owner binding, exact handoff and final integrity, mapping and corruption rollback, release exclusion, 74 host tests, two exact 41-marker qemu64 runs, and 87 controls without reclaim, scalable growth, concurrency, target, N9-exit, or production promotion |
| `FLAG-N9-PMM-RECLAIM-001` | REQUIRED | Closed in Cycle 130 | PKPMM4 admits 70 Boot Services source records as 12 ranges and 11,250 pages only after `PostExitBootServices`, keeps 11 ACPI pages held, and proves complete streamed preflight, retained/active exclusion, full zero/readback, atomic metadata commit, exact idempotence, content-fault ownership preservation, immutable checksummed receipt, 79 host tests, two exact 42-marker qemu64 runs, and 109 controls without scalable growth, concurrency, target, N9-exit, or production promotion |
| `FLAG-N9-PMM-GROWTH-001` | REQUIRED | Closed in Cycle 131 | PKPMM5 moves all active ledgers into guarded generation-owned pages and proves one explicit atomic 4-to-8-page growth, exact copy/seal/switch/revoke/scrub-retire ordering, precommit rollback, postcommit retirement retry, independent oracle agreement, 82 host tests, two exact 43-marker qemu64 runs, and 137 controls without automatic triggering, concurrency, SMP, target, N9-exit, or production promotion |
| `FLAG-N9-PMM-GROWTH-AUTOMATION-001` | REQUIRED | Closed in Cycle 132 | PKPMM6 checks exact demand before automatic scrubbed allocate/free, preserves rollback/retry headroom, grows through 4/8/15/29-page generations, retires three predecessors, proves four soft fallbacks and one hard pre-effect 58-page rejection, and retains explicit interrupt-context/concurrency/SMP/target/production nonclaims |
| `FLAG-N9-PMM-ACPI-CONSUMER-001` | REQUIRED | Closed in Cycle 133 | PBLIVE4 carries the firmware RSDP; PKACPI1 validates and copies the exact required APIC/FACP/HPET/MCFG set into a retained scrubbed snapshot before an opaque evidence-gated ACPI lifecycle transition; independent reclaim, malformed/duplicate/checksum/range, rollback, readback, idempotence, and nonclaim controls pass |
| `FLAG-N9-VM-DIRECT-MAP-001` | REQUIRED | Closed in Cycle 134 | PKVM3 derives a generation-bound sparse write-back direct map from the complete PMM manifest, excludes retained and gap pages, rejects incompatible cache bits, proves transactional write and CR3 rollback, issues exact local invalidation and generation-retirement receipts, defers table release until receipt validation, and rejects multi-processor retirement pending real shootdown support |
| `FLAG-N8-IRQ-001` | REQUIRED | Partial evidence; open in Cycle 141 | PKIRQ1 closes the bounded one-BSP retained-table, local-xAPIC, vector-ledger, guarded-MMIO, calibrated-HPET/local-timer, exact-delivery/EOI, and normal-rollback slice; PKSMP1, PKSMP2, and PKSMP5 separately close one first-AP lifecycle, one AP-local runtime transaction, and the frozen three-AP development IPI/rollback/shootdown slice. I/O APIC/MSI, production capability authority, complete time services, panic-path rollback, general topology and shootdown, scheduler ownership, and target qualification remain required before closure |
| `FLAG-N8-SMP-FIRST-AP-001` | REQUIRED | Closed in Cycle 136 | PKSMP1 starts exactly one selected AP through a guarded below-1-MiB W^X transaction, observes long-mode state, commands stop, validates quiescence/checksum, final-INIT parks before ownership release, revokes aliases, scrubs/releases fourteen pages, and preserves explicit general-SMP/IPI/shootdown/target/production nonclaims |
| `FLAG-N8-SMP-PERCPU-RUNTIME-001` | REQUIRED | Closed in Cycle 137 | PKSMP2 installs and hardware-verifies one AP-local GDT/TSS/IDT, guarded RSP0/IST stacks, x87/SSE owner state, and 27 owned gates, then quiesces, final-INIT parks, revokes, and scrubs/releases all 32 pages while preserving explicit IPI/shootdown/scheduler/target/production nonclaims |
| `FLAG-N8-SMP-IPI-001` | REQUIRED | Closed in Cycle 138 | PKSMP3 proves six fixed development-only IPI operation classes with checksum-bound request/response state, replay controls, acknowledgement, EOI, timeout, panic, stop, park, and exact release while preserving no-arbitrary-callback, no-production-capability, no-scheduler, no-target, and nonproduction boundaries |
| `FLAG-N9-SMP-SHOOTDOWN-001` | REQUIRED | Closed in Cycle 140; extended in Cycle 141 | PKSMP4 proves one generation-bound AP-side `INVLPG` for one AP-owned root/page/generation. PKSMP5 extends the bounded closure to three private AP-owned roots, one page per root, aggregate target/ack mask `0xE`, stale/duplicate/pre-ack-reclaim rejection, one retirement, and exact frame release without claiming general or address-space-wide shootdown, concurrent generations, scheduler, target, N8/N9 exit, or production promotion |
| `FLAG-N8-SMP-MULTI-AP-001` | REQUIRED | Closed in Cycle 141 | PKSMP5 runs three private AP runtimes simultaneously on one exact four-vCPU legacy-xAPIC QEMU topology, proves APIC-4 partial-start timeout, complete park/scrub/release rollback, fresh retry, dynamic local masks and aggregate acknowledgement, final quiesce/park, and exact 102-page release while preserving no-general-topology/no-scheduler/no-target/no-production boundaries |
...
acquisitions plus mutex, notification, reader-writer, and seqlock contention.
Two exact 35-marker four-vCPU Sandy Bridge QEMU/OVMF runs prove one shared
ticket lock across BSP-0 and AP-1,2,3 with exact tickets `0,1,2,3`, CPU mask
`0xF`, four acquisitions, drained queue, cleared owner, and three aliases
installed and revoked before the existing cleanup path. Ten focused tests
within 206 kernel tests and 30 hostile-control categories covering 103 rejected
mutations pass. The canonical kernel is 513,680 bytes in a 585,728-byte,
143-page image with 1,295 relocations and SHA-256
`9029AEE51A4D557EF5B29945985E4A1F07C67DDE9C8C367C80BD1B9EDD9D409E`.
N12.2 is complete only for this exact bounded lock scope. Deferred reclamation
and ABA-safe lifetime, general SMP and topology, ring 3, address-space
switching, target qualification, N12 exit, release, and production remain
open. `N0-HW-KEY-ACQUIRE-001` remains the blocked external move and
`N12-CONCURRENCY-RECLAMATION-001` is the next owner-independent engineering
move. No key, signature, public-key publication, privileged host probe, driver
load, firmware change, physical-media write, tag, release, or production
promotion occurred.

Cycle 148 closes only `FLAG-N12-CONCURRENCY-ATOMICS-001`. PKATOM1 adds
selector 21 and freezes allocation-free typed `u32`, `u64`, `usize`, and
pointer atomics with operation-specific load, store, RMW, compare-exchange, and
fence orders. Nine compare-exchange order pairs are admitted and eleven are
rejected. Host qualification completes 4,096 release/acquire publication
rounds, 20,480 contended RMW/CAS operations, 2,048 sequential-consistency
rounds, and overflow-safe reference-count checks. Seven linked audit symbols
prove the expected x86-64 instruction classes. Two exact 41-marker qemu64
boots prove process-to-interrupt publication, interrupt RMW, ordering before
EOI, rollback, and cleanup on one BSP. Seven focused tests within 196 kernel
host tests, eight independent Rust/Python receipts, and 29 hostile-control
categories pass. The canonical kernel remains 513,672 bytes in a 585,728-byte,
143-page image with 1,289 relocations and SHA-256
`3CBDF56E90D957E62FC35EAEFF376580BEBDA3623FE4591AB6718984AB258EB7`.
N12.1 is complete only for this exact x86-64 scope. General locks, live
multi-AP atomic contention, reclamation and ABA-safe lifetime, non-x86
portability, target qualification, N12 exit, release, and production remain
open. The aggregate PooleBoot validator also fail-closes QEMU stderr to either
empty output or the exact reviewed 352-byte environmental warning digest
`F309CA71408C82207697AE48F7B8398D097A66577F77F66A15FE64D7C5D50864`;
an altered digest is rejected and raw host paths are not published.
`N0-HW-KEY-ACQUIRE-001` remains the blocked external move and
`N12-CONCURRENCY-LOCKS-001` is the next owner-independent engineering move. No
key, signature, public-key publication, privileged host probe, driver load,
firmware change, physical-media write, tag, release, or production promotion
occurred.

Cycle 147 closes only `FLAG-N12-SCHED-SMP-PREEMPT-001`. PKSCHED6 adds selector 20 and composes PKSCHED2 timer/wakeup semantics with PKSCHED4/PKSCHED5 ownership on the exact BSP-0/AP-1,2,3 topology. Four bounded timer/event/frame/run-queue lanes apply deterministic cancel, wake, and migration ordering. Eight live reschedule IPIs and five modeled exact acknowledgements gate ownership; three two-tick quantum switches complete; one APIC-4 timeout restores source ownership and rejects its late acknowledgement; maximum bypass and watchdog age remain two. All eight tasks retire, all timer/frame owners are revoked, all three APs park, and 102 pages or 417,792 bytes are scrubbed, verified, and released. Five focused tests within 189 kernel host tests, seven exact independent Rust/Python host receipts, two exact 38-marker four-vCPU boots, and 34 hostile-control categories covering 232 rejected cases pass. The canonical kernel is 513,672 bytes in a 585,728-byte, 143-page image with 1,264 relocations and SHA-256 `FCE5C1F2478651D010A2F2781B80494FD0D9721880D33CE8C66A499B35C8DAB6`. AP-local timer/frame lanes are bounded semantic inputs, not AP-local timer interrupt delivery or general SMP. General topology, complete atomics/lock/reclamation families, ring-3/address-space switching, complete per-task architectural ownership, target qualification, N12 exit, release, and production remain open. `N0-HW-KEY-ACQUIRE-001` remains the blocked external move and `N12-CONCURRENCY-ATOMICS-001` is the next owner-independent engineering move. No key, signature, public-key publication, privileged host probe, driver load, firmware change, physical-media write, tag, release, or production promotion occurred.

Cycle 146 closes only `FLAG-N12-SCHED-AP-WORKERS-001`. PKSCHED5 adds selector 19 and composes PKSCHED3 deferred work with PKSCHED4's exact BSP-0/AP-1,2,3 topology. Three allocation-free AP-local queues and workers process thirteen enqueued items through twelve fixed typed deliveries: nine timer-driver calls and three generation-reclaim calls. Dispatch is forbidden before the matching EOI; one queued cancellation, one remote in-flight cancellation, one APIC-4 timeout with source-queue rollback, late and stale acknowledgement rejection, flush-before-reclaim, maximum high-priority bypass 2, three worker retirements, and exact cleanup all pass. Ten focused tests within 184 kernel host tests, six exact independent Rust/Python host receipts, two exact 37-marker four-vCPU boots, and 34 hostile-control categories covering 226 rejected cases pass. The canonical kernel is 485,000 bytes in a 557,056-byte, 136-page image with 1,222 relocations and SHA-256 `D11591395FDD8CD7BEEFA0D847A5C99EB133ED15F8DCDDE3392BFA499DCEDC33`. This is a bounded typed-consumer profile, not an arbitrary callback API or complete driver/service runtime. General topology, general SMP timer preemption, ring-3/address-space switching, complete per-task architectural ownership, target qualification, N12 exit, release, and production remain open. `N0-HW-KEY-ACQUIRE-001` remains the blocked external move and `N12-SCHED-SMP-PREEMPT-001` is the next owner-independent engineering move. No key, signature, public-key publication, privileged host probe, driver load, firmware change, physical-media write, tag, release, or production promotion occurred.

Cycle 143 closes only `FLAG-N12-SCHED-PREEMPT-001`. PKSCHED2 adds selector 16 and composes PKSCHED1 with the PKIRQ1 local-APIC timer through an allocation-free eight-event controller and exact 176-byte interrupt frames. Two exact 35-marker qemu64 BSP runs open six one-shot timer windows and reproduce task trace `0,1,2,0,3,3` with causes `none,quantum,wake,block,wake,none`; six frames are saved, four are restored, six EOIs are issued, four hardware switches occur, and each of four isolated 16-KiB task stacks enters exactly once. The pending event count returns to zero, all four tasks retire, LAPIC/HPET mappings are revoked, controller state is restored, and 65,536 stack bytes are cleared. Seven focused tests within 158 kernel host tests, 25 hostile-control categories, and 178 rejected cases pass. The canonical kernel is 443,504 bytes in a 557,056-byte, 136-page image with 1,086 relocations and SHA-256 `A5DE1DBD2ECA9243D90C2EAA2BEDCAC4B0FCC5E4A4779073E398C6722F30B943`. PKSCHED2 is bounded BSP development evidence, not the complete N12 scheduler. Deferred reclamation/workers, live AP dispatch, cross-CPU migration, ring-3/address-space switching, complete per-task FS/GS/xstate/debug/PMU ownership, target qualification, N12 exit, release, and production remain open. `N0-HW-KEY-ACQUIRE-001` remains the blocked external move and `N12-SCHED-DEFERRED-001` is the next owner-independent engineering move. No key, signature, public-key publication, privileged host probe, driver load, firmware change, physical-media write, tag, release, or production promotion occurred.

Cycle 142 closes only `FLAG-N12-SCHED-FOUNDATION-001`. PKSCHED1 freezes an allocation-free four-CPU/eight-task core with generation-safe task identity, four deterministic queues, priorities 1-31, maximum bypass 7, affinity, modeled migration, accounting, lifecycle transitions, exact cancellation and timeout wake delivery, one-mutex direct priority inheritance, bounded references, and a raw spinlock. A deterministic Rust/Python host campaign agrees over 4,096 steps, 1,761 dispatches, 2,334 migrations, and checksum `0x23B76E2F80E2B747`. Two exact 17-marker qemu64 BSP runs execute eight cooperative dispatches and sixteen transitions across two distinct 16-KiB stacks through an exact linked-image-audited 18-instruction, 36-byte switch, preserve one CR3 and the declared excluded state, retire both contexts, and clear 32,768 stack bytes. Fourteen scheduler tests within 151 kernel host tests and 28 hostile-control categories covering 115 rejected cases pass. The canonical kernel is 425,984 bytes in a 507,904-byte, 124-page image with 1,042 relocations and SHA-256 `AFED4AF858404D83CD77215C118F8478C88E91BDDC0F0B1ABAC3C9324B6ED602`. This is a cooperative BSP foundation, not the complete N12 scheduler. Interrupt timer/wakeup preemption, deferred work, live AP dispatch, cross-CPU migration, ring-3/address-space switching, per-task FS/GS and full xstate/debug/PMU ownership, general locks, target qualification, N12 exit, release, and production remain open. `N0-HW-KEY-ACQUIRE-001` remains the blocked external move and `N12-SCHED-PREEMPT-001` is the next owner-independent engineering move. No key, signature, public-key publication, privileged host probe, driver load, firmware change, physical-media write, tag, release, or production promotion occurred.

Cycle 141 closes only `FLAG-N8-SMP-MULTI-AP-001`. PKSMP5 scales the proven one-AP runtime to one exact four-vCPU topology with three private AP runtimes, dynamic local masks `0x2/0x4/0x8`, aggregate target/ack mask `0xE`, one injected APIC-4 partial-start timeout, complete park/scrub/release rollback, fresh retry, three one-page remote invalidations, acknowledgement-gated retirement, and exact final cleanup. Two exact 40-marker runs, 30 hostile-control categories covering 243 rejected cases, and 137 kernel host tests pass; 102 pages or 417,792 bytes are scrubbed, verified, and released. The canonical kernel is 409,600 bytes in a 458,752-byte, 112-page image with 985 relocations and SHA-256 `8118ED5F7761B9D36A4A65EFF1BC1856C5182D5733CE95A8BEEB24D1C2435F8D`. This is not general topology or a general shootdown subsystem. x2APIC/hotplug/NUMA, address-space-wide invalidation, concurrent generations, scheduler ownership, production capability authority, target qualification, N8/N9 exit, release, and production remain open. `N0-HW-KEY-ACQUIRE-001` remains the blocked external move and `N12-SCHED-001` is the next owner-independent engineering move.

Cycle 140 closes only `FLAG-N9-SMP-SHOOTDOWN-001`. PKSMP4 extends the proven one-AP transport with one AP-owned root, one page at `0x001FF000`, one generation transition, exact target and acknowledgement masks, one linked-image-audited remote `INVLPG`, bounded offline timeout with same-attempt retry, premature-reclaim denial, and deferred old-frame release. Two exact 40-marker runs, 25 hostile-control categories covering 169 rejected cases, and 132 kernel host tests pass; 34 pages or 139,264 bytes are scrubbed, verified, and released. The canonical kernel is 409,600 bytes in a 458,752-byte, 112-page image with 969 relocations and SHA-256 `95DDA27784DA944A9C0F5B04029255EDE4DE1BB0684A8EA10DCFC07E686B59A2`. This is not general SMP or a general shootdown subsystem. Multi-AP topology, startup, rollback, masks, and teardown; address-space-wide invalidation; multiple concurrent generations; scheduler ownership; production capability authority; target qualification; N8/N9 exit; release; and production remain open. `N0-HW-KEY-ACQUIRE-001` remains the blocked external move and `N8-SMP-MULTI-AP-001` is the next owner-independent engineering move.
...
| 31 | `N9-VM-001` | Cycle 126 complete bounded foundation: frozen 48-bit layout, generation-bound inactive four-level 4 KiB tables, bounded map/protect/unmap and rollback, W^X/cache-alias rejection, inactive receipts, one revoked bootstrap mapping, two exact runs, and no active-root/SMP/heap/pager/target/production promotion |
| 32 | `N9-VM-ACTIVE-001` | Cycle 127 complete bounded move: kernel-complete one-BSP candidate root, exact inherited mappings, nine-page direct map, transactional CR3 activation/restoration, Accessed/Dirty handling, three local invalidation receipts, exact release, two exact runs, and no SMP/ring-3/heap/pager/target/production promotion |
| 33 | `N9-PMM-SCRUB-001` | Cycle 128 complete bounded move: scrub-before-allocation and scrub-before-reuse, full zero/readback, immutable receipts, exact-reuse residue rejection, ownership-preserving faults, one revoked alias, two exact runs, and no reclaim/concurrency/target/production promotion |
| 34 | `N9-PMM-METADATA-001` | Cycle 129 complete bounded move: five-page guarded generation-owned allocator metadata arena, complete-manager transactional handoff, integrity seal, corruption and mapping rollback, exact release exclusion, two exact runs, and no reclaim/scalable-growth/concurrency/target/production promotion |
| 35 | `N9-PMM-RECLAIM-001` | Cycle 130 complete bounded move: lifecycle-gated Boot Services admission, ACPI hold, complete streamed preflight, retained/active exclusion, full zero/readback, atomic metadata commit, ownership-preserving faults, exact idempotence, immutable receipt, two exact runs, and no scalable-growth/concurrency/target/production promotion |
| 36 | `N9-PMM-GROWTH-001` | Cycle 131 complete bounded move: generation-owned guarded active ledgers, explicit atomic 4-to-8-page growth, old-generation revoke/scrub/retire, rollback/retry, integrity, independent oracle, two exact runs, and no automatic-trigger/concurrency/SMP/target/production promotion |
| 37 | `N9-PMM-GROWTH-AUTOMATION-001` | Cycle 132 complete bounded move: checked automatic 4/8/15/29-page growth, rollback/retry headroom, three predecessor retirements, soft window fallback, hard pre-effect 58-page rejection, independent oracle, two exact runs, and no interrupt-context/concurrency/SMP/target/production promotion |
| 38 | `N9-PMM-ACPI-CONSUMER-001` | Cycle 133 complete bounded move: PBLIVE4 firmware RSDP, exact required-table validation/copy/readback, retained scrubbed snapshot, opaque lifecycle evidence, independent ACPI reclaim receipt, malformed/rollback/idempotence controls, and no-AML/no-SMP/no-target/nonproduction boundary |
| 39 | `N9-VM-DIRECT-MAP-001` | Cycle 134 complete bounded move: PMM-derived generation-owned sparse direct map, exact retained/gap/cache controls, transactional writes and CR3 rollback, local invalidation receipts, generation-retirement receipt, deferred table release, explicit SMP-shootdown dependency, and nonproduction boundary |
| 40 | `N8-IRQ-001` | Cycle 135 complete bounded portion: retained MADT/HPET discovery, one-BSP local xAPIC identity, 51-vector ownership, guarded uncacheable MMIO, PIC masking, checked one-shot timer calibration, eight exact deliveries/EOIs, normal-path rollback, two exact runs, and no AP/IPI/I/O-APIC/MSI/target/production promotion; the encompassing flag remains open |
| 41 | `N8-SMP-FIRST-AP-001` | Cycle 136 complete bounded move: one selected AP, guarded below-1-MiB W^X trampoline/stack/mailbox, INIT-SIPI-SIPI, observed long-mode state, stop/quiesce/final-INIT park, exact alias revocation and 57,344-byte scrub/release, two exact runs, and no general-SMP/IPI/shootdown/target/production promotion |
| 42 | `N8-SMP-PERCPU-RUNTIME-001` | Cycle 137 complete bounded move: one AP-local GDT/TSS/IDT, guarded RSP0/IST1/IST2, x87/SSE owner, 27 owned gates, exact hardware-state validation, final INIT park, 131,072-byte scrub/release, two exact runs, and no IPI/shootdown/scheduler/target/production promotion |
| 43 | `N8-SMP-IPI-001` | Cycle 138 complete bounded move: six fixed development-only IPI classes, checksum-bound request/response, replay controls, six accepted and four denied deliveries, ten EOIs, one offline timeout, panic/stop/park/release evidence, and no real shootdown/arbitrary callback/scheduler/target/production promotion |
| 44 | `N9-SMP-SHOOTDOWN-001` | Cycle 140 complete bounded move: one AP-owned root/page/generation, exact target/ack masks, one linked AP-side `INVLPG`, offline timeout with same-attempt retry, premature-reclaim denial, one retirement receipt, exact 34-page release, and no general-SMP/general-shootdown/scheduler/target/production promotion |
| 45 | `N8-SMP-MULTI-AP-001` | Cycle 141 complete bounded move: three private AP runtimes on one exact four-vCPU legacy-xAPIC topology, APIC-derived local masks, aggregate target/ack mask `0xE`, partial-start timeout and complete rollback, fresh retry, three one-page remote invalidations, final park, exact 102-page release, and no general-topology/general-shootdown/scheduler/target/production promotion |
| 46 | `N12-SCHED-001` | Cycle 142 complete bounded move: fixed-capacity neutral scheduler core, generation-safe task lifecycle, deterministic host-oracle stress, selected synchronization/lifetime primitives, and exact cooperative BSP two-task context switch with stack isolation and cleanup; no interrupt preemption, live AP dispatch, ring 3, address-space switch, target, N12-exit, or production promotion |
| 47 | `N12-SCHED-PREEMPT-001` | Cycle 143 complete bounded move: PKIRQ1-driven BSP timer and higher-priority wakeup preemption through exact interrupt frames, deterministic task/cause trace, balanced EOIs, context/stack ownership, rollback, cleanup, and 178 hostile cases without AP/ring-3/address-space/target/production overclaim |
| 48 | `N12-SCHED-DEFERRED-001` | Cycle 144 complete bounded move: allocation-free typed interrupt-deferred work, EOI-gated dispatch, two fixed BSP workers, cancellation/flush/rollback/shutdown, exact cleanup, and no arbitrary callback/AP/ring-3/target/production promotion |
| 49 | `N12-SCHED-SMP-001` | Cycle 145 complete bounded move: four local queues, acknowledgement-gated cross-CPU wake/migration, live AP dispatch, offline rollback, stale rejection, idle ownership, exact 102-page teardown, and no general-topology/ring-3/address-space/target/production promotion |
| 50 | `N12-SCHED-AP-WORKERS-001` | Cycle 146 complete bounded move: three AP-local workers, fixed typed timer-driver and generation-reclaim consumers, EOI gating, remote cancellation, flush-before-reclaim, rollback, starvation bound, retirement, and exact 102-page teardown |
| 51 | `N12-SCHED-SMP-PREEMPT-001` | Cycle 147 complete bounded move: four timer/event/frame/run-queue lanes, deterministic cancel/wake/migration ordering, eight live reschedule IPIs, five modeled acknowledgements, three quantum switches, offline rollback, watchdog/fairness bounds, exact 102-page teardown, and no AP-timer/general-SMP/ring-3/target/production promotion |
| 52 | `N12-CONCURRENCY-ATOMICS-001` | Cycle 148 complete bounded move: typed integer and pointer atomics, operation-specific order types, invalid-order rejection, host publication/contention/sequential-consistency litmus, overflow-safe references, seven linked x86-64 audits, one-BSP interrupt publication/RMW ordering, exact cleanup, and no lock/live-multi-AP/reclamation/non-x86/target/production promotion |
| 53 | `N12-CONCURRENCY-LOCKS-001` | Cycle 149 complete bounded move: six allocation-free lock primitives, context/rank/order rules, direct bounded donation, owner death, exact rollback, 8,192 host ticket acquisitions, and exact four-vCPU live contention without reclamation/general-SMP/target/production promotion |
| 54 | `N12-CONCURRENCY-RECLAMATION-001` | Next owner-independent move: define and qualify bounded deferred reclamation and ABA-safe lifetime with generation ownership, acknowledged cross-CPU quiescence, stale-handle/premature-reclaim rejection, failure rollback, and exact-topology evidence |
| 55 | `N13-RING3-001` | First user task, syscall, capability object, exception, and clean exit |
| 56 | `N14-IPC-001` | Capability-mediated call/reply, cancellation, quota, and hostile message tests |
| 57 | `N16-VIRTIO-001` | Isolated virtio console/RNG driver domain with revocation and restart |
