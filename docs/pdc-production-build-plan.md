...
Plan version: 2.34.0-native-kernel-page-scrubbing
Roadmap cycle: PooleOS Cycle 128
...
| Native PKLOAD6 / PBLIVE3 / PKMAP2 / PBEXIT1 integration | Exact live PBC1, PSM1, real 229,376-byte canonical PKELF1, six PBART1, PBTP1, and PBTS1 reads; 155/155 Rust host tests; 2/2 exact PooleBoot, kernel, twelve-file media, and QEMU runs; 25 markers; 155/155 controls; exact 97-entry final PBP1, 66-page kernel and nine-file retained mapping, fourteen-page guarded stack, firmware-boundary agreement, live six-format and PBTRUST1 parsing, and independent oracle agreement | Partial N5.1/N5.4-N5.9 | The default path proves retained pages, successful `ExitBootServices`, unsigned denial, and permanent stop. PKXFER1 separately proves the development transfer; signature trust, authenticated state I/O, capability/action enforcement, final framebuffer remap, production transfer, and N5 exit remain open |
| Native PKENTRY1 PooleKernel | Real 229,376-byte canonical PKELF1 product in a 270,336-byte image; fixed 0x8000 entry; 542 relative relocations; 70/70 host tests; 2/2 exact clean builds; 43/43 hostile controls; exact Rust/Python loaded bytes; canonical SHA-256 `06D36FAF5531509097CD03819390AC744DCFD2BD9FB64D1E6F6710DD7796776E` | Partial N6.4-N6.6 | PKXFER1 through PKVM2 separately prove opt-in virtualized entry and bounded BSP trap/CPU/xstate/MSR/PMM/VM slices, while PKERR1 freezes a pure exact-target policy. Authentication, production PBP1, final framebuffer remap, complete per-CPU exception/xstate/runtime policy, target firmware, and N6 exit remain open |
| Native PKPMM2 physical memory | Exact live 97-entry PBP1 intake; 117,923 conventional usable source pages; 117,922 managed pages; 11,250 reclaimable pages held; 821 loader pages protected; DMA/DMA32/Normal zones; generation-safe scrub-before-allocation and scrub-before-reuse; full zero/readback; four immutable receipts; exact-reuse stale-data rejection; ownership-preserving fault rollback; 8 pages and 32,768 bytes scrubbed and verified; 2/2 exact 40-marker QEMU runs; 63/63 hostile controls | Partial N9.1-N9.2 | One BSP and one temporary page alias only. Predecessor raw allocation APIs, durable mapped metadata, reclaim, concurrency, complete mapping policy, heaps, MMIO/cache policy, pressure/OOM, target, and N9 exit remain open |
| Native PKVM2 virtual memory | Eight generation-bound DMA32 table pages plus one data frame; exact inherited kernel/entry/guarded-stack/handoff mappings; bounded nine-page direct map; transactional candidate CR3 activation and exact restoration; Accessed/Dirty handling; three local invalidation receipts; 2/2 exact 40-marker QEMU runs; 46/46 hostile controls; 8,720 physical table writes; 5,368 bootstrap PTE writes and invalidations | Partial N9.3-N9.4 | One BSP only; no complete direct map, SMP shootdown/deferred reclaim, ring 3, huge pages, PCID, COW, user faults, pager, heap, target, N9-exit, or production claim |
...
| Test suite | Cycle 128: 785 tests pass with one Windows symlink-permission skip, including PKPMM2 scrub/readback/receipt/exact-reuse/fault-rollback controls plus all predecessor controls | Partial N36 | Predominantly host/reference, bounded-model, emulator, and artifact tests; no authenticated authority, target-firmware, or physical-hardware result exists |
| Release gate | Cycle 128: 93/93 consistency checks over 88 artifacts; 20 native gaps | Partial N37 | `production_ready=false`; not a release acceptance gate |
...
| Native kernel | Real reproducible 66-page PooleKernel PKELF1 source and image with host-qualified PKREVAL1 and opt-in two-run QEMU live-entry/revalidation, trap, CPU, xstate, MSR, physical-memory, and virtual-memory traces | Partial N6.4-N7.6/N9.1-N9.4 | No authenticated production entry, complete per-CPU runtime, ring 3, IPC, capability, or driver-domain execution |
...
- live PooleBoot reads exact `TRUST.PBT` and `TRUSTST.PBS` development candidates from the twelve-file medium, cross-binds fourteen facts, emits one `TRUST_STATE DENY` marker, grants zero authority, and writes zero state. The Cycle 128-refreshed PKLOAD6 passes 155/155 Rust tests, 25 markers, and the current integrated hostile corpus across two exact QEMU/OVMF runs;
...
- 70/70 Rust tests, 8/8 Python tests, both `x86_64-unknown-none` and `x86_64-unknown-uefi` builds, 36/36 hostile controls, and 32,768/32,768 deterministic mutation rejects pass. The live six-PBART1 retained set is content-bound by the current PKLOAD6 receipt, with PSM1/PBTP1/PBTS1 bound separately;
- the expanded verifier requires a 270,336-byte PooleKernel image with a 229,376-byte canonical file, entry offset `0x8000`, and 542 relative relocations. PKMAP2 covers 66 kernel pages with 14 read-only, 42 executable, and 10 read-write leaves, then places low guard/stack/high guard at pages 66/67-80/81 and the handoff allocation at page 82;
- PKLOAD6 passes 155/155 Rust tests, two exact PooleBoot and PooleKernel builds, two exact media generations, two QEMU/OVMF producer runs, 25 markers, 155/155 hostile controls, and two independent 5,048-byte PBP1 reconstructions with 97 memory descriptors and ten artifact descriptors;
...
- the Cycle 128-refreshed `runs/native_kernel_entry_readiness.json` records 70/70 host tests, two byte-identical clean linked and canonical builds, 43/43 hostile controls, 542 relative relocations, exact independent Rust/Python loaded bytes, and canonical SHA-256 `06D36FAF5531509097CD03819390AC744DCFD2BD9FB64D1E6F6710DD7796776E`;
...
- two exact fresh-vars qemu64 runs record eight table pages, one data page, exact inherited mappings, a bounded nine-page direct map, exact candidate readback and original-root restoration, probe byte `0xA5`, three active local invalidations, 8,720 physical table writes, 5,368 bootstrap temporary-PTE writes and invalidations after the Cycle 128 kernel-layout rebind, exact allocation release, and zero shootdowns, signatures, authority grants, actions, or production claims;
- selector 10 remains one-BSP development evidence. A durable mapped allocator-metadata arena, held-class reclaim, a complete generation-owned direct map, AP startup, SMP shootdown and deferred reclaim, ring 3, huge pages, PCID, COW, user faults, stack growth, pager IPC, heaps, MMIO/cache qualification, target hardware, and N9 exit remain open.
...
| `FLAG-N9-PMM-METADATA-001` | REQUIRED | Open in Cycle 128 | Move fixed bootstrap PMM maps, extents, allocations, receipts, and ownership metadata into explicitly reserved, mapped, guarded, generation-owned pages with validated handoff, corruption checks, rollback, and release exclusion before held-class reclaim |
...
Cycle 128 closes only `FLAG-N9-PMM-SCRUB-001`. PKPMM2 proves bounded one-BSP scrub-before-allocation and scrub-before-reuse with full zero/readback, immutable receipts, exact-reuse stale-data rejection, release preflight, and ownership-preserving fault rollback through 70 kernel host tests, two exact 40-marker qemu64 runs, and 63 hostile controls. The live runs scrub and verify eight pages and 32,768 bytes, emit four receipts, perform 5,120 physical word writes, 6,144 reads, and 28 temporary-PTE writes and invalidations, revoke the alias, and end with zero allocations, reclaim operations, signatures, authority, or actions. The kernel identity is now 229,376 canonical bytes, 270,336 image bytes, 66 pages, 542 relocations, and SHA-256 `06D36FAF5531509097CD03819390AC744DCFD2BD9FB64D1E6F6710DD7796776E`; all dependent evidence through PKVM2 is rebound. N9.1-N9.4 remain partial. `FLAG-N7-ERRATA-SOURCE-001` and `FLAG-N7-MICROCODE-FLOOR-001` remain open stop-ship flags; durable mapped allocator metadata, held-class reclaim, complete mapping and SMP TLB policy, huge pages, PCID, COW, user faults, pager IPC, heap/object caches, MMIO/cache qualification, concurrency, pressure/OOM policy, exact target qualification, and N9 exit remain open. The selected `hardware_fido2_ed25519_sk` device remains physically unavailable, so `N0-HW-KEY-ACQUIRE-001` is still the immediate blocked external move. The owner's broad operation-category authorization remains recorded, but no key, signature, public-key publication, secret use, privileged host probe, driver load, firmware change, physical-media write, tag/release publication, or production promotion occurred in Cycle 128. The next owner-independent engineering move is `N9-PMM-METADATA-001`: freeze and qualify a reserved mapped metadata arena and bootstrap-to-main allocator handoff before reclaim. PooleGlyph Phase 66 may advance in parallel without outranking N0-N9.
...
| 34 | `N9-PMM-METADATA-001` | Next owner-independent move: reserve and map guarded generation-owned allocator metadata pages, migrate bootstrap ledgers transactionally, bind corruption and rollback evidence, and exclude metadata from allocation/reclaim without claiming reclaim, SMP, target, or production readiness |
| 35 | `N8-IRQ-001` | Local APIC, timer, monotonic clock, and first SMP application processor |
| 36 | `N12-SCHED-001` | Neutral scheduler and context switch under SMP stress |
| 37 | `N13-RING3-001` | First user task, syscall, capability object, exception, and clean exit |
| 38 | `N14-IPC-001` | Capability-mediated call/reply, cancellation, quota, and hostile message tests |
| 39 | `N16-VIRTIO-001` | Isolated virtio console/RNG driver domain with revocation and restart |
