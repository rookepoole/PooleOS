...
Plan version: 2.17.0-native-microcode-semantics
Roadmap cycle: PooleOS Cycle 111
...
Cycle 80 reset architecture claims without discarding validated component evidence. Cycles 81-96 established the native constitution, source boundary, pinned one-host toolchain, owner-controlled governance state, hardware and Tier 0 profiles, bounded formal models, and the PooleGlyph co-development lane. Cycles 97-107 qualified the bounded native boot path through exact profile retention and the permanent pre-transfer stop. Cycles 108-110 froze PINIT1, PREC1, and PSYM1 as the first three independently validated inner formats. Cycle 111 freezes PMCU1 as the fourth: a deterministic wrapper around opaque vendor-authenticated processor-microcode bytes with exact `AuthenticAMD` and CPUID `0x00B40F40` identity, per-patch digests, revision and authenticated rollback floors, highest-eligible normal selection, reset-based exact known-good recovery, no in-session downgrade, BSP/AP timing, mixed-revision failure, and post-apply revision and CPUID verification. Independent Python and allocation-free `no_std` Rust implementations agree across three golden bundles, 174 controls, 16,384 parser cases, 16,384 selection cases, and 8,192 post-apply cases. All 35 payload fixtures are synthetic and marked never-apply. PKLOAD6 validates exact PINIT1, PREC1, PSYM1, and PMCU1 bytes through prohibited-from-production host oracles and raises the aggregate corpus to 124 controls. This is format and policy-model evidence only: no vendor container or production payload is included or authenticated, no privileged revision is observed, and PooleBoot and PooleKernel do not enforce or apply PMCU1.
...
| Native PooleBoot proof | `POOLEOS-N5-POOLEBOOT-7`; 8/8 host tests; 2/2 exact 113,664-byte PE32+ builds; 2/2 exact ten-file GPT/FAT32 images; 2/2 pinned OVMF runs; twenty-three ordered markers; 2/2 dual-channel and GOP-frame matches; 124/124 hostile controls | Partial N5.1-N5.9 | Unsigned seven-artifact load, retained PKMAP2/PBLIVE3 storage, host-only PINIT1/PREC1/PSYM1/PMCU1 validation, successful PBEXIT1, and permanent stop before transfer; no signature trust, PooleBoot inner enforcement, PooleKernel activation/recovery/symbol consumption/microcode application, authenticated state persistence, kernel-entry profile, final CR3/RSP, target firmware, physical media, N5 exit, or production claim |
...
| Native PBART1 profile artifacts | Fixed 96-byte envelope; exact roles 2-7; independent Rust/Python validation; six whole-file and payload digest bindings; distinct zero-padded loader pages; final-map and PBP1 retention | Partial N5.6 | Envelopes are unsigned; initial-system and recovery payloads have frozen host-validated inner formats, but no payload is interpreted or applied by PooleBoot or PooleKernel |
...
| Test suite | Cycle 111: 666 tests pass with one Windows symlink-permission skip, including PMCU1 identity/layout/digest/revision/selection/rollback/apply-plan/post-verify controls, cross-language differential agreement, integrated-oracle, claim-boundary, and Doctor external-report nonmutation controls | Partial N36 | Predominantly host/reference, bounded-model, emulator, and artifact tests; no privileged revision probe, CPU update, or PooleKernel entry trace exists |
| Release gate | Cycle 111: 80/80 consistency checks over 75 artifacts; 20 native gaps | Partial N37 | `production_ready=false`; not a release acceptance gate |
...
Cycle 111 control-plane validation passes 666 tests with one Windows symlink-permission skip. Live Doctor passes 467/467 checks, and the 80/80 release gate passes over 75 artifacts while retaining 20 explicit gaps and `production_ready=false`. Live PooleGlyph conformance output is redirected to temporary storage, preserving the tandem repository's generated report. Exact authority and gate hashes are recorded only after their final deterministic generation.
...
| `FLAG-N5-INIT-SEMANTICS-001` | REQUIRED | Open in Cycle 111 | Extend frozen inner formats and independent validation to firmware manifest and policy, then prove live PINIT1/PREC1/PSYM1/PMCU1 enforcement, authenticated state persistence, capability creation, process/recovery launch, diagnostic authorization, lifecycle, rollback, and apply/execute enforcement |
...
Cycle 111 closes only `FLAG-N5-MICROCODE-BUNDLE-001`: PMCU1 exact CPU identity, package and patch digests, bounded revision/floor selection, reset-based known-good behavior, no in-session downgrade, BSP/AP timing prerequisites, mixed-revision failure, apply-plan gating, post-apply revision/CPUID checks, independent parsing, hostile and differential evidence, and unsigned-development activation denial are frozen and reproducible. All 35 qualification payloads and all canonical revisions are visibly synthetic and marked never-apply. N5 remains partial: PSM1 and all seven artifacts are unsigned and untrusted, PBDIGEST1 review and promotion remain open, no real vendor container or redistribution right is validated, no privileged per-processor revision is observed, PooleBoot does not enforce PINIT1, PREC1, PSYM1, or PMCU1, PooleKernel does not create the declared initial-system objects, execute recovery, consume symbols, or update a CPU, no payload instruction executes, firmware-manifest and policy formats remain open, authenticated persistent state I/O is absent, and the retained CR3/RSP are never installed. N6.4-N6.6 remain partial because no PooleKernel instruction executes under the live caller. `FLAG-N5-INIT-SEMANTICS-001`, `FLAG-N6-BOOT-DIGEST-001`, `FLAG-N6-FRAMEBUFFER-MAP-001`, and `FLAG-NATIVE-BOOT-001` remain open until semantics, trust, final transfer state, kernel consumption, and target profiles are proved. The selected `hardware_fido2_ed25519_sk` device remains unavailable, so `N0-HW-KEY-ACQUIRE-001` remains the immediate blocked owner move and `FLAG-N0-GOVERNANCE-KEY-001` stays open. Codex has no authorization to generate or use keys, sign, merge, tag, register a public key, or publish a release. `FLAG-N4-MODELS-001` also remains open. The next owner-independent engineering move is `N5-FIRMWARE-SEMANTICS-001`: freeze the role-6 firmware-manifest identity, compatibility, dependency, rollback, authority, staging, recovery, and post-update verification rules without calling firmware services or mutating firmware. PooleGlyph Phase 66 may advance in parallel without outranking N0-N6.
...
| 17 | `N5-FIRMWARE-SEMANTICS-001` | Next: freeze the role-6 firmware manifest's component and hardware identity, compatibility graph, update order, trust and license inputs, rollback floors, power/recovery prerequisites, authority model, staging, receipt, and post-update verification without calling firmware services or mutating firmware |
| 18 | `N5-POLICY-SEMANTICS-001` | After firmware semantics: freeze the role-7 policy format, scope and precedence, default-deny rules, capability attenuation, version/rollback binding, safe/recovery overrides, audit requirements, and independent hostile evidence without enforcing policy in PooleBoot or PooleKernel |
| 19 | `N5-TRANSFER-001` | After artifact-semantics and trust prerequisites: validate kernel-entry PBP1, install retained CR3/RSP with interrupts disabled, call the frozen entry ABI, and prove kernel consumption or fail-closed halt |
| 20 | `N7-TRAP-001` | GDT/TSS/IDT and deliberate breakpoint/page-fault evidence after bounded transfer exists |
| 21 | `N9-PMM-001` | Normalized map, bootstrap allocator, physical allocator, guarded mappings |
| 22 | `N8-IRQ-001` | Local APIC, timer, monotonic clock, and first SMP application processor |
| 23 | `N12-SCHED-001` | Neutral scheduler and context switch under SMP stress |
| 24 | `N13-RING3-001` | First user task, syscall, capability object, exception, and clean exit |
| 25 | `N14-IPC-001` | Capability-mediated call/reply, cancellation, quota, and hostile message tests |
| 26 | `N16-VIRTIO-001` | Isolated virtio console/RNG driver domain with revocation and restart |
