...
Plan version: 2.39.0-native-kernel-acpi-consumer
Roadmap cycle: PooleOS Cycle 133
...
| Native PKLOAD6 / PBLIVE4 / PKMAP2 / PBEXIT1 integration | Exact live PBC1, PSM1, real 299,008-byte canonical PKELF1, six PBART1, PBTP1, and PBTS1 reads; 88 kernel host tests plus dependent loader tests; 2/2 exact PooleBoot, kernel, twelve-file media, and QEMU runs; 25 markers; 155/155 controls; exact 97-entry final PBP1, 83-page kernel and nine-file retained mapping, 32-page guarded stack, five-page guarded manager plus alternate guarded ledger windows, firmware RSDP record, firmware-boundary agreement, live six-format and PBTRUST1 parsing, and independent oracle agreement | Partial N5.1/N5.4-N5.9 | The default path proves retained pages, successful `ExitBootServices`, unsigned denial, and permanent stop. PKXFER1 separately proves the development transfer; signature trust, authenticated state I/O, capability/action enforcement, final framebuffer remap, production transfer, and N5 exit remain open |
| Native PKENTRY1 PooleKernel | Real 299,008-byte canonical PKELF1 product in a 339,968-byte image; fixed 0x9000 entry; 693 relative relocations; 88 host tests; 2/2 exact clean builds; 43/43 hostile controls; exact Rust/Python loaded bytes; matching manifest/live build ID; canonical SHA-256 `D9EF9B10B56BF779B155BD18DE55853874CCC032D2A3E5E7841B918F08CDE1F2` | Partial N6.4-N6.6 | PKXFER1 through PKVM2 separately prove opt-in virtualized entry and bounded BSP trap/CPU/xstate/MSR/PMM/ACPI-container/VM slices, while PKERR1 freezes a pure exact-target policy. Authentication, production PBP1, final framebuffer remap, complete per-CPU exception/xstate/runtime policy, target firmware, and N6 exit remain open |
| Native PKPMM7 / PKACPI1 physical memory | Exact live 97-entry PBLIVE4/PBP1 intake; stable manager in five guarded pages; external generation-owned active ledgers; checked automatic growth and retirement; required RSDP/XSDT/APIC/FACP/HPET/MCFG validation; retained one-page ACPI snapshot; independent Boot Services and ACPI reclaim receipts; 117,888 source-usable pages; 129,148 final managed pages; 11,250 Boot Services plus 11 ACPI pages admitted; 856 loader pages protected; 11,473 pages and 46,993,408 bytes scrubbed and verified; 2/2 exact 45-marker QEMU runs; 191/191 hostile controls; 88 kernel host tests | Partial N9.1-N9.2 | One BSP and bounded ledger windows only. No AML, complete ACPI resource graph, interrupt-context/concurrent/SMP allocation, complete mapping policy, heaps, MMIO/cache policy, general pressure/OOM, target, or N9 exit |
| Native PKVM2 virtual memory | Eight generation-bound DMA32 table pages plus one data frame; exact inherited kernel/entry/32-page guarded-stack/handoff mappings; bounded nine-page direct map; transactional candidate CR3 activation and exact restoration; Accessed/Dirty handling; three local invalidation receipts; 2/2 exact 40-marker QEMU runs; 46/46 hostile controls; 8,720 physical table writes; 5,640 bootstrap PTE writes and invalidations | Partial N9.3-N9.4 | One BSP only; no complete direct map, SMP shootdown/deferred reclaim, ring 3, huge pages, PCID, COW, user faults, pager, heap, target, N9-exit, or production claim |
...
| Test suite | Cycle 133: 788 Python tests pass with one Windows symlink-permission skip; 88 Rust kernel host tests include PKACPI1 malformed/duplicate/copy/readback/rollback/early-reclaim/idempotence controls plus the full PKPMM7 predecessor surface | Partial N36 | Predominantly host/reference, bounded-model, emulator, and artifact tests; no authenticated authority, target-firmware, or physical-hardware result exists |
| Release gate | Cycle 133: 93/93 canonical consistency checks over 88 artifacts; Doctor 595/595; 20 native gaps | Partial N37 | `production_ready=false`; not a release acceptance gate |
...
| `FLAG-N9-VM-DIRECT-MAP-001` | REQUIRED | Open in Cycle 133 | Replace the bounded nine-page PKVM2 map with a complete generation-owned sparse physical direct map; exclude holes, forbidden and retained ranges; reject incompatible cache aliases; prove transactional switch/rollback, local invalidation receipts, future SMP-shootdown dependencies, deferred reclaim, and explicit nonproduction boundaries |
...
Cycle 133 closes only the bounded serial scope of `FLAG-N9-PMM-ACPI-CONSUMER-001`. PBLIVE4 carries the UEFI ACPI 2.0 RSDP address. PKACPI1 validates RSDP/XSDT and exactly one APIC, FACP, HPET, and MCFG table, copies 600 bytes into a retained 616-byte snapshot, verifies readback, and issues opaque evidence before the AcpiTablesReleased transition. Independent Boot Services and ACPI reclaim receipts admit 11,250 and eleven pages respectively. Eighty-eight kernel host tests, two exact 45-marker qemu64 runs, and 191 hostile controls pass. The runs finish with 117,888 source-usable and 129,148 managed pages, 856 protected loader pages, one retained snapshot page, 11,473 scrubbed and verified pages, 46,993,408 scrubbed and verified bytes, 5,875,277 writes, 5,879,957 reads, and 23,172 temporary-PTE writes and invalidations. Growth checksum `0xCC30B722527718AE` and the two reclaim receipts bind the sequence; PKVM2 is rebound to 5,640 temporary writes and invalidations. The kernel identity is 299,008 canonical bytes, 339,968 image bytes, 83 pages, 693 relocations, and SHA-256 `D9EF9B10B56BF779B155BD18DE55853874CCC032D2A3E5E7841B918F08CDE1F2`; dependent evidence through PKVM2 is rebound. N9.1-N9.4 remain partial. `FLAG-N7-ERRATA-SOURCE-001` and `FLAG-N7-MICROCODE-FLOOR-001` remain open stop-ship flags; AML and a complete ACPI resource graph, complete sparse direct mapping and SMP TLB policy, huge pages, PCID, COW, user faults, pager IPC, heap/object caches, MMIO/cache qualification, interrupt-context/concurrent allocation, general pressure/OOM policy, exact target qualification, and N9 exit remain open. The selected `hardware_fido2_ed25519_sk` device remains physically unavailable, so `N0-HW-KEY-ACQUIRE-001` is still the immediate blocked external move. The owner's broad operation-category authorization remains recorded, but no key, signature, public-key publication, secret use, privileged host probe, driver load, firmware change, physical-media write, tag/release publication, or production promotion occurred in Cycle 133. The next owner-independent engineering move is `N9-VM-DIRECT-MAP-001`. PooleGlyph Phase 66 may advance in parallel without outranking N0-N9.
...
| 39 | `N9-VM-DIRECT-MAP-001` | Next owner-independent move: complete generation-owned sparse physical direct map with hole/forbidden-range/cache-alias rejection, transactional generation switch/rollback, local invalidation receipts, future SMP-shootdown dependency, deferred reclaim, and nonproduction boundary |
| 40 | `N8-IRQ-001` | Local APIC, timer, monotonic clock, and first SMP application processor |
