...
Plan version: 2.47.0-native-smp-multi-ap
Roadmap cycle: PooleOS Cycle 141
...
Cycle 141 closes the bounded scope of `FLAG-N8-SMP-MULTI-AP-001`. PKSMP5
scales the fixed development IPI and one-page remote-invalidation path to one
exact four-vCPU topology with three private AP runtimes, APIC-derived local
masks, aggregate target/ack mask `0xE`, partial-start rollback, fresh retry,
three remote invalidations, and exact park/scrub/release accounting. General
topology, general shootdown, and scheduler ownership remain open; the next
chronological owner-independent move is `N12-SCHED-001`.
...
| Native PKMSR1 privilege/MSR policy | 2 exact TCG qemu64 QEMU/OVMF runs; 35 markers; 47/47 controls; 11 support-gated MSR reads; 10 MCA banks; 0 MCA bank reads; linked LLVM audit with 25 `RDMSR` and 3 `WRMSR`, including exact PKSMP1/PKSMP2/selector-14/PKIRQ1 isolation | Partial N7.3 | One virtual BSP compatibility model only. No syscall/GS/TSC_AUX activation, machine-check handler, MCA recovery, PMU owner, target AP policy, exact target qualification, N7 exit, or production claim |
| Native PKIRQ1 interrupt/time | Complete retained MADT/HPET walk; BSP xAPIC identity and 51-vector ownership; guarded uncacheable LAPIC/HPET mappings; checked HPET calibration; eight exact one-shot timer deliveries and EOIs; exact normal-path controller, PIC, MSR, clock, and mapping rollback; 137/137 kernel tests; 2/2 exact 36-marker runs; 58/58 controls | Partial N8.1/N8.3 | No I/O APIC route, MSI/MSI-X, general time service, panic-path rollback, general multi-AP interrupt routing, target qualification, N8 exit, or production claim |
| Native PKSMP1 first AP | Retained-MADT APIC-ID selection; guarded fourteen-page below-1-MiB W^X transaction; RX pre-accessed-GDT trampoline; guarded RW/NX stack/mailbox; INIT-SIPI-SIPI; exact long-mode observation; stop/quiesce/final-INIT park; alias revocation and 57,344-byte scrub/release; 137/137 kernel tests; 2/2 exact 38-marker two-vCPU runs; 72/72 controls | Partial N8.5 | One AP lifecycle only. PKSMP2 and PKSMP5 separately prove one AP-local runtime and one frozen three-AP fixed-IPI/shootdown topology; general topology, scheduler ownership, target qualification, N8 exit, and production remain open |
| Native PKSMP2 AP-local runtime | One processor-local GDT/TSS/IDT; guarded four-page RSP0 and two-page IST1/IST2 stacks; x87/SSE owner initialization and round trip; 8 exception plus 19 interrupt gates; hardware busy-TSS and gate readback; 32-page transaction; 137/137 kernel tests; 2/2 exact 42-marker two-vCPU runs; 19/19 control categories and 159/159 rejected hostile cases; 131,072-byte scrub/release | Partial N7.4-N7.6/N8.5 | Exactly one AP in one SandyBridge-minus-AVX TCG profile. PKSMP5 separately scales three private copies with bounded rollback/shootdown; scheduler ownership, general topology, target qualification, N8 exit, and production remain open |
| Native PKSMP5 multi-AP IPI and one-page-per-root shootdown | One exact four-vCPU topology; three private AP runtimes; dynamic local masks `0x2/0x4/0x8`; aggregate target/ack mask `0xE`; one APIC-4 partial-start timeout; complete park/scrub/release rollback; fresh retry; nine accepted and three denied deliveries; twelve EOIs; three executions of one linked-image-audited AP-side `INVLPG`; one generation retirement; two premature-reclaim rejections; 137/137 kernel tests; 2/2 exact 40-marker runs; 30/30 control categories and 243/243 rejected hostile cases; 102 pages and 417,792 bytes scrubbed/released | Partial N8.5-N8.6/N9.4-N9.5 | Frozen topology, one page per private root, and one generation only. No general or address-space-wide shootdown, concurrent generations, arbitrary callback, production capability authority, scheduler ownership, target qualification, N8/N9 exit, or production claim |
...
| Test suite | Cycle 141: 828/828 Python tests pass with one expected Windows symlink-permission skip; 137/137 Rust kernel host tests include three-AP startup, partial-start rollback and fresh retry, dynamic masks, aggregate acknowledgement, forged-capability rejection, generation/root/page/mask validation, deferred reclaim, cleanup, source, and linked-instruction controls plus retained PKSMP2/PKSMP1/PKIRQ1/PMM/PKVM3 coverage | Partial N36 | Predominantly host/reference, bounded-model, emulator, and artifact tests; no authenticated authority, target-firmware, or physical-hardware result exists |
| Release gate | Cycle 141: 97/97 canonical consistency checks pass over 92 artifacts while retaining 20 native gaps | Partial N37 | `production_ready=false`; not a release acceptance gate |
...
| Native kernel | Real reproducible 112-page PooleKernel PKELF1 source and image with host-qualified PKREVAL1 and opt-in QEMU live-entry/revalidation, trap, CPU, xstate, MSR, PKPMM7/PKACPI1 physical-memory, PKVM3 virtual-memory, PKIRQ1 interrupt/time, PKSMP1 first-AP, PKSMP2 AP-local runtime, and PKSMP5 exact four-vCPU/three-AP startup, rollback, IPI, aggregate acknowledgement, and one-page-per-root remote-shootdown traces | Partial N6.4-N8.6/N9.1-N9.4 | No authenticated production entry, production capability authority, scheduler ownership, general topology/shootdown, I/O APIC/MSI routing, AML/complete platform graph, ring 3, IPC, capability, or driver-domain execution |
...
| 46 | `N12-SCHED-001` | Next owner-independent move: freeze a neutral scheduler and context-switch contract, then qualify per-CPU run queues, CPU ownership, affinity, preemption, cancellation, teardown, and bounded priority behavior under SMP stress without ring-3 or production overclaim |
| 47 | `N13-RING3-001` | First user task, syscall, capability object, exception, and clean exit |
| 48 | `N14-IPC-001` | Capability-mediated call/reply, cancellation, quota, and hostile message tests |
| 49 | `N16-VIRTIO-001` | Isolated virtio console/RNG driver domain with revocation and restart |
