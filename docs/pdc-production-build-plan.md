...
Plan version: 2.18.0-native-firmware-semantics
Roadmap cycle: PooleOS Cycle 112
...
Cycle 80 reset architecture claims without discarding validated component evidence. Cycles 81-96 established the native constitution, source boundary, pinned one-host toolchain, owner-controlled governance state, hardware and Tier 0 profiles, bounded formal models, and the PooleGlyph co-development lane. Cycles 97-107 qualified the bounded native boot path through exact profile retention and the permanent pre-transfer stop. Cycles 108-111 froze PINIT1, PREC1, PSYM1, and synthetic-only PMCU1 as the first four independently validated inner formats. Cycle 112 freezes PFWM1 as the fifth: a deterministic synthetic-only role-6 manifest for three external-payload firmware components and two dependency edges, exact resource GUID and hardware-instance identity, current/target/lowest/floor/known-good versions, payload/device/vendor-signer/updater-plugin/recovery identities, one active durable transaction in topological order, 47 ordered dry-run prerequisites, and post-reset receipt and driver-rebind checks. Independent Python and allocation-free `no_std` Rust implementations agree across three golden bundles, 101 controls, 16,384 parser cases, 8,192 activation cases, and 8,192 post-reset cases. PFWM1 contains zero payload bytes, observes no live inventory, and creates no apply authority. PKLOAD6 validates exact PINIT1, PREC1, PSYM1, PMCU1, and PFWM1 bytes through prohibited-from-production host oracles and raises the aggregate corpus to 127 controls. This remains format and policy-model evidence only: no vendor payload, updater, live FMP/ESRT/PLDM inventory, capsule submission, driver load, reset, firmware mutation, or physical-media write exists, and PooleBoot and PooleKernel do not enforce or apply PFWM1.
...
| Native PooleBoot proof | `POOLEOS-N5-POOLEBOOT-7`; 8/8 host tests; 2/2 exact 113,664-byte PE32+ builds; 2/2 exact ten-file GPT/FAT32 images; 2/2 pinned OVMF runs; twenty-three ordered markers; 2/2 dual-channel and GOP-frame matches; 127/127 hostile controls | Partial N5.1-N5.9 | Unsigned seven-artifact load, retained PKMAP2/PBLIVE3 storage, host-only PINIT1/PREC1/PSYM1/PMCU1/PFWM1 validation, successful PBEXIT1, and permanent stop before transfer; no signature trust, PooleBoot inner enforcement, PooleKernel activation/recovery/symbol consumption/update application, authenticated state persistence, kernel-entry profile, final CR3/RSP, target firmware, physical media, N5 exit, or production claim |
...
| Native PBART1 profile artifacts | Fixed 96-byte envelope; exact roles 2-7; independent Rust/Python validation; six whole-file and payload digest bindings; distinct zero-padded loader pages; final-map and PBP1 retention | Partial N5.6 | Envelopes are unsigned; PINIT1, PREC1, PSYM1, PMCU1, and PFWM1 have frozen host-validated inner formats, but no payload is interpreted or applied by PooleBoot or PooleKernel |
...
| Test suite | Cycle 112: 678 tests pass with one Windows symlink-permission skip, including PFWM1 identity/layout/dependency/version-floor/dry-run/post-reset controls, cross-language differential agreement, integrated-oracle, claim-boundary, and Doctor external-report nonmutation controls | Partial N36 | Predominantly host/reference, bounded-model, emulator, and artifact tests; no live inventory, firmware apply, privileged probe, or PooleKernel entry trace exists |
| Release gate | Cycle 112: 81/81 consistency checks over 76 artifacts; 20 native gaps | Partial N37 | `production_ready=false`; not a release acceptance gate |
...
Cycle 112 control-plane validation passes 678 tests with one Windows symlink-permission skip. Live Doctor passes its complete generated check set, and the 81/81 release gate passes over 76 artifacts while retaining 20 explicit gaps and `production_ready=false`. Live PooleGlyph conformance output is redirected to temporary storage, preserving the tandem repository's generated report. Exact authority and gate hashes are recorded only after their final deterministic generation.
...
| `FLAG-N5-INIT-SEMANTICS-001` | REQUIRED | Open in Cycle 112 | Extend frozen inner-format validation to role-7 policy, then prove live PINIT1/PREC1/PSYM1/PMCU1/PFWM1 enforcement, authenticated state persistence, capability creation, process/recovery launch, diagnostic authorization, lifecycle, rollback, and apply/execute enforcement |
...
Cycle 112 closes only `FLAG-N5-FIRMWARE-BUNDLE-001`: PFWM1 exact component, resource, hardware-instance, version-floor, external-payload, signer, updater, recovery, dependency-order, dry-run prerequisite, post-reset receipt, and driver-rebind rules are frozen and reproducible. The canonical manifest is synthetic qualification data, contains zero payload bytes, and is marked never-apply. N5 remains partial: PSM1 and all seven artifacts are unsigned and untrusted, PBDIGEST1 review and promotion remain open, no real vendor payload or redistribution right is validated, no privileged per-processor revision or live firmware inventory is observed, PooleBoot does not enforce PINIT1, PREC1, PSYM1, PMCU1, or PFWM1, PooleKernel does not create initial-system objects, execute recovery, consume symbols, update a CPU, or apply firmware, no payload instruction executes, role-7 policy semantics remain open, authenticated persistent state I/O is absent, and the retained CR3/RSP are never installed. N6.4-N6.6 remain partial because no PooleKernel instruction executes under the live caller. `FLAG-N5-INIT-SEMANTICS-001`, `FLAG-N6-BOOT-DIGEST-001`, `FLAG-N6-FRAMEBUFFER-MAP-001`, and `FLAG-NATIVE-BOOT-001` remain open until semantics, trust, final transfer state, kernel consumption, and target profiles are proved. The selected `hardware_fido2_ed25519_sk` device remains unavailable, so `N0-HW-KEY-ACQUIRE-001` remains the immediate blocked owner move and `FLAG-N0-GOVERNANCE-KEY-001` stays open. Codex has no authorization to generate or use keys, sign, merge, tag, register a public key, or publish a release. `FLAG-N4-MODELS-001` also remains open. The next owner-independent engineering move is `N5-POLICY-SEMANTICS-001`: freeze the role-7 policy format, scope, precedence, default-deny, attenuation, version/rollback, safe/recovery override, and audit rules without enforcing policy in PooleBoot or PooleKernel. PooleGlyph Phase 66 may advance in parallel without outranking N0-N6.
...
| 18 | `N5-POLICY-SEMANTICS-001` | Next: freeze the role-7 policy format, scope and precedence, default-deny rules, capability attenuation, version/rollback binding, safe/recovery overrides, audit requirements, and independent hostile evidence without enforcing policy in PooleBoot or PooleKernel |
