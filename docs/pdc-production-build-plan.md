...
Plan version: 2.14.0-native-init-bundle
Roadmap cycle: PooleOS Cycle 108
...
Cycle 80 reset architecture claims without discarding validated component evidence. Cycles 81-96 established the native constitution, source boundary, pinned one-host toolchain, owner-controlled governance state, hardware and Tier 0 profiles, bounded formal models, and the PooleGlyph co-development lane. Cycles 97-100 qualified the first PooleBoot proof, PBP1, PBC1, and PKELF1. Cycle 101 produced the real reproducible PKENTRY1 PooleKernel image, Cycles 102-104 added bounded live load, PSM1 selection, and temporary PBP1, Cycle 105 validated exact temporary higher-half mappings, Cycle 106 retained the transfer objects through `ExitBootServices`, and Cycle 107 loaded and retained the exact seven-artifact development profile before a permanent pre-transfer stop. Cycle 108 freezes PINIT1 as the first inner artifact format: a deterministic initial-system declaration bundle with typed components and services, a strong/weak dependency DAG, abstract resources, attenuated capability routes, lifecycle policy, and reverse rollback. Independent Python and allocation-free `no_std` Rust parsers agree across three golden vectors, 120 controls, and 16,384 differential cases. PKLOAD6 validates the exact inner PINIT1 bytes through its host oracle and raises the aggregate integration corpus to 115 controls. This is declaration evidence only: PooleBoot does not parse PINIT1, PooleKernel does not activate it, declarations confer no authority, unsigned development media must not activate, and no initial-system instruction executes.
...
| Native PooleBoot proof | `POOLEOS-N5-POOLEBOOT-6`; 8/8 host tests; 2/2 exact 104,960-byte PE32+ builds; 2/2 exact four-file GPT/FAT32 images; 2/2 pinned OVMF runs; twenty-two ordered markers; 2/2 dual-channel and GOP-frame matches; 95/95 hostile controls | Partial N5.1-N5.5/N5.7-N5.8 | Unsigned manifest-bound load, retained PKMAP2/PBLIVE2 storage, successful PBEXIT1, and permanent stop before transfer; no signature trust, kernel-entry profile, final CR3/RSP, target firmware, physical media, N5 exit, or production claim |
...
| Native PBART1 profile artifacts | Fixed 96-byte envelope; exact roles 2-7; independent Rust/Python validation; six whole-file and payload digest bindings; distinct zero-padded loader pages; final-map and PBP1 retention | Partial N5.6 | Envelopes are unsigned; only the initial-system payload has a frozen inner format, and no payload is interpreted or applied by PooleBoot or PooleKernel |
...
| Native PKLOAD6 / PBLIVE3 / PKMAP2 / PBEXIT1 integration | Exact live PBC1, PSM1, real 139,264-byte PKELF1, and six PBART1 reads; 76/76 Rust host tests; 2/2 exact PooleBoot, kernel, ten-file media, and QEMU runs; 23 markers; 115/115 controls; exact 4,728-byte final PBP1, retained mapping, firmware-boundary agreement, and host-only PINIT1 oracle agreement | Partial N5.1/N5.4-N5.6/N5.8 | Kernel/artifact/table/stack/handoff pages survive successful `ExitBootServices`; no signature trust, PooleBoot PINIT1 enforcement, kernel activation, final CR3/RSP, transferable handoff, kernel entry, final cache policy, or N5 exit |
...
| Test suite | Cycle 108: 624 tests pass with one Windows symlink-permission skip, including PINIT1 canonicalization, semantic, hostile, differential, activation-denial, integrated-oracle, claim-boundary, and Doctor external-report nonmutation controls | Partial N36 | Predominantly host/reference, bounded-model, emulator, and artifact tests; no PooleKernel entry trace exists |
| Release gate | Cycle 108: 77/77 consistency checks over 72 artifacts; 20 native gaps | Partial N37 | `production_ready=false`; not a release acceptance gate |
...
Cycle 108 control-plane validation passes 624 tests with one Windows symlink-permission skip. Live Doctor passes 414/414 checks, and the 77/77 release gate passes over 72 artifacts while retaining 20 explicit gaps and `production_ready=false`. Live PooleGlyph conformance output is redirected to temporary storage, preserving the tandem repository's generated report. Exact authority and gate hashes are recorded only after their final deterministic generation.
...
Inherited sections: `013-015`. Added: `ADD-BOOT-001`, `ADD-BOOT-004`, `ADD-BOOT-005`.
...
| `FLAG-N5-INIT-SEMANTICS-001` | REQUIRED | Open in Cycle 108 | Extend frozen inner formats and independent validation to recovery, symbols, microcode, firmware, and policy, then prove live capability creation, process launch, lifecycle, rollback, and apply/execute enforcement |
...
Cycle 108 closes only `FLAG-N5-INIT-BUNDLE-001`: PINIT1 declarations, dependency/capability/resource semantics, lifecycle policy, independent parsing, and unsigned activation denial are frozen and reproducible. N5 remains partial: PSM1 and all seven artifacts are unsigned and untrusted, PBDIGEST1 review and promotion remain open, PooleBoot does not parse PINIT1, PooleKernel does not create the declared objects or enforce activation, no initial-system instruction executes, recovery/symbol/microcode/firmware/policy inner formats remain open, and the retained CR3/RSP are never installed. N6.4-N6.6 remain partial because no PooleKernel instruction executes under the live caller. `FLAG-N5-INIT-SEMANTICS-001`, `FLAG-N6-BOOT-DIGEST-001`, `FLAG-N6-FRAMEBUFFER-MAP-001`, and `FLAG-NATIVE-BOOT-001` remain open until semantics, trust, final transfer state, kernel consumption, and target profiles are proved. The selected `hardware_fido2_ed25519_sk` device remains unavailable, so `N0-HW-KEY-ACQUIRE-001` remains the immediate blocked owner move and `FLAG-N0-GOVERNANCE-KEY-001` stays open. Codex has no authorization to generate or use keys, sign, merge, tag, register a public key, or publish a release. `FLAG-N4-MODELS-001` also remains open. The next owner-independent engineering move is `N5-RECOVERY-SEMANTICS-001`: freeze and independently validate the recovery artifact's state machine, eligibility, rollback, authority, failure, and handoff rules before any recovery payload can execute. PooleGlyph Phase 66 may advance in parallel without outranking N0-N6.
...
| 14 | `N5-RECOVERY-SEMANTICS-001` | Next: freeze the recovery inner format, eligibility and attempt state machine, rollback and known-good rules, authority boundary, failure handling, and independent hostile parser evidence before recovery execution |
| 15 | `N5-TRANSFER-001` | After artifact-semantics and trust prerequisites: validate kernel-entry PBP1, install retained CR3/RSP with interrupts disabled, call the frozen entry ABI, and prove kernel consumption or fail-closed halt |
| 16 | `N7-TRAP-001` | GDT/TSS/IDT and deliberate breakpoint/page-fault evidence after bounded transfer exists |
| 17 | `N9-PMM-001` | Normalized map, bootstrap allocator, physical allocator, guarded mappings |
| 18 | `N8-IRQ-001` | Local APIC, timer, monotonic clock, and first SMP application processor |
| 19 | `N12-SCHED-001` | Neutral scheduler and context switch under SMP stress |
| 20 | `N13-RING3-001` | First user task, syscall, capability object, exception, and clean exit |
| 21 | `N14-IPC-001` | Capability-mediated call/reply, cancellation, quota, and hostile message tests |
| 22 | `N16-VIRTIO-001` | Isolated virtio console/RNG driver domain with revocation and restart |
