...
Plan version: 2.36.0-native-kernel-reclaim
Roadmap cycle: PooleOS Cycle 130
...
| Native PKLOAD6 / PBLIVE3 / PKMAP2 / PBEXIT1 integration | Exact live PBC1, PSM1, real 245,760-byte canonical PKELF1, six PBART1, PBTP1, and PBTS1 reads; 164/164 Rust host tests; 2/2 exact PooleBoot, kernel, twelve-file media, and QEMU runs; 25 markers; 155/155 controls; exact 97-entry final PBP1, 70-page kernel and nine-file retained mapping, fourteen-page guarded stack, five-page guarded metadata arena, firmware-boundary agreement, live six-format and PBTRUST1 parsing, and independent oracle agreement | Partial N5.1/N5.4-N5.9 | The default path proves retained pages, successful `ExitBootServices`, unsigned denial, and permanent stop. PKXFER1 separately proves the development transfer; signature trust, authenticated state I/O, capability/action enforcement, final framebuffer remap, production transfer, and N5 exit remain open |
| Native PKENTRY1 PooleKernel | Real 245,760-byte canonical PKELF1 product in a 286,720-byte image; fixed 0x8000 entry; 593 relative relocations; 79/79 host tests; 2/2 exact clean builds; 43/43 hostile controls; exact Rust/Python loaded bytes; canonical SHA-256 `20BCE1C7501BC2344A6D7505BCDA749D9B2738A8435255D0EC2EF5A3E177CC4C` | Partial N6.4-N6.6 | PKXFER1 through PKVM2 separately prove opt-in virtualized entry and bounded BSP trap/CPU/xstate/MSR/PMM/VM slices, while PKERR1 freezes a pure exact-target policy. Authentication, production PBP1, final framebuffer remap, complete per-CPU exception/xstate/runtime policy, target firmware, and N6 exit remain open |
| Native PKPMM4 physical memory | Exact live 97-entry PBP1 intake; 117,919 conventional usable source pages; 129,168 final managed pages; 70 Boot Services source records coalesced into 12 ranges and 11,250 admitted pages; 11 ACPI pages held; 825 loader pages protected; complete 14,928-byte manager in five retained guarded metadata pages; streamed complete preflight, scrub-before-admission, atomic metadata commit, exact idempotence, content-fault ownership preservation, and immutable checksummed receipt; 11,263 pages and 46,133,248 bytes scrubbed and verified; 2/2 exact 42-marker QEMU runs; 109/109 hostile controls | Partial N9.1-N9.2 | One BSP and bounded fixed ledgers only. Scalable metadata growth, complete ACPI lifecycle integration, concurrency, complete mapping policy, heaps, MMIO/cache policy, pressure/OOM, target, and N9 exit remain open |
| Native PKVM2 virtual memory | Eight generation-bound DMA32 table pages plus one data frame; exact inherited kernel/entry/guarded-stack/handoff mappings; bounded nine-page direct map; transactional candidate CR3 activation and exact restoration; Accessed/Dirty handling; three local invalidation receipts; 2/2 exact 40-marker QEMU runs; 46/46 hostile controls; 8,720 physical table writes; 5,432 bootstrap PTE writes and invalidations | Partial N9.3-N9.4 | One BSP only; no complete direct map, SMP shootdown/deferred reclaim, ring 3, huge pages, PCID, COW, user faults, pager, heap, target, N9-exit, or production claim |
...
| Test suite | Cycle 130: 786 Python tests pass with one Windows symlink-permission skip; the 79 Rust kernel host tests include PKPMM4 lifecycle, positive Boot Services/ACPI admission, idempotence, capacity/arithmetic/retained-overlap rejection, content-fault ownership preservation, streamed-plan bounds, and all predecessor kernel controls | Partial N36 | Predominantly host/reference, bounded-model, emulator, and artifact tests; no authenticated authority, target-firmware, or physical-hardware result exists |
| Release gate | Cycle 130: 93/93 consistency checks over 88 artifacts; 20 native gaps | Partial N37 | `production_ready=false`; not a release acceptance gate |
...
| Native kernel | Real reproducible 70-page PooleKernel PKELF1 source and image with host-qualified PKREVAL1 and opt-in two-run QEMU live-entry/revalidation, trap, CPU, xstate, MSR, physical-memory, and virtual-memory traces | Partial N6.4-N7.6/N9.1-N9.4 | No authenticated production entry, complete per-CPU runtime, ring 3, IPC, capability, or driver-domain execution |
...
- live PooleBoot reads exact `TRUST.PBT` and `TRUSTST.PBS` development candidates from the twelve-file medium, cross-binds fourteen facts, emits one `TRUST_STATE DENY` marker, grants zero authority, and writes zero state. The Cycle 130-refreshed PKLOAD6 passes 164/164 Rust tests, 25 markers, and the current 155-control hostile corpus across two exact QEMU/OVMF runs;
...
- the expanded verifier requires a 286,720-byte PooleKernel image with a 245,760-byte canonical file, entry offset `0x8000`, and 569 relative relocations. PKMAP2 covers 70 kernel pages with 14 read-only, 46 executable, and 10 read-write leaves; places low guard/stack/high guard at pages 70/71-84/85 and the handoff allocation at page 86; reserves temporary page 342; and places metadata guards/pages at 343/344-348/349;
- PKLOAD6 passes 164/164 Rust tests, two exact PooleBoot and PooleKernel builds, two exact media generations, two QEMU/OVMF producer runs, 25 markers, 155/155 hostile controls, and two independent 5,048-byte PBP1 reconstructions with 97 memory descriptors and ten artifact descriptors;
...
- two exact kernel builds, two feature-enabled PooleBoot builds, one default isolation build, two byte-identical media generations, and two fresh-vars QEMU/OVMF runs reproduce all 30 markers, exact PBP1 bytes, and exact guest/host denial. The Cycle 130-refreshed 245,760-byte PooleKernel has SHA-256 `20BCE1C7501BC2344A6D7505BCDA749D9B2738A8435255D0EC2EF5A3E177CC4C`; all 58 hostile controls still pass;
...
- the Cycle 130-refreshed `runs/native_kernel_entry_readiness.json` records 79/79 host tests, two byte-identical clean linked and canonical builds, 43/43 hostile controls, 593 relative relocations, exact independent Rust/Python loaded bytes, and canonical SHA-256 `20BCE1C7501BC2344A6D7505BCDA749D9B2738A8435255D0EC2EF5A3E177CC4C`;
...
- two exact fresh-vars qemu64 runs record eight table pages, one data page, exact inherited mappings, a bounded nine-page direct map, exact candidate readback and original-root restoration, probe byte `0xA5`, three active local invalidations, 8,720 physical table writes, 5,432 bootstrap temporary-PTE writes and invalidations after the Cycle 130 kernel-layout rebind, exact allocation release, and zero shootdowns, signatures, authority grants, actions, or production claims;
- selector 10 remains one-BSP development evidence. Held-class reclaim, a complete generation-owned direct map, AP startup, SMP shootdown and deferred reclaim, ring 3, huge pages, PCID, COW, user faults, stack growth, pager IPC, heaps, MMIO/cache qualification, target hardware, and N9 exit remain open.
...
| `FLAG-N9-PMM-GROWTH-001` | REQUIRED | Open in Cycle 130 | Replace bounded fixed-capacity PMM ledgers with generation-owned scalable metadata pages while preserving guards, integrity, complete preflight, failure atomicity, deterministic accounting, retirement safety, rollback, and explicit concurrency/SMP/target nonclaims |
...
Cycle 130 closes only `FLAG-N9-PMM-RECLAIM-001`. PKPMM4 advances to `PostExitBootServices`, streams complete preflight and full zero/readback over the immutable PBP1 source ledger, then atomically admits 70 Boot Services records as 12 coalesced ranges and 11,250 pages while retaining eleven ACPI pages. It excludes retained and live non-target ranges, returns the exact immutable receipt idempotently without physical access, rejects early ACPI admission, and preserves ownership/free/managed/receipt/sequence metadata after content faults. A scalar reclaim cursor replaced an initial dual-array plan that exhausted the live bootstrap stack after PKREVAL1. Seventy-nine kernel host tests, two exact 42-marker qemu64 runs, and 109 hostile controls pass. The runs finish with 129,168 managed pages, 11,263 scrubbed and verified pages, 46,133,248 scrubbed and verified bytes, 5,767,680 physical word writes, 5,768,704 reads, and 22,543 temporary-PTE writes and invalidations. Checksums `0x5A485D4A5725EED8` and `0x4D3EBF743B7F2CCC` bind the range and receipt. The kernel identity is 245,760 canonical bytes, 286,720 image bytes, 70 pages, 593 relocations, and SHA-256 `20BCE1C7501BC2344A6D7505BCDA749D9B2738A8435255D0EC2EF5A3E177CC4C`; dependent evidence through PKVM2 is rebound. N9.1-N9.4 remain partial. `FLAG-N7-ERRATA-SOURCE-001` and `FLAG-N7-MICROCODE-FLOOR-001` remain open stop-ship flags; scalable metadata growth, complete ACPI consumer integration, complete mapping and SMP TLB policy, huge pages, PCID, COW, user faults, pager IPC, heap/object caches, MMIO/cache qualification, concurrency, pressure/OOM policy, exact target qualification, and N9 exit remain open. The selected `hardware_fido2_ed25519_sk` device remains physically unavailable, so `N0-HW-KEY-ACQUIRE-001` is still the immediate blocked external move. The owner's broad operation-category authorization remains recorded, but no key, signature, public-key publication, secret use, privileged host probe, driver load, firmware change, physical-media write, tag/release publication, or production promotion occurred in Cycle 130. The next owner-independent engineering move is `N9-PMM-GROWTH-001`: qualify generation-owned scalable metadata storage, guarded growth, safe retirement, integrity, failure atomicity, deterministic accounting, and rollback. PooleGlyph Phase 66 may advance in parallel without outranking N0-N9.
...
| 36 | `N9-PMM-GROWTH-001` | Next owner-independent move: generation-owned scalable PMM ledgers with guarded growth, retirement safety, integrity, complete preflight, failure atomicity, deterministic accounting, and rollback without concurrency/SMP/target/production promotion |
| 37 | `N8-IRQ-001` | Local APIC, timer, monotonic clock, and first SMP application processor |
| 38 | `N12-SCHED-001` | Neutral scheduler and context switch under SMP stress |
| 39 | `N13-RING3-001` | First user task, syscall, capability object, exception, and clean exit |
| 40 | `N14-IPC-001` | Capability-mediated call/reply, cancellation, quota, and hostile message tests |
| 41 | `N16-VIRTIO-001` | Isolated virtio console/RNG driver domain with revocation and restart |
