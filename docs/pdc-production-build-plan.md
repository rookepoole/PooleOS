...
Plan version: 2.30.0-native-kernel-privilege-msr-policy
Roadmap cycle: PooleOS Cycle 124
...
| Test suite | Cycle 124: 773 tests pass with one Windows symlink-permission skip, including PKMSR1 policy/source/machine-code/TCG controls plus all PKXEXC1, PKXSTATE1, PKERR1, PKCPU1, PKTRAP1, PKXFER1, PKREVAL1, and producer controls | Partial N36 | Predominantly host/reference, bounded-model, emulator, and artifact tests; no authenticated authority, target-firmware, or physical-hardware result exists |
| Release gate | Cycle 124: 91/91 consistency checks over 86 artifacts; 20 native gaps | Partial N37 | `production_ready=false`; not a release acceptance gate |
...
- live PooleBoot reads exact `TRUST.PBT` and `TRUSTST.PBS` development candidates from the twelve-file medium, cross-binds fourteen facts, emits one `TRUST_STATE DENY` marker, grants zero authority, and writes zero state. The Cycle 124-refreshed PKLOAD6 passes 134/134 Rust tests, 25 markers, and the current integrated hostile corpus across two exact QEMU/OVMF runs;
...
- 49/49 Rust tests, 8/8 Python tests, both `x86_64-unknown-none` and `x86_64-unknown-uefi` builds, 36/36 hostile controls, and 32,768/32,768 deterministic mutation rejects pass. After the Cycle 124 PSYM1/PPOL1 identity refresh, the golden nine-file retained-set SHA-256 is `1F2BE74362DEE662AE5E7E9B711EA2060DD659E74548421528637D1904DE9D05`;
- the expanded verifier requires a 262,144-byte PooleKernel image with a 180,224-byte canonical file, entry offset `0x8000`, and, after the Cycle 124 identity refresh, 361 relative relocations. PKMAP2 covers 64 kernel pages with 12 read-only, 32 executable, and 20 read-write leaves, then places low guard/stack/high guard at pages 64/65-72/73 and the handoff allocation at page 80;
- PKLOAD6 passes 134/134 Rust tests, two exact PooleBoot and PooleKernel builds, two exact media generations, two QEMU/OVMF producer runs, 25 markers, 155/155 hostile controls, and two independent 5,008-byte PBP1 reconstructions with 96 memory descriptors and ten artifact descriptors;
...
- two exact kernel builds, two feature-enabled PooleBoot builds, one default isolation build, two byte-identical media generations, and two fresh-vars QEMU/OVMF runs reproduce all 30 markers, exact PBP1 bytes, and exact guest/host denial. The Cycle 124-refreshed 180,224-byte PooleKernel has SHA-256 `062D4EE10BA27F4D0A943D97206ADB5B3761770B6DD9EEC2C281605B2693B883`; all 58 hostile controls still pass;
...
Cycle 124 closes only `FLAG-N7-PRIVILEGE-MSR-POLICY-001`. PKMSR1 proves a support-gated, read-only qemu64 BSP inventory of system-linkage, FS/GS, optional TSC_AUX, global machine-check, and PMU state. Two exact TCG runs, 35 markers, 47 hostile controls, eleven live MSR reads, and a hash-bound linked audit pass with zero activation/write instructions, signatures, authority, actions, post-exit firmware calls, or physical-media effects. N7.3 is partial, not complete. `FLAG-N7-ERRATA-SOURCE-001` and `FLAG-N7-MICROCODE-FLOOR-001` remain open stop-ship flags; syscall and GS/TSC_AUX activation, machine-check and PMU ownership, AP-local state, AVX/extended state, scheduler and user-task delivery, complete per-CPU and guarded trap state, exact target qualification, and N7 exit remain open. The selected `hardware_fido2_ed25519_sk` device remains physically unavailable, so `N0-HW-KEY-ACQUIRE-001` is still the immediate blocked external move. No key, signature, public-key publication, secret use, privileged host probe, driver load, firmware change, physical-media write, tag/release publication, or production promotion occurred in Cycle 124. The next owner-independent engineering move is `N9-PMM-001`: freeze normalized-memory-map ownership and implement the bootstrap and physical-page allocation boundary needed before broader CPU lifecycle work. PooleGlyph Phase 66 may advance in parallel without outranking N0-N9.
...
| 30 | `N9-PMM-001` | Next owner-independent move: freeze normalized-map ownership and implement the bootstrap allocator, physical-page allocator, guarded mappings, and exact allocator evidence |
