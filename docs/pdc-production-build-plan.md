...
Plan version: 2.28.0-native-kernel-xstate-policy
Roadmap cycle: PooleOS Cycle 122
...
| Native PKXSTATE1 x87/SSE ownership | Eager standard `XSAVE64`/`XRSTOR64`; XCR0 `0x3`; XSS zero; 4,096-byte aligned owner images; canonical FCW/MXCSR; 31/31 kernel tests; 2 exact EPYC-Rome-v4 QEMU/OVMF runs; 35 markers; 43/43 controls; 2 saves; 4 restores; 8,192 cleared image bytes; 3 allowlisted privileged configuration writes | Partial N7.4 | One-BSP emulator evidence only. AVX/extended state, deliberate xstate exceptions, scheduler/thread integration, AP initialization, migration, final machine-code SIMD audit, target hardware, N7 exit, and production remain open |
...
| Test suite | Cycle 122: 761 tests pass with one Windows symlink-permission skip, including PKXSTATE1 policy/source/clearing/hostile controls plus all PKERR1, PKCPU1, PKTRAP1, PKXFER1, PKREVAL1, and producer controls | Partial N36 | Predominantly host/reference, bounded-model, emulator, and artifact tests; no authenticated authority, target-firmware, or physical-hardware result exists |
| Release gate | Cycle 122: 89/89 consistency checks over 84 artifacts; 20 native gaps | Partial N37 | `production_ready=false`; not a release acceptance gate |
...
- live PooleBoot reads exact `TRUST.PBT` and `TRUSTST.PBS` development candidates from the twelve-file medium, cross-binds fourteen facts, emits one `TRUST_STATE DENY` marker, grants zero authority, and writes zero state. The Cycle 122-refreshed PKLOAD6 passes 116/116 Rust tests, 25 markers, and the current integrated hostile corpus across two exact QEMU/OVMF runs;
...
- 31/31 Rust tests, 8/8 Python tests, both `x86_64-unknown-none` and `x86_64-unknown-uefi` builds, 36/36 hostile controls, and 32,768/32,768 deterministic mutation rejects pass. After the Cycle 122 PSYM1/PPOL1 identity refresh, the golden nine-file retained-set SHA-256 is `872978D7FB5E1108D04736796A247E5E59D9EEC0C97AE991CFBAC83D819A15DC`;
- the expanded verifier requires a 262,144-byte PooleKernel image with a 180,224-byte canonical file, entry offset `0x8000`, and, after the Cycle 122 identity refresh, 335 relative relocations. PKMAP2 covers 64 kernel pages with 12 read-only, 32 executable, and 20 read-write leaves, then places low guard/stack/high guard at pages 64/65-72/73 and the handoff allocation at page 80;
- PKLOAD6 passes 116/116 Rust tests, two exact PooleBoot and PooleKernel builds, two exact media generations, two QEMU/OVMF producer runs, 25 markers, 155/155 hostile controls, and two independent 5,008-byte PBP1 reconstructions with 96 memory descriptors and ten artifact descriptors;
...
- the Cycle 122-refreshed `runs/native_kernel_entry_readiness.json` records 31/31 host tests, two byte-identical clean linked and canonical builds, 43/43 hostile controls, 335 relative relocations, exact independent Rust/Python loaded bytes, and canonical SHA-256 `7BFD0BFF2ED6F3F391AE09E57ED1559E3B53B14B4F5D4D969B2938FE8D30BAD6`;
...
- `FLAG-N7-XSTATE-POLICY-001` closes only this one-BSP development boundary. N7.4 is partial. AVX and all other extended components, deliberate `#MF`/`#XM`/`#NM` delivery, scheduler/thread integration, AP-local state, SMP/migration, final machine-code SIMD audit, target hardware, N7 exit, release, and production remain open.
...
Cycle 122 closes only `FLAG-N7-XSTATE-POLICY-001`. PKXSTATE1 freezes eager standard-format x87/SSE ownership on one QEMU BSP with exact XCR0/XSS, canonical initialization, two isolated owner images, sensitive-state clearing, fail-closed context-switch preconditions, and a kernel-SIMD prohibition. Thirty-one kernel tests, two exact EPYC-Rome-v4 runs, 35 markers, and 43 hostile controls pass; the only privileged configuration writes are CR0, CR4, and XCR0. N7.4 is partial, not complete. `FLAG-N7-ERRATA-SOURCE-001` and `FLAG-N7-MICROCODE-FLOOR-001` remain open stop-ship flags; AVX/extended state, deliberate xstate exceptions, scheduler/AP/migration integration, final machine-code SIMD audit, complete per-CPU and guarded trap state, target qualification, and N7 exit remain open. The selected `hardware_fido2_ed25519_sk` device remains physically unavailable, so `N0-HW-KEY-ACQUIRE-001` is still the immediate blocked external move. No key, signature, public-key publication, secret use, driver load, firmware change, physical-media write, tag/release publication, or production promotion occurred in Cycle 122. The next owner-independent engineering move is `N7-XSTATE-EXCEPTION-001`: add bounded deliberate `#MF` and `#XM` delivery/recovery evidence while preserving masked default state and without claiming scheduler, AP, migration, target, or production readiness. PooleGlyph Phase 66 may advance in parallel without outranking N0-N7.
...
| 28 | `N7-XSTATE-EXCEPTION-001` | Next owner-independent move: qualify deliberate `#MF` and `#XM` delivery and bounded recovery while preserving masked canonical defaults and explicit `#NM` strategy rejection |
| 29 | `N9-PMM-001` | Normalized map, bootstrap allocator, physical allocator, guarded mappings |
| 30 | `N8-IRQ-001` | Local APIC, timer, monotonic clock, and first SMP application processor |
| 31 | `N12-SCHED-001` | Neutral scheduler and context switch under SMP stress |
| 32 | `N13-RING3-001` | First user task, syscall, capability object, exception, and clean exit |
| 33 | `N14-IPC-001` | Capability-mediated call/reply, cancellation, quota, and hostile message tests |
| 34 | `N16-VIRTIO-001` | Isolated virtio console/RNG driver domain with revocation and restart |
