...
Plan version: 2.40.0-native-kernel-sparse-direct-map
Roadmap cycle: PooleOS Cycle 134
...
| Native PKLOAD6 / PBLIVE4 / PKMAP2 / PBEXIT1 integration | Exact live PBC1, PSM1, real 299,008-byte canonical PKELF1, six PBART1, PBTP1, and PBTS1 reads; 91 kernel host tests and 176/176 aggregate Rust tests; 2/2 exact PooleBoot, kernel, twelve-file media, and QEMU runs; 25 markers; 155/155 controls; exact 97-entry final PBP1, 83-page kernel and nine-file retained mapping, 32-page guarded stack, five-page guarded manager plus alternate guarded ledger windows, firmware RSDP record, firmware-boundary agreement, live six-format and PBTRUST1 parsing, and independent oracle agreement | Partial N5.1/N5.4-N5.9 | The default path proves retained pages, successful `ExitBootServices`, unsigned denial, and permanent stop. PKXFER1 separately proves the development transfer; signature trust, authenticated state I/O, capability/action enforcement, final framebuffer remap, production transfer, and N5 exit remain open |
| Native PKENTRY1 PooleKernel | Real 299,008-byte canonical PKELF1 product in a 339,968-byte image; fixed 0x9000 entry; 729 relative relocations; 91 host tests; 2/2 exact clean builds; 43/43 hostile controls; exact Rust/Python loaded bytes; matching manifest/live build ID; canonical SHA-256 `5B581CC1D1ABEB163D0984D12144CA5016C44B46A28B190A6DFCBDCDA689A255` | Partial N6.4-N6.6 | PKXFER1 through PKVM3 separately prove opt-in virtualized entry and bounded BSP trap/CPU/xstate/MSR/PMM/ACPI-container/sparse-direct-map slices, while PKERR1 freezes a pure exact-target policy. Authentication, production PBP1, final framebuffer remap, complete per-CPU exception/xstate/runtime policy, target firmware, and N6 exit remain open |
...
| Test suite | Cycle 134: 788 Python tests pass with one Windows symlink-permission skip; 91 Rust kernel host tests include PMM-manifest retained-hole coverage plus PKVM3 manifest, cache, rollback, invalidation, and retirement-receipt controls | Partial N36 | Predominantly host/reference, bounded-model, emulator, and artifact tests; no authenticated authority, target-firmware, or physical-hardware result exists |
| Release gate | Cycle 134: 93/93 canonical consistency checks over 88 artifacts; Doctor 595/595; 20 native gaps | Partial N37 | `production_ready=false`; not a release acceptance gate |
...
| Native kernel | Real reproducible 83-page PooleKernel PKELF1 source and image with host-qualified PKREVAL1 and opt-in two-run QEMU live-entry/revalidation, trap, CPU, xstate, MSR, PKPMM7/PKACPI1 physical-memory, and virtual-memory traces | Partial N6.4-N7.6/N9.1-N9.4 | No authenticated production entry, complete per-CPU runtime, AML/complete platform graph, ring 3, IPC, capability, or driver-domain execution |
...
| `FLAG-N8-IRQ-001` | REQUIRED | Open in Cycle 134 | Discover and enable the local APIC; assign bounded vector ownership; establish a monotonic timer with calibrated overflow-safe timekeeping; start the first AP with per-CPU state, failure rollback, and zero unacknowledged interrupts; provide the execution substrate required before PKVM3 can perform SMP shootdown and generation retirement |
...
Cycle 134 closes only the one-BSP scope of `FLAG-N9-VM-DIRECT-MAP-001`. PKPMM7 emits an exact generation-bound manifest; PKVM3 maps 117,887 admitted pages in eleven write-back ranges with 12,878 gap pages absent, uses 243 contiguous DMA32 table pages, and preserves inherited kernel, guarded-stack, handoff, retained-hole, and cache invariants. Ninety-one kernel host tests, two exact 40-marker qemu64 runs, and 46 hostile controls pass with checksum `0x341CF729ADB26B52`, 367,474 physical table writes, 950,234 temporary-PTE writes and invalidations, three local invalidation receipts, and one exact generation-retirement receipt. Write and CR3 rollback, cache drift, premature reuse, and absent/stale receipt paths fail closed. The canonical kernel is 299,008 bytes in a 339,968-byte, 83-page image with 729 relocations and SHA-256 `5B581CC1D1ABEB163D0984D12144CA5016C44B46A28B190A6DFCBDCDA689A255`. N9.1-N9.4 remain partial: multi-processor activation is rejected until AP startup and real inter-processor shootdown exist; remote deferred reclaim, incremental concurrent replacement, huge pages, PCID, COW, user faults, pager IPC, heap/object caches, general MMIO/PAT/MTRR qualification, pressure/OOM, target qualification, and N9 exit remain open. `FLAG-N7-ERRATA-SOURCE-001` and `FLAG-N7-MICROCODE-FLOOR-001` remain stop-ship. The selected `hardware_fido2_ed25519_sk` device remains physically unavailable, so `N0-HW-KEY-ACQUIRE-001` is still the immediate blocked external move. The owner's broad operation-category authorization remains recorded, but no key, signature, public-key publication, secret use, privileged host probe, driver load, firmware change, physical-media write, tag/release publication, or production promotion occurred in Cycle 134. The next owner-independent engineering move is `N8-IRQ-001`; PooleGlyph Phase 66 may advance in parallel without outranking N0-N9.
...
| 40 | `N8-IRQ-001` | Next owner-independent move: local APIC discovery/enablement, bounded vector ownership, monotonic calibrated timer and overflow policy, first SMP application processor with per-CPU state and failure rollback, zero unacknowledged interrupts, and explicit target/nonproduction boundary |
| 41 | `N12-SCHED-001` | Neutral scheduler and context switch under SMP stress |
| 42 | `N13-RING3-001` | First user task, syscall, capability object, exception, and clean exit |
| 43 | `N14-IPC-001` | Capability-mediated call/reply, cancellation, quota, and hostile message tests |
| 44 | `N16-VIRTIO-001` | Isolated virtio console/RNG driver domain with revocation and restart |
