...
Plan version: 2.16.0-native-symbol-semantics
Roadmap cycle: PooleOS Cycle 110
...
Cycle 80 reset architecture claims without discarding validated component evidence. Cycles 81-96 established the native constitution, source boundary, pinned one-host toolchain, owner-controlled governance state, hardware and Tier 0 profiles, bounded formal models, and the PooleGlyph co-development lane. Cycles 97-100 qualified the first PooleBoot proof, PBP1, PBC1, and PKELF1. Cycle 101 produced the real reproducible PKENTRY1 PooleKernel image, Cycles 102-104 added bounded live load, PSM1 selection, and temporary PBP1, Cycle 105 validated exact temporary higher-half mappings, Cycle 106 retained the transfer objects through `ExitBootServices`, and Cycle 107 loaded and retained the exact seven-artifact development profile before a permanent pre-transfer stop. Cycle 108 froze PINIT1 as the first inner artifact format, and Cycle 109 froze PREC1 as the second. Cycle 110 freezes PSYM1 as the third: a deterministic public-only diagnostic index bound to exact stripped and preferred-loaded PooleKernel identities, build ID, private split-debug product, and source manifest; image-relative offsets; explicit KASLR base input; three public symbols; bounded binary search; source-path exclusion; pointer redaction; and fail-closed consumption preconditions. Independent Python and allocation-free `no_std` Rust implementations agree across three golden bundles, 158 controls, 16,384 parser cases, and 16,384 lookup cases. Two full-debug builds are byte-identical and canonicalize to the exact stripped PKELF1 product. PKLOAD6 validates exact PINIT1, PREC1, and PSYM1 bytes through prohibited-from-production host oracles and raises the aggregate integration corpus to 121 controls. This is format, correspondence, privacy, and lookup evidence only: PooleBoot and PooleKernel do not consume PSYM1, no kernel export or diagnostic authority is created, full debug data is absent from media, unsigned development media must not activate, and runtime pointers remain redacted by default.
...
| Native PooleBoot proof | `POOLEOS-N5-POOLEBOOT-7`; 8/8 host tests; 2/2 exact 113,664-byte PE32+ builds; 2/2 exact ten-file GPT/FAT32 images; 2/2 pinned OVMF runs; twenty-three ordered markers; 2/2 dual-channel and GOP-frame matches; 121/121 hostile controls | Partial N5.1-N5.9 | Unsigned seven-artifact load, retained PKMAP2/PBLIVE3 storage, host-only PINIT1/PREC1/PSYM1 validation, successful PBEXIT1, and permanent stop before transfer; no signature trust, PooleBoot inner enforcement, PooleKernel activation/recovery/symbol consumption, authenticated state persistence, kernel-entry profile, final CR3/RSP, target firmware, physical media, N5 exit, or production claim |
...
| Test suite | Cycle 110: 655 tests pass with one Windows symlink-permission skip, including PSYM1 identity/address/privacy canonicalization, hostile parser/lookup/debug-ELF controls, differential agreement, split-debug reproduction, integrated-oracle, claim-boundary, and Doctor external-report nonmutation controls | Partial N36 | Predominantly host/reference, bounded-model, emulator, and artifact tests; no PooleKernel entry, recovery, or symbol-consumption trace exists |
| Release gate | Cycle 110: 79/79 consistency checks over 74 artifacts; 20 native gaps | Partial N37 | `production_ready=false`; not a release acceptance gate |
...
Cycle 110 control-plane validation passes 655 tests with one Windows symlink-permission skip. Live Doctor passes 449/449 checks, and the 79/79 release gate passes over 74 artifacts while retaining 20 explicit gaps and `production_ready=false`. Live PooleGlyph conformance output is redirected to temporary storage, preserving the tandem repository's generated report. Exact authority and gate hashes are recorded only after their final deterministic generation.
...
Inherited sections: `013-015`. Added: `ADD-BOOT-001`, `ADD-BOOT-004`, `ADD-BOOT-005`, `ADD-BOOT-006`, `ADD-BOOT-007`.
...
| `FLAG-N5-INIT-SEMANTICS-001` | REQUIRED | Open in Cycle 110 | Extend frozen inner formats and independent validation to microcode, firmware, and policy, then prove live PINIT1/PREC1/PSYM1 enforcement, authenticated state persistence, capability creation, process/recovery launch, diagnostic authorization, lifecycle, rollback, and apply/execute enforcement |
...
Cycle 110 closes only `FLAG-N5-SYMBOL-BUNDLE-001`: PSYM1 identity, image-relative addressing, bounded lookup, public-name and pointer-redaction policy, stripped/split-debug correspondence, independent parsing, hostile and differential evidence, and unsigned-development consumption denial are frozen and reproducible. N5 remains partial: PSM1 and all seven artifacts are unsigned and untrusted, PBDIGEST1 review and promotion remain open, PooleBoot does not enforce PINIT1, PREC1, or PSYM1 or perform authenticated persistent state I/O, PooleKernel does not create the declared initial-system objects, execute recovery, or consume symbols, no payload instruction executes, microcode/firmware/policy inner formats remain open, and the retained CR3/RSP are never installed. N6.4-N6.6 remain partial because no PooleKernel instruction executes under the live caller. `FLAG-N5-INIT-SEMANTICS-001`, `FLAG-N6-BOOT-DIGEST-001`, `FLAG-N6-FRAMEBUFFER-MAP-001`, and `FLAG-NATIVE-BOOT-001` remain open until semantics, trust, final transfer state, kernel consumption, and target profiles are proved. The selected `hardware_fido2_ed25519_sk` device remains unavailable, so `N0-HW-KEY-ACQUIRE-001` remains the immediate blocked owner move and `FLAG-N0-GOVERNANCE-KEY-001` stays open. Codex has no authorization to generate or use keys, sign, merge, tag, register a public key, or publish a release. `FLAG-N4-MODELS-001` also remains open. The next owner-independent engineering move is `N5-MICROCODE-SEMANTICS-001`: freeze the role-5 microcode artifact's CPU identity, vendor format, revision monotonicity, trust, staging, apply timing, rollback, and post-apply verification rules without executing a microcode update. PooleGlyph Phase 66 may advance in parallel without outranking N0-N6.
...
| 16 | `N5-MICROCODE-SEMANTICS-001` | Next: freeze the role-5 microcode artifact's CPU identity, vendor container, revision floor, trust chain, staging and apply timing, rollback behavior, post-apply verification, resource bounds, and independent hostile parser evidence without executing an update |
| 17 | `N5-TRANSFER-001` | After artifact-semantics and trust prerequisites: validate kernel-entry PBP1, install retained CR3/RSP with interrupts disabled, call the frozen entry ABI, and prove kernel consumption or fail-closed halt |
| 18 | `N7-TRAP-001` | GDT/TSS/IDT and deliberate breakpoint/page-fault evidence after bounded transfer exists |
| 19 | `N9-PMM-001` | Normalized map, bootstrap allocator, physical allocator, guarded mappings |
| 20 | `N8-IRQ-001` | Local APIC, timer, monotonic clock, and first SMP application processor |
| 21 | `N12-SCHED-001` | Neutral scheduler and context switch under SMP stress |
| 22 | `N13-RING3-001` | First user task, syscall, capability object, exception, and clean exit |
| 23 | `N14-IPC-001` | Capability-mediated call/reply, cancellation, quota, and hostile message tests |
| 24 | `N16-VIRTIO-001` | Isolated virtio console/RNG driver domain with revocation and restart |
