...
Plan version: 2.35.0-native-kernel-metadata-arena
Roadmap cycle: PooleOS Cycle 129
...
| Test suite | Cycle 129: 785 Python tests pass with one Windows symlink-permission skip; the 74 Rust kernel host tests include PKPMM3 migration, mapped corruption, rollback, release exclusion, scrub/readback, and all predecessor kernel controls | Partial N36 | Predominantly host/reference, bounded-model, emulator, and artifact tests; no authenticated authority, target-firmware, or physical-hardware result exists |
| Release gate | Cycle 129: 93/93 consistency checks over 88 artifacts; 20 native gaps | Partial N37 | `production_ready=false`; not a release acceptance gate |
...
- live PooleBoot reads exact `TRUST.PBT` and `TRUSTST.PBS` development candidates from the twelve-file medium, cross-binds fourteen facts, emits one `TRUST_STATE DENY` marker, grants zero authority, and writes zero state. The Cycle 129-refreshed PKLOAD6 passes 159/159 Rust tests, 25 markers, and the current 155-control hostile corpus across two exact QEMU/OVMF runs;
...
- PKLOAD6 passes 159/159 Rust tests, two exact PooleBoot and PooleKernel builds, two exact media generations, two QEMU/OVMF producer runs, 25 markers, 155/155 hostile controls, and two independent 5,048-byte PBP1 reconstructions with 97 memory descriptors and ten artifact descriptors;
...
- the Cycle 129-refreshed `runs/native_kernel_entry_readiness.json` records 74/74 host tests, two byte-identical clean linked and canonical builds, 43/43 hostile controls, 569 relative relocations, exact independent Rust/Python loaded bytes, and canonical SHA-256 `F81F4B21F67A4A490AAE049C92B0E890A1D5B286DC708E41557DFCEBB37282DC`;
...
| `FLAG-N9-PMM-RECLAIM-001` | REQUIRED | Open in Cycle 129 | Admit held boot-services and ACPI classes only at their exact lifecycle boundary with retained-range exclusion, metadata-capacity preflight, scrub-before-admission, atomic generation-safe ownership transition, rollback, idempotence, immutable receipts, and explicit concurrency/SMP/target nonclaims |
...
Cycle 129 closes only `FLAG-N9-PMM-METADATA-001`. PKPMM3 moves the complete 14,616-byte physical-memory manager into five explicitly reserved, scrubbed, persistently mapped supervisor RW/NX pages between absent guards; validates generation/owner identity and handoff/final integrity; rolls back mapping and handoff failures; detects mapped corruption before mutation; and rejects every metadata release attempt. Seventy-four kernel host tests, two exact 41-marker qemu64 runs, and 87 hostile controls pass. The live runs manage 117,918 of 117,919 source-usable pages, protect 825 loader pages, scrub and verify 13 pages and 53,248 bytes, perform 7,680 physical word writes, 8,704 reads, and 43 temporary-PTE writes and invalidations, and retain exactly five metadata leaves with two guards and zero reclaim, signatures, authority, or actions. The kernel identity is now 245,760 canonical bytes, 286,720 image bytes, 70 pages, 569 relocations, and SHA-256 `F81F4B21F67A4A490AAE049C92B0E890A1D5B286DC708E41557DFCEBB37282DC`; all dependent evidence through PKVM2 is rebound. N9.1-N9.4 remain partial. `FLAG-N7-ERRATA-SOURCE-001` and `FLAG-N7-MICROCODE-FLOOR-001` remain open stop-ship flags; held-class reclaim, scalable metadata growth, complete mapping and SMP TLB policy, huge pages, PCID, COW, user faults, pager IPC, heap/object caches, MMIO/cache qualification, concurrency, pressure/OOM policy, exact target qualification, and N9 exit remain open. The selected `hardware_fido2_ed25519_sk` device remains physically unavailable, so `N0-HW-KEY-ACQUIRE-001` is still the immediate blocked external move. The owner's broad operation-category authorization remains recorded, but no key, signature, public-key publication, secret use, privileged host probe, driver load, firmware change, physical-media write, tag/release publication, or production promotion occurred in Cycle 129. The next owner-independent engineering move is `N9-PMM-RECLAIM-001`: freeze and qualify exact held-class reclaim timing, scrub-before-admission, atomic ownership transition, rollback, idempotence, retained-range exclusion, and immutable receipts. PooleGlyph Phase 66 may advance in parallel without outranking N0-N9.
...
| 35 | `N9-PMM-RECLAIM-001` | Next owner-independent move: exact held-class lifecycle gates, retained-range exclusion, capacity preflight, scrub-before-admission, atomic generation-safe transition, rollback, idempotence, and immutable receipts without concurrent/SMP/target/production promotion |
| 36 | `N8-IRQ-001` | Local APIC, timer, monotonic clock, and first SMP application processor |
| 37 | `N12-SCHED-001` | Neutral scheduler and context switch under SMP stress |
| 38 | `N13-RING3-001` | First user task, syscall, capability object, exception, and clean exit |
| 39 | `N14-IPC-001` | Capability-mediated call/reply, cancellation, quota, and hostile message tests |
| 40 | `N16-VIRTIO-001` | Isolated virtio console/RNG driver domain with revocation and restart |
