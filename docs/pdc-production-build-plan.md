...
Plan version: 2.25.0-native-kernel-trap
Roadmap cycle: PooleOS Cycle 119
...
| Test suite | Cycle 119: 740 tests pass with one Windows symlink-permission skip, including PKTRAP1 descriptor/frame/scenario/marker controls plus all PKXFER1, PKREVAL1, and producer controls | Partial N36 | Predominantly host/reference, bounded-model, emulator, and artifact tests; no authenticated authority, qualified privileged mechanism, target-firmware, or physical-hardware result exists |
| Release gate | Cycle 119: 86/86 consistency checks over 81 artifacts; 20 native gaps | Partial N37 | `production_ready=false`; not a release acceptance gate |
...
- live PooleBoot reads exact `TRUST.PBT` and `TRUSTST.PBS` development candidates from the twelve-file medium, cross-binds fourteen facts, emits one `TRUST_STATE DENY` marker, grants zero authority, and writes zero state. The Cycle 119-refreshed PKLOAD6 passes 104/104 Rust tests, 25 markers, and the current integrated hostile corpus across two exact QEMU/OVMF runs;
...
- 19/19 Rust tests, 8/8 Python tests, both `x86_64-unknown-none` and `x86_64-unknown-uefi` builds, 36/36 hostile controls, and 32,768/32,768 deterministic mutation rejects pass. After the Cycle 119 PSYM1 identity refresh, the golden nine-file retained-set SHA-256 is `68B51E2BF83867CCB18535FC345888B7F56FA65E477FEFA43E7B5257BAB6F456`;
- the expanded verifier requires a 262,144-byte PooleKernel image with a 180,224-byte canonical file, entry offset `0x8000`, and 325 relative relocations. PKMAP2 now covers 64 kernel pages with 12 read-only, 32 executable, and 20 read-write leaves, then places low guard/stack/high guard at pages 64/65-72/73 and the handoff allocation at page 80;
- PKLOAD6 passes 104/104 Rust tests, two exact PooleBoot and PooleKernel builds, two exact media generations, two QEMU/OVMF producer runs, 25 markers, 155/155 hostile controls, and two independent 5,008-byte PBP1 reconstructions with 96 memory descriptors and ten artifact descriptors;
...
- two exact kernel builds, two feature-enabled PooleBoot builds, one default isolation build, two byte-identical media generations, and two fresh-vars QEMU/OVMF runs reproduce all 30 markers, exact PBP1 bytes, the 180,224-byte PooleKernel SHA-256 `8D1C219B4B6D8FEFE3B325F3A59DF904F1D971686A4A6537D73825A1B6F56FD2`, and exact guest/host denial. All 58 hostile controls pass;
...
- the Cycle 119-refreshed `runs/native_kernel_entry_readiness.json` records 19/19 host tests, two byte-identical clean linked and canonical builds, 43/43 hostile controls, 325 relative relocations, exact independent Rust/Python loaded bytes, and canonical SHA-256 `8D1C219B4B6D8FEFE3B325F3A59DF904F1D971686A4A6537D73825A1B6F56FD2`;
...
- N6.4-N6.6 are only partial. PKXFER1 proves opt-in QEMU CR3/RSP installation, live COM1/debugcon/framebuffer diagnostics, and terminal unsigned denial; PKTRAP1 separately proves only a bounded BSP descriptor/exception slice. KASLR, final debug/unwind policy, framebuffer remap/revocation, complete per-CPU GDT/IDT/TSS, early self-tests, full control-state dumps, all exception/NMI/machine-check paths, retained next-boot panic state, authenticated production transfer, target firmware, and boot trust remain open.
...
- Only subphases N7.5 and N7.6 are partial. The IST arrays are unguarded kernel-data allocations, only five gates are present, and no AP-local state, user transition, asynchronous interrupt preservation, NMI, machine check, recursion recovery, persistent crash record, target-hardware evidence, or N7-exit claim follows.
...
Cycle 119 closes only `FLAG-N7-TRAP-001`. Three opt-in QEMU-only PKTRAP1 profiles extend PKXFER1 with bounded BSP descriptor setup, exact returning synchronous faults, terminal double-fault containment, and semantic malformed-frame rejection; six runs and 51 controls pass with zero authority or effects. N7 remains partial because N7.1-N7.4 are unstarted and N7.5-N7.6 still lack per-CPU and guarded stacks, complete vectors, user transitions, asynchronous state, NMI/machine check, recursion recovery, retained crash evidence, and target-hardware qualification. N5/N6 remain partial for the existing trust, persistent-state, framebuffer, runtime, target, and production gaps. The selected `hardware_fido2_ed25519_sk` device remains physically unavailable, so `N0-HW-KEY-ACQUIRE-001` remains the immediate blocked external move and `FLAG-N0-GOVERNANCE-KEY-001` stays open. Cycle 118 owner authorization still covers the approved operation categories, but no key, signing, privileged or mutating operation, publication, release, or promotion occurred in Cycle 119; owner presence, custody, backup, recovery, safe-target, qualification, and exact-release gates remain mandatory. The next owner-independent engineering move is `N7-CPU-POLICY-001`: freeze a read-only CPUID/control-state policy contract before extending descriptor and exception coverage. PooleGlyph Phase 66 may advance in parallel without outranking N0-N7.
...
| 25 | `N7-CPU-POLICY-001` | Next owner-independent move: freeze bounded CPUID, required-feature, CR0/CR4/EFER, and read-only MSR-policy contracts before expanding trap coverage |
| 26 | `N9-PMM-001` | Normalized map, bootstrap allocator, physical allocator, guarded mappings |
| 27 | `N8-IRQ-001` | Local APIC, timer, monotonic clock, and first SMP application processor |
| 28 | `N12-SCHED-001` | Neutral scheduler and context switch under SMP stress |
| 29 | `N13-RING3-001` | First user task, syscall, capability object, exception, and clean exit |
| 30 | `N14-IPC-001` | Capability-mediated call/reply, cancellation, quota, and hostile message tests |
| 31 | `N16-VIRTIO-001` | Isolated virtio console/RNG driver domain with revocation and restart |
