...
Plan version: 2.3.0-native-poolefs-model
Roadmap cycle: PooleOS Cycle 96
...
| Test suite | Cycle 96: 508 tests pass with one Windows symlink-permission skip, including independent PooleFS torn-write, premature-publication, double-allocation, replay, checksum, and recovery-leak controls | Partial N36 | Predominantly host/reference, bounded-model, and artifact tests; no PooleKernel or PooleFS implementation trace exists |
| Release gate | Cycle 96: 69/69 consistency checks over 64 artifacts; 20 native gaps | Partial N37 | `production_ready=false`; not a release acceptance gate |
...
Cycle 96 completes `N4-POOLEFS-MODEL-001`: `POOLEOS-N4-MODELS-5` binds the drained 74-state PooleFS safe graph and six exact hostile counterexamples, and `FLAG-N4-POOLEFS-MODEL-001` closes. The selected `hardware_fido2_ed25519_sk` device remains unavailable, so `N0-HW-KEY-ACQUIRE-001` remains the immediate blocked owner move and `FLAG-N0-GOVERNANCE-KEY-001` stays open. No key generation, private-key use, signing, merge, tag, public-key registration, or publication is authorized. `FLAG-N4-MODELS-001` remains open because all six native implementation-trace cross-checks and separately scoped liveness/refinement/conformance work are unfinished. The next owner-independent engineering move is `N5-POOLEBOOT-001`, while PooleGlyph Phase 66 may advance in parallel without outranking N0-N5.
...
| 4 | `N5-POOLEBOOT-001` | Poole-authored PE32+ UEFI application prints, draws GOP pixels, reads tables/map, exits cleanly; QEMU/EDK II source assurance and expanded Tier 0 device/fault profiles continue in parallel under N4 |
| 5 | `N5-BOOTPROTO-001` | Canonical boot handoff schema, independent decoder, golden bytes, malformed corpus |
| 6 | `N5-ELF-001` | PooleBoot validates and loads a minimal PooleKernel ELF64 image |
| 7 | `N6-KENTRY-001` | Kernel entry, serial/framebuffer log, panic, build ID, signed manifest continuity |
