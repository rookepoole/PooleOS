...
Plan version: 2.13.0-native-handoff-exit
Roadmap cycle: PooleOS Cycle 107
...
Cycle 80 reset architecture claims without discarding validated component evidence. Cycles 81-96 established the native constitution, source boundary, pinned one-host toolchain, owner-controlled governance state, hardware and Tier 0 profiles, bounded formal models, and the PooleGlyph co-development lane. Cycles 97-100 qualified the first PooleBoot proof, PBP1, PBC1, and PKELF1. Cycle 101 produced the real reproducible PKENTRY1 PooleKernel image, Cycles 102-104 added bounded live load, PSM1 selection, and temporary PBP1, Cycle 105 validated exact temporary higher-half mappings, and Cycle 106 retained the transfer objects through `ExitBootServices` before a permanent pre-transfer stop. Cycle 107 executes the bounded load/retain/PBP1-bind slice of `N5-INIT-SYSTEM-001`: PooleBoot now requires an exact seven-artifact PSM1 profile, validates six PBART1 role/version/payload envelopes, copies each file into a distinct zero-padded loader range, cross-binds all seven artifact roles and digests into final PBLIVE3, exits boot services, and stops before transfer. Two exact QEMU/OVMF runs agree across 23 ordered markers and 112 hostile controls. All artifacts remain unsigned and attacker-controllable; no payload semantics, initial-system instruction, microcode application, trusted kernel-entry profile, final CR3/RSP installation, PooleKernel instruction, target-firmware result, N5 exit, or production claim follows.
...
| Native PBART1 profile artifacts | Fixed 96-byte envelope; exact roles 2-7; independent Rust/Python validation; six whole-file and payload digest bindings; distinct zero-padded loader pages; final-map and PBP1 retention | Partial N5.6 | Envelopes are unsigned, payload semantics are not interpreted, initial system is not executed, and microcode/firmware/policy are not applied |
| Native PKLOAD6 / PBLIVE3 / PKMAP2 / PBEXIT1 integration | Exact live PBC1, PSM1, real 139,264-byte PKELF1, and six PBART1 reads; 76/76 Rust host tests; 2/2 exact PooleBoot, kernel, ten-file media, and QEMU runs; 23 markers; 112/112 controls; exact 4,728-byte final PBP1, retained mapping, and firmware-boundary agreement | Partial N5.1/N5.4-N5.6/N5.8 | Kernel/artifact/table/stack/handoff pages survive successful `ExitBootServices`; no signature trust, payload semantics, final CR3/RSP, transferable handoff, kernel entry, final cache policy, or N5 exit |
...
| Test suite | Cycle 107: 608 tests pass with one Windows symlink-permission skip, including PBART1/PKLOAD6/PBLIVE3/PKMAP2/PBEXIT1 artifact, final-map, retention, guard, retry, post-exit firmware, marker, oracle, claim, and Doctor external-report nonmutation controls | Partial N36 | Predominantly host/reference, bounded-model, emulator, and artifact tests; no PooleKernel entry trace exists |
| Release gate | Cycle 107: 76/76 consistency checks over 71 artifacts; 20 native gaps | Partial N37 | `production_ready=false`; not a release acceptance gate |
...
Cycle 107 control-plane validation passes 608 tests with one Windows symlink-permission skip. Doctor passes 400/400 checks, and the 76/76 release gate passes over 71 artifacts while retaining 20 explicit gaps and `production_ready=false`. Live PooleGlyph conformance output is redirected to temporary storage, preserving the tandem repository's generated report. Exact authority and gate hashes are recorded only after their final deterministic generation.
...
Inherited sections: `013-015`. Added: `ADD-BOOT-001`, `ADD-BOOT-004`.
...
| `FLAG-N5-INIT-SEMANTICS-001` | REQUIRED | Open in Cycle 107 | Freeze and independently validate each inner artifact format, capability/resource declaration, dependency graph, lifecycle, rollback behavior, and apply/execute precondition before any payload is interpreted |
...
Cycle 107 completes only the unsigned envelope/load/retain/PBP1-bind portion of `N5-INIT-SYSTEM-001` and closes only `FLAG-N5-INIT-SYSTEM-001`. N5 remains partial: PSM1 and all seven artifacts are unsigned and untrusted, PBDIGEST1 review and promotion remain open, inner artifact formats and semantics are not frozen, no initial-system instruction executes, no microcode/firmware/policy payload is applied, the final framebuffer cache policy and transferable kernel-entry PBP1 are not established, and the retained CR3/RSP are never installed. N6.4-N6.6 remain partial because no PooleKernel instruction executes under the live caller. `FLAG-N5-INIT-SEMANTICS-001`, `FLAG-N6-BOOT-DIGEST-001`, `FLAG-N6-FRAMEBUFFER-MAP-001`, and `FLAG-NATIVE-BOOT-001` remain open until semantics, trust, final transfer state, kernel consumption, and target profiles are proved. The selected `hardware_fido2_ed25519_sk` device remains unavailable, so `N0-HW-KEY-ACQUIRE-001` remains the immediate blocked owner move and `FLAG-N0-GOVERNANCE-KEY-001` stays open. Codex has no authorization to generate or use keys, sign, merge, tag, register a public key, or publish a release. `FLAG-N4-MODELS-001` also remains open. The next owner-independent engineering move is `N5-INIT-SEMANTICS-001`: freeze and independently validate the initial-system bundle's inner format, dependency/capability/resource declarations, lifecycle, and rejection rules before extending the same discipline to recovery, symbols, microcode, firmware, and policy. PooleGlyph Phase 66 may advance in parallel without outranking N0-N6.
...
| 13 | `N5-INIT-SEMANTICS-001` | Next: freeze the initial-system inner bundle format, dependency graph, capability/resource declarations, lifecycle, and independent hostile parser evidence before any payload interpretation |
| 14 | `N5-TRANSFER-001` | After artifact-semantics and trust prerequisites: validate kernel-entry PBP1, install retained CR3/RSP with interrupts disabled, call the frozen entry ABI, and prove kernel consumption or fail-closed halt |
| 15 | `N7-TRAP-001` | GDT/TSS/IDT and deliberate breakpoint/page-fault evidence after bounded transfer exists |
| 16 | `N9-PMM-001` | Normalized map, bootstrap allocator, physical allocator, guarded mappings |
| 17 | `N8-IRQ-001` | Local APIC, timer, monotonic clock, and first SMP application processor |
| 18 | `N12-SCHED-001` | Neutral scheduler and context switch under SMP stress |
| 19 | `N13-RING3-001` | First user task, syscall, capability object, exception, and clean exit |
| 20 | `N14-IPC-001` | Capability-mediated call/reply, cancellation, quota, and hostile message tests |
| 21 | `N16-VIRTIO-001` | Isolated virtio console/RNG driver domain with revocation and restart |
