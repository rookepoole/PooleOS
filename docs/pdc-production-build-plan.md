...
Plan version: 2.11.0-native-live-pbp1
Roadmap cycle: PooleOS Cycle 104
...
Cycle 80 reset architecture claims without discarding validated component evidence. Cycles 81-96 established the native constitution, source boundary, pinned one-host toolchain, owner-controlled governance state, hardware and Tier 0 profiles, bounded formal models, and the PooleGlyph co-development lane. Cycles 97-100 qualified the first PooleBoot proof, PBP1, PBC1, and PKELF1. Cycle 101 produced the real reproducible PKENTRY1 PooleKernel image, Cycle 102 qualified bounded live load then release, and Cycle 103 added canonical PSM1 manifest selection and digest binding. Cycle 104 executes `N5-PBP1-LIVE-001`: PooleBoot now retains the verified kernel pages through a bounded stride-aware UEFI memory-map snapshot, constructs a canonical temporary pre-exit PBP1 bound to PBC1, PSM1, the live kernel allocation, and GOP, emits exact bytes on both diagnostic channels, rechecks them, and releases every temporary pool and kernel page before return. Two exact QEMU/OVMF runs agree with the independent host oracle across 21 ordered markers and 52 hostile integration controls. The manifest remains unsigned and attacker-controllable; no signature trust, persistent rollback, retained handoff or kernel mapping, page-table activation, final memory map, `ExitBootServices`, kernel consumption, or transfer follows.
...
| Native PKELF1 kernel loader | Bounded ELF64 `ET_DYN` profile; dependency-free `no_std` loader; independent Python oracle; 12/12 Rust tests; 2/2 target and 2/2 PooleBoot integration builds; 3/3 exact loaded images; 129/129 controls; 16,384 differential cases with zero mismatches | Partial N5.5 | Standalone PKELF1 receipt remains caller-buffer evidence; PKLOAD3 separately proves manifest-bound live allocation retained through temporary PBP1 and then released, but no installed page tables or transfer |
| Native PKLOAD3 integration | Exact live PBC1, PSM1, and real 139,264-byte PKELF1 reads; 49/49 Rust host tests; 2/2 exact PooleBoot, kernel, media, and QEMU runs; 21 markers; 52/52 controls; exact guest/oracle manifest, digest, load, and 4,248-byte PBP1 agreement | Partial N5.1/N5.4/N5.5/N5.8 | Unsigned digest-bound development path; pages survive only through temporary pre-exit PBP1 and are then released; no signature trust, installed or retained mappings, final map, `ExitBootServices`, or entry |
| Native PKENTRY1 PooleKernel | Real 139,264-byte canonical PKELF1 product; fixed 0x4000 entry; 40 relative relocations; 13/13 host tests; 2/2 exact clean builds; 43/43 hostile controls; exact Rust/Python loaded bytes | Partial N6.4-N6.6 | Live PooleBoot load is retained only through temporary PBP1 and then released; authentication, retained page tables, `ExitBootServices`, transfer, privileged diagnostics execution, descriptor tables, runtime, target firmware, and N6 exit remain open |
...
| Test suite | Cycle 104: 581 tests pass with one Windows symlink-permission skip, including PKLOAD3 stride/capacity/overlap/cross-binding/lifetime/cleanup/transcript/profile/claim controls | Partial N36 | Predominantly host/reference, bounded-model, emulator, and artifact tests; no PooleKernel entry trace exists |
| Release gate | Cycle 104: 76/76 consistency checks over 71 artifacts; 20 native gaps | Partial N37 | `production_ready=false`; not a release acceptance gate |
...
| Native bootloader | Reproducible unsigned Poole-authored PE32+ UEFI application with bounded live PBC1/PSM1/PKELF1 intake, digest and load-plan validation, stride-aware temporary PBP1 production, cleanup, firmware-table/map/GOP behavior | Partial N5.1-N5.5/N5.7-N5.8 | No signature-authenticated manifest, trusted rollback state, retained mappings or handoff, final map, `ExitBootServices`, transfer, target firmware, or N5 exit |
...
N5.1-N5.5, N5.7, and N5.8 are partial. N5.6 and N5.9 remain not started. PooleBoot now performs bounded live PBC1/PSM1/PKELF1 reads, exact unsigned manifest and digest binding, firmware-page allocation, relocation, mapping-plan validation, and exact temporary pre-exit PBP1 production before full release. It still does not authenticate a signer, enforce persistent rollback, retain the kernel or PBP1, install or activate page tables, load the initial system bundle, capture the final memory map, call `ExitBootServices`, transfer to or execute PooleKernel, enforce Secure Boot, measure through TCG2, use TPM state, test target firmware, write physical media, build an ISO, or satisfy the N5 exit gate. The static identity is not the final animated PooleGlass boot experience or an accessibility result.
...
Cycle 104 completes only bounded `N5-PBP1-LIVE-001` and closes only `FLAG-N5-PBP1-LIVE-001`. N5 remains partial: PSM1 is unsigned and untrusted, PBDIGEST1 review and promotion remain open, the live PBP1 and kernel pages are deliberately released, page tables are not installed or activated, the final memory map is not captured, and no `ExitBootServices` or transfer occurs. N6.4-N6.6 remain partial because no PooleKernel instruction executes under the live caller. `FLAG-N6-BOOT-DIGEST-001` and `FLAG-N6-FRAMEBUFFER-MAP-001` retain provider-review and mapping/cache/revocation dependencies, and `FLAG-NATIVE-BOOT-001` remains open until signature-authenticated manifest selection, trusted rollback state, retained exact mappings and PBP1, final memory-map retry, successful `ExitBootServices`, and transfer are proved. The selected `hardware_fido2_ed25519_sk` device remains unavailable, so `N0-HW-KEY-ACQUIRE-001` remains the immediate blocked owner move and `FLAG-N0-GOVERNANCE-KEY-001` stays open. Codex has no authorization to generate or use keys, sign, merge, tag, register a public key, or publish a release. `FLAG-N4-MODELS-001` also remains open. The next owner-independent engineering move is `N5-KMAP-001`: retain the exact kernel allocation, construct and activate the required W^X mappings, preserve the framebuffer cache policy, bind rollback to every partial failure, and return without entering the kernel. PooleGlyph Phase 66 may advance in parallel without outranking N0-N6.
...
| 10 | `N5-KMAP-001` | Next: retain exact kernel pages, construct and activate required W^X mappings, preserve framebuffer cache policy, and prove rollback without entering the kernel |
