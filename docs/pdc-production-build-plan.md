...
Plan version: 2.37.0-native-kernel-growth
Roadmap cycle: PooleOS Cycle 131
...
| Native PKLOAD6 / PBLIVE3 / PKMAP2 / PBEXIT1 integration | Exact live PBC1, PSM1, real 270,336-byte canonical PKELF1, six PBART1, PBTP1, and PBTS1 reads; 167/167 Rust host tests; 2/2 exact PooleBoot, kernel, twelve-file media, and QEMU runs; 25 markers; 155/155 controls; exact 97-entry final PBP1, 76-page kernel and nine-file retained mapping, fourteen-page guarded stack, five-page guarded manager plus alternate guarded ledger windows, firmware-boundary agreement, live six-format and PBTRUST1 parsing, and independent oracle agreement | Partial N5.1/N5.4-N5.9 | The default path proves retained pages, successful `ExitBootServices`, unsigned denial, and permanent stop. PKXFER1 separately proves the development transfer; signature trust, authenticated state I/O, capability/action enforcement, final framebuffer remap, production transfer, and N5 exit remain open |
| Native PKENTRY1 PooleKernel | Real 270,336-byte canonical PKELF1 product in a 311,296-byte image; fixed 0x9000 entry; 662 relative relocations; 82/82 host tests; 2/2 exact clean builds; 43/43 hostile controls; exact Rust/Python loaded bytes; canonical SHA-256 `30E414826D7DF588C07A66032423DCD6AA8C47B0E6CCFC3D6686BC9224AFB947` | Partial N6.4-N6.6 | PKXFER1 through PKVM2 separately prove opt-in virtualized entry and bounded BSP trap/CPU/xstate/MSR/PMM/VM slices, while PKERR1 freezes a pure exact-target policy. Authentication, production PBP1, final framebuffer remap, complete per-CPU exception/xstate/runtime policy, target firmware, and N6 exit remain open |
| Native PKPMM5 physical memory | Exact live 97-entry PBP1 intake; stable 15,336-byte manager in five guarded pages; external generation-owned active ledgers; explicit atomic generation 2 to 3 growth from four to eight pages with old-generation revoke/scrub/retire and fault-tested rollback/retry; 117,913 source-usable pages; 129,162 final managed pages; 11,250 Boot Services pages admitted; 11 ACPI pages held; 831 loader pages protected; 11,279 pages and 46,198,784 bytes scrubbed and verified; 2/2 exact 43-marker QEMU runs; 137/137 hostile controls; 82/82 kernel host tests | Partial N9.1-N9.2 | One BSP and one explicit growth only. Automatic pressure-triggered/repeated growth, guarded-window exhaustion/fallback, complete ACPI lifecycle integration, concurrency, complete mapping policy, heaps, MMIO/cache policy, pressure/OOM, target, and N9 exit remain open |
| Native PKVM2 virtual memory | Eight generation-bound DMA32 table pages plus one data frame; exact inherited kernel/entry/guarded-stack/handoff mappings; bounded nine-page direct map; transactional candidate CR3 activation and exact restoration; Accessed/Dirty handling; three local invalidation receipts; 2/2 exact 40-marker QEMU runs; 46/46 hostile controls; 8,720 physical table writes; 5,528 bootstrap PTE writes and invalidations | Partial N9.3-N9.4 | One BSP only; no complete direct map, SMP shootdown/deferred reclaim, ring 3, huge pages, PCID, COW, user faults, pager, heap, target, N9-exit, or production claim |
...
| Test suite | Cycle 131: 786 Python tests pass with one Windows symlink-permission skip; the 82 Rust kernel host tests include PKPMM5 generation growth, precommit rollback, postcommit retirement retry, integrity/corruption rejection, lifecycle reclaim, and all predecessor kernel controls | Partial N36 | Predominantly host/reference, bounded-model, emulator, and artifact tests; no authenticated authority, target-firmware, or physical-hardware result exists |
| Release gate | Cycle 131: 93/93 consistency checks over 88 artifacts; 20 native gaps | Partial N37 | `production_ready=false`; not a release acceptance gate |
...
| Native kernel | Real reproducible 76-page PooleKernel PKELF1 source and image with host-qualified PKREVAL1 and opt-in two-run QEMU live-entry/revalidation, trap, CPU, xstate, MSR, physical-memory, and virtual-memory traces | Partial N6.4-N7.6/N9.1-N9.4 | No authenticated production entry, complete per-CPU runtime, ring 3, IPC, capability, or driver-domain execution |
...
- live PooleBoot reads exact `TRUST.PBT` and `TRUSTST.PBS` development candidates from the twelve-file medium, cross-binds fourteen facts, emits one `TRUST_STATE DENY` marker, grants zero authority, and writes zero state. The Cycle 131-refreshed PKLOAD6 passes 167/167 Rust tests, 25 markers, and the current 155-control hostile corpus across two exact QEMU/OVMF runs;
...
- the expanded verifier requires a 311,296-byte PooleKernel image with a 270,336-byte canonical file, entry offset `0x9000`, and 662 relative relocations. PKMAP2 covers 76 kernel pages with 16 read-only, 50 executable, and 10 read-write leaves; places low guard/stack/high guard at pages 76/77-90/91 and the handoff allocation at page 92; reserves temporary page 348; places manager guards/pages at 349/350-354/355; and reserves two guarded 32-page ledger windows at 356/357-388/389 and 390/391-422/423;
...
- two exact kernel builds, two feature-enabled PooleBoot builds, one default isolation build, two byte-identical media generations, and two fresh-vars QEMU/OVMF runs reproduce all 30 markers, exact PBP1 bytes, and exact guest/host denial. The Cycle 131-refreshed 270,336-byte PooleKernel has SHA-256 `30E414826D7DF588C07A66032423DCD6AA8C47B0E6CCFC3D6686BC9224AFB947`; all 58 hostile controls still pass;
...
- the Cycle 131-refreshed `runs/native_kernel_entry_readiness.json` records 82/82 host tests, two byte-identical clean linked and canonical builds, 43/43 hostile controls, 662 relative relocations, exact independent Rust/Python loaded bytes, and canonical SHA-256 `30E414826D7DF588C07A66032423DCD6AA8C47B0E6CCFC3D6686BC9224AFB947`;
...
| `FLAG-N9-PMM-GROWTH-AUTOMATION-001` | REQUIRED | Open in Cycle 131 | Integrate checked pressure-triggered and repeated ledger growth, qualify both guarded-window exhaustion/fallback paths, and preserve deterministic accounting, rollback, retirement retry, integrity, and explicit concurrency/SMP/target nonclaims |
...
Cycle 131 closes only the bounded explicit-transaction scope of `FLAG-N9-PMM-GROWTH-001`. PKPMM5 keeps the 15,336-byte manager stable in five guarded pages while placing every active source, free-extent, allocation, scrub-receipt, and reclaim-receipt ledger in guarded generation-owned pages. Generation 2 starts in four pages at capacities 256/32/256/16/2 and atomically grows to generation 3 in eight pages at capacities 512/64/512/32/4 through complete preflight, reserve/map, copy, seal, switch, old-leaf revocation, full scrub/readback, and retirement. Host faults prove precommit rollback and retryable postcommit retirement; the independent oracle reconstructs stable, retired, and active storage. Eighty-two kernel host tests, two exact 43-marker qemu64 runs, and 137 hostile controls pass. The runs finish with 117,913 source-usable and 129,162 managed pages, 831 protected loader pages, eleven ACPI pages held, 11,279 scrubbed and verified pages, 46,198,784 scrubbed and verified bytes, 5,775,872 writes, 5,776,896 reads, and 22,591 temporary-PTE writes and invalidations. Growth checksum `0xFA339A347E3A3CAF` binds the transaction. The larger layout also exposed and fixed a shared VM-only finalizer assumption; dependent PKVM2 evidence is rebound to 5,528 temporary writes and invalidations. The kernel identity is 270,336 canonical bytes, 311,296 image bytes, 76 pages, 662 relocations, and SHA-256 `30E414826D7DF588C07A66032423DCD6AA8C47B0E6CCFC3D6686BC9224AFB947`; dependent evidence through PKVM2 is rebound. N9.1-N9.4 remain partial. `FLAG-N7-ERRATA-SOURCE-001` and `FLAG-N7-MICROCODE-FLOOR-001` remain open stop-ship flags; automatic pressure-triggered/repeated growth, ledger-window exhaustion/fallback, complete ACPI consumer integration, complete mapping and SMP TLB policy, huge pages, PCID, COW, user faults, pager IPC, heap/object caches, MMIO/cache qualification, concurrency, pressure/OOM policy, exact target qualification, and N9 exit remain open. The selected `hardware_fido2_ed25519_sk` device remains physically unavailable, so `N0-HW-KEY-ACQUIRE-001` is still the immediate blocked external move. The owner's broad operation-category authorization remains recorded, but no key, signature, public-key publication, secret use, privileged host probe, driver load, firmware change, physical-media write, tag/release publication, or production promotion occurred in Cycle 131. The next owner-independent engineering move is `N9-PMM-GROWTH-AUTOMATION-001`. PooleGlyph Phase 66 may advance in parallel without outranking N0-N9.
...
| 37 | `N9-PMM-GROWTH-AUTOMATION-001` | Next owner-independent move: checked pressure-triggered and repeated ledger growth with both guarded-window exhaustion/fallback paths, deterministic accounting, rollback, retirement retry, and explicit concurrency/SMP/target nonclaims |
| 38 | `N8-IRQ-001` | Local APIC, timer, monotonic clock, and first SMP application processor |
| 39 | `N12-SCHED-001` | Neutral scheduler and context switch under SMP stress |
| 40 | `N13-RING3-001` | First user task, syscall, capability object, exception, and clean exit |
| 41 | `N14-IPC-001` | Capability-mediated call/reply, cancellation, quota, and hostile message tests |
| 42 | `N16-VIRTIO-001` | Isolated virtio console/RNG driver domain with revocation and restart |
