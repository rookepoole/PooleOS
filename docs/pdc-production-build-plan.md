...
Plan version: 2.15.0-native-recovery-semantics
Roadmap cycle: PooleOS Cycle 109
...
Cycle 80 reset architecture claims without discarding validated component evidence. Cycles 81-96 established the native constitution, source boundary, pinned one-host toolchain, owner-controlled governance state, hardware and Tier 0 profiles, bounded formal models, and the PooleGlyph co-development lane. Cycles 97-100 qualified the first PooleBoot proof, PBP1, PBC1, and PKELF1. Cycle 101 produced the real reproducible PKENTRY1 PooleKernel image, Cycles 102-104 added bounded live load, PSM1 selection, and temporary PBP1, Cycle 105 validated exact temporary higher-half mappings, Cycle 106 retained the transfer objects through `ExitBootServices`, and Cycle 107 loaded and retained the exact seven-artifact development profile before a permanent pre-transfer stop. Cycle 108 froze PINIT1 as the first inner artifact format. Cycle 109 freezes PREC1 as the second: a deterministic 992-byte immutable two-slot recovery policy plus a separately mutable 128-byte boot-attempt state record, exact decrement-before-handoff transitions, known-good fallback, bounded safe/recovery routing, authenticated success receipts, ten failure routes, seven declarative authority requirements, and physical-presence gates. Independent Python and allocation-free `no_std` Rust implementations agree across three golden policy/state/transition vectors, 144 controls, 16,384 parser/state cases, and 8,192 transition cases. PKLOAD6 validates exact PINIT1 and PREC1 bytes through its prohibited-from-production host oracle and raises the aggregate integration corpus to 118 controls. This is policy and state-machine evidence only: the state checksum is not authentication, PooleBoot does not enforce PREC1 or perform persistent state I/O, PooleKernel does not execute recovery, unsigned development media must not activate, and no authority is granted.
...
| Native PooleBoot proof | `POOLEOS-N5-POOLEBOOT-7`; 8/8 host tests; 2/2 exact 113,664-byte PE32+ builds; 2/2 exact ten-file GPT/FAT32 images; 2/2 pinned OVMF runs; twenty-three ordered markers; 2/2 dual-channel and GOP-frame matches; 118/118 hostile controls | Partial N5.1-N5.9 | Unsigned seven-artifact load, retained PKMAP2/PBLIVE3 storage, host-only PINIT1/PREC1 validation, successful PBEXIT1, and permanent stop before transfer; no signature trust, PooleBoot inner enforcement, PooleKernel recovery, authenticated state persistence, kernel-entry profile, final CR3/RSP, target firmware, physical media, N5 exit, or production claim |
...
| Test suite | Cycle 109: 643 tests pass with one Windows symlink-permission skip, including PREC1 policy/state/transition canonicalization, hostile, differential, receipt, activation-denial, integrated-oracle, claim-boundary, and Doctor external-report nonmutation controls | Partial N36 | Predominantly host/reference, bounded-model, emulator, and artifact tests; no PooleKernel entry or recovery trace exists |
| Release gate | Cycle 109: 78/78 consistency checks over 73 artifacts; 20 native gaps | Partial N37 | `production_ready=false`; not a release acceptance gate |
...
Cycle 109 control-plane validation passes 643 tests with one Windows symlink-permission skip. Live Doctor passes 431/431 checks, and the 78/78 release gate passes over 73 artifacts while retaining 20 explicit gaps and `production_ready=false`. Live PooleGlyph conformance output is redirected to temporary storage, preserving the tandem repository's generated report. Exact authority and gate hashes are recorded only after their final deterministic generation.
...
Inherited sections: `013-015`. Added: `ADD-BOOT-001`, `ADD-BOOT-004`, `ADD-BOOT-005`, `ADD-BOOT-006`.
...
| `FLAG-N5-INIT-SEMANTICS-001` | REQUIRED | Open in Cycle 109 | Extend frozen inner formats and independent validation to symbols, microcode, firmware, and policy, then prove live PINIT1/PREC1 enforcement, authenticated state persistence, capability creation, process/recovery launch, lifecycle, rollback, and apply/execute enforcement |
...
Cycle 109 closes only `FLAG-N5-RECOVERY-BUNDLE-001`: PREC1 immutable policy and mutable state, A/B eligibility, decrement-before-handoff, known-good fallback, bounded safe/recovery routing, authenticated receipt binding, declarative authority rules, physical-presence requirements, independent parsing/transitions, and unsigned activation denial are frozen and reproducible. N5 remains partial: PSM1 and all seven artifacts are unsigned and untrusted, PBDIGEST1 review and promotion remain open, PooleBoot does not enforce PINIT1 or PREC1 or perform authenticated persistent state I/O, PooleKernel does not create the declared initial-system objects or execute recovery, no payload instruction executes, symbol/microcode/firmware/policy inner formats remain open, and the retained CR3/RSP are never installed. N6.4-N6.6 remain partial because no PooleKernel instruction executes under the live caller. `FLAG-N5-INIT-SEMANTICS-001`, `FLAG-N6-BOOT-DIGEST-001`, `FLAG-N6-FRAMEBUFFER-MAP-001`, and `FLAG-NATIVE-BOOT-001` remain open until semantics, trust, final transfer state, kernel consumption, and target profiles are proved. The selected `hardware_fido2_ed25519_sk` device remains unavailable, so `N0-HW-KEY-ACQUIRE-001` remains the immediate blocked owner move and `FLAG-N0-GOVERNANCE-KEY-001` stays open. Codex has no authorization to generate or use keys, sign, merge, tag, register a public key, or publish a release. `FLAG-N4-MODELS-001` also remains open. The next owner-independent engineering move is `N5-SYMBOLS-SEMANTICS-001`: freeze and independently validate the symbol artifact's identity, address-space, integrity, privacy, stripping, and bounded-consumption rules before any boot or kernel path consumes symbols. PooleGlyph Phase 66 may advance in parallel without outranking N0-N6.
...
| 15 | `N5-SYMBOLS-SEMANTICS-001` | Next: freeze the symbol inner format, exact image/build identity and address model, integrity and privacy rules, stripped/public split, resource bounds, and independent hostile parser evidence before symbol consumption |
| 16 | `N5-TRANSFER-001` | After artifact-semantics and trust prerequisites: validate kernel-entry PBP1, install retained CR3/RSP with interrupts disabled, call the frozen entry ABI, and prove kernel consumption or fail-closed halt |
| 17 | `N7-TRAP-001` | GDT/TSS/IDT and deliberate breakpoint/page-fault evidence after bounded transfer exists |
| 18 | `N9-PMM-001` | Normalized map, bootstrap allocator, physical allocator, guarded mappings |
| 19 | `N8-IRQ-001` | Local APIC, timer, monotonic clock, and first SMP application processor |
| 20 | `N12-SCHED-001` | Neutral scheduler and context switch under SMP stress |
| 21 | `N13-RING3-001` | First user task, syscall, capability object, exception, and clean exit |
| 22 | `N14-IPC-001` | Capability-mediated call/reply, cancellation, quota, and hostile message tests |
| 23 | `N16-VIRTIO-001` | Isolated virtio console/RNG driver domain with revocation and restart |
