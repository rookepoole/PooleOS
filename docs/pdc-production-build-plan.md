...
Plan version: 2.54.0-native-atomics
Roadmap cycle: PooleOS Cycle 148
...
Cycle 148 closes only the bounded scope of
`FLAG-N12-CONCURRENCY-ATOMICS-001`. PKATOM1 adds allocation-free typed
`u32`, `u64`, `usize`, and pointer atomics; operation-specific load, store,
read-modify-write, compare-exchange, and fence order types; nine accepted and
eleven rejected compare-exchange order pairs; overflow-safe reference counts;
host-thread publication, contended RMW/CAS, and sequential-consistency tests;
seven exact linked x86-64 instruction audits; and a live BSP
process-to-interrupt release/acquire plus interrupt read-modify-write path that
orders publication before EOI. Eight exact Rust/Python host receipts, two exact
41-marker QEMU/OVMF runs, and 29 hostile-control categories pass. N12.1 is
complete for this frozen x86-64 atomic contract only. General locks, live
multi-AP atomic contention, reclamation, non-x86 portability, target
qualification, N12 exit, and production remain open. The next chronological
owner-independent move is `N12-CONCURRENCY-LOCKS-001`.

The Cycle 148 image uses a 585,728-byte, 143-page memory envelope. PKENTRY1
records a 513,672-byte canonical kernel, 1,289 relocations, entry `0xA000`,
...
PKSMP5, PKSCHED1-PKSCHED6, and PKATOM1 completed the refreshed replay required
by the Cycle 148 boundary.
...
| Test suite | Cycle 148: 889 Python tests collected; 196/196 Rust kernel host tests add typed atomic/order/refcount coverage, exact host contention/litmus receipts, linked-instruction review, BSP process-to-interrupt publication/RMW ordering, rollback, cleanup, and hostile controls while retaining PKSCHED6 through PKSCHED1/PKSMP5/PKSMP2/PKIRQ1/PMM/PKVM3 coverage | Partial N36 | Predominantly host/reference, bounded-model, emulator, and artifact tests; no authenticated authority, target-firmware, or physical-hardware result exists |
| Release gate | Cycle 148: the roadmap inventories 101 canonical evidence artifacts; the exact gate invocation evaluates 104 consistency checks over 61 explicit/default artifact paths while retaining 20 native gaps | Partial N37 | `production_ready=false`; not a release acceptance gate |
...
| `FLAG-N12-CONCURRENCY-LOCKS-001` | REQUIRED | Open in Cycle 148 | Implement and qualify raw and IRQ-save spinlocks, sleeping mutexes with bounded priority inheritance, reader-writer locks, seqlocks, try/timed paths, recursion and IRQ/preemption nesting rejection, lock ranks and order graph, owner-death teardown, fairness/starvation bounds, exact rollback, and exact-topology contention without claiming reclamation, general SMP, target, N12 exit, or production |
...
| 53 | `N12-CONCURRENCY-LOCKS-001` | Next owner-independent move: implement raw and IRQ-save spinlocks, mutexes with bounded priority inheritance, reader-writer locks, seqlocks, try/timed paths, lock ranks/order graph, owner-death teardown, fairness bounds, rollback, and exact-topology contention |
| 54 | `N13-RING3-001` | First user task, syscall, capability object, exception, and clean exit |
| 55 | `N14-IPC-001` | Capability-mediated call/reply, cancellation, quota, and hostile message tests |
| 56 | `N16-VIRTIO-001` | Isolated virtio console/RNG driver domain with revocation and restart |
...
- every required checklist item from all 8,998 implementation items is complete or is excluded by a signed release-profile disposition;
