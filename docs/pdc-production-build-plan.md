...
Plan version: 2.29.0-native-kernel-xstate-exception
Roadmap cycle: PooleOS Cycle 123
...
| Test suite | Cycle 123: 767 tests pass with one Windows symlink-permission skip, including PKXEXC1 policy/source/machine-code/TCG/WHPX controls plus all PKXSTATE1, PKERR1, PKCPU1, PKTRAP1, PKXFER1, PKREVAL1, and producer controls | Partial N36 | Predominantly host/reference, bounded-model, emulator, and artifact tests; no authenticated authority, target-firmware, or physical-hardware result exists |
| Release gate | Cycle 123: 90/90 consistency checks over 85 artifacts; 20 native gaps | Partial N37 | `production_ready=false`; not a release acceptance gate |
...
- live PooleBoot reads exact `TRUST.PBT` and `TRUSTST.PBS` development candidates from the twelve-file medium, cross-binds fourteen facts, emits one `TRUST_STATE DENY` marker, grants zero authority, and writes zero state. The Cycle 123-refreshed PKLOAD6 passes 123/123 Rust tests, 25 markers, and the current integrated hostile corpus across two exact QEMU/OVMF runs;
...
- 38/38 Rust tests, 8/8 Python tests, both `x86_64-unknown-none` and `x86_64-unknown-uefi` builds, 36/36 hostile controls, and 32,768/32,768 deterministic mutation rejects pass. After the Cycle 123 PSYM1/PPOL1 identity refresh, the golden nine-file retained-set SHA-256 is `331E38A7C071FFC3491DB13132726CCA12B26901059AA08690C9963D8652435D`;
- the expanded verifier requires a 262,144-byte PooleKernel image with a 180,224-byte canonical file, entry offset `0x8000`, and, after the Cycle 123 identity refresh, 352 relative relocations. PKMAP2 covers 64 kernel pages with 12 read-only, 32 executable, and 20 read-write leaves, then places low guard/stack/high guard at pages 64/65-72/73 and the handoff allocation at page 80;
- PKLOAD6 passes 123/123 Rust tests, two exact PooleBoot and PooleKernel builds, two exact media generations, two QEMU/OVMF producer runs, 25 markers, 155/155 hostile controls, and two independent 5,008-byte PBP1 reconstructions with 96 memory descriptors and ten artifact descriptors;
...
- two exact kernel builds, two feature-enabled PooleBoot builds, one default isolation build, two byte-identical media generations, and two fresh-vars QEMU/OVMF runs reproduce all 30 markers, exact PBP1 bytes, and exact guest/host denial. The Cycle 123-refreshed 180,224-byte PooleKernel has SHA-256 `36582C1CBEB4CCCB9E872C432CA095EC0896FB8E5507FA10305AA406C13D37F7`; all 58 hostile controls still pass;
...
- the Cycle 123-refreshed `runs/native_kernel_entry_readiness.json` records 38/38 host tests, two byte-identical clean linked and canonical builds, 43/43 hostile controls, 352 relative relocations, exact independent Rust/Python loaded bytes, and canonical SHA-256 `36582C1CBEB4CCCB9E872C432CA095EC0896FB8E5507FA10305AA406C13D37F7`;
...
Cycle 123 closes only `FLAG-N7-XSTATE-EXCEPTION-001`. PKXEXC1 proves deliberate processor-delivered x87 `#MF` and SIMD `#XM`, exact bounded recovery and resume, terminal test-only `#NM` eager-policy rejection, and a hash-bound linked machine-code scope audit on one WHPX BSP. Two exact runs, one expected TCG limitation probe, 41 markers, and 43 hostile controls pass; configuration writes are limited to CR0, CR4, XCR0, and test-only CR0.TS, while recovery writes are limited to `FNINIT` and `LDMXCSR`. N7.4 is partial, not complete. `FLAG-N7-ERRATA-SOURCE-001` and `FLAG-N7-MICROCODE-FLOOR-001` remain open stop-ship flags; AVX/extended state, scheduler and user-task delivery, AP/migration integration, complete per-CPU and guarded trap state, exact target qualification, and N7 exit remain open. The selected `hardware_fido2_ed25519_sk` device remains physically unavailable, so `N0-HW-KEY-ACQUIRE-001` is still the immediate blocked external move. No key, signature, public-key publication, secret use, driver load, firmware change, physical-media write, tag/release publication, or production promotion occurred in Cycle 123. The next owner-independent engineering move is `N7-PRIVILEGE-MSR-POLICY-001`: freeze and qualify the syscall/GS/TSC_AUX/MCE/performance-MSR policy with reserved-bit, read-only, virtualization, and zero-authority controls. PooleGlyph Phase 66 may advance in parallel without outranking N0-N7.
...
| 29 | `N7-PRIVILEGE-MSR-POLICY-001` | Next owner-independent move: freeze and qualify syscall, GS-base, TSC_AUX, machine-check, and performance-MSR policy with exact support gates, reserved-bit rejection, virtualization boundaries, zero writes, and zero authority |
| 30 | `N9-PMM-001` | Normalized map, bootstrap allocator, physical allocator, guarded mappings |
| 31 | `N8-IRQ-001` | Local APIC, timer, monotonic clock, and first SMP application processor |
| 32 | `N12-SCHED-001` | Neutral scheduler and context switch under SMP stress |
| 33 | `N13-RING3-001` | First user task, syscall, capability object, exception, and clean exit |
| 34 | `N14-IPC-001` | Capability-mediated call/reply, cancellation, quota, and hostile message tests |
| 35 | `N16-VIRTIO-001` | Isolated virtio console/RNG driver domain with revocation and restart |
