...
Plan version: 2.53.0-native-scheduler-smp-preempt
Roadmap cycle: PooleOS Cycle 147
...
Cycle 147 closes only the bounded scope of
`FLAG-N12-SCHED-SMP-PREEMPT-001`. PKSCHED6 composes PKSCHED2 timer/wakeup
semantics with PKSCHED4/PKSCHED5 ownership on the exact BSP-0/AP-1,2,3
topology. Four allocation-free timer/event/frame/run-queue lanes enforce
deterministic cancel, wake, and migration ordering. Eight live reschedule IPIs
and five modeled exact acknowledgements gate ownership; three quantum switches,
one offline-target rollback, watchdog/fairness bounds, and exact teardown pass.
All 102 pages or 417,792 bytes are scrubbed, verified, and released. AP-local
timer interrupts, general topology/hotplug/x2APIC, general SMP preemption,
ring-3/address-space switching, complete per-task architectural state, target
qualification, N12 exit, and production remain open. The next chronological
owner-independent move is `N12-CONCURRENCY-ATOMICS-001`.

The Cycle 147 image uses a 585,728-byte, 143-page memory envelope. PKENTRY1
records a 513,672-byte canonical kernel, 1,264 relocations, entry `0xA000`,
...
PKSMP5, and PKSCHED1-PKSCHED6 completed the refreshed replay required by the
Cycle 147 boundary.
...
| Test suite | Cycle 147: 874 Python tests collected; 189/189 Rust kernel host tests add PKSCHED6 per-CPU semantic timer/frame ownership, deterministic event ordering, live acknowledgement-gated reschedule IPIs, quantum switching, rollback, watchdog/fairness, cleanup, oracle, source, linked-instruction, and hostile controls while retaining PKSCHED5 through PKSCHED1/PKSMP5/PKSMP2/PKIRQ1/PMM/PKVM3 coverage | Partial N36 | Predominantly host/reference, bounded-model, emulator, and artifact tests; no authenticated authority, target-firmware, or physical-hardware result exists |
| Release gate | Cycle 147: the roadmap inventories 100 canonical evidence artifacts; the exact gate invocation evaluates 103 consistency checks over 60 explicit/default artifact paths while retaining 20 native gaps | Partial N37 | `production_ready=false`; not a release acceptance gate |
...
| Native kernel | Real reproducible 143-page PooleKernel PKELF1 source and image with host-qualified PKREVAL1 and opt-in QEMU live-entry/revalidation, trap, CPU, xstate, MSR, PKPMM7/PKACPI1 physical-memory, PKVM3 virtual-memory, PKIRQ1 interrupt/time, PKSMP5 exact multi-AP/shootdown, PKSCHED1 scheduler/context-switch, PKSCHED2 BSP timer/wakeup preemption, PKSCHED3 BSP deferred workers, PKSCHED4 exact-topology AP ownership/wake/migration/dispatch, PKSCHED5 AP-local typed workers, and PKSCHED6 exact-topology SMP-preemption traces | Partial N6.4-N9.4/N12.1-N12.7 | No authenticated production entry, production capability authority, arbitrary callback or general driver/service worker framework, general topology/shootdown or AP-local timer-driven SMP preemption, I/O APIC/MSI routing, AML/complete platform graph, ring 3, IPC, capability, or driver-domain execution |
...
target qualification, and N12 exit remain open. `ADD-N12-CONCURRENCY-ATOMICS-001`
and `FLAG-N12-CONCURRENCY-ATOMICS-001` bind the next chronological N12.1 move.
...
| `FLAG-N12-CONCURRENCY-ATOMICS-001` | REQUIRED | Open in Cycle 147 | Freeze typed atomics and compiler/x86-64 memory-order contracts; qualify supported load/store/exchange/compare-exchange/RMW/fence operations through deterministic litmus tests, generated-instruction review, invalid-order rejection, and explicit interrupt/SMP/progress/portability boundaries |
...
| 52 | `N12-CONCURRENCY-ATOMICS-001` | Next owner-independent move: freeze typed atomics and compiler/x86-64 memory-order contracts; run deterministic litmus and generated-instruction reviews; reject invalid orderings; document interrupt, SMP, progress, and portability boundaries |
| 53 | `N13-RING3-001` | First user task, syscall, capability object, exception, and clean exit |
| 54 | `N14-IPC-001` | Capability-mediated call/reply, cancellation, quota, and hostile message tests |
| 55 | `N16-VIRTIO-001` | Isolated virtio console/RNG driver domain with revocation and restart |
