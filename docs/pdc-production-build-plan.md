...
Plan version: 2.51.0-native-scheduler-smp
Roadmap cycle: PooleOS Cycle 145
...
Cycle 145 closes only the bounded scope of `FLAG-N12-SCHED-SMP-001`.
PKSCHED4 composes four allocation-free local run queues with PKSMP5's exact
BSP-0/AP-1,2,3 topology. It proves acknowledgement-gated ownership, one
generation-safe cross-CPU wake, two migrations, six live AP dispatches,
bounded fairness, one offline-target timeout rollback, stale acknowledgement
rejection, idle ownership, and exact task/AP teardown with 417,792 bytes
scrubbed and released. General topology, hotplug, x2APIC, general SMP timer
preemption, AP-local deferred workers and real driver/service consumers,
arbitrary callbacks, ring-3/address-space switching, complete per-task
architectural state, target qualification, N12 exit, and production remain
open. The next chronological owner-independent move is
`N12-SCHED-AP-WORKERS-001`.

The first selector-18 execution also exposed an exact bootstrap-stack boundary
fault at `RSP/CR2=0xFFFFFFFF80088BC8`, 1,080 bytes below the former 32-page
stack. The cross-stage layout now reserves a 36-page, 144-KiB stack between the
same absent guards, preserves all retained regions, and leaves page-table leaf
511 spare. The repaired PKVM3 path and every dependent native profile through
PKSCHED4 completed a full sequential replay with the refreshed identities and
...
| Test suite | Cycle 145: 857/857 Python tests pass with one expected Windows symlink-permission skip and 445 passing subtests; 173/173 Rust kernel host tests add PKSCHED4 exact-topology CPU ownership, acknowledged wake/migration, live AP dispatch, rollback, stale rejection, teardown, oracle, source, linked-instruction, and hostile controls while retaining PKSCHED3/PKSCHED2/PKSCHED1/PKSMP5/PKSMP2/PKIRQ1/PMM/PKVM3 coverage | Partial N36 | Predominantly host/reference, bounded-model, emulator, and artifact tests; no authenticated authority, target-firmware, or physical-hardware result exists |
| Release gate | Cycle 145: 101/101 canonical consistency checks pass over 96 artifacts while retaining 20 native gaps | Partial N37 | `production_ready=false`; not a release acceptance gate |
...
`ADD-N12-SCHED-SMP-001`, plus `ADD-N12-SCHED-AP-WORKERS-001`.
...
Cycle 142 PKSCHED1, Cycle 143 PKSCHED2, Cycle 144 PKSCHED3, and Cycle 145
PKSCHED4 evidence
...
417,792 cleared bytes. Eight focused tests within 173 kernel tests, five exact
...
`FLAG-N12-SCHED-SMP-001` for the bounded topology and still does not complete an
N12 subphase or start N12.8. `ADD-N12-SCHED-AP-WORKERS-001` and
`FLAG-N12-SCHED-AP-WORKERS-001` now track AP-local deferred workers, real
driver/service consumers, remote cancellation/flush/reclamation, offline
rollback, starvation bounds, and exact worker cleanup. General topology and SMP
preemption, complete lock/reclamation families, ring 3, address-space and full
...
| `FLAG-N12-SCHED-AP-WORKERS-001` | REQUIRED | Open in Cycle 145 | Extend PKSCHED3 to AP-local deferred workers and real kernel driver/service consumers with explicit queue and CPU ownership, remote wake/cancellation, flush and reclamation ordering, offline rollback, starvation bounds, exact worker-stack cleanup, and independent SMP stress evidence |
