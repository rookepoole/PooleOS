...
Plan version: 2.52.0-native-scheduler-ap-workers
Roadmap cycle: PooleOS Cycle 146
...
Cycle 146 closes only the bounded scope of
`FLAG-N12-SCHED-AP-WORKERS-001`. PKSCHED5 composes PKSCHED3 deferred work with
PKSCHED4's exact BSP-0/AP-1,2,3 topology. Three allocation-free AP-local queues
and workers execute twelve fixed typed calls from thirteen enqueues only after
the matching EOI; remote cancellation, flush-before-reclaim, bounded priority
bypass, one offline-target rollback, stale acknowledgement rejection, and
exact worker/AP teardown pass. All 102 pages or 417,792 bytes are scrubbed,
verified, and released. Arbitrary callbacks, a general driver/service
framework, general topology/hotplug/x2APIC, general SMP timer preemption,
...
owner-independent move is `N12-SCHED-SMP-PREEMPT-001`.

The Cycle 146 image remains inside the 557,056-byte, 136-page memory envelope.
PKENTRY1 now records a 485,000-byte canonical kernel, 1,222 relocations, entry
`0xA000`, text end `0x69000`, RELRO end and data start `0x76000`, and SHA-256
...
PKSMP5, PKSCHED1-PKSCHED4, and PKSCHED5 completed the refreshed replay.
...
| Test suite | Cycle 146: 866 Python tests collected; focused Cycle 146 closeout passes 37 tests and 46 subtests; 184/184 Rust kernel host tests add PKSCHED5 AP-worker ownership, typed consumers, cancellation, flush/reclaim, rollback, cleanup, oracle, source, linked-instruction, and hostile controls while retaining PKSCHED4/PKSCHED3/PKSCHED2/PKSCHED1/PKSMP5/PKSMP2/PKIRQ1/PMM/PKVM3 coverage | Partial N36 | Predominantly host/reference, bounded-model, emulator, and artifact tests; no authenticated authority, target-firmware, or physical-hardware result exists |
| Release gate | Cycle 146: the roadmap inventories 99 canonical evidence artifacts; the exact gate invocation passes 102/102 consistency checks over 59 explicit/default artifact paths while retaining 20 native gaps | Partial N37 | `production_ready=false`; not a release acceptance gate |
...
| Native kernel | Real reproducible 136-page PooleKernel PKELF1 source and image with host-qualified PKREVAL1 and opt-in QEMU live-entry/revalidation, trap, CPU, xstate, MSR, PKPMM7/PKACPI1 physical-memory, PKVM3 virtual-memory, PKIRQ1 interrupt/time, PKSMP5 exact multi-AP/shootdown, PKSCHED1 scheduler/context-switch, PKSCHED2 BSP timer/wakeup preemption, PKSCHED3 BSP deferred workers, PKSCHED4 exact-topology AP ownership/wake/migration/dispatch, and PKSCHED5 AP-local typed worker traces | Partial N6.4-N9.4/N12.1-N12.7 | No authenticated production entry, production capability authority, arbitrary callback or general driver/service worker framework, general topology/shootdown or SMP preemption, I/O APIC/MSI routing, AML/complete platform graph, ring 3, IPC, capability, or driver-domain execution |
...
Cycle 142 PKSCHED1 through Cycle 146 PKSCHED5 evidence
...
417,792 cleared bytes. Eight focused tests within 184 kernel tests, five exact
...
102-page cleanup pass. Ten focused tests within 184 kernel tests, six exact
...
unstarted. `ADD-N12-SCHED-SMP-PREEMPT-001` and
`FLAG-N12-SCHED-SMP-PREEMPT-001` now track per-CPU timer/frame/run-queue
ownership, remote reschedule acknowledgement, concurrent tick/wake/cancel and
migration ordering, offline rollback, watchdog/latency bounds, and exact
teardown. General topology, arbitrary callbacks or a general driver/service
framework, complete lock/reclamation families, ring 3, address-space and full
architectural switching, target qualification, and N12 exit remain open.
...
| `FLAG-N12-SCHED-SMP-PREEMPT-001` | REQUIRED | Open in Cycle 146 | Compose PKSCHED2 timer/wakeup preemption with PKSCHED4/PKSCHED5 exact-topology CPU and worker ownership; prove per-CPU timer/event/frame/run-queue ownership, acknowledgement-gated remote reschedule, deterministic concurrent ordering, offline rollback, starvation/watchdog latency bounds, and exact teardown |
...
| 48 | `N12-SCHED-DEFERRED-001` | Next owner-independent move: implement bounded interrupt-deferred work and kernel workers with cancellation, flush, duplicate suppression, recursion prevention, reclamation-safe teardown, rollback, shutdown ordering, starvation bounds, and exact cleanup |
| 49 | `N13-RING3-001` | First user task, syscall, capability object, exception, and clean exit |
| 50 | `N14-IPC-001` | Capability-mediated call/reply, cancellation, quota, and hostile message tests |
| 51 | `N16-VIRTIO-001` | Isolated virtio console/RNG driver domain with revocation and restart |
