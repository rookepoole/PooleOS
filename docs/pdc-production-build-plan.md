...
Plan version: 2.21.0-native-trust-state-contract
Roadmap cycle: PooleOS Cycle 115
...
Cycle 80 reset architecture claims without discarding validated component evidence. Cycles 81-96 established the native constitution, source boundary, pinned one-host toolchain, owner-controlled governance state, hardware and Tier 0 profiles, bounded formal models, and the PooleGlyph co-development lane. Cycles 97-107 qualified the bounded native boot path through exact profile retention and the permanent pre-transfer stop. Cycles 108-113 froze PINIT1, PREC1, PSYM1, synthetic-only PMCU1, synthetic-only PFWM1, and qualification-only PPOL1 as six independently validated inner formats. Cycle 114 integrates those parsers into live PooleBoot over the exact retained firmware-page bytes. Cycle 115 freezes PBTRUST1 as separate 320-byte immutable-policy and 256-byte mutable acceptance-state records, distinct from PREC1 boot-attempt state. The contract binds exact manifest, kernel, retained set, revocation set, role mask, policy version, secure-version floor, trust epoch, state generation, redundant-copy shape, previous-state chain, and external verification evidence. Live PooleBoot parses the two ESP development candidates, proves fourteen bindings, and denies exactly at `pbtrust_policy_unsigned` with zero signature verification, authority grants, or state writes. PBTRUST1 passes 4/4 Rust tests, 88/88 controls, and 24,576 differential cases; PKLOAD6 passes 85/85 Rust tests, two exact QEMU/OVMF runs, 25 markers, and 148/148 integrated controls. This is format, binding, precedence, and mandatory-denial evidence only: the ESP state is not authenticated, monotonic, writable, selected, repaired, migrated, or accepted as authority; no cryptographic verifier, authenticated revocation, Secure Boot-state verification, PooleKernel revalidation, authority creation, policy application, PooleGlyph executable authority, hardware action, or physical-media write exists.
...
| Native PooleBoot proof | `POOLEOS-N5-POOLEBOOT-7`; 8/8 host tests; 2/2 exact 202,752-byte PE32+ builds; 2/2 exact twelve-file GPT/FAT32 images; 2/2 pinned OVMF runs; twenty-five ordered markers; 2/2 dual-channel and GOP-frame matches; 148/148 hostile controls | Partial N5.1-N5.9 | Unsigned seven-artifact load, retained PKMAP2/PBLIVE3 storage, live retained-page six-format parsing, exact PBTRUST1 policy/state parsing and fourteen cross-bindings, mandatory unsigned-policy denial, successful PBEXIT1, and permanent stop before transfer; no policy-signature verification, authenticated revocation or monotonic writable backend, Secure Boot-state verification, PooleKernel revalidation or activation, authority/action/state/hardware effect, kernel-entry profile, final CR3/RSP, target firmware, physical media, N5 exit, or production claim |
| Native PBP1 handoff | Canonical 64-byte header and 32-byte descriptors; twelve typed records; 8/8 Rust tests; 2/2 `no_std` target builds; 3/3 golden vectors; 32/32 controls; 16,384 Rust/Python differential cases with zero mismatches; PKLOAD6 independently reconstructs 2/2 exact 4,728-byte post-exit development messages with seven artifacts and 95 stride-48 memory entries | Partial N5.8 | Live bytes bind retained root, guarded stack, handoff, all seven artifacts, GOP, and final-map ranges, but the unsigned development profile rejects kernel entry and no transfer or ABI ratification is proved |
...
| Native PBART1 profile artifacts | Fixed 96-byte envelope; exact roles 2-7; independent Rust/Python validation; six whole-file and payload digest bindings; distinct zero-padded loader pages; final-map and PBP1 retention; live six-format retained-page reparse | Partial N5.6 | Envelopes are unsigned and untrusted; parsing and mandatory denial create no authority, and PooleKernel revalidation plus every payload action remain open |
| Native PINIT1 initial-system bundle | Canonical 1,764-byte typed declaration bundle; 3 components, 3 services, 3 dependencies, 4 resources, 11 attenuated routes; 3/3 Rust tests; 2/2 `no_std` targets; 3/3 vectors; 120/120 controls; 16,384 Rust/Python differential cases; exact live retained-page parse with zero effects | Partial N5.6/N19/N21 | Declarations confer no authority, unsigned activation is denied, and PooleKernel revalidation, authenticated loading, capability creation, process launch, lifecycle execution, and rollback remain open |
...
| Native PBTRUST1 boot trust | Separate 320-byte PBTP1 policy and 256-byte PBTS1 acceptance-state contracts; 4/4 Rust tests; 2/2 `no_std` targets; one PooleBoot integration build; 88/88 controls; 24,576 differential cases; fourteen live bindings and exact unsigned-policy denial | Partial N5.1/N5.4/N5.5/N5.8/N5.9 | ESP records are development candidates only; no signature verification, trusted signer or revocation store, authenticated monotonic writable backend, Secure Boot-state evidence, authority, state write, PooleKernel revalidation, or production promotion |
| Native PKLOAD6 / PBLIVE3 / PKMAP2 / PBEXIT1 integration | Exact live PBC1, PSM1, real 139,264-byte PKELF1, six PBART1, PBTP1, and PBTS1 reads; 85/85 Rust host tests; 2/2 exact PooleBoot, kernel, twelve-file media, and QEMU runs; 25 markers; 148/148 controls; exact final PBP1, retained mapping, firmware-boundary agreement, live six-format and PBTRUST1 parsing, and independent oracle agreement | Partial N5.1/N5.4-N5.9 | Kernel/artifact/table/stack/handoff pages survive successful `ExitBootServices`; trust candidates deny without authority and are not persistent state; no signature trust, authenticated state I/O, PooleKernel reparse or capability creation/activation/recovery/symbol/policy/microcode/firmware application, final CR3/RSP, transferable handoff, kernel entry, final cache policy, or N5 exit |
| Native PKENTRY1 PooleKernel | Real 139,264-byte canonical PKELF1 product; fixed 0x4000 entry; 40 relative relocations; 13/13 host tests; 2/2 exact clean builds; 43/43 hostile controls; exact Rust/Python loaded bytes | Partial N6.4-N6.6 | Live PooleBoot now retains the image through `ExitBootServices` and stops before entry; authentication, transferable PBP1, final CR3/RSP, privileged diagnostics execution, descriptor tables, runtime, target firmware, and N6 exit remain open |
...
| Test suite | Cycle 115: 709 tests pass with one Windows symlink-permission skip, including PBTRUST1 policy/state/authorization precedence, hostile and differential checks, live unsigned-denial integration, and all prior retained-page controls | Partial N36 | Predominantly host/reference, bounded-model, emulator, and artifact tests; no authenticated authority, privileged probe, or PooleKernel entry trace exists |
| Release gate | Cycle 115: 83/83 consistency checks over 78 artifacts; 20 native gaps | Partial N37 | `production_ready=false`; not a release acceptance gate |
...
| Native bootloader | Reproducible unsigned Poole-authored PE32+ UEFI application with live PBC1/PSM1/PKELF1/PBART1 intake, retained PKMAP2 kernel/artifact/root/stack/handoff ranges, final PBLIVE3, bounded PBEXIT1, successful `ExitBootServices`, and firmware-free stop before transfer | Partial N5.1-N5.8 | No signature-authenticated manifest/artifacts, trusted rollback state, payload semantics, initial-system execution, kernel-entry profile, final CR3/RSP, final framebuffer cache policy, transfer, target firmware, or N5 exit |
| Native kernel | Real reproducible PooleKernel PKELF1 source and image, loaded and retained through `ExitBootServices` by PooleBoot under OVMF | Partial N6.4-N6.6 | No live entry, descriptor/exception runtime, ring 3, IPC, capability, or driver-domain execution |
...
Cycle 115 control-plane validation passes 709 tests with one Windows symlink-permission skip. Live Doctor passes its complete generated check set, and the 83/83 release gate passes over 78 artifacts while retaining 20 explicit gaps and `production_ready=false`. Live PooleGlyph conformance output is redirected to temporary storage, preserving the tandem repository's generated report. Exact authority and gate hashes are recorded only after their final deterministic generation.
...
- `ADD-BOOT-011` requires independent PooleBoot and PooleKernel reparsing of exact retained bytes before authority. Cycle 114 closes only the PooleBoot half through `FLAG-N5-INNER-PARSE-001`; `FLAG-N5-INNER-TRUST-STATE-001` and broad `FLAG-N5-INNER-ENFORCEMENT-001` remain open.

Cycle 115 boot trust policy/state contract evidence:
...
- `native/trust` provides allocation-free `no_std` parsing and authorization-order modeling. `runtime/native_boot_trust.py` independently encodes, parses, authorizes, and differentially checks both formats. Four Rust tests, both native target builds, one PooleBoot UEFI integration build, 88/88 hostile controls, and 8,192 policy-parser, 8,192 state-parser, and 8,192 authorization cases pass with zero mismatch;
- live PooleBoot reads exact `TRUST.PBT` and `TRUSTST.PBS` development candidates from the twelve-file medium, cross-binds fourteen facts, emits one `TRUST_STATE DENY` marker, grants zero authority, and writes zero state. PKLOAD6 passes 85/85 Rust tests, 25 markers, and 148/148 integrated controls across two exact QEMU/OVMF runs;
- the ESP state candidate is explicitly not persistent authority: it is unauthenticated, non-monotonic, non-writable, and neither selected, repaired, migrated, nor committed. No key, cryptographic signature verification, revocation store, Secure Boot variable observation, TPM state, firmware mutation, or physical-media write exists;
- `ADD-BOOT-012` requires separate trust-policy, acceptance-state, and recovery-attempt contracts plus a redundant authenticated transactional backend, deterministic copy selection, previous-state chaining, rollback floors, torn-write/power-loss recovery, and independent PooleKernel revalidation. Cycle 115 closes only `FLAG-N5-INNER-TRUST-CONTRACT-001`; broad `FLAG-N5-INNER-TRUST-STATE-001` and `FLAG-N5-INNER-ENFORCEMENT-001` remain open.

N5.1-N5.9 are partial. PooleBoot now performs bounded live PBC1/PSM1/PKELF1/PBART1/PBTP1/PBTS1 reads, unsigned digest and envelope binding, transactional firmware-page loading, exact retained-page parsing of all six inner formats, payload and route cross-binding, fourteen boot-trust cross-bindings, exact unsigned-policy denial, retained PKMAP2 construction, final PBLIVE3 production, bounded `ExitBootServices`, and firmware-free stop before transfer. It does not authenticate the files, verify a policy signature or revocation store, select or persist authenticated monotonic state, consume symbols, apply policy, observe Secure Boot state, privileged revisions, or live firmware inventory, or apply microcode or firmware. PooleKernel does not independently reparse the retained files or selected trust state, create capabilities, or execute lifecycle/recovery/policy actions. The path still does not authenticate a signer, enforce authenticated rollback, validate a real vendor payload or redistribution right, establish the final active CR3/RSP or framebuffer cache policy, produce a kernel-entry-valid PBP1, transfer to or execute PooleKernel, measure through TCG2, use TPM state, test target firmware, write physical media, build an ISO, or satisfy the N5 exit gate. The static identity is not the final animated PooleGlass boot experience or an accessibility result.
...
| `FLAG-N5-INNER-TRUST-STATE-001` | BLOCKER | Open in Cycle 115 | Implement and authenticate a redundant monotonic writable acceptance-state backend, revocation and Secure Boot evidence, deterministic selection/repair/migration under torn writes and power loss, and independent PooleKernel revalidation before capability creation |
| `FLAG-N5-INNER-ENFORCEMENT-001` | REQUIRED | Open in Cycle 115 | Complete authenticated state, independent PooleKernel retained-byte parsing, attenuated capability creation, authorized lifecycle/recovery/diagnostic/policy/update actions, durable audit, and rollback evidence |
...
Cycle 115 closes only `FLAG-N5-INNER-TRUST-CONTRACT-001`: PBTRUST1 freezes separate immutable-policy and mutable acceptance-state bytes, deterministic parser and authorization precedence, fourteen exact artifact/state/rollback bindings, and live unsigned-policy denial with zero signature verification, authority grants, or state writes. N5 remains partial: PSM1, all seven artifacts, and both ESP trust candidates are unsigned or unauthenticated and untrusted; authenticated revocation, Secure Boot evidence, and a redundant monotonic writable backend are absent; PBDIGEST1 review and promotion remain open; no real vendor payload or redistribution right is validated; no privileged per-processor revision or live firmware inventory is observed; and PooleKernel does not independently reparse retained bytes or selected trust state, create capabilities, execute lifecycle or recovery, consume symbols, apply policy, update a CPU, or apply firmware. Retained CR3/RSP are never installed. N6.4-N6.6 remain partial because no PooleKernel instruction executes under the live caller. `FLAG-N5-INNER-TRUST-STATE-001`, `FLAG-N5-INNER-ENFORCEMENT-001`, `FLAG-N6-BOOT-DIGEST-001`, `FLAG-N6-FRAMEBUFFER-MAP-001`, and `FLAG-NATIVE-BOOT-001` remain open until trust, state, kernel revalidation, authorized execution, final transfer state, and target profiles are proved. The selected `hardware_fido2_ed25519_sk` device remains unavailable, so `N0-HW-KEY-ACQUIRE-001` remains the immediate blocked owner move and `FLAG-N0-GOVERNANCE-KEY-001` stays open. Codex has no authorization to generate or use keys, sign, merge, tag, register a public key, or publish a release. `FLAG-N4-MODELS-001` also remains open. The next owner-independent engineering move is `N5-INNER-TRUST-BACKEND-001`: build the authenticated redundant acceptance-state backend selection/repair model and power-loss corpus without generating or using a governance key, then require independent PooleKernel reparsing before authority. PooleGlyph Phase 66 may advance in parallel without outranking N0-N6.
...
| 21 | `N5-INNER-TRUST-BACKEND-001` | Next: implement authenticated redundant monotonic acceptance-state read/select/validate/repair/migrate semantics and torn-write/power-loss evidence while preserving zero authority until every external evidence gate passes |
| 22 | `N5-INNER-KERNEL-REVALIDATE-001` | Independently reparse the exact retained files and selected PBTRUST1 state in PooleKernel before capability creation, reject loader-summary substitution and byte changes, and close the loader-to-kernel TOCTOU boundary required by `ADD-BOOT-011` and `ADD-BOOT-012` |
| 23 | `N5-TRANSFER-001` | After artifact-enforcement and trust prerequisites: validate kernel-entry PBP1, install retained CR3/RSP with interrupts disabled, call the frozen entry ABI, and prove kernel consumption or fail-closed halt |
