...
Plan version: 2.6.0-native-boot-config
Roadmap cycle: PooleOS Cycle 99
...
| Test suite | Cycle 99: 542 tests pass with one Windows symlink-permission skip, including PBP1 and PBC1 grammar/golden/version/staleness/claim controls | Partial N36 | Predominantly host/reference, bounded-model, emulator, and artifact tests; no PooleKernel implementation trace exists |
| Release gate | Cycle 99: 72/72 consistency checks over 67 artifacts; 20 native gaps | Partial N37 | `production_ready=false`; not a release acceptance gate |
...
Cycle 99 control-plane validation passes 542 tests with one Windows symlink-permission skip, Doctor passes 335/335 checks against live PooleGlyph Phase 65, and the complete consistency release gate passes 72/72 checks over 67 artifacts while retaining 20 explicit gaps and `production_ready=false`. Exact public-index, release-gate, and handoff hashes are recorded in the Cycle 99 log after staging.
...
N5.1-N5.4, N5.7, and N5.8 are partial. N5.5, N5.6, and N5.9 remain not started. PooleBoot compiles against PBC1 but does not open or parse a live configuration, select a trusted entry, load or authenticate PooleKernel, load system artifacts, populate PBP1 from live firmware, call `ExitBootServices`, transfer to or execute PooleKernel, enforce Secure Boot, measure through TCG2, use TPM state, create a signature, test target firmware, write physical media, build an ISO, or satisfy the N5 exit gate. The static identity is not the final animated PooleGlass boot experience or an accessibility result.
...
Cycle 99 completes the bounded `N5-BOOTCFG-001` parser qualification and closes only `FLAG-N5-BOOTCFG-001`; N5.4 remains partial until PooleBoot discovers the loaded-image filesystem, performs bounded reads, parses the live file, and binds trusted selection to signed manifests and recovery policy. `FLAG-NATIVE-BOOT-001` remains STOP_SHIP because PooleBoot still lacks live configuration loading, signed-artifact and ELF loading, PBP1 producer integration, `ExitBootServices` transfer, the complete hostile loader corpus, second-host reproduction, target-firmware proof, and physical-media qualification. The selected `hardware_fido2_ed25519_sk` device remains unavailable, so `N0-HW-KEY-ACQUIRE-001` remains the immediate blocked owner move and `FLAG-N0-GOVERNANCE-KEY-001` stays open. No key generation, private-key use, signing, merge, tag, public-key registration, or publication is authorized. `FLAG-N4-MODELS-001` also remains open because all six native implementation-trace cross-checks and separately scoped liveness/refinement/conformance work are unfinished. The next owner-independent engineering move is `N5-ELF-001`, while PooleGlyph Phase 66 may advance in parallel without outranking N0-N5.
...
| 4 | `N5-BOOTCFG-001` | Bounded boot configuration grammar with duplicate, unknown-key, traversal, range, truncation, incompatible-version, and oversized-artifact rejection |
| 5 | `N5-ELF-001` | PooleBoot validates and loads a minimal PooleKernel ELF64 image |
| 6 | `N6-KENTRY-001` | Kernel entry, serial/framebuffer log, panic, build ID, signed manifest continuity |
| 7 | `N7-TRAP-001` | GDT/TSS/IDT and deliberate breakpoint/page-fault evidence |
| 8 | `N9-PMM-001` | Normalized map, bootstrap allocator, physical allocator, guarded mappings |
| 9 | `N8-IRQ-001` | Local APIC, timer, monotonic clock, and first SMP application processor |
| 10 | `N12-SCHED-001` | Neutral scheduler and context switch under SMP stress |
| 11 | `N13-RING3-001` | First user task, syscall, capability object, exception, and clean exit |
| 12 | `N14-IPC-001` | Capability-mediated call/reply, cancellation, quota, and hostile message tests |
| 13 | `N16-VIRTIO-001` | Isolated virtio console/RNG driver domain with revocation and restart |
