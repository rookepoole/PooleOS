...
Plan version: 2.4.0-native-pooleboot-proof
Roadmap cycle: PooleOS Cycle 97
...
| Test suite | Cycle 97: 521 tests pass with one Windows symlink-permission skip, including PooleBoot PE/media/path-policy/firmware-contract/marker/frame/claim controls | Partial N36 | Predominantly host/reference, bounded-model, emulator, and artifact tests; no PooleKernel implementation trace exists |
| Release gate | Cycle 97: 70/70 consistency checks over 65 artifacts; 20 native gaps | Partial N37 | `production_ready=false`; not a release acceptance gate |
...
Cycle 97 control-plane validation passes 521 tests with one Windows symlink-permission skip, Doctor passes its complete check set against live PooleGlyph Phase 65, and the complete consistency release gate passes 70/70 checks over 65 artifacts while retaining 20 explicit gaps and `production_ready=false`. Exact public-index, release-gate, and handoff hashes are recorded in the Cycle 97 log after staging.
...
Cycle 97 completes the bounded `N5-POOLEBOOT-001` proof and closes only `FLAG-N5-POOLEBOOT-PROOF-001`. `FLAG-NATIVE-BOOT-001` remains STOP_SHIP because PooleBoot still lacks its boot grammar, signed-artifact and ELF loading, frozen handoff, `ExitBootServices` transfer, hostile loader corpus, second-host reproduction, target-firmware proof, and physical-media qualification. The selected `hardware_fido2_ed25519_sk` device remains unavailable, so `N0-HW-KEY-ACQUIRE-001` remains the immediate blocked owner move and `FLAG-N0-GOVERNANCE-KEY-001` stays open. No key generation, private-key use, signing, merge, tag, public-key registration, or publication is authorized. `FLAG-N4-MODELS-001` also remains open because all six native implementation-trace cross-checks and separately scoped liveness/refinement/conformance work are unfinished. The next owner-independent engineering move is `N5-BOOTPROTO-001`, while PooleGlyph Phase 66 may advance in parallel without outranking N0-N5.
...
| 4 | `N5-BOOTPROTO-001` | Canonical boot handoff schema, independent decoder, layout assertions, golden bytes, downgrade controls, malformed corpus, and fuzz receipt |
