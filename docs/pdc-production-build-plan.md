...
Roadmap cycle: PooleOS Cycle 106
...
Cycle 80 reset architecture claims without discarding validated component evidence. Cycles 81-96 established the native constitution, source boundary, pinned one-host toolchain, owner-controlled governance state, hardware and Tier 0 profiles, bounded formal models, and the PooleGlyph co-development lane. Cycles 97-100 qualified the first PooleBoot proof, PBP1, PBC1, and PKELF1. Cycle 101 produced the real reproducible PKENTRY1 PooleKernel image, Cycles 102-104 added bounded live load, PSM1 selection, and temporary PBP1, and Cycle 105 validated exact temporary higher-half mappings. Cycle 106 executes `N5-HANDOFF-001`: PooleBoot retains the 48-page kernel, four private table pages, an eight-page guarded stack, and a one-MiB read-only handoff; captures and normalizes the final map; serializes exact post-exit development PBP1 bytes; calls `ExitBootServices` under bounded stale-key retry rules; makes zero later firmware calls; and halts permanently before transfer. Two exact QEMU/OVMF runs agree across 22 ordered markers and 95 hostile controls. The manifest and kernel remain unsigned and attacker-controllable; no trusted kernel-entry profile, initial-system load, final CR3/RSP installation, PooleKernel instruction, target-firmware result, N5 exit, or production claim follows.
...
| Native PKELF1 kernel loader | Bounded ELF64 `ET_DYN` profile; dependency-free `no_std` loader; independent Python oracle; 12/12 Rust tests; 2/2 target and 2/2 PooleBoot integration builds; 3/3 exact loaded images; 129/129 controls; 16,384 differential cases with zero mismatches | Partial N5.5 | Standalone PKELF1 receipt remains caller-buffer evidence; PKLOAD5 separately proves manifest-bound live allocation retained through successful `ExitBootServices`, but not transfer or trust |
| Native PKLOAD5 / PKMAP2 / PBEXIT1 integration | Exact live PBC1, PSM1, and real 139,264-byte PKELF1 reads; 70/70 Rust host tests; 2/2 exact PooleBoot, kernel, media, and QEMU runs; 22 markers; 95/95 controls; exact 4,208-byte final PBP1, retained mapping, and firmware-boundary agreement | Partial N5.1/N5.4/N5.5/N5.8 | Kernel/table/stack/handoff pages survive successful `ExitBootServices`; no signature trust, final CR3/RSP, transferable handoff, kernel entry, final cache policy, or N5 exit |
...
| Test suite | Cycle 106: 605 tests pass with one Windows symlink-permission skip, including PKLOAD5/PBLIVE2/PKMAP2/PBEXIT1 final-map, retention, guard, retry, post-exit firmware, marker, oracle, claim, and Doctor external-report nonmutation controls | Partial N36 | Predominantly host/reference, bounded-model, emulator, and artifact tests; no PooleKernel entry trace exists |
| Release gate | Cycle 106: 76/76 consistency checks over 71 artifacts; 20 native gaps | Partial N37 | `production_ready=false`; not a release acceptance gate |
...
| Native bootloader | Reproducible unsigned Poole-authored PE32+ UEFI application with live PBC1/PSM1/PKELF1 intake, retained PKMAP2 root/stack/handoff, final PBLIVE2, bounded PBEXIT1, successful `ExitBootServices`, and firmware-free stop before transfer | Partial N5.1-N5.5/N5.7-N5.8 | No signature-authenticated manifest, trusted rollback state, initial-system load, kernel-entry profile, final CR3/RSP, final framebuffer cache policy, transfer, target firmware, or N5 exit |
...
Cycle 106 control-plane validation passes 605 tests with one Windows symlink-permission skip. Doctor passes 400/400 checks, and the release gate passes 76/76 checks over 71 evidence artifacts while retaining 20 explicit gaps and `production_ready=false`. Live PooleGlyph conformance output is redirected to temporary storage, preserving the tandem repository's generated report. Exact authority, gate, public-index, and handoff hashes are recorded only after their final deterministic generation.
...
N5.1-N5.5, N5.7, and N5.8 are partial. N5.6 and N5.9 remain not started. PooleBoot now performs bounded live PBC1/PSM1/PKELF1 reads, unsigned digest binding, firmware-page loading, retained PKMAP2 construction, final PBLIVE2 production, bounded `ExitBootServices`, and firmware-free stop before transfer. It still does not authenticate a signer, enforce persistent rollback, load initial-system/recovery/policy artifacts, establish the final active CR3/RSP or framebuffer cache policy, produce a kernel-entry-valid PBP1, transfer to or execute PooleKernel, enforce Secure Boot, measure through TCG2, use TPM state, test target firmware, write physical media, build an ISO, or satisfy the N5 exit gate. The static identity is not the final animated PooleGlass boot experience or an accessibility result.
...
Cycle 106 completes only the bounded exit-and-stop portion of `N5-HANDOFF-001` and closes only `FLAG-N5-HANDOFF-EXIT-001`. N5 remains partial: PSM1 and PooleKernel are unsigned and untrusted, PBDIGEST1 review and promotion remain open, initial-system/recovery/policy artifacts are absent, the final framebuffer cache policy and transferable kernel-entry PBP1 are not established, and the retained CR3/RSP are never installed. N6.4-N6.6 remain partial because no PooleKernel instruction executes under the live caller. `FLAG-N6-BOOT-DIGEST-001`, `FLAG-N6-FRAMEBUFFER-MAP-001`, and `FLAG-NATIVE-BOOT-001` remain open until trust, initial-system loading, final transfer state, kernel consumption, and target profiles are proved. The selected `hardware_fido2_ed25519_sk` device remains unavailable, so `N0-HW-KEY-ACQUIRE-001` remains the immediate blocked owner move and `FLAG-N0-GOVERNANCE-KEY-001` stays open. Codex has no authorization to generate or use keys, sign, merge, tag, register a public key, or publish a release. `FLAG-N4-MODELS-001` also remains open. The next owner-independent engineering move is `N5-INIT-SYSTEM-001`: define, parse, digest-bind, load, retain, and PBP1-bind the profile-required initial-system, recovery, symbol, microcode, firmware-manifest, and policy artifacts without claiming signature trust. PooleGlyph Phase 66 may advance in parallel without outranking N0-N6.
...
| 12 | `N5-INIT-SYSTEM-001` | Next: freeze profile artifact requirements, extend PSM1/PBP1 records, and digest-bind/load/retain initial-system, recovery, symbol, microcode, firmware-manifest, and policy artifacts without signature-trust overreach |
| 13 | `N5-TRANSFER-001` | After artifact and trust prerequisites: validate kernel-entry PBP1, install retained CR3/RSP with interrupts disabled, call the frozen entry ABI, and prove kernel consumption or fail-closed halt |
| 14 | `N7-TRAP-001` | GDT/TSS/IDT and deliberate breakpoint/page-fault evidence after bounded transfer exists |
| 15 | `N9-PMM-001` | Normalized map, bootstrap allocator, physical allocator, guarded mappings |
| 16 | `N8-IRQ-001` | Local APIC, timer, monotonic clock, and first SMP application processor |
| 17 | `N12-SCHED-001` | Neutral scheduler and context switch under SMP stress |
| 18 | `N13-RING3-001` | First user task, syscall, capability object, exception, and clean exit |
| 19 | `N14-IPC-001` | Capability-mediated call/reply, cancellation, quota, and hostile message tests |
| 20 | `N16-VIRTIO-001` | Isolated virtio console/RNG driver domain with revocation and restart |
