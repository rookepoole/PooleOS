...
Plan version: 2.38.0-native-kernel-growth-automation
Roadmap cycle: PooleOS Cycle 132
...
Cycle 132 upgrades selector `8` to PKPMM6 and closes `FLAG-N9-PMM-GROWTH-AUTOMATION-001`. Every automatic scrubbed allocate/free computes exact post-operation ledger demand and reserves one allocation plus four scrub-receipt slots for failed-growth rollback and retry. Checked pressure grows through 4/8/15/29-page generations with capacities ending at 2048/256/2048/128/16. Three predecessors totaling 27 pages are revoked, scrubbed, and retired. The live sequence records 121 checks, eight triggers, three automatic growths, sixty successful cycles, and four soft fallbacks; a host control proves hard rejection before physical or ownership effects when required demand cannot fit and the 58-page next layout exceeds a 32-page window. Two exact fresh-vars qemu64 runs retain 43 markers while 147 hostile controls and 84 kernel host tests pass. They manage 117,911 source-usable and 129,160 final pages, protect 833 loader pages, admit 11,250 Boot Services pages while eleven ACPI pages remain held, scrub and verify 11,462 pages and 46,948,352 bytes, and perform 5,869,568 physical writes, 5,870,592 reads, and 22,798 temporary-PTE writes and invalidations. Growth checksum `0xF7AD111CA266071D` binds the sequence. The retained-layout shift rebinds PKVM2 to 5,560 temporary writes and invalidations. Closeout also corrects and guards a split build ID so PKMID1 and live diagnostics both carry `PKBUILD1-CYCLE132-N9-PMM-GROWTH-AUTOMATION-001`. The canonical kernel is 278,528 bytes in a 319,488-byte, 78-page image with 667 relocations and SHA-256 `CDF33067B2421550BB03A4796FF9A92AE54D40B2575188632BF2C208449B882E`. Complete ACPI consumer integration, interrupt-context/concurrent/SMP allocation, complete direct mapping and shootdown policy, heaps, MMIO/cache qualification, general pressure/OOM policy, target hardware, and N9 exit remain open under `FLAG-N9-PMM-ACPI-CONSUMER-001` and existing N9 gaps.
...
| Native PKLOAD6 / PBLIVE3 / PKMAP2 / PBEXIT1 integration | Exact live PBC1, PSM1, real 278,528-byte canonical PKELF1, six PBART1, PBTP1, and PBTS1 reads; 167/167 Rust host tests; 2/2 exact PooleBoot, kernel, twelve-file media, and QEMU runs; 25 markers; 155/155 controls; exact 97-entry final PBP1, 78-page kernel and nine-file retained mapping, fourteen-page guarded stack, five-page guarded manager plus alternate guarded ledger windows, firmware-boundary agreement, live six-format and PBTRUST1 parsing, and independent oracle agreement | Partial N5.1/N5.4-N5.9 | The default path proves retained pages, successful `ExitBootServices`, unsigned denial, and permanent stop. PKXFER1 separately proves the development transfer; signature trust, authenticated state I/O, capability/action enforcement, final framebuffer remap, production transfer, and N5 exit remain open |
| Native PKENTRY1 PooleKernel | Real 278,528-byte canonical PKELF1 product in a 319,488-byte image; fixed 0x9000 entry; 667 relative relocations; 84/84 host tests; 2/2 exact clean builds; 43/43 hostile controls; exact Rust/Python loaded bytes; matching manifest/live build ID; canonical SHA-256 `CDF33067B2421550BB03A4796FF9A92AE54D40B2575188632BF2C208449B882E` | Partial N6.4-N6.6 | PKXFER1 through PKVM2 separately prove opt-in virtualized entry and bounded BSP trap/CPU/xstate/MSR/PMM/VM slices, while PKERR1 freezes a pure exact-target policy. Authentication, production PBP1, final framebuffer remap, complete per-CPU exception/xstate/runtime policy, target firmware, and N6 exit remain open |
| Native PKPMM6 physical memory | Exact live 97-entry PBP1 intake; stable 15,376-byte manager in five guarded pages; external generation-owned active ledgers; checked automatic 4/8/15/29-page growth with final capacities 2048/256/2048/128/16, three predecessor retirements, rollback/retry headroom, four soft fallbacks, and hard pre-effect 58-page rejection; 117,911 source-usable pages; 129,160 final managed pages; 11,250 Boot Services pages admitted; 11 ACPI pages held; 833 loader pages protected; 11,462 pages and 46,948,352 bytes scrubbed and verified; 2/2 exact 43-marker QEMU runs; 147/147 hostile controls; 84/84 kernel host tests | Partial N9.1-N9.2 | One BSP and bounded 32-page windows only. Complete ACPI consumer integration, interrupt-context/concurrent/SMP allocation, complete mapping policy, heaps, MMIO/cache policy, general pressure/OOM, target, and N9 exit remain open |
| Native PKVM2 virtual memory | Eight generation-bound DMA32 table pages plus one data frame; exact inherited kernel/entry/guarded-stack/handoff mappings; bounded nine-page direct map; transactional candidate CR3 activation and exact restoration; Accessed/Dirty handling; three local invalidation receipts; 2/2 exact 40-marker QEMU runs; 46/46 hostile controls; 8,720 physical table writes; 5,560 bootstrap PTE writes and invalidations | Partial N9.3-N9.4 | One BSP only; no complete direct map, SMP shootdown/deferred reclaim, ring 3, huge pages, PCID, COW, user faults, pager, heap, target, N9-exit, or production claim |
...
| Test suite | Cycle 132: 787 Python tests pass with one Windows symlink-permission skip; the 84 Rust kernel host tests include PKPMM6 automatic repeated growth, bounded-window fallback/rejection, rollback/retry headroom, retirement, integrity/corruption rejection, lifecycle reclaim, and all predecessor kernel controls | Partial N36 | Predominantly host/reference, bounded-model, emulator, and artifact tests; no authenticated authority, target-firmware, or physical-hardware result exists |
| Release gate | Cycle 132: 93/93 consistency checks over 88 artifacts; 20 native gaps | Partial N37 | `production_ready=false`; not a release acceptance gate |
...
| Native kernel | Real reproducible 78-page PooleKernel PKELF1 source and image with host-qualified PKREVAL1 and opt-in two-run QEMU live-entry/revalidation, trap, CPU, xstate, MSR, physical-memory, and virtual-memory traces | Partial N6.4-N7.6/N9.1-N9.4 | No authenticated production entry, complete per-CPU runtime, ring 3, IPC, capability, or driver-domain execution |
...
- live PooleBoot reads exact `TRUST.PBT` and `TRUSTST.PBS` development candidates from the twelve-file medium, cross-binds fourteen facts, emits one `TRUST_STATE DENY` marker, grants zero authority, and writes zero state. The Cycle 132-refreshed PKLOAD6 passes 167/167 Rust tests, 25 markers, and the current 155-control hostile corpus across two exact QEMU/OVMF runs;
...
- the expanded verifier requires a 319,488-byte PooleKernel image with a 278,528-byte canonical file, entry offset `0x9000`, and 667 relative relocations. PKMAP2 covers 78 kernel pages with 16 read-only, 52 executable, and 10 read-write leaves; places low guard/stack/high guard at pages 78/79-92/93 and the handoff allocation at page 94; reserves temporary page 350; places manager guards/pages at 351/352-356/357; and reserves two guarded 32-page ledger windows at 358/359-390/391 and 392/393-424/425;
...
- two exact kernel builds, two feature-enabled PooleBoot builds, one default isolation build, two byte-identical media generations, and two fresh-vars QEMU/OVMF runs reproduce all 30 markers, exact PBP1 bytes, and exact guest/host denial. The Cycle 132-refreshed 278,528-byte PooleKernel has SHA-256 `CDF33067B2421550BB03A4796FF9A92AE54D40B2575188632BF2C208449B882E`; all 58 hostile controls still pass;
...
- the Cycle 132-refreshed `runs/native_kernel_entry_readiness.json` records 84/84 Rust host tests, two byte-identical clean linked and canonical builds, 43/43 hostile controls, 667 relative relocations, exact independent Rust/Python loaded bytes, an exact matching PKMID1/live build ID, and canonical SHA-256 `CDF33067B2421550BB03A4796FF9A92AE54D40B2575188632BF2C208449B882E`;
...
| `FLAG-N9-PMM-ACPI-CONSUMER-001` | REQUIRED | Open in Cycle 132 | Implement a bounded native ACPI table consumer, prove required-table validation/copy before lifecycle release, qualify ACPI reclaim, idempotence, rollback, retained exclusions and malformed-table rejection, and retain no-AML/no-SMP/no-target/no-production boundaries |
...
Cycle 132 closes only the bounded serial scope of `FLAG-N9-PMM-GROWTH-AUTOMATION-001`. PKPMM6 keeps the 15,376-byte manager stable in five guarded pages while exact-demand checks automatically grow every active ledger through 4/8/15/29 pages and final capacities 2048/256/2048/128/16. One allocation and four scrub-receipt slots remain reserved for complete failed-growth rollback and retry. Three predecessors totaling 27 pages are revoked, scrubbed, and retired. The measured sequence records 121 pressure checks, eight triggers, three automatic growths, sixty successful cycles, four soft fallbacks, and one host-proved rejection before physical or ownership effects when a 58-page next generation cannot fit. Eighty-four kernel host tests, two exact 43-marker qemu64 runs, and 147 hostile controls pass. The runs finish with 117,911 source-usable and 129,160 managed pages, 833 protected loader pages, eleven ACPI pages held, 11,462 scrubbed and verified pages, 46,948,352 scrubbed and verified bytes, 5,869,568 writes, 5,870,592 reads, and 22,798 temporary-PTE writes and invalidations. Growth checksum `0xF7AD111CA266071D` binds the sequence, and PKVM2 is rebound to 5,560 temporary writes and invalidations. Closeout fixes and directly guards the PKMID1/live build-ID match. The kernel identity is 278,528 canonical bytes, 319,488 image bytes, 78 pages, 667 relocations, and SHA-256 `CDF33067B2421550BB03A4796FF9A92AE54D40B2575188632BF2C208449B882E`; dependent evidence through PKVM2 is rebound. N9.1-N9.4 remain partial. `FLAG-N7-ERRATA-SOURCE-001` and `FLAG-N7-MICROCODE-FLOOR-001` remain open stop-ship flags; complete ACPI consumer integration, complete mapping and SMP TLB policy, huge pages, PCID, COW, user faults, pager IPC, heap/object caches, MMIO/cache qualification, interrupt-context/concurrent allocation, general pressure/OOM policy, exact target qualification, and N9 exit remain open. The selected `hardware_fido2_ed25519_sk` device remains physically unavailable, so `N0-HW-KEY-ACQUIRE-001` is still the immediate blocked external move. The owner's broad operation-category authorization remains recorded, but no key, signature, public-key publication, secret use, privileged host probe, driver load, firmware change, physical-media write, tag/release publication, or production promotion occurred in Cycle 132. The next owner-independent engineering move is `N9-PMM-ACPI-CONSUMER-001`. PooleGlyph Phase 66 may advance in parallel without outranking N0-N9.
...
| 38 | `N9-PMM-ACPI-CONSUMER-001` | Next owner-independent move: bounded native ACPI table validation/copy/release consumer, exact lifecycle transition and ACPI reclaim admission, malformed-table and rollback controls, and no-AML/no-SMP/no-target/nonproduction boundary |
| 39 | `N8-IRQ-001` | Local APIC, timer, monotonic clock, and first SMP application processor |
| 40 | `N12-SCHED-001` | Neutral scheduler and context switch under SMP stress |
| 41 | `N13-RING3-001` | First user task, syscall, capability object, exception, and clean exit |
| 42 | `N14-IPC-001` | Capability-mediated call/reply, cancellation, quota, and hostile message tests |
| 43 | `N16-VIRTIO-001` | Isolated virtio console/RNG driver domain with revocation and restart |
