...
Plan version: 2.26.0-native-kernel-cpu-policy
Roadmap cycle: PooleOS Cycle 120
...
| Test suite | Cycle 120: 747 tests pass with one Windows symlink-permission skip, including PKCPU1 policy/marker/zero-write controls plus all PKTRAP1, PKXFER1, PKREVAL1, and producer controls | Partial N36 | Predominantly host/reference, bounded-model, emulator, and artifact tests; no authenticated authority, qualified privileged mechanism, target-firmware, or physical-hardware result exists |
| Release gate | Cycle 120: 87/87 consistency checks over 82 artifacts; 20 native gaps | Partial N37 | `production_ready=false`; not a release acceptance gate |
...
- N6.4-N6.6 are only partial. PKXFER1 proves opt-in QEMU CR3/RSP installation, live COM1/debugcon/framebuffer diagnostics, and terminal unsigned denial; PKTRAP1 separately proves only a bounded BSP descriptor/exception slice; PKCPU1 separately proves only a bounded qemu64 read-only CPU-policy slice. KASLR, final debug/unwind policy, framebuffer remap/revocation, exact target-family/errata/xstate policy, complete per-CPU GDT/IDT/TSS, early self-tests, full control-state dumps, all exception/NMI/machine-check paths, retained next-boot panic state, authenticated production transfer, target firmware, and boot trust remain open.
...
Cycle 120 closes only `FLAG-N7-CPU-POLICY-001`. One opt-in QEMU-only selector-4 PKCPU1 profile extends PKXFER1 with bounded BSP read-only CPUID, CR0/CR4/EFER, XCR0, APIC/PAT/MTRR, topology, and address-width observation; two runs, 35 markers, and 41 controls pass with exact Rust/Python agreement, five MSR reads, zero CPU/MSR writes, and zero authority or actions. N7.1 and N7.3 are now partial, while N7.2 target-family/errata/microcode policy and N7.4 xstate ownership remain unstarted; N7.5-N7.6 still lack per-CPU and guarded stacks, complete vectors, user transitions, asynchronous state, NMI/machine check, recursion recovery, retained crash evidence, and target-hardware qualification. N5/N6 remain partial for the existing trust, persistent-state, framebuffer, runtime, target, and production gaps. The selected `hardware_fido2_ed25519_sk` device remains physically unavailable, so `N0-HW-KEY-ACQUIRE-001` remains the immediate blocked external move and `FLAG-N0-GOVERNANCE-KEY-001` stays open. Owner authorization covers the approved operation categories, but no key, signing, privileged or mutating operation, publication, release, or promotion occurred in Cycle 120; owner presence, custody, backup, recovery, safe-target, qualification, and exact-release gates remain mandatory. The next owner-independent engineering move is `N7-ERRATA-POLICY-001`: freeze exact Ryzen 7 9800X3D family/model/stepping, microcode-revision, errata, mitigation, and rejection rules without using qemu64 observations as target evidence. PooleGlyph Phase 66 may advance in parallel without outranking N0-N7.
...
| 26 | `N7-ERRATA-POLICY-001` | Next owner-independent move: freeze exact Ryzen 7 9800X3D identity, microcode-revision floor, errata/mitigation matrix, mandatory-feature rejection, source bindings, and target-observation nonclaims |
| 27 | `N9-PMM-001` | Normalized map, bootstrap allocator, physical allocator, guarded mappings |
| 28 | `N8-IRQ-001` | Local APIC, timer, monotonic clock, and first SMP application processor |
| 29 | `N12-SCHED-001` | Neutral scheduler and context switch under SMP stress |
| 30 | `N13-RING3-001` | First user task, syscall, capability object, exception, and clean exit |
| 31 | `N14-IPC-001` | Capability-mediated call/reply, cancellation, quota, and hostile message tests |
| 32 | `N16-VIRTIO-001` | Isolated virtio console/RNG driver domain with revocation and restart |
