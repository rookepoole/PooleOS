...
| Native PKENTRY1 PooleKernel | Real 409,600-byte canonical PKELF1 product in a 458,752-byte image; fixed 0x9000 entry; 959 relative relocations; 130 host tests; 2/2 exact clean builds; 43/43 hostile controls; exact Rust/Python loaded bytes; matching manifest/live build ID; canonical SHA-256 `6B8A9C2C3EAC559E1D9CB5965800A1671DB8F487F149861300D4EDCB279B3A11` | Partial N6.4-N6.6 | PKXFER1 through PKSMP3 separately prove opt-in virtualized entry and bounded trap/CPU/xstate/MSR/PMM/ACPI-container/sparse-direct-map/interrupt-time/first-AP/AP-local-runtime/fixed-vector-IPI slices, while PKERR1 freezes a pure exact-target policy. Authentication, production PBP1, final framebuffer remap, production capability authority, target firmware, and N6 exit remain open |
...
| Native PKMSR1 privilege/MSR policy | 2 exact TCG qemu64 QEMU/OVMF runs; 35 markers; 47/47 controls; 11 support-gated MSR reads; 10 MCA banks; 0 MCA bank reads; linked LLVM audit with 25 `RDMSR` and 3 `WRMSR`, including exact PKSMP1/PKSMP2/PKSMP3/PKIRQ1 isolation | Partial N7.3 | One virtual BSP compatibility model only. No syscall/GS/TSC_AUX activation, machine-check handler, MCA recovery, PMU owner, target AP policy, exact target qualification, N7 exit, or production claim |
| Native PKIRQ1 interrupt/time | Complete retained MADT/HPET walk; BSP xAPIC identity and 51-vector ownership; guarded uncacheable LAPIC/HPET mappings; checked HPET calibration; eight exact one-shot timer deliveries and EOIs; exact normal-path controller, PIC, MSR, clock, and mapping rollback; 130/130 kernel tests; 2/2 exact 36-marker runs; 58/58 controls | Partial N8.1/N8.3 | No I/O APIC route, MSI/MSI-X, general time service, panic-path rollback, AP interrupt state, IPI, shootdown, target qualification, N8 exit, or production claim |
| Native PKSMP1 first AP | Retained-MADT APIC-ID selection; guarded fourteen-page below-1-MiB W^X transaction; RX pre-accessed-GDT trampoline; guarded RW/NX stack/mailbox; INIT-SIPI-SIPI; exact long-mode observation; stop/quiesce/final-INIT park; alias revocation and 57,344-byte scrub/release; 130/130 kernel tests; 2/2 exact 38-marker two-vCPU runs; 72/72 controls | Partial N8.5 | One AP lifecycle only. PKSMP2 and PKSMP3 separately prove one AP-local runtime and one fixed development-only IPI transport; multi-AP startup, live partial-start rollback, real shootdown, target qualification, N8 exit, and production remain open |
| Native PKSMP2 AP-local runtime | One processor-local GDT/TSS/IDT; guarded four-page RSP0 and two-page IST1/IST2 stacks; x87/SSE owner initialization and round trip; 8 exception plus 19 interrupt gates; hardware busy-TSS and gate readback; 32-page transaction; 130/130 kernel tests; 2/2 exact 42-marker two-vCPU runs; 19/19 control categories and 159/159 rejected hostile cases; 131,072-byte scrub/release | Partial N7.4-N7.6/N8.5 | Exactly one AP in one SandyBridge-minus-AVX TCG profile. PKSMP3 separately proves only a fixed development IPI transport; real TLB shootdown, scheduler ownership, multi-AP, target qualification, N8 exit, and production remain open |
| Native PKSMP3 IPI transport | Six fixed vectors and operation classes; checksum-bound development capability, request, and response; sequence/attempt replay controls; six accepted and four denied deliveries; ten EOIs; one offline-target timeout; panic/stop/park/release transaction; 130/130 kernel tests; 2/2 exact 39-marker two-vCPU runs; 18/18 control categories and 120/120 rejected hostile cases; 131,072-byte scrub/release | Partial N8.6 | Fixed development transport only. Zero real TLB invalidations, no arbitrary callback, no production capability authority, scheduler ownership, multi-AP, target qualification, N8/N9 exit, or production claim |
...
| Test suite | Cycle 138: 819/819 Python tests pass with one Windows symlink-permission skip; 130/130 Rust kernel host tests include PKSMP3 request/response, capability, replay, acknowledgement, EOI, timeout, panic, lifecycle, cleanup, and hostile-source controls plus retained PKSMP2/PKSMP1/PKIRQ1/PMM/PKVM3 coverage | Partial N36 | Predominantly host/reference, bounded-model, emulator, and artifact tests; no authenticated authority, target-firmware, or physical-hardware result exists |
| Release gate | Cycle 138: 97/97 canonical consistency checks over 92 artifacts; 20 native gaps | Partial N37 | `production_ready=false`; not a release acceptance gate |
...
| `FLAG-N8-IRQ-001` | REQUIRED | Partial evidence; open in Cycle 138 | PKIRQ1 closes the bounded one-BSP retained-table, local-xAPIC, vector-ledger, guarded-MMIO, calibrated-HPET/local-timer, exact-delivery/EOI, and normal-rollback slice; PKSMP1, PKSMP2, and PKSMP3 separately close one first-AP lifecycle, one AP-local runtime transaction, and one fixed development-only IPI transport. I/O APIC/MSI, production capability authority, complete time services, panic-path rollback, target qualification, and real SMP shootdown remain required before closure |
...
| 44 | `N9-SMP-SHOOTDOWN-001` | Next owner-independent move: generation-bound remote TLB invalidation over PKSMP3 with target masks, per-CPU acknowledgement, stale-generation and duplicate-ack rejection, timeout/offline handling, deferred-reclaim ordering, rollback, and exact no-reuse-before-retirement evidence |
| 45 | `N12-SCHED-001` | Neutral scheduler and context switch under SMP stress |
| 46 | `N13-RING3-001` | First user task, syscall, capability object, exception, and clean exit |
| 47 | `N14-IPC-001` | Capability-mediated call/reply, cancellation, quota, and hostile message tests |
| 48 | `N16-VIRTIO-001` | Isolated virtio console/RNG driver domain with revocation and restart |
