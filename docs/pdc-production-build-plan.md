...
Plan version: 2.22.0-native-trust-backend-model
Roadmap cycle: PooleOS Cycle 116
...
| Test suite | Cycle 116: 716 tests pass with one Windows symlink-permission skip, including PBTRUST1 policy/state/authorization and PBSTATE1 anchor/selection/repair/migration/power-loss checks plus all prior live unsigned-denial and retained-page controls | Partial N36 | Predominantly host/reference, bounded-model, emulator, and artifact tests; no authenticated authority, privileged probe, or PooleKernel entry trace exists |
| Release gate | Cycle 116: 83/83 consistency checks over 78 artifacts; 20 native gaps | Partial N37 | `production_ready=false`; not a release acceptance gate |
...
| `FLAG-N5-INNER-TRUST-STATE-001` | BLOCKER | Open in Cycle 116 | Implement and authenticate the real redundant monotonic writable acceptance-state provider, revocation and Secure Boot evidence, executed selection/repair/migration under storage faults and power loss, and independent PooleKernel revalidation before capability creation |
| `FLAG-N5-INNER-ENFORCEMENT-001` | REQUIRED | Open in Cycle 116 | Complete authenticated state, independent PooleKernel retained-byte parsing, attenuated capability creation, authorized lifecycle/recovery/diagnostic/policy/update actions, durable audit, and rollback evidence |
...
| 22 | `N5-INNER-KERNEL-REVALIDATE-001` | Next: independently reparse the exact retained files and selected PBTRUST1 state in PooleKernel before capability creation, reject loader-summary substitution and byte changes, and close the loader-to-kernel TOCTOU boundary required by `ADD-BOOT-011` and `ADD-BOOT-012` |
