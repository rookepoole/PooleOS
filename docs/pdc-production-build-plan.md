...
Plan version: 2.23.0-native-kernel-revalidation
Roadmap cycle: PooleOS Cycle 117
...
| Native PBART1 profile artifacts | Fixed 96-byte envelope; exact roles 2-7; independent Rust/Python validation; six whole-file and payload digest bindings; distinct zero-padded loader pages; final-map and PBP1 retention; live PooleBoot reparse plus host-qualified independent PooleKernel reparse | Partial N5.6 | Envelopes are unsigned and untrusted; parsing and mandatory denial create no authority, and live PooleKernel execution plus every payload action remain open |
...
| Native PKLOAD6 / PBLIVE3 / PKMAP2 / PBEXIT1 integration | Exact live PBC1, PSM1, real 180,224-byte canonical PKELF1, six PBART1, PBTP1, and PBTS1 reads; 93/93 Rust host tests; 2/2 exact PooleBoot, kernel, twelve-file media, and QEMU runs; 25 markers; 155/155 controls; exact ten-descriptor final PBP1, 64-page kernel and nine-file retained mapping, firmware-boundary agreement, live six-format and PBTRUST1 parsing, and independent oracle agreement | Partial N5.1/N5.4-N5.9 | Kernel/file/table/stack/handoff pages survive successful `ExitBootServices`; trust candidates deny without authority and are not persistent state; no signature trust, authenticated state I/O, live PooleKernel reparse or capability creation/activation/recovery/symbol/policy/microcode/firmware application, final CR3/RSP, transferable handoff, kernel entry, final cache policy, or N5 exit |
| Native PKENTRY1 PooleKernel | Real 180,224-byte canonical PKELF1 product in a 262,144-byte image; fixed 0x8000 entry; 302 relative relocations; 13/13 host tests; 2/2 exact clean builds; 43/43 hostile controls; exact Rust/Python loaded bytes | Partial N6.4-N6.6 | Live PooleBoot retains the image through `ExitBootServices` and stops before entry; authentication, transferable PBP1, final CR3/RSP, privileged diagnostics execution, descriptor tables, runtime, target firmware, and N6 exit remain open |
| Native PKREVAL1 PooleKernel revalidation | Allocation-free `no_std` verifier over exact retained PSM1, six PBART1 files, PBTP1, and PBTS1; 13/13 Rust tests; 8/8 Python tests; both target builds; 36/36 hostile controls; 32,768/32,768 role-complete mutation rejects; exact Rust/Python denial receipt; zero authority, actions, and state writes | Partial N5.8/N6.4-N6.6 | Host-executed and target-built only; PooleBoot stops before transfer, the ESP state is non-authoritative, and no live kernel entry, cryptographic trust, persistent state selection, capability creation, action, write, N5/N6 exit, or production claim follows |
...
| Test suite | Cycle 117: 724 tests pass with one Windows symlink-permission skip, including PKREVAL1 exact-file/role/range/digest/binding/substitution/mutation/denial checks plus PBTRUST1/PBSTATE1 and all prior producer controls | Partial N36 | Predominantly host/reference, bounded-model, emulator, and artifact tests; no authenticated authority, privileged probe, or live PooleKernel entry trace exists |
| Release gate | Cycle 117: 84/84 consistency checks over 79 artifacts; 20 native gaps | Partial N37 | `production_ready=false`; not a release acceptance gate |
...
| ADR ratification | Frozen 16-source owner packet, completed public-safe response receipt, 2/2 ADR dispositions, 38/38 definition dispositions, canonical OpenSSH `SSHSIG` contract, public trust/revocation files, signed-tag/remote verifier, and 38 combined packet/response/ceremony controls | Partial N0/N1/N37 | Packet stays historically unselected; all seven ADRs are owner-directed but unsigned; selected hardware key unavailable; zero trusted keys. Amendment V2 permits only fully gated clean PR merges; keys, signing, public-key publication, tags, releases, and production promotion remain separately unauthorized |
...
| Native bootloader | Reproducible unsigned Poole-authored PE32+ UEFI application with live PBC1/PSM1/PKELF1/PBART1/PBTP1/PBTS1 intake, retained PKMAP2 kernel/nine-file/root/stack/handoff ranges, ten-descriptor final PBLIVE3, bounded PBEXIT1, successful `ExitBootServices`, and firmware-free stop before transfer | Partial N5.1-N5.8 | No signature-authenticated manifest/artifacts, trusted rollback state, payload authority, initial-system execution, final CR3/RSP, final framebuffer cache policy, transfer, target firmware, or N5 exit |
| Native kernel | Real reproducible 64-page PooleKernel PKELF1 source and image with host-qualified PKREVAL1, loaded and retained through `ExitBootServices` by PooleBoot under OVMF | Partial N6.4-N6.6 | No live entry or revalidation trace, descriptor/exception runtime, ring 3, IPC, capability, or driver-domain execution |
...
- Public-key/fingerprint review, signer registration, detached signing, signed tagging, and publication remain individually gated and unauthorized. Standing Authority Amendment V2 separately permits only fully qualified clean PR merges under the Goal Charter; that execution authority does not ratify an ADR, create a key/signature, or promote a release. Codex generated or used no key and performed none of those cryptographic actions.
...
- PKMAP2 preserves the exact 48-page kernel plan and adds four retained page-table pages, eight RW/NX stack pages bracketed by absent guards, and 256 RO/NX handoff pages; the independent Rust/Python retained-layout fingerprint is `DCA9DC32D914438D`;
...
- `ADD-BOOT-011` requires independent PooleBoot and PooleKernel reparsing of exact retained bytes before authority. Cycle 114 closes the live PooleBoot half through `FLAG-N5-INNER-PARSE-001`; Cycle 117 closes the allocation-free host-executed PooleKernel half through `FLAG-N5-INNER-KERNEL-REVALIDATE-001`. Live post-transfer kernel execution remains open under `FLAG-N5-KERNEL-TRANSFER-001`, while authenticated state and authority enforcement remain open.
...
- live PooleBoot reads exact `TRUST.PBT` and `TRUSTST.PBS` development candidates from the twelve-file medium, cross-binds fourteen facts, emits one `TRUST_STATE DENY` marker, grants zero authority, and writes zero state. PKLOAD6 passes 93/93 Rust tests, 25 markers, and 148/148 integrated controls across two exact QEMU/OVMF runs;
...
- `ADD-BOOT-012` requires separate trust-policy, acceptance-state, and recovery-attempt contracts plus a redundant authenticated transactional backend, deterministic copy selection, previous-state chaining, rollback floors, torn-write/power-loss recovery, and independent PooleKernel revalidation. Cycle 115 closes `FLAG-N5-INNER-TRUST-CONTRACT-001`; Cycle 116 closes the pure PBSTATE1 backend model; Cycle 117 closes exact host-executed PooleKernel PBTP1/PBTS1 reparsing and unsigned-policy reconstruction. A real cryptographic monotonic writable provider, persistent backend I/O, live verifier execution, and broad enforcement remain open.
...
- 13/13 Rust tests, 8/8 Python tests, both `x86_64-unknown-none` and `x86_64-unknown-uefi` builds, 36/36 hostile controls, and 32,768/32,768 deterministic mutation rejects pass. The golden nine-file retained-set SHA-256 is `FEE092995AF574AABDC329154E27E9464252923B87D58171DD4455B89FCBBA49`;
- the expanded verifier requires a 262,144-byte PooleKernel image with a 180,224-byte canonical file, entry offset `0x8000`, and 302 relative relocations. PKMAP2 now covers 64 kernel pages with 12 read-only, 32 executable, and 20 read-write leaves, then places low guard/stack/high guard at pages 64/65-72/73 and the handoff allocation at page 80;
- PKLOAD6 passes 93/93 Rust tests, two exact PooleBoot and PooleKernel builds, two exact media generations, two QEMU/OVMF producer runs, 25 markers, 155/155 hostile controls, and two independent 5,008-byte PBP1 reconstructions with 96 memory descriptors and ten artifact descriptors;
...
N5.1-N5.9 are partial. PooleBoot now performs bounded live PBC1/PSM1/PKELF1/PBART1/PBTP1/PBTS1 reads, unsigned digest and envelope binding, transactional firmware-page loading, exact retention of PSM1, six PBART1 files, PBTP1, and PBTS1, live parsing of all six inner formats, payload and route cross-binding, fourteen boot-trust cross-bindings, exact unsigned-policy denial, retained PKMAP2 construction, final ten-descriptor PBLIVE3 production, bounded `ExitBootServices`, and firmware-free stop before transfer. PBSTATE1 separately models authenticated-anchor-based redundant-copy selection and recovery but is not wired into the live path. PKREVAL1 independently reparses all nine retained files and reconstructs exact denial in host-executed PooleKernel code, but it has no live post-transfer trace. PooleBoot does not authenticate the files, verify a policy signature or revocation store, use a real monotonic provider, perform persistent trust-state I/O, consume symbols, apply policy, observe Secure Boot state, privileged revisions, or live firmware inventory, or apply microcode or firmware. PooleKernel does not yet execute under the live caller, create capabilities, or execute lifecycle/recovery/policy actions. The path still does not authenticate a signer, enforce authenticated rollback, validate a real vendor payload or redistribution right, establish the final active CR3/RSP or framebuffer cache policy, transfer to PooleKernel, measure through TCG2, use TPM state, test target firmware, write physical media, build an ISO, or satisfy the N5 exit gate. The static identity is not the final animated PooleGlass boot experience or an accessibility result.
...
| `FLAG-N5-KERNEL-TRANSFER-001` | REQUIRED | Open in Cycle 117 | Install final retained CR3 and guarded RSP after `ExitBootServices`, preserve required framebuffer and ABI state, enter PooleKernel exactly once, live-execute PKREVAL1, and emit an independently reconstructed terminal denial receipt with zero authority, actions, writes, or firmware calls |
| `FLAG-N5-INNER-TRUST-STATE-001` | BLOCKER | Open in Cycle 117 | Implement and authenticate a real cryptographic monotonic writable acceptance-state provider, revocation and Secure Boot evidence, and executed persistent selection/repair/migration under storage faults and power loss |
| `FLAG-N5-INNER-ENFORCEMENT-001` | REQUIRED | Open in Cycle 117 | Complete authenticated state, live PooleKernel revalidation, attenuated capability creation, authorized lifecycle/recovery/diagnostic/policy/update actions, durable audit, and rollback evidence |
...
Cycle 117 closes only `FLAG-N5-INNER-KERNEL-REVALIDATE-001`: exact retained PSM1, six PBART1 files, PBTP1, and PBTS1 are represented in the final PBP1 and independently reparsed by allocation-free `no_std` PooleKernel code. Host Rust and Python agree on exact binding reconstruction and unsigned-policy denial across the golden bundle, 36 hostile controls, and 32,768 deterministic mutations, with zero authority, actions, and writes. N5 remains partial: all development files are unsigned or unauthenticated and untrusted; authenticated revocation, Secure Boot evidence, a real cryptographic monotonic provider, persistent backend I/O, and executed repair/migration are absent; PBDIGEST1 review and promotion remain open; no real vendor payload or redistribution right is validated; no privileged per-processor revision or live firmware inventory is observed; and PooleKernel does not execute under the live caller, create capabilities, execute lifecycle or recovery, consume symbols, apply policy, update a CPU, or apply firmware. Retained CR3/RSP are never installed. N6.4-N6.6 remain partial because no PooleKernel instruction executes after `ExitBootServices`. `FLAG-N5-KERNEL-TRANSFER-001`, `FLAG-N5-INNER-TRUST-STATE-001`, `FLAG-N5-INNER-ENFORCEMENT-001`, `FLAG-N6-BOOT-DIGEST-001`, `FLAG-N6-FRAMEBUFFER-MAP-001`, and `FLAG-NATIVE-BOOT-001` remain open until live transfer, trust, state, authorized execution, final transfer state, and target profiles are proved. The selected `hardware_fido2_ed25519_sk` device remains unavailable, so `N0-HW-KEY-ACQUIRE-001` remains the immediate blocked owner move and `FLAG-N0-GOVERNANCE-KEY-001` stays open. Standing Authority Amendment V2 permits fully gated clean PR merges but does not authorize key generation/use, signing, public-key publication, tags, releases, secrets, privileged probes, drivers, firmware, physical-media writes, weakened governance, or production promotion. `FLAG-N4-MODELS-001` also remains open. The next owner-independent engineering move is `N5-KERNEL-TRANSFER-001`: install final CR3/RSP, enter PooleKernel once, and prove live PKREVAL1 terminal denial without granting authority. PooleGlyph Phase 66 may advance in parallel without outranking N0-N6.
...
| 23 | `N5-KERNEL-TRANSFER-001` | Next: validate the ten-descriptor kernel-entry PBP1, install retained CR3/RSP with interrupts disabled, preserve the temporary framebuffer mapping and ABI state, call the frozen entry exactly once, live-execute PKREVAL1, and prove a terminal zero-authority denial or fail-closed halt |
| 24 | `N7-TRAP-001` | GDT/TSS/IDT and deliberate breakpoint/page-fault evidence after bounded transfer exists |
| 25 | `N9-PMM-001` | Normalized map, bootstrap allocator, physical allocator, guarded mappings |
| 26 | `N8-IRQ-001` | Local APIC, timer, monotonic clock, and first SMP application processor |
| 27 | `N12-SCHED-001` | Neutral scheduler and context switch under SMP stress |
| 28 | `N13-RING3-001` | First user task, syscall, capability object, exception, and clean exit |
| 29 | `N14-IPC-001` | Capability-mediated call/reply, cancellation, quota, and hostile message tests |
| 30 | `N16-VIRTIO-001` | Isolated virtio console/RNG driver domain with revocation and restart |
