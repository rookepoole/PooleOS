...
Plan version: 2.48.0-native-scheduler-foundation
Roadmap cycle: PooleOS Cycle 142
...
Cycle 142 closes only the bounded scope of `FLAG-N12-SCHED-FOUNDATION-001`.
PKSCHED1 adds an allocation-free four-CPU/eight-task scheduler core, neutral
fixed-priority round-robin policy, generation-safe task lifecycle, selected
synchronization and lifetime primitives, a deterministic independent oracle,
and an exact live two-task cooperative BSP context switch with isolated and
cleared kernel stacks. Interrupt-driven preemption, deferred work, live AP
dispatch, ring-3/address-space switching, full per-task architectural state,
target qualification, N12 exit, and production remain open. The next
chronological owner-independent move is `N12-SCHED-PREEMPT-001`.
...
| Native PKMSR1 privilege/MSR policy | 2 exact TCG qemu64 QEMU/OVMF runs; 35 markers; 47/47 controls; 11 support-gated MSR reads; 10 MCA banks; 0 MCA bank reads; linked LLVM audit with 31 `RDMSR` and 3 `WRMSR`, including exact PKSMP1/PKSMP2/selector-14/PKSCHED1/PKIRQ1 isolation | Partial N7.3 | One virtual BSP compatibility model only. No syscall/GS/TSC_AUX activation, machine-check handler, MCA recovery, PMU owner, target AP policy, exact target qualification, N7 exit, or production claim |
| Native PKIRQ1 interrupt/time | Complete retained MADT/HPET walk; BSP xAPIC identity and 51-vector ownership; guarded uncacheable LAPIC/HPET mappings; checked HPET calibration; eight exact one-shot timer deliveries and EOIs; exact normal-path controller, PIC, MSR, clock, and mapping rollback; 151/151 kernel tests; 2/2 exact 36-marker runs; 58/58 controls | Partial N8.1/N8.3 | No I/O APIC route, MSI/MSI-X, general time service, panic-path rollback, general multi-AP interrupt routing, target qualification, N8 exit, or production claim |
| Native PKSMP1 first AP | Retained-MADT APIC-ID selection; guarded fourteen-page below-1-MiB W^X transaction; RX pre-accessed-GDT trampoline; guarded RW/NX stack/mailbox; INIT-SIPI-SIPI; exact long-mode observation; stop/quiesce/final-INIT park; alias revocation and 57,344-byte scrub/release; 151/151 kernel tests; 2/2 exact 38-marker two-vCPU runs; 72/72 controls | Partial N8.5 | One AP lifecycle only. PKSMP2 and PKSMP5 separately prove one AP-local runtime and one frozen three-AP fixed-IPI/shootdown topology; general topology, scheduler ownership, target qualification, N8 exit, and production remain open |
| Native PKSMP2 AP-local runtime | One processor-local GDT/TSS/IDT; guarded four-page RSP0 and two-page IST1/IST2 stacks; x87/SSE owner initialization and round trip; 8 exception plus 19 interrupt gates; hardware busy-TSS and gate readback; 32-page transaction; 151/151 kernel tests; 2/2 exact 42-marker two-vCPU runs; 19/19 control categories and 159/159 rejected hostile cases; 131,072-byte scrub/release | Partial N7.4-N7.6/N8.5 | Exactly one AP in one SandyBridge-minus-AVX TCG profile. PKSMP5 separately scales three private copies with bounded rollback/shootdown; scheduler ownership, general topology, target qualification, N8 exit, and production remain open |
| Native PKSMP5 multi-AP IPI and one-page-per-root shootdown | One exact four-vCPU topology; three private AP runtimes; dynamic local masks `0x2/0x4/0x8`; aggregate target/ack mask `0xE`; one APIC-4 partial-start timeout; complete park/scrub/release rollback; fresh retry; nine accepted and three denied deliveries; twelve EOIs; three executions of one linked-image-audited AP-side `INVLPG`; one generation retirement; two premature-reclaim rejections; 151/151 kernel tests; 2/2 exact 40-marker runs; 30/30 control categories and 243/243 rejected hostile cases; 102 pages and 417,792 bytes scrubbed/released | Partial N8.5-N8.6/N9.4-N9.5 | Frozen topology, one page per private root, and one generation only. No general or address-space-wide shootdown, concurrent generations, arbitrary callback, production capability authority, scheduler ownership, target qualification, N8/N9 exit, or production claim |
| Native PKSCHED1 scheduler foundation | Allocation-free four-CPU/eight-task core; generation-safe IDs; four deterministic run queues; priorities 1-31 and maximum bypass 7; affinity, modeled migration, accounting, yield/block/wake/cancel/timeout/teardown; one-mutex direct priority inheritance; bounded reference and raw-spinlock primitives; 4,096-step Rust/Python trace with 1,761 dispatches and 2,334 migrations; exact 18-instruction live BSP switch across two 16-KiB stacks; 14/14 scheduler tests within 151/151 kernel tests; 2/2 exact 17-marker runs; 28/28 control categories and 115/115 rejected cases | Partial N12.1-N12.2/N12.5-N12.7 | Cooperative BSP foundation only. No interrupt/wakeup preemption, deferred workers, live AP dispatch, ring 3, address-space switch, full FS/GS/xstate/debug/PMU ownership, general locks, target qualification, N12 exit, or production claim |
...
| Test suite | Cycle 142: 837/837 Python tests pass with one expected Windows symlink-permission skip; 151/151 Rust kernel host tests add scheduler lifecycle, fairness, synchronization, context, stack, host-oracle, source, linked-instruction, and hostile controls while retaining PKSMP5/PKSMP2/PKIRQ1/PMM/PKVM3 coverage | Partial N36 | Predominantly host/reference, bounded-model, emulator, and artifact tests; no authenticated authority, target-firmware, or physical-hardware result exists |
| Release gate | Cycle 142: 98/98 canonical consistency checks pass over 93 artifacts while retaining 20 native gaps | Partial N37 | `production_ready=false`; not a release acceptance gate |
...
| Native kernel | Real reproducible 124-page PooleKernel PKELF1 source and image with host-qualified PKREVAL1 and opt-in QEMU live-entry/revalidation, trap, CPU, xstate, MSR, PKPMM7/PKACPI1 physical-memory, PKVM3 virtual-memory, PKIRQ1 interrupt/time, PKSMP5 exact multi-AP/shootdown, and PKSCHED1 bounded scheduler/context-switch traces | Partial N6.4-N9.4/N12.1-N12.2/N12.5-N12.7 | No authenticated production entry, production capability authority, interrupt preemption, live scheduler AP ownership, general topology/shootdown, I/O APIC/MSI routing, AML/complete platform graph, ring 3, IPC, capability, or driver-domain execution |
...
Cycle 142 PKSCHED1 evidence partially advances N12.1, N12.2, N12.5, N12.6,
and N12.7. The fixed-capacity core provides generation-safe task IDs, explicit
CPU ownership and affinity, four run queues, bounded priority bypass, lifecycle
transitions, accounting, one direct-inheritance mutex, bounded references, and
a raw spinlock. The host probe and independent Python oracle agree across 4,096
steps. Two qemu64 BSP runs execute an exact 18-instruction cooperative switch
for two kernel tasks on distinct 16-KiB stacks, preserve the declared common
address-space boundary, retire both task contexts, and clear all 32,768 stack
bytes. Fourteen scheduler tests within 151 kernel tests and 115 hostile cases
pass. This evidence does not start N12.3, N12.4, or N12.8 and does not complete
any N12 subphase. `FLAG-N12-SCHED-PREEMPT-001` tracks the next bounded move:
PKIRQ1-driven BSP timer and wake/cancel preemption with exact interrupt-frame,
deferred-reschedule, accounting, rollback, and cleanup evidence.
...
| `FLAG-N12-SCHED-PREEMPT-001` | REQUIRED | Open in Cycle 142 | Integrate PKIRQ1 timer and wake/cancel paths with PKSCHED1 through a bounded interrupt-frame and deferred-reschedule contract; prove deterministic BSP quantum and wakeup preemption, nesting/interrupt-state rules, context and stack ownership, rollback, cleanup, accounting, and starvation bounds without live-AP, ring-3, address-space, target, N12-exit, or production promotion |
...
| 47 | `N12-SCHED-PREEMPT-001` | Next owner-independent move: integrate PKIRQ1 timer and wake/cancel events with a bounded BSP interrupt-frame/deferred-reschedule path, then prove exact quantum and wakeup preemption, interrupt-state rules, context ownership, accounting, rollback, cleanup, and hostile behavior without AP/ring-3/address-space/production overclaim |
| 48 | `N13-RING3-001` | First user task, syscall, capability object, exception, and clean exit |
| 49 | `N14-IPC-001` | Capability-mediated call/reply, cancellation, quota, and hostile message tests |
| 50 | `N16-VIRTIO-001` | Isolated virtio console/RNG driver domain with revocation and restart |
