...
Plan version: 2.24.0-native-kernel-transfer
Roadmap cycle: PooleOS Cycle 118
...
| Test suite | Cycle 118: 733 tests pass with one Windows symlink-permission skip, including PKXFER1 default-isolation, marker/order/runtime/PBP1/revalidation/terminal/zero-effect controls plus all prior PKREVAL1 and producer controls | Partial N36 | Predominantly host/reference, bounded-model, emulator, and artifact tests; no authenticated authority, qualified privileged mechanism, target-firmware, or physical-hardware result exists |
| Release gate | Cycle 118: 85/85 consistency checks over 79 artifacts; 20 native gaps | Partial N37 | `production_ready=false`; not a release acceptance gate |
...
- live PooleBoot reads exact `TRUST.PBT` and `TRUSTST.PBS` development candidates from the twelve-file medium, cross-binds fourteen facts, emits one `TRUST_STATE DENY` marker, grants zero authority, and writes zero state. The Cycle 118-refreshed PKLOAD6 passes 99/99 Rust tests, 25 markers, and the current integrated hostile corpus across two exact QEMU/OVMF runs;
...
- 14/14 Rust tests, 8/8 Python tests, both `x86_64-unknown-none` and `x86_64-unknown-uefi` builds, 36/36 hostile controls, and 32,768/32,768 deterministic mutation rejects pass. After the Cycle 118 PSYM1 identity refresh, the golden nine-file retained-set SHA-256 is `652F3743DC6C033D41C8F634F359BF3A8A4B4AFF8D21F7EA329A0E349619A069`;
...
- PKLOAD6 passes 99/99 Rust tests, two exact PooleBoot and PooleKernel builds, two exact media generations, two QEMU/OVMF producer runs, 25 markers, 155/155 hostile controls, and two independent 5,008-byte PBP1 reconstructions with 96 memory descriptors and ten artifact descriptors;
...
- two exact kernel builds, two feature-enabled PooleBoot builds, one default isolation build, two byte-identical media generations, and two fresh-vars QEMU/OVMF runs reproduce all 30 markers, exact PBP1 bytes, the 180,224-byte PooleKernel SHA-256 `5D06ABFC9BD525931C63CC8A48FACD66A478640CE793E20E6D66EAEEB2BCEEEA`, and exact guest/host denial. All 57 hostile controls pass;
...
- the Cycle 118-refreshed `runs/native_kernel_entry_readiness.json` records 14/14 host tests, two byte-identical clean linked and canonical builds, 43/43 hostile controls, exact independent Rust/Python loaded bytes, and canonical SHA-256 `5D06ABFC9BD525931C63CC8A48FACD66A478640CE793E20E6D66EAEEB2BCEEEA`;
...
- N6.4-N6.6 are only partial. PKXFER1 now proves opt-in QEMU CR3/RSP installation, live COM1/debugcon/framebuffer diagnostics, and terminal unsigned denial. KASLR, final debug/unwind policy, framebuffer remap/revocation, GDT/IDT/TSS, early self-tests, full register/control-state dumps, page-fault/double-fault/NMI paths, retained next-boot panic state, authenticated production transfer, target firmware, and boot trust remain open.
...
| `FLAG-N5-INNER-TRUST-STATE-001` | BLOCKER | Open in Cycle 118 | Implement and authenticate a real cryptographic monotonic writable acceptance-state provider, revocation and Secure Boot evidence, and executed persistent selection/repair/migration under storage faults and power loss |
| `FLAG-N5-INNER-ENFORCEMENT-001` | REQUIRED | Open in Cycle 118 | Complete authenticated state, production-profile PooleKernel revalidation, attenuated capability creation, authorized lifecycle/recovery/diagnostic/policy/update actions, durable audit, and rollback evidence |
...
Cycle 118 closes only `FLAG-N5-KERNEL-TRANSFER-001`. The ordinary PooleBoot build still stops before transfer; the opt-in QEMU-only `development-transfer` path installs retained CR3/RSP, clears IF/DF, enters PooleKernel once, validates runtime/PBP1 continuity, independently live-executes PKREVAL1, emits exact dual-channel terminal denial, and halts. Two exact runs agree on all 30 markers, exact PBP1 bytes, and the nine-file denial; all 57 hostile controls pass with zero signatures, authority, actions, writes, or post-exit firmware calls. N5 remains partial because every development file is unsigned or unauthenticated and untrusted; authenticated revocation, Secure Boot evidence, a real cryptographic monotonic provider, persistent backend I/O, executed repair/migration, PBDIGEST1 review, licensed vendor payloads, privileged hardware evidence, capability creation/enforcement, final framebuffer remap/revocation, production transfer, target firmware, and N5 exit are absent. N6.4-N6.6 remain partial because the live path is a terminal development denial with no descriptors, exceptions, retained crash path, memory manager, or runtime. The selected `hardware_fido2_ed25519_sk` device remains physically unavailable, so `N0-HW-KEY-ACQUIRE-001` remains the immediate blocked external move and `FLAG-N0-GOVERNANCE-KEY-001` stays open. Cycle 118 owner authorization now covers compatible-key acquisition, key generation/use, publication, signing, secrets, privileged probes, drivers, firmware changes, physical writes, releases, and promotion, but no such action occurred and owner-presence, custody, backup, recovery, safe-target, qualification, and exact-release gates remain mandatory. `FLAG-N5-INNER-TRUST-STATE-001`, `FLAG-N5-INNER-ENFORCEMENT-001`, `FLAG-N6-BOOT-DIGEST-001`, `FLAG-N6-FRAMEBUFFER-MAP-001`, `FLAG-NATIVE-BOOT-001`, and `FLAG-N4-MODELS-001` remain open. The next owner-independent engineering move is `N7-TRAP-001`: install bounded GDT/TSS/IDT state and prove deliberate breakpoint/page-fault containment after PKXFER1. PooleGlyph Phase 66 may advance in parallel without outranking N0-N6.
...
| 23 | `N5-KERNEL-TRANSFER-001` | Cycle 118 complete development boundary: default stop preserved; exact QEMU-only feature installs retained CR3/RSP, clears IF/DF, transfers once, live-executes PKREVAL1, and emits a terminal zero-authority denial across two exact runs and 57 hostile controls |
| 24 | `N7-TRAP-001` | Next: install bounded GDT/TSS/IDT state and produce deliberate breakpoint, page-fault, invalid-opcode, double-fault, and malformed-frame fail-closed evidence after PKXFER1 |
