...
Plan version: 2.43.0-native-kernel-smp-percpu-runtime
Roadmap cycle: PooleOS Cycle 137
...
| Native PKMSR1 privilege/MSR policy | 2 exact TCG qemu64 QEMU/OVMF runs; 35 markers; 47/47 controls; 11 support-gated MSR reads; 10 MCA banks; 0 MCA bank reads; linked LLVM audit with 22 `RDMSR` and 2 `WRMSR`, including exact PKSMP1/PKSMP2/PKIRQ1 isolation | Partial N7.3 | One virtual BSP compatibility model only. No syscall/GS/TSC_AUX activation, machine-check handler, MCA recovery, PMU owner, target AP policy, exact target qualification, N7 exit, or production claim |
| Native PKIRQ1 interrupt/time | Complete retained MADT/HPET walk; BSP xAPIC identity and 51-vector ownership; guarded uncacheable LAPIC/HPET mappings; checked HPET calibration; eight exact one-shot timer deliveries and EOIs; exact normal-path controller, PIC, MSR, clock, and mapping rollback; 122/122 kernel tests; 2/2 exact 36-marker runs; 58/58 controls | Partial N8.1/N8.3 | No I/O APIC route, MSI/MSI-X, general time service, panic-path rollback, AP interrupt state, IPI, shootdown, target qualification, N8 exit, or production claim |
| Native PKSMP1 first AP | Retained-MADT APIC-ID selection; guarded fourteen-page below-1-MiB W^X transaction; RX pre-accessed-GDT trampoline; guarded RW/NX stack/mailbox; INIT-SIPI-SIPI; exact long-mode observation; stop/quiesce/final-INIT park; alias revocation and 57,344-byte scrub/release; 122/122 kernel tests; 2/2 exact 38-marker two-vCPU runs; 72/72 controls | Partial N8.5 | One AP lifecycle only. PKSMP2 separately proves one AP-local runtime; multi-AP startup, live partial-start rollback, IPI/shootdown, target qualification, N8 exit, and production remain open |
| Native PKSMP2 AP-local runtime | One processor-local GDT/TSS/IDT; guarded four-page RSP0 and two-page IST1/IST2 stacks; x87/SSE owner initialization and round trip; 8 exception plus 19 interrupt gates; hardware busy-TSS and gate readback; 32-page transaction; 122/122 kernel tests; 2/2 exact 42-marker two-vCPU runs; 19/19 control categories and 159/159 rejected hostile cases; 131,072-byte scrub/release | Partial N7.4-N7.6/N8.5 | Exactly one AP in one SandyBridge-minus-AVX TCG profile. No capability-gated IPI service, TLB shootdown, scheduler ownership, multi-AP, target qualification, N8 exit, or production claim |
...
| Test suite | Cycle 137: 812/812 Python tests pass with one Windows symlink-permission skip; 122/122 Rust kernel host tests include PKSMP2 layout, topology, descriptor, guarded-stack, xstate, vector, lifecycle, cleanup, and hostile source controls plus retained PKSMP1/PKIRQ1/PMM/PKVM3 coverage | Partial N36 | Predominantly host/reference, bounded-model, emulator, and artifact tests; no authenticated authority, target-firmware, or physical-hardware result exists |
| Release gate | Cycle 137: 96/96 canonical consistency checks over 91 artifacts; 20 native gaps | Partial N37 | `production_ready=false`; not a release acceptance gate |
...
| `FLAG-N8-IRQ-001` | REQUIRED | Partial evidence; open in Cycle 137 | PKIRQ1 closes the bounded one-BSP retained-table, local-xAPIC, vector-ledger, guarded-MMIO, calibrated-HPET/local-timer, exact-delivery/EOI, and normal-rollback slice; PKSMP1 and PKSMP2 separately close one first-AP lifecycle and one AP-local runtime transaction. I/O APIC/MSI, capability-gated IPIs, complete time services, panic-path rollback, target qualification, and real SMP shootdown remain required before closure |
...
Cycle 137 closes only `FLAG-N8-SMP-PERCPU-RUNTIME-001`. PKSMP2 extends the proven one-AP lifecycle with a processor-local GDT/TSS/IDT, guarded RSP0/IST1/IST2 stacks, x87/SSE owner state, and eight exception plus nineteen interrupt gates. The BSP verifies hardware-loaded descriptor state, both mailbox checksums, quiescence, final INIT parking, and exact zero/readback release of all 32 pages or 131,072 bytes. One hundred twenty-two kernel host tests, two exact 42-marker two-vCPU SandyBridge-minus-AVX TCG runs, 19 hostile-control categories, and 159 independently rejected cases pass. The canonical kernel is 409,600 bytes in a 458,752-byte, 112-page image with 919 relocations and SHA-256 `214F32214494E632063238337551C355BFED150B9B49846DB8A927584B8E47F0`. N8.5 remains partial and `FLAG-N8-IRQ-001` remains open. Capability-gated IPI delivery, acknowledgement/timeouts, TLB shootdown, scheduler ownership, multi-AP startup, I/O APIC/MSI, complete time services, live partial-start and panic rollback, target qualification, and N8 exit remain open. N9.1-N9.4 remain partial and PKVM3 still rejects multi-processor retirement. `FLAG-N7-ERRATA-SOURCE-001` and `FLAG-N7-MICROCODE-FLOOR-001` remain stop-ship. The selected `hardware_fido2_ed25519_sk` device remains physically unavailable, so `N0-HW-KEY-ACQUIRE-001` is still the immediate blocked external move. No key, signature, public-key publication, secret use, privileged host probe, driver load, firmware change, physical-media write, tag/release publication, or production promotion occurred in Cycle 137. The next owner-independent engineering move is `N8-SMP-IPI-001`; PooleGlyph Phase 66 may advance in parallel without outranking N0-N9.
...
| 43 | `N8-SMP-IPI-001` | Next owner-independent move: capability-gated IPI request/delivery/acknowledgement with sequence, timeout, stale-request, duplicate-delivery, offline-CPU, stop/panic, and bounded rollback evidence before any TLB-shootdown promotion |
| 44 | `N12-SCHED-001` | Neutral scheduler and context switch under SMP stress |
| 45 | `N13-RING3-001` | First user task, syscall, capability object, exception, and clean exit |
| 46 | `N14-IPC-001` | Capability-mediated call/reply, cancellation, quota, and hostile message tests |
| 47 | `N16-VIRTIO-001` | Isolated virtio console/RNG driver domain with revocation and restart |
