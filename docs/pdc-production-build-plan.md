...
Plan version: 2.46.0-native-smp-shootdown
Roadmap cycle: PooleOS Cycle 140
...
Cycle 140 closes the bounded scope of `FLAG-N9-SMP-SHOOTDOWN-001`. PKSMP4
retains the fixed development IPI transport and adds one generation-bound
remote TLB invalidation for one AP-owned root, one page, and one retired
generation. Exact target/ack masks, stale and duplicate rejection, an offline
timeout with same-attempt retry, pre-ack reclaim denial, linked-image `INVLPG`
scope, and old/new frame readback and release are independently qualified.
General multi-AP SMP and general shootdown remain open; the next chronological
...
| Native PKIRQ1 interrupt/time | Complete retained MADT/HPET walk; BSP xAPIC identity and 51-vector ownership; guarded uncacheable LAPIC/HPET mappings; checked HPET calibration; eight exact one-shot timer deliveries and EOIs; exact normal-path controller, PIC, MSR, clock, and mapping rollback; 132/132 kernel tests; 2/2 exact 36-marker runs; 58/58 controls | Partial N8.1/N8.3 | No I/O APIC route, MSI/MSI-X, general time service, panic-path rollback, multi-AP interrupt state, target qualification, N8 exit, or production claim |
| Native PKSMP1 first AP | Retained-MADT APIC-ID selection; guarded fourteen-page below-1-MiB W^X transaction; RX pre-accessed-GDT trampoline; guarded RW/NX stack/mailbox; INIT-SIPI-SIPI; exact long-mode observation; stop/quiesce/final-INIT park; alias revocation and 57,344-byte scrub/release; 132/132 kernel tests; 2/2 exact 38-marker two-vCPU runs; 72/72 controls | Partial N8.5 | One AP lifecycle only. PKSMP2 and PKSMP4 separately prove one AP-local runtime, fixed IPI transport, and one-page shootdown; multi-AP startup, live partial-start rollback, target qualification, N8 exit, and production remain open |
| Native PKSMP2 AP-local runtime | One processor-local GDT/TSS/IDT; guarded four-page RSP0 and two-page IST1/IST2 stacks; x87/SSE owner initialization and round trip; 8 exception plus 19 interrupt gates; hardware busy-TSS and gate readback; 32-page transaction; 132/132 kernel tests; 2/2 exact 42-marker two-vCPU runs; 19/19 control categories and 159/159 rejected hostile cases; 131,072-byte scrub/release | Partial N7.4-N7.6/N8.5 | Exactly one AP in one SandyBridge-minus-AVX TCG profile. PKSMP4 separately proves fixed IPI transport and one-page remote invalidation; scheduler ownership, multi-AP, target qualification, N8 exit, and production remain open |
| Native PKSMP4 IPI and one-page shootdown | Six fixed vectors and operation classes; checksum-bound development capability, request, response, generation, root, page, target, and acknowledgement state; six accepted and four denied deliveries; ten EOIs; one offline-target timeout with same-attempt retry; exactly one linked-image-audited AP-side `INVLPG`; one generation retirement; one premature-reclaim rejection; 132/132 kernel tests; 2/2 exact 40-marker two-vCPU runs; 25/25 control categories and 169/169 rejected hostile cases; 34 pages and 139,264 bytes scrubbed/released | Partial N8.6/N9.4 | One AP-owned root, one page, and one generation only. No general or address-space-wide shootdown, multiple concurrent generations, arbitrary callback, production capability authority, scheduler ownership, multi-AP, target qualification, N8/N9 exit, or production claim |
...
| Test suite | Cycle 140: 823/823 Python tests pass with one expected Windows symlink-permission skip; 132/132 Rust kernel host tests include request/response, generation/root/page/mask acknowledgement, stale/duplicate rejection, timeout/retry, deferred reclaim, cleanup, source, and linked-instruction controls plus retained PKSMP2/PKSMP1/PKIRQ1/PMM/PKVM3 coverage | Partial N36 | Predominantly host/reference, bounded-model, emulator, and artifact tests; no authenticated authority, target-firmware, or physical-hardware result exists |
| Release gate | Cycle 140: 97/97 canonical consistency checks pass over 92 artifacts while retaining 20 native gaps | Partial N37 | `production_ready=false`; not a release acceptance gate |
...
| `FLAG-N8-IRQ-001` | REQUIRED | Partial evidence; open in Cycle 140 | PKIRQ1 closes the bounded one-BSP retained-table, local-xAPIC, vector-ledger, guarded-MMIO, calibrated-HPET/local-timer, exact-delivery/EOI, and normal-rollback slice; PKSMP1, PKSMP2, and PKSMP4 separately close one first-AP lifecycle, one AP-local runtime transaction, one fixed development-only IPI transport, and one bounded remote invalidation. I/O APIC/MSI, production capability authority, complete time services, panic-path rollback, multi-AP operation, target qualification, and general SMP shootdown remain required before closure |
...
| `FLAG-N9-SMP-SHOOTDOWN-001` | REQUIRED | Closed in Cycle 140 | PKSMP4 proves one generation-bound AP-side `INVLPG` for one AP-owned root and page, exact target/ack masks, stale and duplicate rejection, offline timeout with same-attempt retry, premature-reclaim denial, one retirement receipt, linked instruction scope, and exact release without general SMP/shootdown, scheduler, target, N8/N9-exit, or production promotion |
| `FLAG-N8-SMP-MULTI-AP-001` | REQUIRED | Open in Cycle 140 | Scale the AP-local runtime and IPI path across the frozen Tier 0 topology with unique APIC ownership, per-CPU resources, multi-target masks, partial-start and timeout rollback, exact park/release accounting, and explicit no-scheduler/no-target/no-production boundaries |
...
| 45 | `N8-SMP-MULTI-AP-001` | Next owner-independent move: scale the proven AP-local runtime and IPI path across the complete frozen Tier 0 topology with unique APIC ownership, per-CPU resources, multi-target masks, partial-start and timeout rollback, exact park/release accounting, and no scheduler/target/production overclaim |
| 46 | `N12-SCHED-001` | Neutral scheduler and context switch under SMP stress |
