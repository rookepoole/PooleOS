...
Plan version: 2.33.0-native-kernel-active-memory
Roadmap cycle: PooleOS Cycle 127
...
| Test suite | Cycle 127: 785 tests pass with one Windows symlink-permission skip, including PKVM2 activation/rollback/Accessed-Dirty/invalidation/reuse controls plus all predecessor controls | Partial N36 | Predominantly host/reference, bounded-model, emulator, and artifact tests; no authenticated authority, target-firmware, or physical-hardware result exists |
| Release gate | Cycle 127: 93/93 consistency checks over 88 artifacts; 20 native gaps | Partial N37 | `production_ready=false`; not a release acceptance gate |
...
- live PooleBoot reads exact `TRUST.PBT` and `TRUSTST.PBS` development candidates from the twelve-file medium, cross-binds fourteen facts, emits one `TRUST_STATE DENY` marker, grants zero authority, and writes zero state. The Cycle 127-refreshed PKLOAD6 passes 151/151 Rust tests, 25 markers, and the current integrated hostile corpus across two exact QEMU/OVMF runs;
...
- 66/66 Rust tests, 8/8 Python tests, both `x86_64-unknown-none` and `x86_64-unknown-uefi` builds, 36/36 hostile controls, and 32,768/32,768 deterministic mutation rejects pass. The live six-PBART1 retained set is content-bound by the current PKLOAD6 receipt, with PSM1/PBTP1/PBTS1 bound separately;
- the expanded verifier requires a 262,144-byte PooleKernel image with a 221,184-byte canonical file, entry offset `0x8000`, and 525 relative relocations. PKMAP2 covers 64 kernel pages with 14 read-only, 40 executable, and 10 read-write leaves, then places low guard/stack/high guard at pages 64/65-78/79 and the handoff allocation at page 80;
- PKLOAD6 passes 151/151 Rust tests, two exact PooleBoot and PooleKernel builds, two exact media generations, two QEMU/OVMF producer runs, 25 markers, 155/155 hostile controls, and two independent 5,048-byte PBP1 reconstructions with 97 memory descriptors and ten artifact descriptors;
...
- two exact fresh-vars qemu64 runs record eight table pages, one data page, exact inherited mappings, a bounded nine-page direct map, exact candidate readback and original-root restoration, probe byte `0xA5`, three active local invalidations, 8,720 physical table writes, 5,336 bootstrap temporary-PTE writes and invalidations, exact allocation release, and zero shootdowns, signatures, authority grants, actions, or production claims;
- selector 10 remains one-BSP development evidence. Page-content scrub policy, held-class reclaim, a complete generation-owned direct map, AP startup, SMP shootdown and deferred reclaim, ring 3, huge pages, PCID, COW, user faults, stack growth, pager IPC, heaps, MMIO/cache qualification, target hardware, and N9 exit remain open.
...
Cycle 127 closes only `FLAG-N9-VM-ACTIVE-001`. PKVM2 proves a bounded one-BSP active-root transaction over generation-bound PKPMM1 allocations through CR3 and leaf-write fault injection, two exact 40-marker qemu64 runs, and 46 hostile controls. It preserves exact inherited kernel/entry/guarded-stack/handoff mappings, directly maps exactly eight table pages and one data page, handles architectural Accessed/Dirty updates, binds protect and both unmaps to three local invalidation receipts, restores the exact original CR3, and releases both allocations only after scrub and alias revocation. The live runs record 8,720 physical table writes, 5,336 bootstrap temporary-PTE writes and invalidations, two CR3 writes, three active local invalidations, zero shootdowns, and zero signatures, authority, or actions. N9.1-N9.4 remain partial, not complete. `FLAG-N7-ERRATA-SOURCE-001` and `FLAG-N7-MICROCODE-FLOOR-001` remain open stop-ship flags; page-content scrub policy, held-class reclaim, a complete generation-owned direct map, SMP shootdown/deferred reclaim, huge pages, PCID, COW, user faults, pager IPC, heap/object caches, MMIO/cache qualification, concurrency, pressure/OOM policy, exact target qualification, and N9 exit remain open. The selected `hardware_fido2_ed25519_sk` device remains physically unavailable, so `N0-HW-KEY-ACQUIRE-001` is still the immediate blocked external move. The owner's broad operation-category authorization remains recorded, but no key, signature, public-key publication, secret use, privileged host probe, driver load, firmware change, physical-media write, tag/release publication, or production promotion occurred in Cycle 127. The next owner-independent engineering move is `N9-PMM-SCRUB-001`: freeze and qualify scrub-before-allocation and scrub-before-reuse with exact zeroization, fault rollback, generation ownership, and bounded live evidence. PooleGlyph Phase 66 may advance in parallel without outranking N0-N9.
...
| 33 | `N9-PMM-SCRUB-001` | Next owner-independent move: freeze scrub-before-allocation and scrub-before-reuse semantics, exact zeroization verification, fault rollback, generation and alias preconditions, and bounded live evidence without reclaim, concurrency, target, or production overclaim |
| 34 | `N8-IRQ-001` | Local APIC, timer, monotonic clock, and first SMP application processor |
| 35 | `N12-SCHED-001` | Neutral scheduler and context switch under SMP stress |
| 36 | `N13-RING3-001` | First user task, syscall, capability object, exception, and clean exit |
| 37 | `N14-IPC-001` | Capability-mediated call/reply, cancellation, quota, and hostile message tests |
| 38 | `N16-VIRTIO-001` | Isolated virtio console/RNG driver domain with revocation and restart |
