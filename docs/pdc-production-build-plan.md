...
Plan version: 2.7.0-native-elf-loader
Roadmap cycle: PooleOS Cycle 100
...
| Test suite | Cycle 100: 557 tests pass with one Windows symlink-permission skip, including PBP1, PBC1, and PKELF1 grammar/golden/version/staleness/hostile/differential/claim controls | Partial N36 | Predominantly host/reference, bounded-model, emulator, and artifact tests; no PooleKernel implementation trace exists |
| Release gate | Cycle 100: 73/73 consistency checks over 68 artifacts; 20 native gaps | Partial N37 | `production_ready=false`; not a release acceptance gate |
...
Cycle 100 control-plane validation passes 557 tests with one Windows symlink-permission skip, Doctor passes 350/350 checks against live PooleGlyph Phase 65, and the complete consistency release gate passes 73/73 checks over 68 artifacts while retaining 20 explicit gaps and `production_ready=false`. Exact public-index, release-gate, and handoff hashes are recorded in the Cycle 100 log after staging.
...
Cycle 100 completes only the bounded `N5-ELF-001` synthetic-loader qualification and closes only `FLAG-N5-ELF-001`; N5.5 remains partial until PooleBoot performs live bounded reads, authenticates the selected kernel, allocates exact firmware pages, installs and enforces the returned mappings, records the loaded image in PBP1, and transfers to a functional PooleKernel. `FLAG-NATIVE-BOOT-001` remains STOP_SHIP because PooleBoot still lacks live configuration and kernel loading, signed-artifact verification, paging, a live PBP1 producer/consumer, `ExitBootServices` transfer, second-host reproduction, target-firmware proof, and physical-media qualification. The selected `hardware_fido2_ed25519_sk` device remains unavailable, so `N0-HW-KEY-ACQUIRE-001` remains the immediate blocked owner move and `FLAG-N0-GOVERNANCE-KEY-001` stays open. Codex has no authorization to generate or use keys, sign, merge, tag, register a public key, or publish a release. `FLAG-N4-MODELS-001` also remains open because all six native implementation-trace cross-checks and separately scoped liveness/refinement/conformance work are unfinished. The next owner-independent engineering move is `N6-KENTRY-001`, while PooleGlyph Phase 66 may advance in parallel without outranking N0-N5.
...
| 6 | `N6-KENTRY-001` | Next: functional minimal PKELF1 PooleKernel image with entry, serial/framebuffer log, panic, build ID, and manifest-continuity boundary |
