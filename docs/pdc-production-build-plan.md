...
Plan version: 2.12.0-native-kmap
Roadmap cycle: PooleOS Cycle 105
...
Cycle 80 reset architecture claims without discarding validated component evidence. Cycles 81-96 established the native constitution, source boundary, pinned one-host toolchain, owner-controlled governance state, hardware and Tier 0 profiles, bounded formal models, and the PooleGlyph co-development lane. Cycles 97-100 qualified the first PooleBoot proof, PBP1, PBC1, and PKELF1. Cycle 101 produced the real reproducible PKENTRY1 PooleKernel image, Cycle 102 qualified bounded live load then release, Cycle 103 added canonical PSM1 manifest selection and digest binding, and Cycle 104 added an exact temporary pre-exit PBP1 producer. Cycle 105 executes `N5-KMAP-001`: PooleBoot clones the active four-level root, installs four private page-table pages for 48 exact supervisor 4 KiB higher-half kernel leaves, activates the candidate CR3, verifies the complete alias and framebuffer invariants, restores the exact original CR3 before firmware use, and releases every private table and kernel page. Two exact QEMU/OVMF runs agree with the independent host oracle across 23 ordered markers and 77 hostile integration controls. The manifest remains unsigned and attacker-controllable; no signature trust, persistent rollback, retained handoff, kernel, or page table, final framebuffer cache policy, final memory map, `ExitBootServices`, kernel consumption, or transfer follows.
...
| Native PooleBoot proof | `POOLEOS-N5-POOLEBOOT-5`; 8/8 host tests; 2/2 exact 94,720-byte PE32+ builds; 2/2 exact four-file GPT/FAT32 images; 2/2 pinned OVMF runs; twenty-three ordered markers; 2/2 dual-channel and GOP-frame matches; 77/77 hostile controls | Partial N5.1-N5.5/N5.7-N5.8 | Unsigned manifest-bound load, temporary pre-exit PBP1, temporary PKMAP1 activation and exact rollback, then complete release; no signature trust, retained mappings or handoff, final cache policy/map, `ExitBootServices`, transfer, target firmware, physical media, N5 exit, or production claim |
| Native PBP1 handoff | Canonical 64-byte header and 32-byte descriptors; twelve typed records; 8/8 Rust tests; 2/2 `no_std` target builds; 3/3 golden vectors; 32/32 controls; 16,384 Rust/Python differential cases with zero mismatches; PKLOAD4 independently reconstructs 2/2 exact 4,248-byte live snapshots with 95 stride-48 memory entries | Partial N5.8 | Live producer evidence is temporary and pre-exit; stack and CR3 are zero, kernel-entry validation rejects, all storage is released, and no final map, kernel consumer, transfer, ABI ratification, target firmware, or N5 exit is proved |
| Native PBC1 boot configuration | Canonical ASCII/LF grammar; allocation-free `no_std` parser; independent Python oracle; 12/12 Rust tests; 2/2 parser and 2/2 PooleBoot integration target builds; 3/3 vectors; 64/64 controls; 16,384 differential cases with zero mismatches | Partial N5.4 | Standalone PBC1 receipt remains parser-only; PKLOAD4 separately proves a live exact read and PSM1/PBP1 binding, but not signature trust or persistent rollback |
...
| Native PKELF1 kernel loader | Bounded ELF64 `ET_DYN` profile; dependency-free `no_std` loader; independent Python oracle; 12/12 Rust tests; 2/2 target and 2/2 PooleBoot integration builds; 3/3 exact loaded images; 129/129 controls; 16,384 differential cases with zero mismatches | Partial N5.5 | Standalone PKELF1 receipt remains caller-buffer evidence; PKLOAD4 separately proves manifest-bound live allocation through temporary PBP1 and mapping activation before release, but no retained page tables or transfer |
| Native PKLOAD4 / PKMAP1 integration | Exact live PBC1, PSM1, and real 139,264-byte PKELF1 reads; 59/59 Rust host tests; 2/2 exact PooleBoot, kernel, media, and QEMU runs; 23 markers; 77/77 controls; exact 4,248-byte PBP1 and 48-page higher-half map agreement | Partial N5.1/N5.4/N5.5/N5.8 | Candidate CR3 activates only for a bounded audit and rolls back exactly; no signature trust, retained mapping/handoff, final cache policy/map, `ExitBootServices`, or entry |
| Native PKENTRY1 PooleKernel | Real 139,264-byte canonical PKELF1 product; fixed 0x4000 entry; 40 relative relocations; 13/13 host tests; 2/2 exact clean builds; 43/43 hostile controls; exact Rust/Python loaded bytes | Partial N6.4-N6.6 | Live PooleBoot load survives temporary PBP1 and PKMAP1 activation, then is released; authentication, retained page tables, `ExitBootServices`, transfer, privileged diagnostics execution, descriptor tables, runtime, target firmware, and N6 exit remain open |
...
| Test suite | Cycle 105: 592 tests pass with one Windows symlink-permission skip, including PKLOAD4/PKMAP1 CPU-profile, coverage, W^X, leaf, activation, rollback, framebuffer, firmware-call, cleanup, marker, oracle, and claim controls | Partial N36 | Predominantly host/reference, bounded-model, emulator, and artifact tests; no PooleKernel entry trace exists |
| Release gate | Cycle 105: 76/76 consistency checks over 71 artifacts; 20 native gaps | Partial N37 | `production_ready=false`; not a release acceptance gate |
...
| Native bootloader | Reproducible unsigned Poole-authored PE32+ UEFI application with bounded live PBC1/PSM1/PKELF1 intake, digest and load-plan validation, stride-aware temporary PBP1 production, temporary PKMAP1 activation/rollback, cleanup, firmware-table/map/GOP behavior | Partial N5.1-N5.5/N5.7-N5.8 | No signature-authenticated manifest, trusted rollback state, retained mappings or handoff, final framebuffer cache policy/map, `ExitBootServices`, transfer, target firmware, or N5 exit |
...
N5.1-N5.5, N5.7, and N5.8 are partial. N5.6 and N5.9 remain not started. PooleBoot now performs bounded live PBC1/PSM1/PKELF1 reads, exact unsigned manifest and digest binding, firmware-page allocation, relocation, exact temporary pre-exit PBP1 production, and temporary audited PKMAP1 candidate-root activation with exact rollback before full release. It still does not authenticate a signer, enforce persistent rollback, retain the kernel, PBP1, or page tables, establish the final kernel address space or framebuffer cache policy, load the initial system bundle, capture the final memory map, call `ExitBootServices`, transfer to or execute PooleKernel, enforce Secure Boot, measure through TCG2, use TPM state, test target firmware, write physical media, build an ISO, or satisfy the N5 exit gate. The static identity is not the final animated PooleGlass boot experience or an accessibility result.
...
Cycle 105 completes only bounded `N5-KMAP-001` and closes only `FLAG-N5-KMAP-001`. N5 remains partial: PSM1 is unsigned and untrusted, PBDIGEST1 review and promotion remain open, the live PBP1, private page tables, and kernel pages are deliberately released after exact candidate-root rollback, the final framebuffer cache policy and final memory map are not established, and no `ExitBootServices` or transfer occurs. N6.4-N6.6 remain partial because no PooleKernel instruction executes under the live caller. `FLAG-N6-BOOT-DIGEST-001` and `FLAG-N6-FRAMEBUFFER-MAP-001` retain provider-review and final mapping/cache/revocation dependencies, and `FLAG-NATIVE-BOOT-001` remains open until signature-authenticated manifest selection, trusted rollback state, retained exact mappings and PBP1, final memory-map retry, successful `ExitBootServices`, and transfer are proved. The selected `hardware_fido2_ed25519_sk` device remains unavailable, so `N0-HW-KEY-ACQUIRE-001` remains the immediate blocked owner move and `FLAG-N0-GOVERNANCE-KEY-001` stays open. Codex has no authorization to generate or use keys, sign, merge, tag, register a public key, or publish a release. `FLAG-N4-MODELS-001` also remains open. The next owner-independent engineering move is `N5-HANDOFF-001`: retain the qualified kernel mappings and immutable PBP1, normalize and retry the final memory map, call `ExitBootServices`, prove no later boot-service call, and stop before kernel transfer. PooleGlyph Phase 66 may advance in parallel without outranking N0-N6.
...
| 11 | `N5-HANDOFF-001` | Next: bind the qualified live PBP1 to retained ranges, normalize and retry the final memory map, call `ExitBootServices`, and prove no later boot-service call before transfer work |
