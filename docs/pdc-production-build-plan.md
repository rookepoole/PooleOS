...
Roadmap cycle: PooleOS Cycle 93
...
| Test suite | Cycle 93: 505 tests pass with one Windows symlink-permission skip, including frozen-packet and completed-response staleness, placeholder, custody, measurement, private-material, and authorization controls | Partial N36 | Predominantly host/reference, bounded-model, and artifact tests; no PooleKernel implementation trace exists |
| Release gate | Cycle 93: 69/69 consistency checks over 64 artifacts; 20 native gaps | Partial N37 | `production_ready=false`; not a release acceptance gate |
...
Cycle 89-90 bounded model evidence:
...
- all seven model cases reproduce exactly over two runs, and 14/14 controls reject prerelease/toolchain substitution, stale runtime/model bindings, path escape, arbitrary TLC modes, unexpected safe violations, all four missing hostile violations, and implementation-trace overclaim;
...
- IPC, scheduler, and PooleFS transaction recovery remain unmodeled; all three current model families have zero native implementation-trace cross-checks.
...
| `FLAG-N4-MODELS-001` | BLOCKER | Open; boot/capability slice landed in Cycle 89 and VM slice landed in Cycle 90 | Boot-slot, capability, and VM checks pass within frozen bounds, but IPC, scheduler, PooleFS, and implementation-trace cross-checks remain required before dependent ABI freezes |
...
Cycle 93 completes `N0-OWNER-RESPONSE-001`: the exact completed response is validated against the frozen Cycle 91 packet, ADR-0003/0004 and all 38 objective definitions advance to owner-directed unsigned state, and 16/16 hostile response controls pass. The selected `hardware_fido2_ed25519_sk` device is unavailable, so `N0-HW-KEY-ACQUIRE-001` is the immediate blocked owner move and `FLAG-N0-GOVERNANCE-KEY-001` is open. No key generation, private-key use, signing, merge, tag, public-key registration, or publication is authorized. `FLAG-N4-MODELS-001` remains open because IPC, scheduler, PooleFS recovery, and every native implementation-trace cross-check remain unfinished. The next owner-independent engineering move remains `N4-IPC-MODEL-001`, while PooleGlyph Phase 66 may advance in parallel without outranking N0-N5.
