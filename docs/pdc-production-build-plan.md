...
- `runs/n0_owner_decision_packet.json` and `docs/n0-owner-decision-packet.md` bind 16 exact review sources and present both proposed ADRs, all 38 exact targets, all three allowed custody profiles, five bounded decisions, and an unfilled owner response form. Twelve additional controls reject stale sources, omitted or changed targets, inferred acceptance or key choice, private-material fields, signing/publication authorization, production promotion, and dropped owner actions.
