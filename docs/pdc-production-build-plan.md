...
Roadmap cycle: PooleOS Cycle 95
...
| Native formal models | `POOLEOS-N4-MODELS-4`; five safe state spaces drained; fifteen required hostile invariant violations; 20/20 repeat matches; 25/25 negative controls | Partial N4.5-N4.6 | Frozen finite constants only; PooleFS and all five implementation-trace comparisons remain open; no theorem, liveness, refinement, ABI-freeze, kernel-execution, or promotion claim |
...
| Test suite | Cycle 95: 507 tests pass with one Windows symlink-permission skip, including independent scheduler wake, duplicate-runnable, inheritance, priority, bypass, and teardown controls | Partial N36 | Predominantly host/reference, bounded-model, and artifact tests; no PooleKernel implementation trace exists |
| Release gate | Cycle 95: 69/69 consistency checks over 64 artifacts; 20 native gaps | Partial N37 | `production_ready=false`; not a release acceptance gate |
...
Cycle 89-90 and Cycle 94-95 bounded model evidence:
...
- all twenty model cases reproduce exactly over two runs, and 25/25 controls reject prerelease/toolchain substitution, stale runtime/model bindings, path escape, arbitrary TLC modes, unexpected safe violations, all fifteen missing hostile violations, and implementation-trace overclaim;
...
- PooleFS transaction recovery remains unmodeled; all five current model families have zero native implementation-trace cross-checks.
...
| `FLAG-N4-MODELS-001` | BLOCKER | Open; boot/capability slice landed in Cycle 89, VM in Cycle 90, IPC in Cycle 94, and scheduler in Cycle 95 | Boot-slot, capability, VM, IPC, and scheduler checks pass within frozen bounds, but PooleFS and implementation-trace cross-checks remain required before dependent ABI freezes |
...
Cycle 95 completes `N4-SCHEDULER-MODEL-001`: `POOLEOS-N4-MODELS-4` binds the drained 2,391-state scheduler safe graph and seven exact hostile counterexamples, and `FLAG-N4-SCHEDULER-MODEL-001` closes. The selected `hardware_fido2_ed25519_sk` device remains unavailable, so `N0-HW-KEY-ACQUIRE-001` remains the immediate blocked owner move and `FLAG-N0-GOVERNANCE-KEY-001` stays open. No key generation, private-key use, signing, merge, tag, public-key registration, or publication is authorized. `FLAG-N4-MODELS-001` remains open because PooleFS recovery and every native implementation-trace cross-check remain unfinished. The next owner-independent engineering move is `N4-POOLEFS-MODEL-001`, while PooleGlyph Phase 66 may advance in parallel without outranking N0-N5.
...
| 4 | `N4-POOLEFS-MODEL-001` | Bounded transaction begin/write/commit, journal or copy-on-write publication, crash points, replay, idempotence, old-or-new visibility, allocation ownership, checksum rejection, and recovery teardown with independent torn-write, premature-publish, double-allocation, and non-idempotent-replay mutants; QEMU source-rebuild and real PooleBoot launch evidence remain parallel N4 work |
