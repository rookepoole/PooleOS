...
Plan version: 2.20.0-native-inner-live-parse
Roadmap cycle: PooleOS Cycle 114
...
Cycle 80 reset architecture claims without discarding validated component evidence. Cycles 81-96 established the native constitution, source boundary, pinned one-host toolchain, owner-controlled governance state, hardware and Tier 0 profiles, bounded formal models, and the PooleGlyph co-development lane. Cycles 97-107 qualified the bounded native boot path through exact profile retention and the permanent pre-transfer stop. Cycles 108-113 froze PINIT1, PREC1, PSYM1, synthetic-only PMCU1, synthetic-only PFWM1, and qualification-only PPOL1 as six independently validated inner formats. Cycle 114 integrates those parsers into live PooleBoot over the exact retained firmware-page bytes. It cross-binds PPOL1's five payload digests and eleven PINIT1 routes, requires all six development gates to fail first at the missing outer signature, and emits a domain-separated retained-set SHA-256 with zero authority grants, authorized actions, state writes, and hardware observations. The target and independent oracle agree on SHA-256 `F3154B354C77D0567207994EFDDA4FE2D203611CA21D60B63872BC9FFC73C675`; PKLOAD6 passes 81/81 Rust tests, two exact QEMU/OVMF runs, 24 markers, and 139/139 integrated controls. This is parsing and mandatory-denial evidence only: no authentication, persistent-state verification, PooleKernel revalidation, authority creation, policy application, PooleGlyph executable authority, state mutation, hardware action, or physical-media write exists.
...
| Native PooleBoot proof | `POOLEOS-N5-POOLEBOOT-7`; 8/8 host tests; 2/2 exact 192,512-byte PE32+ builds; 2/2 exact ten-file GPT/FAT32 images; 2/2 pinned OVMF runs; twenty-four ordered markers; 2/2 dual-channel and GOP-frame matches; 139/139 hostile controls | Partial N5.1-N5.9 | Unsigned seven-artifact load, retained PKMAP2/PBLIVE3 storage, live retained-page PINIT1/PREC1/PSYM1/PMCU1/PFWM1/PPOL1 parsing, exact payload/route cross-binding, six mandatory denials, successful PBEXIT1, and permanent stop before transfer; no authentication, persistent-state trust, PooleKernel revalidation or activation, authority/action/state/hardware effect, kernel-entry profile, final CR3/RSP, target firmware, physical media, N5 exit, or production claim |
...
| Native PKLOAD6 / PBLIVE3 / PKMAP2 / PBEXIT1 integration | Exact live PBC1, PSM1, real 139,264-byte PKELF1, and six PBART1 reads; 81/81 Rust host tests; 2/2 exact PooleBoot, kernel, ten-file media, and QEMU runs; 24 markers; 139/139 controls; exact final PBP1, retained mapping, firmware-boundary agreement, live six-format retained-page parsing, and independent oracle agreement | Partial N5.1/N5.4-N5.9 | Kernel/artifact/table/stack/handoff pages survive successful `ExitBootServices`; no signature trust, authenticated state I/O, PooleKernel reparse or capability creation/activation/recovery/symbol/policy/microcode/firmware application, final CR3/RSP, transferable handoff, kernel entry, final cache policy, or N5 exit |
...
| Test suite | Cycle 114: 696 tests pass with one Windows symlink-permission skip, including retained-page six-format parsing, digest/route cross-binding, denial-order, zero-effect, marker/oracle divergence, and prior PPOL1 differential controls | Partial N36 | Predominantly host/reference, bounded-model, emulator, and artifact tests; no authenticated authority, privileged probe, or PooleKernel entry trace exists |
| Release gate | Cycle 114: 82/82 consistency checks over 77 artifacts; 20 native gaps | Partial N37 | `production_ready=false`; not a release acceptance gate |
...
Cycle 114 control-plane validation passes 696 tests with one Windows symlink-permission skip. Live Doctor passes its complete generated check set, and the 82/82 release gate passes over 77 artifacts while retaining 20 explicit gaps and `production_ready=false`. Live PooleGlyph conformance output is redirected to temporary storage, preserving the tandem repository's generated report. Exact authority and gate hashes are recorded only after their final deterministic generation.
...
Inherited sections: `013-015`. Added: `ADD-BOOT-001`, `ADD-BOOT-004`, `ADD-BOOT-005`, `ADD-BOOT-006`, `ADD-BOOT-007`, `ADD-BOOT-008`, `ADD-BOOT-009`, `ADD-BOOT-010`, `ADD-BOOT-011`.
...
N5.1-N5.9 are partial. PooleBoot now performs bounded live PBC1/PSM1/PKELF1/PBART1 reads, unsigned digest and envelope binding, transactional firmware-page loading, exact retained-page parsing of all six inner formats, payload and route cross-binding, mandatory development denial, retained PKMAP2 construction, final PBLIVE3 production, bounded `ExitBootServices`, and firmware-free stop before transfer. It does not authenticate the files, persist authenticated state, consume symbols, apply policy, observe privileged revisions or live firmware inventory, or apply microcode or firmware. PooleKernel does not independently reparse the retained files, create capabilities, or execute lifecycle/recovery/policy actions. The path still does not authenticate a signer, enforce authenticated rollback, validate a real vendor payload or redistribution right, establish the final active CR3/RSP or framebuffer cache policy, produce a kernel-entry-valid PBP1, transfer to or execute PooleKernel, enforce Secure Boot, measure through TCG2, use TPM state, test target firmware, write physical media, build an ISO, or satisfy the N5 exit gate. The static identity is not the final animated PooleGlass boot experience or an accessibility result.
...
| `FLAG-N5-INNER-TRUST-STATE-001` | BLOCKER | Open in Cycle 114 | Authenticate exact retained artifacts and monotonic rollback/revocation state before any loader decision, without generating or using a governance key, and require independent PooleKernel revalidation before capability creation |
| `FLAG-N5-INNER-ENFORCEMENT-001` | REQUIRED | Open in Cycle 114 | Complete authenticated state, independent PooleKernel retained-byte parsing, attenuated capability creation, authorized lifecycle/recovery/diagnostic/policy/update actions, durable audit, and rollback evidence |
...
Cycle 114 closes only `FLAG-N5-INNER-PARSE-001`: live PooleBoot reparses all six exact retained PBART1 files, cross-binds the PPOL1 payload identities and PINIT1 routes, requires all six development gates to deny at their missing outer signature, and reports zero authority, action, state-write, and hardware-observation effects. N5 remains partial: PSM1 and all seven artifacts are unsigned and untrusted, authenticated monotonic rollback/revocation state is absent, PBDIGEST1 review and promotion remain open, no real vendor payload or redistribution right is validated, no privileged per-processor revision or live firmware inventory is observed, and PooleKernel does not independently reparse retained bytes, create capabilities, execute lifecycle or recovery, consume symbols, apply policy, update a CPU, or apply firmware. Retained CR3/RSP are never installed. N6.4-N6.6 remain partial because no PooleKernel instruction executes under the live caller. `FLAG-N5-INNER-TRUST-STATE-001`, `FLAG-N5-INNER-ENFORCEMENT-001`, `FLAG-N6-BOOT-DIGEST-001`, `FLAG-N6-FRAMEBUFFER-MAP-001`, and `FLAG-NATIVE-BOOT-001` remain open until trust, state, kernel revalidation, authorized execution, final transfer state, and target profiles are proved. The selected `hardware_fido2_ed25519_sk` device remains unavailable, so `N0-HW-KEY-ACQUIRE-001` remains the immediate blocked owner move and `FLAG-N0-GOVERNANCE-KEY-001` stays open. Codex has no authorization to generate or use keys, sign, merge, tag, register a public key, or publish a release. `FLAG-N4-MODELS-001` also remains open. The next owner-independent engineering move is `N5-INNER-TRUST-STATE-001`: freeze and enforce authenticated trust and monotonic persistent-state inputs over exact retained bytes without generating a governance key, then require independent PooleKernel reparsing before authority. PooleGlyph Phase 66 may advance in parallel without outranking N0-N6.
...
| 20 | `N5-INNER-TRUST-STATE-001` | Next: define and enforce authenticated artifact trust plus monotonic rollback/revocation state over exact retained bytes without generating or using a governance key; preserve zero authority until all preconditions pass |
| 21 | `N5-INNER-KERNEL-REVALIDATE-001` | Independently reparse the exact retained files in PooleKernel before capability creation, reject loader-summary substitution and byte changes, and close the loader-to-kernel TOCTOU boundary required by `ADD-BOOT-011` |
| 22 | `N5-TRANSFER-001` | After artifact-enforcement and trust prerequisites: validate kernel-entry PBP1, install retained CR3/RSP with interrupts disabled, call the frozen entry ABI, and prove kernel consumption or fail-closed halt |
| 23 | `N7-TRAP-001` | GDT/TSS/IDT and deliberate breakpoint/page-fault evidence after bounded transfer exists |
| 24 | `N9-PMM-001` | Normalized map, bootstrap allocator, physical allocator, guarded mappings |
| 25 | `N8-IRQ-001` | Local APIC, timer, monotonic clock, and first SMP application processor |
| 26 | `N12-SCHED-001` | Neutral scheduler and context switch under SMP stress |
| 27 | `N13-RING3-001` | First user task, syscall, capability object, exception, and clean exit |
| 28 | `N14-IPC-001` | Capability-mediated call/reply, cancellation, quota, and hostile message tests |
| 29 | `N16-VIRTIO-001` | Isolated virtio console/RNG driver domain with revocation and restart |
