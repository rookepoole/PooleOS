...
Plan version: 2.2.0-native-owner-response
Roadmap cycle: PooleOS Cycle 94
...
| Native formal models | `POOLEOS-N4-MODELS-3`; four safe state spaces drained; eight required hostile invariant violations; 12/12 repeat matches; 18/18 negative controls | Partial N4.5-N4.6 | Frozen finite constants only; scheduler, PooleFS, and all four implementation-trace comparisons remain open; no theorem, liveness, refinement, ABI-freeze, kernel-execution, or promotion claim |
...
| Test suite | Cycle 94: 506 tests pass with one Windows symlink-permission skip, including independent IPC authorization, reply-token, stale-reply, and teardown-leak controls | Partial N36 | Predominantly host/reference, bounded-model, and artifact tests; no PooleKernel implementation trace exists |
| Release gate | Cycle 94: 69/69 consistency checks over 64 artifacts; 20 native gaps | Partial N37 | `production_ready=false`; not a release acceptance gate |
...
Cycle 94 control-plane validation passes 506 tests with one Windows symlink-permission skip, Doctor passes its complete check set against live PooleGlyph Phase 65, and the complete consistency release gate passes 69/69 checks over 64 artifacts while retaining 20 explicit gaps and `production_ready=false`. Exact public-index, release-gate, and handoff hashes are recorded in the Cycle 94 log after staging.
...
Cycle 89-90 and Cycle 94 bounded model evidence:
...
- all twelve model cases reproduce exactly over two runs, and 18/18 controls reject prerelease/toolchain substitution, stale runtime/model bindings, path escape, arbitrary TLC modes, unexpected safe violations, all eight missing hostile violations, and implementation-trace overclaim;
...
- Scheduler transitions and PooleFS transaction recovery remain unmodeled; all four current model families have zero native implementation-trace cross-checks.
...
| `FLAG-N4-MODELS-001` | BLOCKER | Open; boot/capability slice landed in Cycle 89, VM in Cycle 90, and IPC in Cycle 94 | Boot-slot, capability, VM, and IPC checks pass within frozen bounds, but scheduler, PooleFS, and implementation-trace cross-checks remain required before dependent ABI freezes |
...
Cycle 94 completes `N4-IPC-MODEL-001`: `POOLEOS-N4-MODELS-3` binds the drained 621-state IPC safe graph and four exact hostile counterexamples, and `FLAG-N4-IPC-MODEL-001` closes. The selected `hardware_fido2_ed25519_sk` device remains unavailable, so `N0-HW-KEY-ACQUIRE-001` remains the immediate blocked owner move and `FLAG-N0-GOVERNANCE-KEY-001` stays open. No key generation, private-key use, signing, merge, tag, public-key registration, or publication is authorized. `FLAG-N4-MODELS-001` remains open because scheduler, PooleFS recovery, and every native implementation-trace cross-check remain unfinished. The next owner-independent engineering move is `N4-SCHEDULER-MODEL-001`, while PooleGlyph Phase 66 may advance in parallel without outranking N0-N5.
...
| 4 | `N4-SCHEDULER-MODEL-001` | Bounded runnable/blocked/running transitions, cancellation and timeout wakeups, priority inversion controls, teardown, and explicitly scoped fairness assumptions with independent unsafe mutants and no liveness or implementation-equivalence overclaim; QEMU source-rebuild and real PooleBoot launch evidence remain parallel N4 work |
