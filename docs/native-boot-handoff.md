...
UEFI's memory-map descriptor size is explicitly extensible, and the current map key must be used for `ExitBootServices`. PBP1 therefore does not expose the volatile map key or copy raw descriptor strides into the kernel ABI. PBLIVE2 uses the returned descriptor size and version, preserves source type and attributes, rejects malformed or overlapping ranges, and records no volatile key. PKLOAD5 now allocates final retained objects, recaptures the map after those allocations, uses that map's current key for bounded `ExitBootServices`, makes no later firmware call, and retains read-only PBP1 bytes. The current bytes remain a nontransferable development profile because signatures are absent. The official UEFI 2.11 requirements are [GetMemoryMap and ExitBootServices](https://uefi.org/specs/UEFI/2.11/07_Services_Boot_Services.html).
...
PKLOAD5 separately proves that PooleBoot opens live PBC1/PSM1/PKELF1 inputs, retains kernel/table/guarded-stack/handoff pages, produces exact PBLIVE2 bytes from the final map, reconstructs identical bytes from serial and debugcon, cross-binds every retained range, calls `ExitBootServices`, makes zero later firmware calls, and halts before transfer. It does not authenticate the manifest or kernel, install the retained CR3/RSP as final state, execute transfer assembly, consume PBP1 in PooleKernel, test target firmware, or satisfy N5. CRC-32 and FNV are corruption/equality checks, not authentication. Finite corpus and differential agreement is not a proof over every input. No production, signing, Secure Boot, measured-boot, TPM, release, or physical-hardware claim follows.
