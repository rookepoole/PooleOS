...
PKLOAD6 separately proves that the ordinary PooleBoot path opens live PBC1/PSM1/PKELF1/PBART1/PBTP1/PBTS1 inputs, retains every required range, reparses and cross-binds the files, produces exact final PBLIVE3 bytes, calls `ExitBootServices`, makes zero later firmware calls, and halts before transfer. PKXFER1 separately proves that an opt-in QEMU-only build installs retained CR3/RSP, passes those exact PBP1 bytes once, live-executes independent nine-file PKREVAL1, and halts on unsigned denial with zero authority or effects. This evidence does not authenticate the manifest, artifacts, trust policy, revocation state, or acceptance state; provide a monotonic writable backend; authorize or apply payload semantics; establish a production transfer profile; test target firmware; or satisfy N5. CRC-32 and FNV are corruption/equality checks, not authentication. Finite corpus and differential agreement is not a proof over every input. No production, Secure Boot, measured-boot, TPM, release, or physical-hardware claim follows.
