...
PKLOAD3 now loads this image and binds it into a temporary pre-exit PBP1 snapshot, then releases both snapshot and kernel pages without executing the entry point. The qualification does not execute `ExitBootServices`, install page tables or final W^X permissions, perform privileged COM1 or framebuffer I/O, initialize GDT/IDT/TSS, start the kernel runtime, test target firmware, reproduce on a second host, produce a bootable ISO, close N6, or establish production readiness. It performs no signing, merging, release publication, firmware modification, driver loading, privileged probing, or physical-media write.
