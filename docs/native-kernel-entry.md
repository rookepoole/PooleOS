...
The qualifier uses the workspace-local pinned Rust toolchain, executes 165 host tests, checks formatting and Clippy, performs two clean offline builds in separate target directories, compares linked and canonical bytes, requires the exact contract build ID once in PKMID1 and once in the live diagnostic literal, runs hostile linked-ELF and canonical-PKELF controls, scans for host leakage, and compares exact loaded bytes from the independent Python and Rust PKELF1 implementations. Its deterministic public receipt is `runs/native_kernel_entry_readiness.json`; the current product has a 460,424-byte canonical file, a 557,056-byte loaded image, entry offset `0xA000`, 1,125 relative relocations, and canonical SHA-256 `FC13CF79E94318FAE10AFF9E7198036B30C587CF2BFD10457A045ACC6EB7665E`. The product file under `outputs/` remains local generated state.
