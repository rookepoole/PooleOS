...
The qualifier uses the workspace-local pinned Rust toolchain, executes thirteen host tests, checks formatting and Clippy, performs two clean offline builds in separate target directories, compares linked and canonical bytes, runs hostile linked-ELF and canonical-PKELF controls, scans for host leakage, and compares exact loaded bytes from the independent Python and Rust PKELF1 implementations. Its deterministic public receipt is `runs/native_kernel_entry_readiness.json`; the product file under `outputs/` remains local generated state.

## Nonclaims

No current PooleBoot path loads or executes this image. The qualification does not execute `ExitBootServices`, install page tables or final W^X permissions, perform privileged COM1 or framebuffer I/O, initialize GDT/IDT/TSS, start the kernel runtime, run under QEMU, test target firmware, reproduce on a second host, produce a bootable ISO, close N6, or establish production readiness. It performs no signing, merging, release publication, firmware modification, driver loading, privileged probing, or physical-media write.
