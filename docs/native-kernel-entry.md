...
The qualifier uses the workspace-local pinned Rust toolchain, executes nineteen host tests, checks formatting and Clippy, performs two clean offline builds in separate target directories, compares linked and canonical bytes, runs hostile linked-ELF and canonical-PKELF controls, scans for host leakage, and compares exact loaded bytes from the independent Python and Rust PKELF1 implementations. Its deterministic public receipt is `runs/native_kernel_entry_readiness.json`; the current product has a 3,492,624-byte private linked input, a 180,224-byte canonical file, a 262,144-byte loaded image, entry offset `0x8000`, 325 relative relocations, and canonical SHA-256 `8D1C219B4B6D8FEFE3B325F3A59DF904F1D971686A4A6537D73825A1B6F56FD2`. The product file under `outputs/` remains local generated state.

## Nonclaims

This standalone product qualification does not itself execute the image. `PKXFER1` separately proves that an opt-in feature-enabled PooleBoot build installs retained CR3/RSP, enters PooleKernel under QEMU, executes PKREVAL1, emits matching serial/debugcon evidence, and halts on the unsigned denial; default PooleBoot still stops before transfer. `PKTRAP1` separately proves a bounded BSP-only GDT/TSS/IDT and deliberate exception profile. None of these receipts authenticates a production PBP1 profile, finalizes framebuffer cache/remap/revocation policy, completes the kernel runtime, tests target firmware or physical hardware, reproduces on a second host, produces a bootable ISO, closes N5-N7, or establishes production readiness.
