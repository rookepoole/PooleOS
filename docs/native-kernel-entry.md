...
PKLOAD5 now loads this image, audits exact higher-half W^X mappings, retains the kernel, private tables, guarded stack, and PBP1 pages, calls `ExitBootServices`, proves zero later firmware calls, and halts before executing the entry point. The qualification does not install retained CR3/RSP as final state, accept the unsigned development PBP1 under the kernel-entry profile, perform privileged COM1 or framebuffer I/O from PooleKernel, initialize GDT/IDT/TSS, start the kernel runtime, test target firmware, reproduce on a second host, produce a bootable ISO, close N6, or establish production readiness. It performs no signing, merging, release publication, firmware modification, driver loading, privileged probing, or physical-media write.
