...
The qualifier uses the workspace-local pinned Rust toolchain, executes fourteen host tests, checks formatting and Clippy, performs two clean offline builds in separate target directories, compares linked and canonical bytes, runs hostile linked-ELF and canonical-PKELF controls, scans for host leakage, and compares exact loaded bytes from the independent Python and Rust PKELF1 implementations. Its deterministic public receipt is `runs/native_kernel_entry_readiness.json`; the current product has a 3,449,288-byte private linked input, a 180,224-byte canonical file, a 262,144-byte loaded image, entry offset `0x8000`, 302 relative relocations, and canonical SHA-256 `5D06ABFC9BD525931C63CC8A48FACD66A478640CE793E20E6D66EAEEB2BCEEEA`. The product file under `outputs/` remains local generated state.

## Nonclaims

This standalone product qualification does not itself execute the image. `PKXFER1` separately proves that an opt-in feature-enabled PooleBoot build installs retained CR3/RSP, enters PooleKernel under QEMU, executes PKREVAL1, emits matching serial/debugcon evidence, and halts on the unsigned denial; default PooleBoot still stops before transfer. Neither receipt authenticates a production PBP1 profile, finalizes framebuffer cache/remap/revocation policy, initializes GDT/IDT/TSS or the kernel runtime, tests target firmware or physical hardware, reproduces on a second host, produces a bootable ISO, closes N5/N6, or establishes production readiness.
