...
The qualifier uses the workspace-local pinned Rust toolchain, executes 84 host tests, checks formatting and Clippy, performs two clean offline builds in separate target directories, compares linked and canonical bytes, requires the exact contract build ID once in PKMID1 and once in the live diagnostic literal, runs hostile linked-ELF and canonical-PKELF controls, scans for host leakage, and compares exact loaded bytes from the independent Python and Rust PKELF1 implementations. Its deterministic public receipt is `runs/native_kernel_entry_readiness.json`; the current product has a 4,570,744-byte private linked input, a 278,528-byte canonical file, a 319,488-byte loaded image, entry offset `0x9000`, 667 relative relocations, and canonical SHA-256 `CDF33067B2421550BB03A4796FF9A92AE54D40B2575188632BF2C208449B882E`. The product file under `outputs/` remains local generated state.
