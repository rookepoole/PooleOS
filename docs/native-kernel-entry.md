...
The qualifier uses the workspace-local pinned Rust toolchain, executes 91 host tests, checks formatting and Clippy, performs two clean offline builds in separate target directories, compares linked and canonical bytes, requires the exact contract build ID once in PKMID1 and once in the live diagnostic literal, runs hostile linked-ELF and canonical-PKELF controls, scans for host leakage, and compares exact loaded bytes from the independent Python and Rust PKELF1 implementations. Its deterministic public receipt is `runs/native_kernel_entry_readiness.json`; the current product has a 299,008-byte canonical file, a 339,968-byte loaded image, entry offset `0x9000`, 729 relative relocations, and canonical SHA-256 `5B581CC1D1ABEB163D0984D12144CA5016C44B46A28B190A6DFCBDCDA689A255`. The product file under `outputs/` remains local generated state.
