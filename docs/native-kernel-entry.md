...
The qualifier uses the workspace-local pinned Rust toolchain, executes 184 host tests, checks formatting and Clippy, performs two clean offline builds in separate target directories, compares linked and canonical bytes, requires the exact contract build ID once in PKMID1 and once in the live diagnostic literal, runs hostile linked-ELF and canonical-PKELF controls, scans for host leakage, and compares exact loaded bytes from the independent Python and Rust PKELF1 implementations. Its deterministic public receipt is `runs/native_kernel_entry_readiness.json`; the current product has a 485,000-byte canonical file, a 557,056-byte loaded image, entry offset `0xA000`, text end `0x69000`, RELRO end `0x76000`, 1,222 relative relocations, and canonical SHA-256 `D11591395FDD8CD7BEEFA0D847A5C99EB133ED15F8DCDDE3392BFA499DCEDC33`. The product file under `outputs/` remains local generated state.
