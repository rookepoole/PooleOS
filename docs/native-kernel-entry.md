...
The qualifier uses the workspace-local pinned Rust toolchain, executes 151 host tests, checks formatting and Clippy, performs two clean offline builds in separate target directories, compares linked and canonical bytes, requires the exact contract build ID once in PKMID1 and once in the live diagnostic literal, runs hostile linked-ELF and canonical-PKELF controls, scans for host leakage, and compares exact loaded bytes from the independent Python and Rust PKELF1 implementations. Its deterministic public receipt is `runs/native_kernel_entry_readiness.json`; the current product has a 425,984-byte canonical file, a 507,904-byte loaded image, entry offset `0xA000`, 1,042 relative relocations, and canonical SHA-256 `AFED4AF858404D83CD77215C118F8478C88E91BDDC0F0B1ABAC3C9324B6ED602`. The product file under `outputs/` remains local generated state.
