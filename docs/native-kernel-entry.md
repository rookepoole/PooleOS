...
The qualifier uses the workspace-local pinned Rust toolchain, executes 173 host tests, checks formatting and Clippy, performs two clean offline builds in separate target directories, compares linked and canonical bytes, requires the exact contract build ID once in PKMID1 and once in the live diagnostic literal, runs hostile linked-ELF and canonical-PKELF controls, scans for host leakage, and compares exact loaded bytes from the independent Python and Rust PKELF1 implementations. Its deterministic public receipt is `runs/native_kernel_entry_readiness.json`; the current product has a 476,808-byte canonical file, a 557,056-byte loaded image, entry offset `0xA000`, text end `0x66000`, RELRO end `0x74000`, 1,181 relative relocations, and canonical SHA-256 `9C23236E85A6D2C7AEEFDA12F3CEC202DC3BF34B89D9CEAEEBB7037A079DA168`. The product file under `outputs/` remains local generated state.
