...
The qualifier uses the workspace-local pinned Rust toolchain, executes 99 host tests, checks formatting and Clippy, performs two clean offline builds in separate target directories, compares linked and canonical bytes, requires the exact contract build ID once in PKMID1 and once in the live diagnostic literal, runs hostile linked-ELF and canonical-PKELF controls, scans for host leakage, and compares exact loaded bytes from the independent Python and Rust PKELF1 implementations. Its deterministic public receipt is `runs/native_kernel_entry_readiness.json`; the current product has a 323,584-byte canonical file, a 364,544-byte loaded image, entry offset `0x9000`, 812 relative relocations, and canonical SHA-256 `2ACD4A5EF30CA1A4A22711FD31E2A259A5C87D97BCE7FB1BF49A3488B3FC02B2`. The product file under `outputs/` remains local generated state.
