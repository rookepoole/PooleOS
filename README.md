...
- `specs/native-kernel-entry-contract.json`, `native/kernel`, `runtime/native_kernel_image.py`, `docs/native-kernel-entry.md`, and `runs/native_kernel_entry_readiness.json`: real freestanding PKELF1 PooleKernel product, PKENTRY1 handoff intake, bounded early diagnostics and panic classes, 49 host tests, exact two-build reproduction, 43 hostile controls, and an explicit standalone-product boundary; live development transfer, traps, read-only CPU policy, x87/SSE ownership/exceptions, and privileged-MSR policy are qualified separately by PKXFER1, PKTRAP1, PKCPU1, PKXSTATE1, PKXEXC1, and PKMSR1.
...
Current Cycle 124 status: 40 phases, 0 complete, 15 partial, 1 blocked, and 24 not started; 45 explicit `ADD-*` requirements and 68 implementation flags are tracked. The selected `hardware_fido2_ed25519_sk` profile is still physically unavailable; no key was generated or used, the signer store remains empty, and no ADR is cryptographically signed. Rooke Poole has authorized the listed cryptographic, privileged, media, release, and promotion operation categories, but that authority does not supply hardware, evidence, owner presence, backups, recovery media, a safe target, or permission to bypass qualification and exact-release gates; no such operation occurred in Cycle 124. `N0-HW-KEY-ACQUIRE-001` remains the immediate blocked external move, and the PooleGlyph Phase 65/66 lane remains active without policy authority. Cycle 124 closes only `FLAG-N7-PRIVILEGE-MSR-POLICY-001`: PKMSR1 proves a bounded support-gated read-only system-linkage, FS/GS, optional TSC_AUX, global MCA, and PMU policy on one TCG qemu64 BSP, with eleven live MSR reads and a linked no-activation/no-write audit. N7.3 is partial, not complete; syscall, GS/TSC_AUX, MCE, PMU, AP, target-hardware, and N7 exit work remain open. The next owner-independent move is `N9-PMM-001` for normalized-map ownership and bootstrap/physical allocation.
