...
- `specs/native-pooleboot-proof.json`, `runs/native_pooleboot_readiness.json`, `docs/native-pooleboot-proof.md`, and `tools/qualify_native_pooleboot.py`: bounded unsigned PooleBoot PE32+ aggregate proof, deterministic four-file GPT/FAT32 development media, two exact pinned OVMF runs, twenty-two ordered serial/debugcon markers, retained PKMAP2/PBLIVE2/PBEXIT1 evidence, static GOP-frame evidence, 95 hostile controls, and explicit signature/kernel-entry/target-firmware/N5 nonclaims.
...
- `specs/native-kernel-load-contract.json`, `native/bootload`, `native/boot/src/exit.rs`, `native/bootexit`, `runtime/native_kernel_load.py`, `docs/native-kernel-load.md`, and `runs/native_kernel_load_readiness.json`: PKLOAD5 live UEFI PBC1/PSM1/PKELF1 intake, retained PKMAP2 root/guarded-stack/handoff, final PBLIVE2, bounded PBEXIT1, successful `ExitBootServices`, exact guest/oracle agreement, 95 hostile controls, and explicit unsigned/stop-before-transfer nonclaims.
...
Current Cycle 106 status: 40 phases, 0 complete, 14 partial, 1 blocked, and 25 not started; 34 explicit `ADD-*` requirements and 45 implementation flags are tracked. The selected `hardware_fido2_ed25519_sk` profile remains unavailable; no key was generated or used, the signer store remains empty, and no ADR is cryptographically signed. Codex still has no authority to generate or use keys, sign, merge, tag, register a public key, or publish a release. `N0-HW-KEY-ACQUIRE-001` remains the blocked owner move, and the PooleGlyph Phase 65/66 co-development lane remains active. Cycle 106 closes only `FLAG-N5-HANDOFF-EXIT-001`: PooleBoot retains the exact kernel, four table pages, guarded stack, and read-only handoff; binds final PBP1 bytes to the successful UEFI map; calls `ExitBootServices`; proves zero later firmware calls; and halts before transfer. PKLOAD5 passes 70/70 Rust host tests, two exact 104,960-byte PooleBoot builds, two exact kernel and media generations, two exact OVMF runs, 22 markers, 95/95 controls, and exact PBLIVE2/PKMAP2/PBEXIT1 oracle agreement. The manifest and kernel remain unsigned and untrusted, independent PBDIGEST1 review remains open, and no final CR3/RSP, kernel entry, target firmware, N5 exit, or production claim follows. The complete loader, executing PooleKernel system, ring-3 execution, capabilities, native drivers, PooleFS, PooleGlass, and production ISO do not exist. `N5-INIT-SYSTEM-001` is the next owner-independent move.
...
The generators and qualifiers are deterministic: tests reproduce the checklist, roadmap, architecture baseline, objectives-readiness, ADR-readiness, frozen N0 owner-packet, completed owner-response receipt, native-toolchain, hardware-readiness, native Tier 0, native model, native PooleBoot, PBP1, PBC1, PSM1, PKELF1, PKENTRY1, and PKLOAD5/PKMAP2/PBEXIT1 ledgers byte for byte. Cycle 106 contains 605 tests with one expected Windows symlink-permission skip. Doctor passes 400/400 checks and writes live PooleGlyph conformance output to temporary storage so verification does not rewrite the tandem repository's generated report. The 76/76 non-promoting consistency release gate verifies the locked checklist hash, coverage digest, N0-N39 sequence, governance non-authorization controls, native toolchain and target contracts, hardware and Tier 0 boundaries, bounded models, boot evidence and nonclaims, public-index controls, PooleGlyph co-development boundaries, PDC evidence, and retained historical consistency artifacts while keeping 20 explicit gaps and `production_ready=false`.
