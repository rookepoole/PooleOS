...
- `specs/native-kernel-entry-contract.json`, `native/kernel`, `runtime/native_kernel_image.py`, `docs/native-kernel-entry.md`, and `runs/native_kernel_entry_readiness.json`: real 221,184-byte canonical PKELF1 PooleKernel product in a 262,144-byte image, PKENTRY1 handoff intake, bounded early diagnostics and panic classes, 66 host tests, exact two-build reproduction, 43 hostile controls, 525 relocations, and canonical SHA-256 `5533347F0DA41B0F08B4E0ADF643379706BE548B5486D49C57960FEFE799FE93`; live bounded subsystem slices are qualified separately through PKVM2.
...
Current Cycle 127 status: 40 phases, 0 complete, 16 partial, 1 blocked, and 23 not started; 46 explicit `ADD-*` requirements and 71 implementation flags are tracked. The selected `hardware_fido2_ed25519_sk` profile is still physically unavailable; no key was generated or used, the signer store remains empty, and no ADR is cryptographically signed. Rooke Poole has authorized the listed cryptographic, privileged, media, release, and promotion operation categories, but that authority does not supply hardware, evidence, owner presence, backups, recovery media, a safe target, or permission to bypass qualification and exact-release gates; no such operation occurred in Cycle 127. `N0-HW-KEY-ACQUIRE-001` remains the immediate blocked external move, and the PooleGlyph Phase 65/66 lane remains active without policy authority. Cycle 127 closes `FLAG-N9-VM-ACTIVE-001`: PKVM2 proves one bounded active candidate root, a nine-page direct map, exact CR3 restoration, and three local invalidation receipts across two exact 40-marker qemu64 runs and 46 hostile controls. N9.1-N9.4 remain partial; page-content scrub policy, held-class reclaim, complete direct-map and SMP TLB policy, deferred reclaim, huge pages, PCID, COW, user faults, pager IPC, heaps/object caches, MMIO/cache qualification, concurrency, pressure/OOM policy, target hardware, and N9 exit remain open. The next owner-independent move is `N9-PMM-SCRUB-001`.
