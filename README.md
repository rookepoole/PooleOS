...
- `specs/native-kernel-entry-contract.json`, `native/kernel`, `runtime/native_kernel_image.py`, `docs/native-kernel-entry.md`, and `runs/native_kernel_entry_readiness.json`: real 323,584-byte canonical PKELF1 PooleKernel product in a 364,544-byte image, PKENTRY1 handoff intake, bounded early diagnostics and panic classes, 99 host tests, exact two-build reproduction, 43 hostile controls, 812 relocations, and canonical SHA-256 `2ACD4A5EF30CA1A4A22711FD31E2A259A5C87D97BCE7FB1BF49A3488B3FC02B2`; live bounded subsystem slices are qualified separately through PKIRQ1.
...
- `specs/native-kernel-map-contract.json`, `native/kmap`, `runtime/native_kernel_map.py`, `docs/native-kernel-map.md`, and `tests/test_native_kernel_map.py`: PKMAP2 exact 89-page supervisor 4 KiB kernel mapping, 32-page guarded stack, read-only handoff, one temporary leaf, five retained supervisor RW/NX manager leaves, two alternate guarded 32-page PMM ledger windows, and a guarded five-page IRQ MMIO reservation, with CR0.WP/NX/W^X enforcement, active-root audit, framebuffer translation/cache preservation, exact CR3 restoration, complete nine-file retained coverage, and retained/no-transfer boundaries.
...
Current Cycle 135 status: 40 phases, 0 complete, 17 partial, 1 blocked, and 22 not started; 47 explicit `ADD-*` requirements and 79 implementation flags are tracked, with 33 open. The selected `hardware_fido2_ed25519_sk` profile is still physically unavailable; no key was generated or used, the signer store remains empty, and no ADR is cryptographically signed. Rooke Poole has authorized the listed cryptographic, privileged, media, release, and promotion operation categories, but that authority does not supply hardware, evidence, owner presence, backups, recovery media, a safe target, or permission to bypass qualification and exact-release gates; no such operation occurred in Cycle 135. `N0-HW-KEY-ACQUIRE-001` remains the immediate blocked external move, and the PooleGlyph Phase 65/66 lane remains active without policy authority. Cycle 135 makes N8 partial through PKIRQ1: 99 kernel host tests, two exact 36-marker qemu64 runs, and 58 hostile controls prove bounded retained MADT/HPET parsing, local xAPIC identity, vector ownership, guarded uncacheable MMIO, checked one-shot calibration, eight exact timer deliveries/EOIs, and normal-path restoration. `FLAG-N8-IRQ-001` stays open because no AP, IPI, I/O APIC route, MSI/MSI-X, complete time service, panic-path rollback, or target run exists. The next owner-independent move is `N8-SMP-FIRST-AP-001`.
