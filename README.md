...
- `specs/n0-owner-response.json`, `runs/n0_owner_response_receipt.json`, and `docs/n0-owner-response-receipt.md`: deterministic completed response binding, 2/2 ADR and 38/38 definition dispositions, selected unavailable FIDO2 profile, 16 fail-closed controls, and explicit zero authority for key generation, signing, merge, tagging, or publication.
...
- `specs/native-pooleboot-proof.json`, `runs/native_pooleboot_readiness.json`, `docs/native-pooleboot-proof.md`, and `tools/qualify_native_pooleboot.py`: bounded unsigned PooleBoot PE32+ proof, deterministic GPT/FAT32 development media, two exact pinned OVMF runs, ordered serial/debugcon markers, static GOP-frame evidence, fifteen hostile controls, and explicit complete-loader/N5 nonclaims.
- `specs/native-boot-handoff-contract.json`, `native/handoff`, `runtime/native_boot_handoff.py`, and `runs/native_boot_handoff_readiness.json`: candidate PBP1 firmware-to-kernel bytes, dependency-free `no_std` codec, independent oracle, golden vectors, hostile controls, and non-promoting differential evidence.
...
Current Cycle 101 status: 40 phases, 0 complete, 14 partial, 1 blocked, and 25 not started; 33 explicit `ADD-*` requirements and 39 implementation flags remain tracked. Cycle 91's 16-source owner packet remains byte-frozen with every original field `UNSELECTED`. Cycle 93 records ADR-0003/0004 and all 38 target definitions as owner-directed but unsigned while retaining zero measurements. The selected `hardware_fido2_ed25519_sk` profile remains unavailable; no key was generated or used, the signer store remains empty, and no ADR is cryptographically signed. PRs #14, #15, and #16 were merged externally by the owner; Codex still has no authority to generate or use keys, sign, merge, tag, register a public key, or publish a release. `N0-HW-KEY-ACQUIRE-001` remains the blocked owner move. Cycle 92's PooleGlyph co-development lane remains active. Cycles 97-100 close only the bounded PooleBoot, PBP1, PBC1, and PKELF1 qualification flags. Cycle 101 closes only `FLAG-N6-KENTRY-001`: a real freestanding PooleKernel product canonicalizes to 139,264 PKELF1 bytes at SHA-256 `8D79F687B92EE4C3592F45BB8B2C33E805D3D246319D54AB7EC354458490CA5B`; 13/13 host tests, 2/2 exact clean builds, 43/43 hostile controls, and independent Rust/Python loaded-byte comparison pass. PKENTRY1 validates the candidate PBP1 entry envelope and continuity and supplies bounded ring, COM1-candidate, framebuffer, and panic paths. It does not prove live PooleBoot file I/O, authentication, allocation, page tables, `ExitBootServices`, transfer, privileged diagnostics execution, descriptor tables, exceptions, kernel runtime, target firmware, second-host reproduction, or N6 exit. The newly explicit framebuffer mapping/cache/revocation dependency remains open as `FLAG-N6-FRAMEBUFFER-MAP-001`. The complete PooleBoot loader, executing PooleKernel system, ring-3 execution, capabilities, native drivers, PooleFS, PooleGlass, and production ISO do not exist. `N5-KLOAD-001` is the next owner-independent move.
...
The generators and qualifiers are deterministic: tests reproduce the checklist, roadmap, architecture baseline, objectives-readiness, ADR-readiness, frozen N0 owner-packet, completed owner-response receipt, native-toolchain, hardware-readiness, native Tier 0, native model, native PooleBoot, PBP1, PBC1, PKELF1, and PKENTRY1 ledgers byte for byte. Cycle 101 passes 565 tests with one expected Windows symlink-permission skip, Doctor passes 368/368 checks, and the non-promoting consistency release gate passes 74/74 checks over 69 artifacts with 20 explicit gaps. Doctor verifies the locked checklist hash, coverage digest, N0-N39 sequence, ADR/source hashes, owner-directed definitions and zero-measurement boundary, ratification scope, frozen packet, completed response, non-authorization controls, repository identity, native toolchain/target contracts, hardware target/readiness/privacy boundaries, Tier 0 runtime closure and non-boot boundary, bounded-model contracts/results/claim boundaries, bounded PooleBoot evidence, PBP1, PBC1, PKELF1, and PKENTRY1 product/canonical/hostile/exact-loaded-byte evidence and nonclaims, public index boundary, PooleGlyph bridge and co-development boundaries, PDC evidence, and retained historical consistency artifacts.

The public repository currently carries source, specifications, tests, and explicitly allowlisted deterministic ledgers. The historical N0 packet publishes exact source digests and advisory recommendations while retaining every field unselected. The owner-response ledger publishes only explicit choices, content hashes, false authorization fields, and source-status evidence; it contains no public key, private key, key handle, signature, credential, or secret. The public hardware observation is rebuilt from a fixed whitelist and binds only the byte count and SHA-256 of its ignored private capture; raw firmware-table bytes, raw CPUID registers, processor serials, device serials, MAC addresses, UUIDs, host/user names, absolute local paths, full PnP instances, and TPM key material are not published. The native Tier 0 ledger binds exact hashes and relative roles without publishing the 1.18 GB QEMU/OVMF runtime or installer. The native model, PooleBoot, PBP1, PBC1, PKELF1, and PKENTRY1 ledgers publish only exact relative bindings, bounded results, synthetic or product-byte hashes, and claim limits; operational entropy, differential corpora, local PooleKernel product bytes, JRE, TLC JAR, QEMU/OVMF runtime, generated media, screenshots, raw logs, and metadata remain ignored. Public ratification manifest, detached signature, and signing receipt paths remain absent until separately authorized owner action. Raw internal PDC inputs, private benchmark evidence, other local run artifacts, historical lab images, firmware without redistribution clearance, secrets, and private signing material remain in the ignored private evidence vault.
