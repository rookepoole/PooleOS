...
- `specs/native-kernel-entry-contract.json`, `native/kernel`, `runtime/native_kernel_image.py`, `docs/native-kernel-entry.md`, and `runs/native_kernel_entry_readiness.json`: real 299,008-byte canonical PKELF1 PooleKernel product in a 339,968-byte image, PKENTRY1 handoff intake, bounded early diagnostics and panic classes, 91 host tests, exact two-build reproduction, 43 hostile controls, 729 relocations, and canonical SHA-256 `5B581CC1D1ABEB163D0984D12144CA5016C44B46A28B190A6DFCBDCDA689A255`; live bounded subsystem slices are qualified separately through PKVM3.
...
Current Cycle 134 status: 40 phases, 0 complete, 16 partial, 1 blocked, and 23 not started; 47 explicit `ADD-*` requirements and 79 implementation flags are tracked, with 33 open. The selected `hardware_fido2_ed25519_sk` profile is still physically unavailable; no key was generated or used, the signer store remains empty, and no ADR is cryptographically signed. Rooke Poole has authorized the listed cryptographic, privileged, media, release, and promotion operation categories, but that authority does not supply hardware, evidence, owner presence, backups, recovery media, a safe target, or permission to bypass qualification and exact-release gates; no such operation occurred in Cycle 134. `N0-HW-KEY-ACQUIRE-001` remains the immediate blocked external move, and the PooleGlyph Phase 65/66 lane remains active without policy authority. Cycle 134 closes the bounded one-BSP scope of `FLAG-N9-VM-DIRECT-MAP-001`: PKPMM7 supplies the generation-bound ownership manifest and PKVM3 installs 117,887 admitted pages in eleven write-back ranges while leaving 12,878 gaps absent. Ninety-one kernel host tests, two exact 40-marker qemu64 runs, and 46 hostile controls pass with write/CR3 rollback, cache-alias rejection, local invalidation receipts, and exact generation-retirement gating. N9.1-N9.4 remain partial; AP startup, real SMP TLB shootdown/remote deferred reclaim, concurrent replacement, huge pages, PCID, COW, user faults, pager IPC, heaps/object caches, general MMIO/cache qualification, interrupt-context/concurrent allocation, pressure/OOM policy, target hardware, and N9 exit remain open. The next owner-independent move is `N8-IRQ-001`.
