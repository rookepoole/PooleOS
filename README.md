...
- `specs/native-pooleboot-proof.json`, `runs/native_pooleboot_readiness.json`, `docs/native-pooleboot-proof.md`, and `tools/qualify_native_pooleboot.py`: bounded unsigned PooleBoot PE32+ aggregate proof, deterministic four-file GPT/FAT32 development media, two exact pinned OVMF runs, twenty-three ordered serial/debugcon markers, exact temporary PBP1 and PKMAP1 evidence, static GOP-frame evidence, seventy-seven hostile controls, and explicit signature-trust/retention/final-map/transfer/N5 nonclaims.
- `specs/native-boot-handoff-contract.json`, `native/handoff`, `native/livehandoff`, `runtime/native_boot_handoff.py`, `runtime/native_live_boot_handoff.py`, and `runs/native_boot_handoff_readiness.json`: candidate PBP1 firmware-to-kernel bytes, dependency-free `no_std` codec and live builder, independent synthetic and transcript oracles, golden vectors, hostile controls, temporary pre-exit production, and non-promoting differential evidence.
...
- `specs/native-kernel-load-contract.json`, `native/bootload`, `native/boot/src/kload.rs`, `native/boot/src/livehandoff.rs`, `native/boot/src/kmap.rs`, `runtime/native_kernel_load.py`, `docs/native-kernel-load.md`, and `runs/native_kernel_load_readiness.json`: PKLOAD4 live UEFI PBC1/PSM1/PKELF1 intake, exact manifest and digest binding, page allocation/load/relocation, stride-aware temporary PBP1 production, temporary PKMAP1 activation and exact rollback, deterministic cleanup, guest/oracle agreement, 77 hostile controls, and explicit unsigned/pre-exit/release nonclaims.
- `specs/native-kernel-map-contract.json`, `native/kmap`, `runtime/native_kernel_map.py`, `docs/native-kernel-map.md`, and `tests/test_native_kernel_map.py`: PKMAP1 exact supervisor 4 KiB higher-half mapping, CR0.WP/NX/W^X enforcement, active-root cloning, full alias audit, framebuffer translation/cache preservation, exact CR3 rollback, cleanup, and no-retention/no-transfer boundaries.
...
Current Cycle 105 status: 40 phases, 0 complete, 14 partial, 1 blocked, and 25 not started; 34 explicit `ADD-*` requirements and 44 implementation flags are tracked. The selected `hardware_fido2_ed25519_sk` profile remains unavailable; no key was generated or used, the signer store remains empty, and no ADR is cryptographically signed. Codex still has no authority to generate or use keys, sign, merge, tag, register a public key, or publish a release. `N0-HW-KEY-ACQUIRE-001` remains the blocked owner move, and the PooleGlyph Phase 65/66 co-development lane remains active. Cycle 105 closes only `FLAG-N5-KMAP-001`: PooleBoot builds and temporarily activates an exact 48-page supervisor higher-half mapping, verifies W^X/WP/NX, hashes the complete alias, preserves framebuffer translation/cache bits, restores the exact original CR3 before firmware use, and releases all four private table pages plus all kernel pages. PKLOAD4 passes 59/59 Rust host tests, two exact 94,720-byte PooleBoot builds, two exact kernel and media generations, two exact OVMF runs, 23 markers, 77/77 controls, and exact PBP1 and PKMAP1 oracle agreement. The manifest remains unsigned and untrusted, and independent PBDIGEST1 security review remains open. No mapping or handoff is retained, no final framebuffer cache policy or final memory map is established, and no `ExitBootServices` or entry is claimed. The complete loader, executing PooleKernel system, ring-3 execution, capabilities, native drivers, PooleFS, PooleGlass, and production ISO do not exist. `N5-HANDOFF-001` is the next owner-independent move.
...
The generators and qualifiers are deterministic: tests reproduce the checklist, roadmap, architecture baseline, objectives-readiness, ADR-readiness, frozen N0 owner-packet, completed owner-response receipt, native-toolchain, hardware-readiness, native Tier 0, native model, native PooleBoot, PBP1, PBC1, PSM1, PKELF1, PKENTRY1, and PKLOAD4/PKMAP1 ledgers byte for byte. Cycle 105 passes 592 tests with one expected Windows symlink-permission skip, Doctor passes 398/398 checks without mutating PooleGlyph runtime outputs, and the non-promoting consistency release gate passes 76/76 checks over 71 artifacts with 20 explicit gaps. Doctor verifies the locked checklist hash, coverage digest, N0-N39 sequence, governance non-authorization controls, native toolchain and target contracts, hardware and Tier 0 boundaries, bounded models, PooleBoot/PBP1/PBC1/PSM1/PKELF1/PKENTRY1/PKLOAD4/PKMAP1 evidence and nonclaims, public-index controls, PooleGlyph co-development boundaries, PDC evidence, and retained historical consistency artifacts.
