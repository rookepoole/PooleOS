...
- `specs/native-kernel-smp-ipi-contract.json`, `native/kernel/src/smp_ipi.rs`, `runtime/native_kernel_smp_ipi.py`, `tools/qualify_native_kernel_smp_ipi.py`, `docs/native-kernel-smp-ipi.md`, and `runs/native-kernel-smp-ipi-readiness.json`: PKSMP4 retains six fixed development-only IPI vectors and adds one generation-bound remote one-page invalidation for one AP-owned root. Exact request/ack masks, stale and duplicate rejection, offline timeout with same-attempt retry, pre-ack reclaim denial, one linked-image-audited AP-side `INVLPG`, old/new frame observation, retirement, and cleanup pass. Two exact 40-marker runs observe six accepted and four denied deliveries, ten EOIs, one timeout, one retirement, and exact release of 34 pages or 139,264 bytes; 25 hostile-control categories cover 169 rejected cases and 132 kernel host tests pass. This is not general SMP or general shootdown, and no arbitrary callback, production capability, scheduler, target, N8/N9-exit, or production claim is made.
...
Current Cycle 140 status: 40 phases, 0 complete, 17 partial, 1 blocked, and 22 not started; 47 explicit `ADD-*` requirements and 84 implementation flags are tracked, with 34 open. `FLAG-N9-SMP-SHOOTDOWN-001` is closed only for PKSMP4's one-AP, one-root, one-page, one-generation remote invalidation and deferred-reclaim transaction; `FLAG-N8-SMP-MULTI-AP-001` records the next required scale-out. General SMP, scheduler ownership, production capability authority, target hardware, N8/N9 exit, and production remain open, so `production_ready=false`. No key, signature, firmware mutation, physical-media write, tag, release, or production promotion occurred. The selected `hardware_fido2_ed25519_sk` profile remains physically unavailable, `N0-HW-KEY-ACQUIRE-001` remains the immediate blocked external move, and `N8-SMP-MULTI-AP-001` is the next owner-independent kernel move.
