...
- `specs/native-kernel-entry-contract.json`, `native/kernel`, `runtime/native_kernel_image.py`, `docs/native-kernel-entry.md`, and `runs/native_kernel_entry_readiness.json`: real 513,680-byte canonical PKELF1 PooleKernel product in a 585,728-byte, 143-page image, fixed `0xA000` entry, page-aligned `0x70000` text and `0x7D000` RELRO/data boundaries, PKENTRY1 handoff intake, bounded early diagnostics and panic classes, 206 host tests, exact two-build reproduction, 43 hostile controls, 1,295 relocations, and canonical SHA-256 `9029AEE51A4D557EF5B29945985E4A1F07C67DDE9C8C367C80BD1B9EDD9D409E`; live bounded subsystem slices are qualified separately through PKLOCK1.
...
Current Cycle 149 status: 40 phases remain non-complete. `FLAG-N12-CONCURRENCY-LOCKS-001` is closed and N12.2 is complete only for PKLOCK1's bounded allocation-free lock family, context/rank rules, direct priority donation, fairness, owner death, rollback, host contention, and one exact four-vCPU live ticket-lock profile. Deferred reclamation and ABA-safe lifetime, AP-local timer interrupt delivery, arbitrary callbacks, a general driver/service framework, general topology/hotplug/x2APIC, general SMP preemption, ring 3, address-space and full per-task architectural switching, target hardware, N12 exit, and production remain open, so `production_ready=false`. No key, signature, firmware mutation, physical-media write, tag, release, or production promotion occurred. The selected `hardware_fido2_ed25519_sk` profile remains physically unavailable, `N0-HW-KEY-ACQUIRE-001` remains the immediate blocked external move, and `N12-CONCURRENCY-RECLAMATION-001` is the next owner-independent kernel move.
