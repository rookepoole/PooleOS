...
- `specs/native-pooleboot-proof.json`, `runs/native_pooleboot_readiness.json`, `docs/native-pooleboot-proof.md`, and `tools/qualify_native_pooleboot.py`: bounded unsigned PooleBoot PE32+ aggregate proof, deterministic ten-file GPT/FAT32 development media, two exact pinned OVMF runs, twenty-three ordered serial/debugcon markers, retained PBART1/PKMAP2/PBLIVE3/PBEXIT1 evidence, static GOP-frame evidence, 112 hostile controls, and explicit signature/payload-semantics/kernel-entry/target-firmware/N5 nonclaims.
...
Current Cycle 107 status: 40 phases, 0 complete, 14 partial, 1 blocked, and 25 not started; 35 explicit `ADD-*` requirements and 47 implementation flags are tracked. The selected `hardware_fido2_ed25519_sk` profile remains unavailable; no key was generated or used, the signer store remains empty, and no ADR is cryptographically signed. Codex still has no authority to generate or use keys, sign, merge, tag, register a public key, or publish a release. `N0-HW-KEY-ACQUIRE-001` remains the blocked owner move, and the PooleGlyph Phase 65/66 co-development lane remains active. Cycle 107 closes only the unsigned development boundary `FLAG-N5-INIT-SYSTEM-001`: PooleBoot requires an exact seven-artifact PSM1 profile, validates six PBART1 envelopes, loads them into distinct zero-padded pages, retains all artifact ranges, binds roles 1 through 7 into final PBLIVE3, calls `ExitBootServices`, and halts before transfer. PKLOAD6 passes 76/76 Rust checks, two exact 113,664-byte PooleBoot builds, two exact kernel and ten-file media generations, two exact OVMF runs, 23 markers, 112/112 controls, and exact PBART1/PBLIVE3/PKMAP2/PBEXIT1 oracle agreement. All artifacts remain unsigned and untrusted; payload semantics, initial-system execution, microcode application, independent PBDIGEST1 review, final CR3/RSP, kernel entry, target firmware, N5 exit, and production readiness remain open. The complete loader, executing PooleKernel system, ring-3 execution, capabilities, native drivers, PooleFS, PooleGlass, and production ISO do not exist. `N5-INIT-SEMANTICS-001` is the next owner-independent move.
