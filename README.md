...
- `specs/native-pooleboot-proof.json`, `runs/native_pooleboot_readiness.json`, `docs/native-pooleboot-proof.md`, and `tools/qualify_native_pooleboot.py`: bounded unsigned PooleBoot PE32+ aggregate proof, deterministic ten-file GPT/FAT32 development media, two exact pinned OVMF runs, twenty-three ordered serial/debugcon markers, retained PBART1/PINIT1/PREC1/PSYM1/PKMAP2/PBLIVE3/PBEXIT1 evidence, static GOP-frame evidence, 121 hostile controls, and explicit signature/PooleBoot-inner-enforcement/kernel-activation-recovery-or-symbol-consumption/kernel-entry/target-firmware/N5 nonclaims.
...
Current Cycle 110 status: 40 phases, 0 complete, 14 partial, 1 blocked, and 25 not started; 38 explicit `ADD-*` requirements and 50 implementation flags are tracked. The selected `hardware_fido2_ed25519_sk` profile remains unavailable; no key was generated or used, the signer store remains empty, and no ADR is cryptographically signed. Codex still has no authority to generate or use keys, sign, merge, tag, register a public key, or publish a release. `N0-HW-KEY-ACQUIRE-001` remains the blocked owner move, and the PooleGlyph Phase 65/66 co-development lane remains active. Cycle 110 closes only `FLAG-N5-SYMBOL-BUNDLE-001`: canonical PSYM1 freezes exact stripped/loaded/build/debug/source identity, image-relative offsets, explicit KASLR-base input, bounded lookup, three public diagnostic symbols, source-path exclusion, pointer redaction, split-debug correspondence, and fail-closed consumption preconditions. Independent Python and allocation-free `no_std` Rust implementations pass 4/4 tests, 2/2 targets, 3/3 vectors, 158/158 controls, 16,384 parser cases, and 16,384 lookup cases with zero mismatches; two full-debug builds reproduce exactly. PKLOAD6 and aggregate PooleBoot evidence bind the exact inner bytes through host-only oracles and pass 121/121 controls. All artifacts remain unsigned and untrusted; PooleBoot and PooleKernel do not consume PSYM1, full debug data is absent from media, no kernel export or diagnostic authority is created, and runtime pointers remain redacted by default. Microcode/firmware/policy semantics, live PINIT1/PREC1/PSYM1 enforcement, authenticated persistent rollback state, independent PBDIGEST1 review, final CR3/RSP, kernel entry, target firmware, N5 exit, and production readiness remain open. The executing PooleKernel system, ring-3 execution, capabilities, native drivers, PooleFS, PooleGlass, and production ISO do not exist. `N5-MICROCODE-SEMANTICS-001` is the next owner-independent move.
