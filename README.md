...
- `specs/native-kernel-physical-memory-contract.json`, `native/kernel/src/physical_memory.rs`, `runtime/native_kernel_physical_memory.py`, `tools/qualify_native_kernel_physical_memory.py`, `docs/native-kernel-physical-memory.md`, and `runs/native-kernel-physical-memory-readiness.json`: PKPMM3 bounded one-BSP live PBP1 physical-page ownership with generation-safe scrubbed lifecycle transactions and a complete 14,616-byte manager migrated into five retained supervisor RW/NX metadata pages between absent guards; exact handoff/final integrity, mapped-corruption rejection, rollback, release exclusion, 74 kernel host tests, two exact 41-marker QEMU runs, 87 hostile controls, and explicit no-reclaim/no-scalable-growth/no-concurrency/no-production nonclaims.
...
Current Cycle 129 status: 40 phases, 0 complete, 16 partial, 1 blocked, and 23 not started; 46 explicit `ADD-*` requirements and 74 implementation flags are tracked. The selected `hardware_fido2_ed25519_sk` profile is still physically unavailable; no key was generated or used, the signer store remains empty, and no ADR is cryptographically signed. Rooke Poole has authorized the listed cryptographic, privileged, media, release, and promotion operation categories, but that authority does not supply hardware, evidence, owner presence, backups, recovery media, a safe target, or permission to bypass qualification and exact-release gates; no such operation occurred in Cycle 129. `N0-HW-KEY-ACQUIRE-001` remains the immediate blocked external move, and the PooleGlyph Phase 65/66 lane remains active without policy authority. Cycle 129 closes `FLAG-N9-PMM-METADATA-001`: PKPMM3 migrates the complete manager into five retained guarded metadata pages with generation/owner binding, exact handoff/final integrity, corruption rejection, rollback, and release exclusion across 74 host tests, two exact 41-marker qemu64 runs, and 87 hostile controls. N9.1-N9.4 remain partial; held-class reclaim, scalable metadata growth, complete direct-map and SMP TLB policy, deferred reclaim, huge pages, PCID, COW, user faults, pager IPC, heaps/object caches, MMIO/cache qualification, concurrency, pressure/OOM policy, target hardware, and N9 exit remain open. The next owner-independent move is `N9-PMM-RECLAIM-001`.
