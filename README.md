...
Current Cycle 88 status: 40 phases, 0 complete, 12 partial, 1 blocked, and 27 not started. The public repository, protected workflow, bounded one-host toolchain fixtures, scope-hardened ADR ceremony, candidate v1 objectives, exact Tier 1 target, read-only hardware collector, sanitized observation, and fail-closed readiness ledgers exist. The ceremony binds six exact sources and all 38 objective definitions but explicitly binds zero measurements and can never grant production promotion by itself. Rooke Poole has not accepted the profile or values, every target remains unmeasured, and the public ADR trust store intentionally has zero keys. No ADR is cryptographically signed; ADR-0003 and ADR-0004 remain proposed; and `N0-RATIFY-001` awaits owner target acceptance or amendment, ADR disposition, and custody choice. The hardware target matches 24/24 required identity checks; its collector captures 16 allowlisted user-mode CPUID records and passes 14/14 hostile controls with zero privacy violations. N2 remains partial because seven required channels are not fully observed, including CPU/MSR and SPD/topology, while 15 standards hashes, ten lab-safety prerequisites, and native-parser comparison remain open. No driver or privileged probe was authorized. Cycle 88 closes `FLAG-N4-PROFILE-001` with one workspace-local QEMU 11.0.0 candidate, two exact versioned Q35 profiles, 4/4 paused QMP machine probes, and 18/18 negative controls. No probe started a guest CPU or used native media, so no PooleBoot, PooleKernel, driver, Secure Boot, or formal-model claim follows. N4 remains partial: current upstream QEMU/EDK II source rebuilds, provider-delta review, license/SBOM/vulnerability closure, real PooleBoot evidence, executable models, malformed-device campaigns, and second-host reproduction remain open. Native PooleBoot, PooleKernel, ring-3 execution, capability enforcement, native drivers, PooleFS, PooleGlass, and the production ISO do not exist.
...
The generators are deterministic: tests reproduce the checklist, roadmap, architecture baseline, objectives-readiness, ADR-readiness, native-toolchain, hardware-readiness, and native Tier 0 ledgers byte for byte. Cycle 88 passes 455 tests with one Windows symlink-permission skip, and the non-promoting consistency release gate passes 66/66 checks over 61 artifacts with 20 explicit gaps. Doctor verifies the locked checklist hash, coverage digest, N0-N39 sequence, ADR/source hashes, candidate objectives and owner boundary, ratification scope and promotion boundary, repository identity, native toolchain/target contracts, hardware target/readiness/privacy boundaries, Tier 0 runtime closure and non-boot boundary, public index boundary, PooleGlyph bridge, PDC evidence, and retained historical consistency artifacts.

The public repository currently carries source, specifications, tests, and nine allowlisted deterministic ledgers. The public hardware observation is rebuilt from a fixed whitelist and binds only the byte count and SHA-256 of its ignored private capture; raw firmware-table bytes, raw CPUID registers, processor serials, device serials, MAC addresses, UUIDs, host/user names, absolute local paths, full PnP instances, and TPM key material are not published. The native Tier 0 ledger binds exact hashes and relative roles without publishing the 1.18 GB QEMU/OVMF runtime or installer. Public ratification manifest, detached signature, and receipt paths are reserved but remain absent until owner action. Raw internal PDC inputs, private benchmark evidence, other local run artifacts, historical lab images, firmware without redistribution clearance, secrets, and private signing material remain in the ignored private evidence vault. Full private-evidence reproduction therefore requires an owner-authorized local vault in addition to the public checkout.
