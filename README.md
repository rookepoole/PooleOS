...
- `specs/adr-ratification-policy.json`, `docs/adr-ratification-ceremony.md`, and `runs/adr_ratification_readiness.json`: secret-free owner-signing contract binding six exact decision sources and 38 objective definitions, explicit zero-measurement boundary, and deterministic pending ledger.
- `docs/n0-owner-decision-packet.md`, `specs/n0-owner-decision-packet.schema.json`, and `runs/n0_owner_decision_packet.json`: deterministic 16-source owner review surface covering both proposed ADRs, all 38 objective definitions, all allowed custody profiles, six required owner actions, and 12 fail-closed packet controls.
...
- `models/tla/`, `docs/native-formal-models.md`, `tools/bootstrap_native_models.ps1`, and `tools/qualify_native_models.py`: boot-slot rollback, capability derivation/revocation, and virtual-memory ownership/map/unmap/shootdown models plus workspace-local deterministic reproduction.
...
Current Cycle 92 status: 40 phases, 0 complete, 12 partial, 1 blocked, and 27 not started. The public repository, protected workflow, bounded one-host toolchain fixtures, scope-hardened ADR ceremony, candidate v1 objectives, exact Tier 1 target, read-only hardware collector, sanitized observation, Tier 0 profile, and fail-closed readiness ledgers exist. The ceremony binds six exact decision sources and all 38 objective definitions but binds zero measurements and cannot grant production promotion. Cycle 91 adds a deterministic owner packet binding 16 exact sources and presenting both proposed ADRs, all 38 targets, all allowed custody profiles, six required owner actions, and an unfilled response form. Cycle 92 receives an owner response with clear selections in the other fields but one unresolved three-choice placeholder; fail-closed handling leaves every packet selection `UNSELECTED`, every target unmeasured, and the public ADR trust store empty. No key was generated or used, no ADR is cryptographically signed, ADR-0003 and ADR-0004 remain proposed, and `N0-RATIFY-001` awaits one explicit replacement value before any separate source-disposition step. Cycle 92 also expands N34 from 8 to 22 subphases and from one to six `ADD-PGL-*` requirements so PooleGlyph machine language can be developed in tandem across language governance, Phase 66, frontend, Core IR, PGASM, PGB2, PGVM2, host ABI, policy, tooling, conformance, native integration, release, and IP boundaries. The hardware target matches 24/24 required identity checks; its collector captures 16 allowlisted user-mode CPUID records and passes 14/14 hostile controls with zero privacy violations. No driver or privileged probe was authorized. Cycle 88 closed `FLAG-N4-PROFILE-001` with one workspace-local QEMU 11.0.0 candidate, two exact Q35 profiles, 4/4 paused QMP probes, and 18/18 negative controls. Cycles 89-90 partially advance N4.5-N4.6: three finite TLC models drain 3/3 safe state spaces, detect 4/4 required hostile invariant violations, reproduce 7/7 case pairs exactly, and pass 14/14 negative controls. `FLAG-N4-MODELS-001` remains open because IPC, scheduler, PooleFS, and all three implementation-trace comparisons are unfinished. These checks are not theorem proofs, liveness/refinement checks, ABI-freeze authority, or kernel evidence. `N4-IPC-MODEL-001` remains the next owner-independent move, while PooleGlyph Phase 66 may advance in parallel. Native PooleBoot, PooleKernel, ring-3 execution, capability enforcement, native drivers, PooleFS, PooleGlass, and the production ISO do not exist.
...
The generators and qualifiers are deterministic: tests reproduce the checklist, roadmap, architecture baseline, objectives-readiness, ADR-readiness, N0 owner-packet, native-toolchain, hardware-readiness, native Tier 0, and native model ledgers byte for byte. Cycle 92 passes 493 tests with one expected Windows symlink-permission skip, and the non-promoting consistency release gate passes 68/68 checks over 63 artifacts with 20 explicit gaps. Doctor verifies the locked checklist hash, coverage digest, N0-N39 sequence, ADR/source hashes, candidate objectives and owner boundary, ratification scope and promotion boundary, N0 packet completeness and non-authorization controls, repository identity, native toolchain/target contracts, hardware target/readiness/privacy boundaries, Tier 0 runtime closure and non-boot boundary, bounded-model contracts/results/claim boundaries, public index boundary, PooleGlyph bridge and co-development boundaries, PDC evidence, and retained historical consistency artifacts.

The public repository currently carries source, specifications, tests, and eleven allowlisted deterministic ledgers. The N0 owner-packet ledger publishes exact source digests, advisory recommendations, and an unfilled response surface; it contains no inferred selection, private key material, signature, signing authorization, publication authorization, or production promotion. The public hardware observation is rebuilt from a fixed whitelist and binds only the byte count and SHA-256 of its ignored private capture; raw firmware-table bytes, raw CPUID registers, processor serials, device serials, MAC addresses, UUIDs, host/user names, absolute local paths, full PnP instances, and TPM key material are not published. The native Tier 0 ledger binds exact hashes and relative roles without publishing the 1.18 GB QEMU/OVMF runtime or installer. The native model ledger publishes only exact relative bindings, finite result counts, normalized traces, and claim limits; the JRE, TLC JAR, raw logs, and metadata remain ignored. Public ratification manifest, detached signature, and receipt paths are reserved but remain absent until owner action. Raw internal PDC inputs, private benchmark evidence, other local run artifacts, historical lab images, firmware without redistribution clearance, secrets, and private signing material remain in the ignored private evidence vault. Full private-evidence reproduction therefore requires an owner-authorized local vault in addition to the public checkout.
