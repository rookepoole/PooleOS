...
- `specs/native-kernel-entry-contract.json`, `native/kernel`, `runtime/native_kernel_image.py`, `docs/native-kernel-entry.md`, and `runs/native_kernel_entry_readiness.json`: real 229,376-byte canonical PKELF1 PooleKernel product in a 270,336-byte image, PKENTRY1 handoff intake, bounded early diagnostics and panic classes, 70 host tests, exact two-build reproduction, 43 hostile controls, 542 relocations, and canonical SHA-256 `06D36FAF5531509097CD03819390AC744DCFD2BD9FB64D1E6F6710DD7796776E`; live bounded subsystem slices are qualified separately through PKVM2.
...
- `specs/native-kernel-physical-memory-contract.json`, `native/kernel/src/physical_memory.rs`, `runtime/native_kernel_physical_memory.py`, `tools/qualify_native_kernel_physical_memory.py`, `docs/native-kernel-physical-memory.md`, and `runs/native-kernel-physical-memory-readiness.json`: PKPMM2 bounded one-BSP live PBP1 physical-page ownership with generation-safe scrub-before-allocation and scrub-before-reuse, full zero/readback, immutable receipts, exact-reuse stale-data rejection, release preflight, ownership-preserving fault rollback, one revoked temporary alias, 70 kernel host tests, two exact 40-marker QEMU runs, 63 hostile controls, and explicit no-predecessor-raw-API/no-reclaim/no-concurrency/no-production nonclaims.
...
- `specs/native-kernel-map-contract.json`, `native/kmap`, `runtime/native_kernel_map.py`, `docs/native-kernel-map.md`, and `tests/test_native_kernel_map.py`: PKMAP2 exact 66-page supervisor 4 KiB kernel mapping, guarded stack, read-only handoff, CR0.WP/NX/W^X enforcement, active-root audit, framebuffer translation/cache preservation, exact CR3 restoration, complete nine-file retained coverage, and retained/no-transfer boundaries.
...
Current Cycle 128 status: 40 phases, 0 complete, 16 partial, 1 blocked, and 23 not started; 46 explicit `ADD-*` requirements and 73 implementation flags are tracked. The selected `hardware_fido2_ed25519_sk` profile is still physically unavailable; no key was generated or used, the signer store remains empty, and no ADR is cryptographically signed. Rooke Poole has authorized the listed cryptographic, privileged, media, release, and promotion operation categories, but that authority does not supply hardware, evidence, owner presence, backups, recovery media, a safe target, or permission to bypass qualification and exact-release gates; no such operation occurred in Cycle 128. `N0-HW-KEY-ACQUIRE-001` remains the immediate blocked external move, and the PooleGlyph Phase 65/66 lane remains active without policy authority. Cycle 128 closes `FLAG-N9-PMM-SCRUB-001`: PKPMM2 proves full-page scrub-before-allocation and scrub-before-reuse, readback, receipts, exact-reuse residue rejection, fault rollback, and alias revocation across 70 host tests, two exact 40-marker qemu64 runs, and 63 hostile controls. N9.1-N9.4 remain partial; durable mapped allocator metadata, held-class reclaim, complete direct-map and SMP TLB policy, deferred reclaim, huge pages, PCID, COW, user faults, pager IPC, heaps/object caches, MMIO/cache qualification, concurrency, pressure/OOM policy, target hardware, and N9 exit remain open. The next owner-independent move is `N9-PMM-METADATA-001`.
