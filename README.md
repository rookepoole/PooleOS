...
- `specs/native-kernel-entry-contract.json`, `native/kernel`, `runtime/native_kernel_image.py`, `docs/native-kernel-entry.md`, and `runs/native_kernel_entry_readiness.json`: real 335,872-byte canonical PKELF1 PooleKernel product in a 376,832-byte image, PKENTRY1 handoff intake, bounded early diagnostics and panic classes, 111 host tests, exact two-build reproduction, 43 hostile controls, 855 relocations, and canonical SHA-256 `6596CB332EB24813089F95A00AC979C892C47235943CB0E73E3979ED9901B725`; live bounded subsystem slices are qualified separately through PKSMP1.
...
- `specs/native-kernel-map-contract.json`, `native/kmap`, `runtime/native_kernel_map.py`, `docs/native-kernel-map.md`, and `tests/test_native_kernel_map.py`: PKMAP2 exact 92-page supervisor 4 KiB kernel mapping, 32-page guarded stack, read-only handoff, one temporary leaf, five retained supervisor RW/NX manager leaves, two alternate guarded 32-page PMM ledger windows, and a guarded five-page IRQ MMIO reservation, with CR0.WP/NX/W^X enforcement, active-root audit, framebuffer translation/cache preservation, exact CR3 restoration, complete nine-file retained coverage, and retained/no-transfer boundaries.
...
Current Cycle 136 status: 40 phases, 0 complete, 17 partial, 1 blocked, and 22 not started; 47 explicit `ADD-*` requirements and 80 implementation flags are tracked, with 33 open. The selected `hardware_fido2_ed25519_sk` profile is still physically unavailable; no key was generated or used, the signer store remains empty, and no ADR is cryptographically signed. Rooke Poole has authorized the listed cryptographic, privileged, media, release, and promotion operation categories, but that authority does not supply hardware, evidence, owner presence, backups, recovery media, a safe target, or permission to bypass qualification and exact-release gates; no such operation occurred in Cycle 136. `N0-HW-KEY-ACQUIRE-001` remains the immediate blocked external move, and the PooleGlyph Phase 65/66 lane remains active without policy authority. PKSMP1 now proves exactly one AP start/online/quiesce/final-INIT-park lifecycle on two-vCPU qemu64, including an RX trampoline, four guards, dynamic mailbox validation, alias revocation, and exact 57,344-byte scrub/release. `FLAG-N8-SMP-FIRST-AP-001` is closed by 111 kernel host tests, two exact 38-marker runs, and 72 hostile controls; `FLAG-N8-IRQ-001` remains open because AP-local runtime state, IPIs, shootdown, I/O APIC/MSI, complete time services, panic-path rollback, target execution, and N8 exit are absent. The next owner-independent move is `N8-SMP-PERCPU-RUNTIME-001`.
