...
- `specs/native-kernel-entry-contract.json`, `native/kernel`, `runtime/native_kernel_image.py`, `docs/native-kernel-entry.md`, and `runs/native_kernel_entry_readiness.json`: real 204,800-byte canonical PKELF1 PooleKernel product, PKENTRY1 handoff intake, bounded early diagnostics and panic classes, 60 host tests, exact two-build reproduction, 43 hostile controls, and an explicit standalone-product boundary; live development transfer, traps, read-only CPU policy, x87/SSE ownership/exceptions, privileged-MSR policy, physical memory, and inactive virtual memory are qualified separately by PKXFER1, PKTRAP1, PKCPU1, PKXSTATE1, PKXEXC1, PKMSR1, PKPMM1, and PKVM1.
...
- `specs/native-kernel-physical-memory-contract.json`, `native/kernel/src/physical_memory.rs`, `runtime/native_kernel_physical_memory.py`, `tools/qualify_native_kernel_physical_memory.py`, `docs/native-kernel-physical-memory.md`, and `runs/native-kernel-physical-memory-readiness.json`: PKPMM1 bounded one-BSP live PBP1 physical-page ownership and allocator foundation with source-kind validation, usable-only admission, held reclaim classes, page-zero exclusion, retained-loader protection, DMA/DMA32/Normal accounting, generation-safe handles, quotas, poisoning, double-free rejection, coalescing, two exact 40-marker QEMU runs, 48 hostile controls, and explicit no-scrub/no-mapping/no-reclaim/no-concurrency/no-production nonclaims.
- `specs/native-kernel-virtual-memory-contract.json`, `native/kernel/src/virtual_memory.rs`, `runtime/native_kernel_virtual_memory.py`, `tools/qualify_native_kernel_virtual_memory.py`, `docs/native-kernel-virtual-memory.md`, and `runs/native-kernel-virtual-memory-readiness.json`: PKVM1 bounded one-BSP inactive four-level 4 KiB page-table foundation with frozen 48-bit bands, PMM generation ownership, map/protect/unmap rollback, W^X and mixed-cache-alias rejection, inactive-root receipts, one exactly revoked PKMAP2 bootstrap leaf, two exact 40-marker QEMU runs, 39 hostile controls, and explicit no-active-root/no-shootdown/no-heap/no-pager/no-production nonclaims.
...
Current Cycle 126 status: 40 phases, 0 complete, 16 partial, 1 blocked, and 23 not started; 46 explicit `ADD-*` requirements and 70 implementation flags are tracked. The selected `hardware_fido2_ed25519_sk` profile is still physically unavailable; no key was generated or used, the signer store remains empty, and no ADR is cryptographically signed. Rooke Poole has authorized the listed cryptographic, privileged, media, release, and promotion operation categories, but that authority does not supply hardware, evidence, owner presence, backups, recovery media, a safe target, or permission to bypass qualification and exact-release gates; no such operation occurred in Cycle 126. `N0-HW-KEY-ACQUIRE-001` remains the immediate blocked external move, and the PooleGlyph Phase 65/66 lane remains active without policy authority. Cycle 126 closes `FLAG-N9-VM-FOUNDATION-001`: PKVM1 proves a PMM-bound inactive four-level hierarchy and a narrowly scoped PKMAP2 bootstrap mapping across two exact 40-marker qemu64 runs and 39 hostile controls. N9.1-N9.4 are partial only; page scrubbing, reclaim, kernel-complete active roots, direct-map and SMP TLB policy, huge pages, PCID, COW, user faults, pager IPC, heaps/object caches, MMIO/cache qualification, concurrency, pressure/OOM policy, target hardware, and N9 exit remain open. The next owner-independent move is `N9-VM-ACTIVE-001`.
