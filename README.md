...
- `specs/native-kernel-entry-contract.json`, `native/kernel`, `runtime/native_kernel_image.py`, `docs/native-kernel-entry.md`, and `runs/native_kernel_entry_readiness.json`: real 443,504-byte canonical PKELF1 PooleKernel product in a 557,056-byte, 136-page image, PKENTRY1 handoff intake, bounded early diagnostics and panic classes, 158 host tests, exact two-build reproduction, 43 hostile controls, 1,086 relocations, and canonical SHA-256 `A5DE1DBD2ECA9243D90C2EAA2BEDCAC4B0FCC5E4A4779073E398C6722F30B943`; live bounded subsystem slices are qualified separately through PKSCHED2.
...
Current Cycle 143 status: 40 phases, 0 complete, 18 partial, 1 blocked, and 21 not started; 50 explicit `ADD-*` requirements and 87 implementation flags are tracked, with 34 open. `FLAG-N12-SCHED-PREEMPT-001` is closed only for PKSCHED2's bounded BSP timer/wakeup preemption, exact interrupt-frame ownership, deterministic task/cause trace, balanced EOIs, rollback, teardown, and complete stack/MMIO cleanup. Deferred work and kernel workers, live AP dispatch, ring 3, address-space switching, complete per-task architectural state, target hardware, N12 exit, and production remain open, so `production_ready=false`. No key, signature, firmware mutation, physical-media write, tag, release, or production promotion occurred. The selected `hardware_fido2_ed25519_sk` profile remains physically unavailable, `N0-HW-KEY-ACQUIRE-001` remains the immediate blocked external move, and `N12-SCHED-DEFERRED-001` is the next owner-independent kernel move.
