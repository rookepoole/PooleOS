...
- `specs/native-pooleboot-proof.json`, `runs/native_pooleboot_readiness.json`, `docs/native-pooleboot-proof.md`, and `tools/qualify_native_pooleboot.py`: bounded unsigned PooleBoot PE32+ aggregate proof, deterministic four-file GPT/FAT32 development media, two exact pinned OVMF runs, nineteen ordered serial/debugcon markers, static GOP-frame evidence, forty hostile controls, and explicit signature-trust/transfer/N5 nonclaims.
...
- `specs/native-kernel-load-contract.json`, `native/bootload`, `native/boot/src/kload.rs`, `runtime/native_kernel_load.py`, `docs/native-kernel-load.md`, and `runs/native_kernel_load_readiness.json`: PKLOAD2 live UEFI PBC1/PSM1/PKELF1 intake, exact manifest and digest binding, page allocation/load/relocation, W^X mapping-plan validation, deterministic cleanup, guest/oracle agreement, 40 hostile controls, and explicit unsigned/load-then-release nonclaims.
...
Current Cycle 103 status: 40 phases, 0 complete, 14 partial, 1 blocked, and 25 not started; 34 explicit `ADD-*` requirements and 42 implementation flags are tracked. The selected `hardware_fido2_ed25519_sk` profile remains unavailable; no key was generated or used, the signer store remains empty, and no ADR is cryptographically signed. Codex still has no authority to generate or use keys, sign, merge, tag, register a public key, or publish a release. `N0-HW-KEY-ACQUIRE-001` remains the blocked owner move, and the PooleGlyph Phase 65/66 co-development lane remains active. Cycle 103 closes only `FLAG-N5-MANIFEST-001`: PooleBoot now consumes live PBC1 and PSM1, binds the selected slot/version/path/file/image/SHA-256/PKENTRY1 fields, verifies the real 139,264-byte PKELF1 kernel, materializes 196,608 bytes with 40 relocations, validates four W^X-safe mapping ranges, and releases all resources. PSM1 passes 8/8 Rust tests, 64/64 hostile controls, 16,384 differential cases, and 1,027 digest cases; PKLOAD2 passes 35/35 Rust host tests, two exact builds of each product, two exact four-file media generations, two exact OVMF runs, 19 markers, and 40/40 controls. The manifest remains unsigned and untrusted, and independent PBDIGEST1 security review remains open. No retained mapping, live PBP1, `ExitBootServices`, or entry is claimed. The complete loader, executing PooleKernel system, ring-3 execution, capabilities, native drivers, PooleFS, PooleGlass, and production ISO do not exist. `N5-PBP1-LIVE-001` is the next owner-independent move.
