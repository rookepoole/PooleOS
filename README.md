...
- `specs/native-kernel-entry-contract.json`, `native/kernel`, `runtime/native_kernel_image.py`, `docs/native-kernel-entry.md`, and `runs/native_kernel_entry_readiness.json`: real 245,760-byte canonical PKELF1 PooleKernel product in a 286,720-byte image, PKENTRY1 handoff intake, bounded early diagnostics and panic classes, 79 host tests, exact two-build reproduction, 43 hostile controls, 593 relocations, and canonical SHA-256 `20BCE1C7501BC2344A6D7505BCDA749D9B2738A8435255D0EC2EF5A3E177CC4C`; live bounded subsystem slices are qualified separately through PKVM2.
...
- `specs/native-kernel-privilege-msr-policy-contract.json`, `native/kernel/src/privilege_msr.rs`, `runtime/native_kernel_privilege_msr_policy.py`, `tools/qualify_native_kernel_privilege_msr_policy.py`, `docs/native-kernel-privilege-msr-policy.md`, and `runs/native-kernel-privilege-msr-policy-readiness.json`: PKMSR1 bounded one-BSP qemu64 read-only system-linkage, FS/GS, support-gated TSC_AUX, global MCA, and PMU policy with two exact TCG QEMU/OVMF runs, 35 markers, 47 hostile controls, eleven live MSR reads, a hash-bound no-activation/no-write linked audit, and explicit syscall/MCE/PMU/AP/target/production nonclaims.
- `specs/native-kernel-physical-memory-contract.json`, `native/kernel/src/physical_memory.rs`, `runtime/native_kernel_physical_memory.py`, `tools/qualify_native_kernel_physical_memory.py`, `docs/native-kernel-physical-memory.md`, and `runs/native-kernel-physical-memory-readiness.json`: PKPMM6 bounded one-BSP live PBP1 physical-page ownership with a stable five-page guarded manager and external generation-owned active ledgers. Exact-demand checks automatically grow through 4/8/15/29-page generations, preserve rollback/retry headroom, scrub-retire three predecessors, record bounded-window soft fallback, and hard-reject unsatisfied 58-page demand before physical or ownership effects; lifecycle reclaim, 84 kernel host tests, two exact 43-marker QEMU runs, 147 hostile controls, and explicit no-interrupt-context/no-concurrency/no-SMP/no-production boundaries pass.
- `specs/native-kernel-virtual-memory-contract.json`, `native/kernel/src/active_virtual_memory.rs`, `runtime/native_kernel_virtual_memory.py`, `tools/qualify_native_kernel_virtual_memory.py`, `docs/native-kernel-virtual-memory.md`, and `runs/native-kernel-virtual-memory-readiness.json`: PKVM2 bounded one-BSP active-root profile with exact inherited mappings, eight table pages plus one data page, a nine-page supervisor direct map, transactional CR3 activation and exact restoration, architectural Accessed/Dirty handling, three local invalidation receipts, host rollback injection, two exact 40-marker QEMU runs, 46 hostile controls, and explicit no-SMP/no-ring-3/no-heap/no-pager/no-production nonclaims. PKVM1 remains the inactive predecessor foundation.
- `specs/native-kernel-map-contract.json`, `native/kmap`, `runtime/native_kernel_map.py`, `docs/native-kernel-map.md`, and `tests/test_native_kernel_map.py`: PKMAP2 exact 78-page supervisor 4 KiB kernel mapping, guarded stack, read-only handoff, one temporary leaf, five retained supervisor RW/NX manager leaves, and two alternate guarded 32-page PMM ledger windows, with CR0.WP/NX/W^X enforcement, active-root audit, framebuffer translation/cache preservation, exact CR3 restoration, complete nine-file retained coverage, and retained/no-transfer boundaries.
...
Current Cycle 132 status: 40 phases, 0 complete, 16 partial, 1 blocked, and 23 not started; 46 explicit `ADD-*` requirements and 77 implementation flags are tracked, with 33 open. The selected `hardware_fido2_ed25519_sk` profile is still physically unavailable; no key was generated or used, the signer store remains empty, and no ADR is cryptographically signed. Rooke Poole has authorized the listed cryptographic, privileged, media, release, and promotion operation categories, but that authority does not supply hardware, evidence, owner presence, backups, recovery media, a safe target, or permission to bypass qualification and exact-release gates; no such operation occurred in Cycle 132. `N0-HW-KEY-ACQUIRE-001` remains the immediate blocked external move, and the PooleGlyph Phase 65/66 lane remains active without policy authority. Cycle 132 closes the bounded serial scope of `FLAG-N9-PMM-GROWTH-AUTOMATION-001`: PKPMM6 checks exact demand before automatic scrubbed allocate/free, grows through 4/8/15/29-page generations, preserves rollback/retry headroom, scrub-retires three predecessors, records four soft fallbacks, and host-proves hard rejection before effects when the 58-page next layout cannot fit. The receipt passes 84 kernel host tests, two exact 43-marker qemu64 runs, and 147 hostile controls. N9.1-N9.4 remain partial; complete ACPI consumer integration, complete direct-map and SMP TLB policy, deferred reclaim, huge pages, PCID, COW, user faults, pager IPC, heaps/object caches, MMIO/cache qualification, interrupt-context/concurrent allocation, general pressure/OOM policy, target hardware, and N9 exit remain open. The next owner-independent move is `N9-PMM-ACPI-CONSUMER-001`.
...
The generators and qualifiers are deterministic: tests reproduce the checklist, roadmap, architecture baseline, objectives-readiness, governance receipts, native toolchain, hardware/Tier 0/model evidence, native PooleBoot, PBP1/PBC1/PSM1/PBART1 and all six inner formats, PBTRUST1/PBSTATE1, PKELF1, PKENTRY1, PKLOAD6/PKMAP2/PBEXIT1, PKREVAL1, PKXFER1, PKTRAP1, PKCPU1, PKERR1, PKXSTATE1, PKXEXC1, PKMSR1, PKPMM6, PKVM1, and PKVM2 ledgers byte for byte. Cycle 132 contains 787 Python tests with one expected Windows symlink-permission skip; the growth/reclaim cases are included in the 84 Rust kernel host tests. Doctor runs the full PooleGlyph stack from a temporary source mirror while preserving its generated reports and run log. The 93-check non-promoting consistency release gate verifies 88 artifacts, retains 20 explicit gaps, and keeps `production_ready=false`.
