...
- `specs/native-pooleboot-proof.json`, `runs/native_pooleboot_readiness.json`, `docs/native-pooleboot-proof.md`, and `tools/qualify_native_pooleboot.py`: bounded unsigned PooleBoot PE32+ aggregate proof, deterministic ten-file GPT/FAT32 development media, two exact pinned OVMF runs, twenty-four ordered serial/debugcon markers, exact retained-page PINIT1/PREC1/PSYM1/PMCU1/PFWM1/PPOL1 parsing and mandatory denial, retained PKMAP2/PBLIVE3/PBEXIT1 evidence, static GOP-frame evidence, 139 hostile controls, zero authority/action/state/hardware effects, and explicit authentication/PooleKernel-revalidation/kernel-activation-or-consumption/kernel-entry/target-firmware/N5 nonclaims.
...
- `docs/native-initial-system-profile.md`, `native/artifact`, `native/inner`, `runtime/native_boot_artifact.py`, `runtime/native_inner_live.py`, `specs/native-kernel-load-contract.json`, `native/bootload`, `native/boot/src/exit.rs`, `native/bootexit`, `runtime/native_kernel_load.py`, `docs/native-kernel-load.md`, and `runs/native_kernel_load_readiness.json`: PKLOAD6 live UEFI PBC1/PSM1/PKELF1/PBART1 intake, exact seven-artifact development profile, transactional zero-padded load/cleanup, six-format retained-page parsing with PPOL1 payload/PINIT1 route cross-binding and missing-signature denial, retained PKMAP2 artifact/root/guarded-stack/handoff ranges, final PBLIVE3, bounded PBEXIT1, successful `ExitBootServices`, exact guest/oracle agreement, 139 hostile controls, and explicit unsigned/no-authority/no-state-I/O/no-PooleKernel-revalidation-or-activation/stop-before-transfer nonclaims.
...
Current Cycle 114 status: 40 phases, 0 complete, 14 partial, 1 blocked, and 25 not started; 42 explicit `ADD-*` requirements and 56 implementation flags are tracked. The selected `hardware_fido2_ed25519_sk` profile remains unavailable; no key was generated or used, the signer store remains empty, and no ADR is cryptographically signed. Codex still has no authority to generate or use keys, sign, merge, tag, register a public key, or publish a release. `N0-HW-KEY-ACQUIRE-001` remains the blocked owner move, and the PooleGlyph Phase 65/66 co-development lane remains active without policy authority. Cycle 114 closes only `FLAG-N5-INNER-PARSE-001`: live PooleBoot reparses all six exact retained PBART1 files, binds PPOL1's five payload digests and eleven PINIT1 routes, requires six development gates to deny at the missing outer signature, and emits exact retained-set SHA-256 `F3154B354C77D0567207994EFDDA4FE2D203611CA21D60B63872BC9FFC73C675` with zero authority grants, authorized actions, state writes, or hardware observations. PKLOAD6 passes 81/81 Rust tests, two exact QEMU/OVMF runs, 24 markers, and 139/139 controls. `FLAG-N5-INNER-TRUST-STATE-001`, broad `FLAG-N5-INNER-ENFORCEMENT-001`, independent PooleKernel revalidation, authenticated persistent rollback state, PBDIGEST1 review, final CR3/RSP, kernel entry, target firmware, N5 exit, and production readiness remain open. The executing PooleKernel system, ring-3 execution, capabilities, native drivers, PooleFS, PooleGlass, and production ISO do not exist. `N5-INNER-TRUST-STATE-001` is the next owner-independent move.
