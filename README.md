...
- `specs/native-pooleboot-proof.json`, `runs/native_pooleboot_readiness.json`, `docs/native-pooleboot-proof.md`, and `tools/qualify_native_pooleboot.py`: bounded unsigned PooleBoot PE32+ aggregate proof, deterministic four-file GPT/FAT32 development media, two exact pinned OVMF runs, twenty-one ordered serial/debugcon markers, exact temporary PBP1 reconstruction, static GOP-frame evidence, fifty-two hostile controls, and explicit signature-trust/final-map/transfer/N5 nonclaims.
...
- `specs/native-kernel-load-contract.json`, `native/bootload`, `native/boot/src/kload.rs`, `native/boot/src/livehandoff.rs`, `runtime/native_kernel_load.py`, `docs/native-kernel-load.md`, and `runs/native_kernel_load_readiness.json`: PKLOAD3 live UEFI PBC1/PSM1/PKELF1 intake, exact manifest and digest binding, page allocation/load/relocation, W^X mapping-plan validation, stride-aware temporary PBP1 production, deterministic cleanup, guest/oracle agreement, 52 hostile controls, and explicit unsigned/pre-exit/release nonclaims.
...
Current Cycle 104 status: 40 phases, 0 complete, 14 partial, 1 blocked, and 25 not started; 34 explicit `ADD-*` requirements and 43 implementation flags are tracked. The selected `hardware_fido2_ed25519_sk` profile remains unavailable; no key was generated or used, the signer store remains empty, and no ADR is cryptographically signed. Codex still has no authority to generate or use keys, sign, merge, tag, register a public key, or publish a release. `N0-HW-KEY-ACQUIRE-001` remains the blocked owner move, and the PooleGlyph Phase 65/66 co-development lane remains active. Cycle 104 closes only `FLAG-N5-PBP1-LIVE-001`: PooleBoot now retains the verified real 139,264-byte PKELF1 kernel through a stride-aware live memory-map snapshot, produces exact config/manifest/kernel/GOP-bound temporary PBP1 bytes, emits and independently reconstructs them on both diagnostic channels, rechecks their lifetime, and releases all PBP1 pools and kernel pages. PKLOAD3 passes 49/49 Rust host tests, two exact 81,408-byte PooleBoot builds, two exact kernel and media generations, two exact OVMF runs, 21 markers, 52/52 controls, and two exact 4,248-byte PBP1 matches. The manifest remains unsigned and untrusted, and independent PBDIGEST1 security review remains open. No retained mapping or handoff, installed page table, final memory map, `ExitBootServices`, or entry is claimed. The complete loader, executing PooleKernel system, ring-3 execution, capabilities, native drivers, PooleFS, PooleGlass, and production ISO do not exist. `N5-KMAP-001` is the next owner-independent move.
