...
- `specs/native-kernel-entry-contract.json`, `native/kernel`, `runtime/native_kernel_image.py`, `docs/native-kernel-entry.md`, and `runs/native_kernel_entry_readiness.json`: real 409,600-byte canonical PKELF1 PooleKernel product in a 458,752-byte image, PKENTRY1 handoff intake, bounded early diagnostics and panic classes, 130 host tests, exact two-build reproduction, 43 hostile controls, 959 relocations, and canonical SHA-256 `6B8A9C2C3EAC559E1D9CB5965800A1671DB8F487F149861300D4EDCB279B3A11`; live bounded subsystem slices are qualified separately through PKSMP3.
...
- `specs/native-kernel-smp-ipi-contract.json`, `native/kernel/src/smp_ipi.rs`, `runtime/native_kernel_smp_ipi.py`, `tools/qualify_native_kernel_smp_ipi.py`, `docs/native-kernel-smp-ipi.md`, and `runs/native-kernel-smp-ipi-readiness.json`: PKSMP3 adds six fixed development-only IPI vectors and operation classes with checksum-bound request/response state, sequence and attempt replay controls, checked acknowledgement/EOI, bounded offline timeout, panic/stop/park/release ordering, and exact cleanup. Two exact 39-marker runs observe six accepted and four denied deliveries, ten EOIs, and one timeout; 18 hostile-control categories cover 120 rejected cases and 130 kernel host tests pass. The shootdown class performs zero TLB invalidations, call-function exposes no arbitrary callback, and no production capability, scheduler, target, N8/N9-exit, or production claim is made.
- `specs/native-kernel-map-contract.json`, `native/kmap`, `runtime/native_kernel_map.py`, `docs/native-kernel-map.md`, and `tests/test_native_kernel_map.py`: PKMAP2 exact 112-page supervisor 4 KiB kernel mapping, 32-page guarded stack, read-only handoff, one temporary leaf, five retained supervisor RW/NX manager leaves, two alternate guarded 32-page PMM ledger windows, and a guarded five-page IRQ MMIO reservation, with CR0.WP/NX/W^X enforcement, active-root audit, framebuffer translation/cache preservation, exact CR3 restoration, complete nine-file retained coverage, and retained/no-transfer boundaries.
...
Current Cycle 138 status: 40 phases, 0 complete, 17 partial, 1 blocked, and 22 not started; 47 explicit `ADD-*` requirements and 83 implementation flags are tracked, with 34 open. The selected `hardware_fido2_ed25519_sk` profile is still physically unavailable; no key was generated or used, the signer store remains empty, and no ADR is cryptographically signed. Rooke Poole has authorized the listed cryptographic, privileged, media, release, and promotion operation categories, but that authority does not supply hardware, evidence, owner presence, backups, recovery media, a safe target, or permission to bypass qualification and exact-release gates; no such operation occurred in Cycle 138. `N0-HW-KEY-ACQUIRE-001` remains the immediate blocked external move, and the PooleGlyph Phase 65/66 lane remains active without policy authority. PKSMP3 now proves six fixed development-only IPI vectors and operation classes with checked acknowledgement, EOI, denial, timeout, panic, stop, park, revocation, and exact 131,072-byte scrub/release on a two-vCPU SandyBridge-minus-AVX TCG profile. `FLAG-N8-SMP-IPI-001` is closed by 130 kernel host tests, two exact 39-marker runs, and 18 hostile-control categories covering 120 independently rejected cases; `FLAG-N8-IRQ-001` remains open, and real remote TLB invalidation, deferred reclaim, I/O APIC/MSI, complete time services, panic-path rollback, target execution, and N8/N9 exit remain absent. The next owner-independent move is `N9-SMP-SHOOTDOWN-001`.
...
The generators and qualifiers are deterministic: tests reproduce the checklist, roadmap, architecture baseline, objectives-readiness, governance receipts, native toolchain, hardware/Tier 0/model evidence, native PooleBoot, PBP1/PBC1/PSM1/PBART1 and all six inner formats, PBTRUST1/PBSTATE1, PKELF1, PKENTRY1, PKLOAD6/PBLIVE4/PKMAP2/PBEXIT1, PKREVAL1, PKXFER1, PKTRAP1, PKCPU1, PKERR1, PKXSTATE1, PKXEXC1, PKMSR1, PKPMM7, PKACPI1, PKVM1, PKVM2, PKVM3, PKIRQ1, PKSMP1, PKSMP2, and PKSMP3 ledgers byte for byte. Cycle 138 passes 819/819 Python tests with one expected Windows symlink-permission skip, 215/215 aggregate PKLOAD6 Rust tests, and 130/130 kernel host tests. PKSMP3 request/response, capability, replay, acknowledgement, EOI, timeout, panic, lifecycle, cleanup, and hostile-source cases are included. Doctor runs the full PooleGlyph stack from a temporary source mirror while preserving its generated reports and run log. The 97-check non-promoting consistency release gate covers 92 artifacts and keeps `production_ready=false`.

The public repository currently carries source, specifications, tests, and explicitly allowlisted deterministic ledgers. The historical N0 packet retains every field unselected, and the owner-response ledger contains no public key, private key, signature, credential, or secret. The sanitized hardware observation excludes raw firmware bytes, raw CPUID registers, device identifiers, user paths, and TPM material. Native model, boot, protocol, parser, loader, kernel-entry, live-load, revalidation, and transfer ledgers publish only relative bindings, bounded results, hashes, and claim limits; local product bytes, QEMU/OVMF runtime, generated media, screenshots, raw logs, and operational metadata remain ignored. Public-key publication and signing are now authorized but remain unperformed because the selected hardware key is unavailable and the custody ceremony has not occurred. Raw internal PDC inputs, private benchmarks, historical images, uncleared firmware, secrets, and private signing material remain in the ignored private evidence vault.
