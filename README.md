...
- `specs/native-kernel-entry-contract.json`, `native/kernel`, `runtime/native_kernel_image.py`, `docs/native-kernel-entry.md`, and `runs/native_kernel_entry_readiness.json`: real freestanding PKELF1 PooleKernel product, PKENTRY1 handoff intake, bounded early diagnostics and panic classes, 52 host tests, exact two-build reproduction, 43 hostile controls, and an explicit standalone-product boundary; live development transfer, traps, read-only CPU policy, x87/SSE ownership/exceptions, privileged-MSR policy, and early physical memory are qualified separately by PKXFER1, PKTRAP1, PKCPU1, PKXSTATE1, PKXEXC1, PKMSR1, and PKPMM1.
...
Current Cycle 125 status: 40 phases, 0 complete, 16 partial, 1 blocked, and 23 not started; 46 explicit `ADD-*` requirements and 69 implementation flags are tracked. The selected `hardware_fido2_ed25519_sk` profile is still physically unavailable; no key was generated or used, the signer store remains empty, and no ADR is cryptographically signed. Rooke Poole has authorized the listed cryptographic, privileged, media, release, and promotion operation categories, but that authority does not supply hardware, evidence, owner presence, backups, recovery media, a safe target, or permission to bypass qualification and exact-release gates; no such operation occurred in Cycle 125. `N0-HW-KEY-ACQUIRE-001` remains the immediate blocked external move, and the PooleGlyph Phase 65/66 lane remains active without policy authority. Cycle 125 closes `FLAG-N9-PMM-FOUNDATION-001`: PKPMM1 consumes the exact live PBP1 map, protects retained ownership, and proves a deterministic zoned early allocator across two exact 40-marker qemu64 runs and 48 hostile controls. N9.1/N9.2 are partial only; page scrubbing, reclaim, virtual memory, heaps, MMIO/cache aliases, concurrency, pressure/OOM policy, target hardware, and N9 exit remain open. The next owner-independent move is `N9-VM-001` for virtual-layout and page-table primitives.
