...
- `specs/native-pooleboot-proof.json`, `runs/native_pooleboot_readiness.json`, `docs/native-pooleboot-proof.md`, and `tools/qualify_native_pooleboot.py`: bounded unsigned PooleBoot PE32+ aggregate proof, deterministic ten-file GPT/FAT32 development media, two exact pinned OVMF runs, twenty-three ordered serial/debugcon markers, retained PBART1/PINIT1/PKMAP2/PBLIVE3/PBEXIT1 evidence, static GOP-frame evidence, 115 hostile controls, and explicit signature/PooleBoot-PINIT1-enforcement/kernel-activation/kernel-entry/target-firmware/N5 nonclaims.
...
Current Cycle 108 status: 40 phases, 0 complete, 14 partial, 1 blocked, and 25 not started; 36 explicit `ADD-*` requirements and 48 implementation flags are tracked. The selected `hardware_fido2_ed25519_sk` profile remains unavailable; no key was generated or used, the signer store remains empty, and no ADR is cryptographically signed. Codex still has no authority to generate or use keys, sign, merge, tag, register a public key, or publish a release. `N0-HW-KEY-ACQUIRE-001` remains the blocked owner move, and the PooleGlyph Phase 65/66 co-development lane remains active. Cycle 108 closes only `FLAG-N5-INIT-BUNDLE-001`: canonical PINIT1 initial-system declarations bind three components, three services, a deterministic dependency DAG, four abstract resources, eleven attenuated routes, lifecycle policy, reverse rollback, and fail-closed activation requirements. Its independent Python and allocation-free `no_std` Rust implementations pass 3/3 tests, 2/2 targets, 3/3 vectors, 120/120 controls, and 16,384 differential cases. PKLOAD6 and aggregate PooleBoot evidence bind the exact inner bytes through a host-only oracle and pass 115/115 controls. All artifacts remain unsigned and untrusted; declarations confer no authority, PooleBoot does not parse PINIT1, PooleKernel does not activate it, and no initial-system instruction executes. Recovery/symbol/microcode/firmware/policy semantics, independent PBDIGEST1 review, final CR3/RSP, kernel entry, target firmware, N5 exit, and production readiness remain open. The executing PooleKernel system, ring-3 execution, capabilities, native drivers, PooleFS, PooleGlass, and production ISO do not exist. `N5-RECOVERY-SEMANTICS-001` is the next owner-independent move.
