...
- `specs/native-pooleboot-proof.json`, `runs/native_pooleboot_readiness.json`, `docs/native-pooleboot-proof.md`, and `tools/qualify_native_pooleboot.py`: bounded unsigned PooleBoot PE32+ aggregate proof, deterministic ten-file GPT/FAT32 development media, two exact pinned OVMF runs, twenty-three ordered serial/debugcon markers, retained PBART1/PINIT1/PREC1/PSYM1/PMCU1/PFWM1/PKMAP2/PBLIVE3/PBEXIT1 evidence, static GOP-frame evidence, 127 hostile controls, and explicit signature/PooleBoot-inner-enforcement/kernel-activation-recovery-symbol-microcode-or-firmware-consumption/kernel-entry/target-firmware/N5 nonclaims.
...
Current Cycle 112 status: 40 phases, 0 complete, 14 partial, 1 blocked, and 25 not started; 40 explicit `ADD-*` requirements and 52 implementation flags are tracked. The selected `hardware_fido2_ed25519_sk` profile remains unavailable; no key was generated or used, the signer store remains empty, and no ADR is cryptographically signed. Codex still has no authority to generate or use keys, sign, merge, tag, register a public key, or publish a release. `N0-HW-KEY-ACQUIRE-001` remains the blocked owner move, and the PooleGlyph Phase 65/66 co-development lane remains active. Cycle 112 closes only `FLAG-N5-FIRMWARE-BUNDLE-001`: canonical PFWM1 freezes three exact synthetic components, two dependency edges, external payload identities, hardware and version floors, signer/updater/recovery bindings, a single-transaction topological plan, 47 ordered dry-run prerequisites, and post-reset receipt and driver-rebind checks. Independent Python and allocation-free `no_std` Rust implementations pass 5/5 tests, 2/2 targets, 3/3 vectors, 101/101 controls, 16,384 parser cases, 8,192 activation cases, and 8,192 post-reset cases with zero mismatches. PKLOAD6 and aggregate PooleBoot bind exact PFWM1 bytes through a prohibited-from-production host oracle and pass 127/127 controls. PFWM1 embeds zero payload bytes, observes no live inventory, creates no apply authority, and performs no firmware call, capsule submission, driver load, reset, mutation, or physical-media write. Role-7 policy semantics, live PINIT1/PREC1/PSYM1/PMCU1/PFWM1 enforcement, authenticated persistent rollback state, independent PBDIGEST1 review, final CR3/RSP, kernel entry, target firmware, N5 exit, and production readiness remain open. The executing PooleKernel system, ring-3 execution, capabilities, native drivers, PooleFS, PooleGlass, and production ISO do not exist. `N5-POLICY-SEMANTICS-001` is the next owner-independent move.
...
The generators and qualifiers are deterministic: tests reproduce the checklist, roadmap, architecture baseline, objectives-readiness, ADR-readiness, frozen N0 owner-packet, completed owner-response receipt, native-toolchain, hardware-readiness, native Tier 0, native model, native PooleBoot, PBP1, PBC1, PSM1, PBART1, PINIT1, PREC1, PSYM1, PMCU1, PFWM1, PKELF1, PKENTRY1, and PKLOAD6/PKMAP2/PBEXIT1 ledgers byte for byte. Cycle 112 contains 678 tests with one expected Windows symlink-permission skip. Doctor writes live PooleGlyph conformance output to temporary storage so verification does not rewrite the tandem repository's generated report. The 81-check non-promoting consistency release gate verifies 76 artifacts covering the locked checklist hash, coverage digest, N0-N39 sequence, governance non-authorization controls, native toolchain and target contracts, hardware and Tier 0 boundaries, bounded models, boot evidence and nonclaims, public-symbol, microcode-package, and firmware-manifest controls, PooleGlyph co-development boundaries, PDC evidence, and retained historical consistency artifacts while keeping 20 explicit gaps and `production_ready=false`.
