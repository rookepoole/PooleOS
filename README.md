...
- `specs/native-kernel-entry-contract.json`, `native/kernel`, `runtime/native_kernel_image.py`, `docs/native-kernel-entry.md`, and `runs/native_kernel_entry_readiness.json`: real freestanding PKELF1 PooleKernel product, PKENTRY1 handoff intake, bounded early diagnostics and panic classes, 38 host tests, exact two-build reproduction, 43 hostile controls, and an explicit standalone-product boundary; live development transfer, traps, read-only CPU policy, x87/SSE ownership, and xstate exceptions are qualified separately by PKXFER1, PKTRAP1, PKCPU1, PKXSTATE1, and PKXEXC1.
...
Current Cycle 123 status: 40 phases, 0 complete, 15 partial, 1 blocked, and 24 not started; 45 explicit `ADD-*` requirements and 67 implementation flags are tracked. The selected `hardware_fido2_ed25519_sk` profile is still physically unavailable; no key was generated or used, the signer store remains empty, and no ADR is cryptographically signed. Rooke Poole has authorized the listed cryptographic, privileged, media, release, and promotion operation categories, but that authority does not supply hardware, evidence, owner presence, backups, recovery media, a safe target, or permission to bypass qualification and exact-release gates; no such operation occurred in Cycle 123. `N0-HW-KEY-ACQUIRE-001` remains the immediate blocked external move, and the PooleGlyph Phase 65/66 lane remains active without policy authority. Cycle 123 closes only `FLAG-N7-XSTATE-EXCEPTION-001`: PKXEXC1 proves bounded processor-delivered x87 `#MF` and SIMD `#XM`, exact recovery and resume, terminal test-only `#NM` eager-policy rejection, and a hash-bound linked-image machine-code audit on one WHPX BSP. N7.4 is partial, not complete; AVX/extended state, scheduler and user-task delivery, AP/migration integration, target hardware, and N7 exit remain open. The next owner-independent move is `N7-PRIVILEGE-MSR-POLICY-001` for the syscall/GS/TSC_AUX/MCE/performance-MSR policy boundary.
