...
- `specs/native-kernel-entry-contract.json`, `native/kernel`, `runtime/native_kernel_image.py`, `docs/native-kernel-entry.md`, and `runs/native_kernel_entry_readiness.json`: real 485,000-byte canonical PKELF1 PooleKernel product in a 557,056-byte, 136-page image, fixed `0xA000` entry, page-aligned `0x69000` text and `0x76000` RELRO/data boundaries, PKENTRY1 handoff intake, bounded early diagnostics and panic classes, 184 host tests, exact two-build reproduction, 43 hostile controls, 1,222 relocations, and canonical SHA-256 `D11591395FDD8CD7BEEFA0D847A5C99EB133ED15F8DCDDE3392BFA499DCEDC33`; live bounded subsystem slices are qualified separately through PKSCHED5.
...
Current Cycle 146 status: 40 phases remain non-complete. `FLAG-N12-SCHED-AP-WORKERS-001` is closed only for PKSCHED5's bounded BSP-0/AP-1,2,3 typed AP-worker profile, including EOI-gated dispatch, fixed timer-driver and generation-reclaim consumers, remote cancellation, offline rollback, flush/reclamation ordering, bounded starvation, and exact park/scrub/release cleanup. Arbitrary callbacks, a general driver/service framework, general topology/hotplug/x2APIC, general SMP preemption, complete atomics/lock/reclamation families, ring 3, address-space and full per-task architectural switching, target hardware, N12 exit, and production remain open, so `production_ready=false`. No key, signature, firmware mutation, physical-media write, tag, release, or production promotion occurred. The selected `hardware_fido2_ed25519_sk` profile remains physically unavailable, `N0-HW-KEY-ACQUIRE-001` remains the immediate blocked external move, and `N12-SCHED-SMP-PREEMPT-001` is the next owner-independent kernel move.
