...
Current state: PooleKernel is implemented only as a bounded early freestanding product and is not production-ready. Cycles 97-118 establish unsigned PooleBoot intake, exact retained PKMAP2/PBLIVE3/PBEXIT1 state, a reproducible 64-page PKELF1 image, allocation-free nine-file PKREVAL1, and opt-in QEMU-only PKXFER1 with no authority. Cycles 119-124 add bounded BSP trap, qemu64 CPU observation, exact-target rejection policy, eager x87/SSE ownership/exceptions, and privileged-MSR observation. Cycle 125 adds PKPMM1 physical ownership. Cycle 126 adds PKVM1: a fixed-capacity inactive four-level 4 KiB hierarchy over generation-bound PKPMM1 allocations, frozen 48-bit bands, map/protect/unmap rollback, W^X and mixed-cache-alias rejection, inactive-root receipts, and deferred frame reuse. Two exact fresh-vars qemu64 runs emit 40 markers and pass 39 hostile controls with 4,104 inactive-table writes. Since newly allocated conventional pages are not inherited identity mappings, the adapter validates live BSP CPUID width and exclusively uses one absent PKMAP2 leaf as a supervisor RW/NX bootstrap alias; 40 active-leaf writes and 40 local `INVLPG` operations occur, and the alias is revoked before table free. PKVM1 performs zero root-installing CR3 writes, shootdowns, signatures, authority grants, or actions. This is not authenticated boot, production transfer, target qualification, direct per-processor microcode evidence, syscall or GS/TSC_AUX activation, machine-check handling, PMU ownership, AVX/extended-state qualification, scheduler or user-task delivery, AP/migration integration, complete per-CPU descriptor state, page scrubbing, reclaim, a kernel-complete active root, direct map, active-address-space invalidation/SMP shootdown, huge pages, PCID, COW, user faults, pager IPC, heaps, MMIO/cache qualification, concurrent/SMP allocation, pressure/OOM policy, capabilities, ring 3, N5/N6/N7/N9 exit, or production readiness. The applicable Model 40h-4Fh errata source and a direct numeric client microcode floor or ratified replacement remain stop-ship gaps. The selected FIDO2 key remains physically unavailable despite authorization for acquisition and later key/signing operations. The immediate external move is `N0-HW-KEY-ACQUIRE-001`; the next owner-independent engineering move is `N9-VM-ACTIVE-001`.
