...
Current state: PooleKernel is implemented only as a bounded early freestanding product and is not production-ready. Cycles 97-117 establish unsigned PooleBoot intake, exact retained PKMAP2/PBLIVE3/PBEXIT1 state, a reproducible 64-page PKELF1 image, and allocation-free nine-file PKREVAL1 with no authority. Cycle 118 adds PKXFER1 as an opt-in QEMU-only path while preserving the default permanent stop: PooleBoot installs retained CR3/RSP, clears IF/DF, transfers once, and PooleKernel validates runtime/PBP1 continuity, live-executes PKREVAL1, emits exact serial/debugcon terminal denial, and halts. Two exact runs and 57 hostile controls pass with zero signatures, authority, actions, writes, or post-exit firmware calls. This is not authenticated boot, a production transfer profile, descriptor/exception containment, memory management, capabilities, ring 3, target-firmware evidence, physical-hardware evidence, N5/N6 exit, or production readiness. The selected FIDO2 key remains physically unavailable despite Cycle 118 authorization for acquisition and later key/signing operations. The immediate external move is `N0-HW-KEY-ACQUIRE-001`; the next owner-independent engineering move is `N7-TRAP-001`.
