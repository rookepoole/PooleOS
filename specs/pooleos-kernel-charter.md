...
Current state: PooleKernel remains chartered, not implemented, and not production-ready. Cycle 87 closes only the bounded user-mode CPUID sub-capability of `N2-HW-002`; no privileged probe, native parser, driver, or kernel mechanism is implied. Cycle 88 adds a pinned host-side Tier 0 Q35/QEMU/OVMF profile and proves only deterministic command construction plus paused machine instantiation. Cycles 89-90 and 94-96 add bounded boot-slot rollback, capability derivation/revocation, virtual-memory ownership/map/unmap/shootdown, capability-mediated IPC, scheduler, and PooleFS transaction/recovery checks: all six safe finite spaces drain and all twenty-one deliberately unsafe configurations yield their required invariant violation. Cycle 97 separately executes one unsigned PooleBoot UEFI proof application twice under pinned OVMF with deterministic GPT/FAT32 media, bounded firmware-table/map handling, ordered dual-channel diagnostics, and matching static GOP frames. Cycle 98 qualifies the candidate PBP1 byte protocol with `no_std` Rust and independent Python codecs, three golden vectors, 32 hostile controls, and 16,384 zero-mismatch differential cases. No current PooleBoot path populates PBP1, no PooleKernel path consumes it, and no `ExitBootServices` or transfer execution occurred. No liveness, refinement, fingerprint-collision, signed ABI-ratification, hardware-durability, N5-exit, or production claim follows. The immediate owner move remains blocked `N0-HW-KEY-ACQUIRE-001`; the next owner-independent engineering move is `N5-BOOTCFG-001`.
