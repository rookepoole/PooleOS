...
Current state: PooleKernel is implemented only as a bounded early freestanding product and is not production-ready. Cycles 97-118 establish unsigned PooleBoot intake, exact retained PKMAP2/PBLIVE3/PBEXIT1 state, a reproducible PKELF1 image, allocation-free nine-file PKREVAL1, and opt-in QEMU-only PKXFER1 with no authority. Cycles 119-124 add bounded BSP trap, qemu64 CPU observation, exact-target rejection policy, eager x87/SSE ownership/exceptions, and privileged-MSR observation. Cycles 125-127 add PKPMM1 generation-safe physical ownership, PKVM1 inactive four-level transactions, and one bounded PKVM2 active root with a nine-page direct map, exact CR3 restoration, and three local invalidation receipts. Cycles 128-130 add PKPMM2 scrubbed lifecycle transactions, the PKPMM3 retained five-page guarded manager, and PKPMM4 lifecycle-gated Boot Services reclaim while ACPI remains held. Cycle 131 upgrades selector 8 to PKPMM5: every active source, free-extent, allocation, scrub-receipt, and reclaim-receipt ledger moves into guarded generation-owned pages behind the stable manager; generation 2 starts in four pages and atomically grows to generation 3 in eight pages through complete preflight, map/copy/seal/switch, old-leaf revocation, scrub, and retirement; precommit failures roll back and postcommit retirement can retry. Eighty-two kernel host tests, two exact fresh-vars qemu64 runs with 43 markers, and 137 hostile controls pass. The runs finish with 129,162 managed pages while 11 ACPI pages remain held, protect 831 loader pages, scrub and verify 11,279 pages and 46,198,784 bytes, and perform 5,775,872 physical writes, 5,776,896 reads, and 22,591 temporary-PTE writes and invalidations. The current kernel is 76 pages with 662 relocations and SHA-256 `30E414826D7DF588C07A66032423DCD6AA8C47B0E6CCFC3D6686BC9224AFB947`. This is not authenticated boot, production transfer, target qualification, direct per-processor microcode evidence, syscall or GS/TSC_AUX activation, machine-check handling, PMU ownership, AVX/extended-state qualification, scheduler or user-task delivery, AP/migration integration, complete per-CPU descriptor state, automatic pressure-triggered or repeated allocator-ledger growth, guarded-window exhaustion/fallback, complete ACPI consumer integration, complete generation-owned address spaces, SMP shootdown, huge pages, PCID, COW, user faults, pager IPC, heaps, MMIO/cache qualification, concurrent/SMP allocation, pressure/OOM policy, capabilities, ring 3, N5/N6/N7/N9 exit, or production readiness. The applicable Model 40h-4Fh errata source and a direct numeric client microcode floor or ratified replacement remain stop-ship gaps. The selected FIDO2 key remains physically unavailable despite authorization for acquisition and later key/signing operations. The immediate external move is `N0-HW-KEY-ACQUIRE-001`; the next owner-independent engineering move is `N9-PMM-GROWTH-AUTOMATION-001`.
