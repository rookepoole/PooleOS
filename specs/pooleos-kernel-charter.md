...
Current state: PooleKernel is implemented only as a bounded early freestanding product and is not production-ready. Cycles 97-118 establish unsigned PooleBoot intake, exact retained PKMAP2/PBLIVE3/PBEXIT1 state, a reproducible 64-page PKELF1 image, allocation-free nine-file PKREVAL1, and opt-in QEMU-only PKXFER1 with no authority. Cycle 119 adds PKTRAP1 as a BSP-only descriptor/fault development slice. Cycle 120 adds PKCPU1 as a mutually exclusive BSP-only qemu64 read-only CPU-policy slice: required CPUID identity/features/topology/address widths, CR0/CR4/EFER, XCR0, and support-gated APIC/PAT/MTRR MSRs pass exact Rust/Python agreement across two QEMU/OVMF runs, 35 ordered markers, and 41 hostile controls with five MSR reads, zero CPU/MSR writes, zero signatures, authority, actions, state writes, or post-exit firmware calls. This is not authenticated boot, a production transfer profile, exact Ryzen target or errata/microcode policy, xstate ownership, AP-local state, per-CPU or complete exception containment, memory management, capabilities, ring 3, target-firmware evidence, physical-hardware evidence, N5/N6/N7 exit, or production readiness. The selected FIDO2 key remains physically unavailable despite authorization for acquisition and later key/signing operations. The immediate external move is `N0-HW-KEY-ACQUIRE-001`; the next owner-independent engineering move is `N7-ERRATA-POLICY-001`.
