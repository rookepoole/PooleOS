...
Current state: PooleKernel is implemented only as a bounded early freestanding product and is not production-ready. Cycles 97-118 establish unsigned PooleBoot intake, exact retained PKMAP2/PBLIVE3/PBEXIT1 state, a reproducible 64-page PKELF1 image, allocation-free nine-file PKREVAL1, and opt-in QEMU-only PKXFER1 with no authority. Cycles 119-123 add bounded BSP trap, qemu64 CPU observation, exact-target rejection policy, eager x87/SSE ownership, and xstate-exception slices. Cycle 124 adds PKMSR1: two exact fresh-vars TCG qemu64 runs prove support-gated read-only system-linkage, FS/GS, optional TSC_AUX, global MCA, and unsupported-PMU policy. Thirty-five ordered markers, 47 hostile controls, eleven live MSR reads, and a hash-bound linked-image audit pass with zero activation/write instructions, signatures, authority, actions, post-exit firmware calls, or media effects. QEMU MCA values remain compatibility observations only. This is not authenticated boot, production transfer, target qualification, direct per-processor microcode evidence, syscall or GS/TSC_AUX activation, machine-check handling, PMU ownership, AVX/extended-state qualification, scheduler or user-task delivery, AP/migration integration, complete per-CPU descriptor and guarded-stack state, complete exception containment, memory management, capabilities, ring 3, N5/N6/N7 exit, or production readiness. The applicable Model 40h-4Fh errata source and a direct numeric client microcode floor or ratified replacement remain stop-ship gaps. The selected FIDO2 key remains physically unavailable despite authorization for acquisition and later key/signing operations. The immediate external move is `N0-HW-KEY-ACQUIRE-001`; the next owner-independent engineering move is `N9-PMM-001`.
