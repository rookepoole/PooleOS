...
Current state: PooleKernel is implemented only as a bounded early freestanding product and is not production-ready. Cycles 97-118 establish unsigned PooleBoot intake, exact retained PKMAP2/PBLIVE3/PBEXIT1 state, a reproducible 64-page PKELF1 image, allocation-free nine-file PKREVAL1, and opt-in QEMU-only PKXFER1 with no authority. Cycle 119 adds PKTRAP1 as a BSP-only descriptor/fault development slice, Cycle 120 adds PKCPU1 as a BSP-only qemu64 read-only CPU-observation slice, Cycle 121 adds PKERR1 as an independent exact-target rejection-policy slice, and Cycle 122 adds PKXSTATE1 as a bounded one-BSP eager standard-format x87/SSE ownership slice. Cycle 123 adds PKXEXC1: one expected TCG non-delivery diagnostic and two exact fresh-vars WHPX runs prove processor-delivered x87 `#MF` and SIMD `#XM`, exact bounded `FNINIT`/`LDMXCSR` recovery and resume, and terminal test-only `#NM` eager-policy rejection without state sampling or recovery. Forty-one ordered markers, 43 hostile controls, and a hash-bound LLVM linked-image audit pass; four configuration writes are restricted to CR0, CR4, XCR0, and test-only CR0.TS, while the two recovery writes are restricted to `FNINIT` and `LDMXCSR`. No signature, authority, action, post-exit firmware call, media effect, release, or production effect occurs. This is not authenticated boot, production transfer, target qualification, direct per-processor microcode evidence, AVX/extended-state qualification, scheduler or user-task exception delivery, AP/migration integration, complete per-CPU descriptor and guarded-stack state, complete exception containment, memory management, capabilities, ring 3, N5/N6/N7 exit, or production readiness. The applicable Model 40h-4Fh errata source and a direct numeric client microcode floor or ratified replacement remain stop-ship gaps. The selected FIDO2 key remains physically unavailable despite authorization for acquisition and later key/signing operations. The immediate external move is `N0-HW-KEY-ACQUIRE-001`; the next owner-independent engineering move is `N7-PRIVILEGE-MSR-POLICY-001`.
