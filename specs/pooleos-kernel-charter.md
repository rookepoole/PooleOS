...
Current state: PooleKernel is implemented only as a bounded early freestanding product and is not production-ready. Cycles 97-118 establish unsigned PooleBoot intake, exact retained PKMAP2/PBLIVE3/PBEXIT1 state, a reproducible 64-page PKELF1 image, allocation-free nine-file PKREVAL1, and opt-in QEMU-only PKXFER1 with no authority. Cycle 119 adds PKTRAP1 as a BSP-only development slice: one GDT/TSS/IDT setup, five present exception gates, distinct bounded but unguarded IST arrays, a uniform integer frame, three exact returning synchronous faults, terminal double-fault containment, and semantic malformed-frame rejection. Six exact QEMU/OVMF runs and 51 hostile controls pass with zero signatures, authority, actions, writes, enabled interrupts, or post-exit firmware calls. This is not authenticated boot, a production transfer profile, per-CPU or complete exception containment, memory management, capabilities, ring 3, target-firmware evidence, physical-hardware evidence, N5/N6/N7 exit, or production readiness. The selected FIDO2 key remains physically unavailable despite authorization for acquisition and later key/signing operations. The immediate external move is `N0-HW-KEY-ACQUIRE-001`; the next owner-independent engineering move is `N7-CPU-POLICY-001`.
