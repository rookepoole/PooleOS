"""Independent PKSMP3 capability-gated IPI transport and live-marker oracle."""
...
COMPLETION_MARKER = b"POOLEOS:KERNEL:SMP-IPI-RESULT PASS contract=PKSMP3"

PAGE_BYTES = 4096
RESOURCE_PAGE_COUNT = 32
TRAMPOLINE_BYTES = 1964
...
EXTENSION_MAGIC = 0x504B_534D_5033_4950
EXTENSION_VERSION = 1
EXTENSION_BASE_OFFSET = 352
EXTENSION_BYTES = 200
MAILBOX_BYTES = 552
CAPABILITY_HIGH = 0x504F_4F4C_454F_5349
CAPABILITY_LOW = 0x504B_534D_5033_0001
...
OPERATIONS = {
    ...
    2: {"name": "shootdown_transport", "vector": 225, "payload": 0x1, "result": 0x5348_4F4F_5400_0001},
...
NEGATIVE_CONTROL_IDS = (
    "NEG-N8-PKSMP3-MARKER-OMISSION",
    "NEG-N8-PKSMP3-MARKER-ORDER",
    "NEG-N8-PKSMP3-MARKER-DUPLICATE",
    "NEG-N8-PKSMP3-TRANSFER-PREFIX-SELECTOR-MATRIX",
    "NEG-N8-PKSMP3-TOPOLOGY-FIELD-MATRIX",
    "NEG-N8-PKSMP3-RESOURCE-GEOMETRY-FIELD-MATRIX",
    "NEG-N8-PKSMP3-ONLINE-FIELD-MATRIX",
    "NEG-N8-PKSMP3-ACCEPTED-FIELD-MATRIX",
    "NEG-N8-PKSMP3-DENIAL-FIELD-MATRIX",
    "NEG-N8-PKSMP3-TIMEOUT-FIELD-MATRIX",
    "NEG-N8-PKSMP3-STOP-CHECKSUM-FIELD-MATRIX",
    "NEG-N8-PKSMP3-RELEASE-FIELD-MATRIX",
    "NEG-N8-PKSMP3-CLAIM-BOUNDARY-FIELD-MATRIX",
    "NEG-N8-PKSMP3-SOURCE-AUDIT",
    "NEG-N8-PKSMP3-RESOURCE-LAYOUT-MODEL",
    "NEG-N8-PKSMP3-REQUEST-CAPABILITY-MODEL",
    "NEG-N8-PKSMP3-REQUEST-VECTOR-MODEL",
    "NEG-N8-PKSMP3-REQUEST-SEQUENCE-MODEL",
)

EARLY = re.compile(r"^POOLEOS:KERNEL:SMP-IPI-EARLY PASS contract=(?P<contract>PKSMP3) selector=(?P<selector>[0-9]+) bsp=(?P<bsp>[0-9]+) if=(?P<iflag>[0-9]+) stack=validated_by_wrapper serial=initialized$")
TOPOLOGY = re.compile(r"^POOLEOS:KERNEL:SMP-IPI-TOPOLOGY PASS contract=(?P<contract>PKSMP3) processors=(?P<processors>[0-9]+) enabled=(?P<enabled>[0-9]+) bsp_apic_id=(?P<bsp>[0-9]+) target_apic_id=(?P<target>[0-9]+) apic_physical=0x(?P<apic>[0-9A-F]{16}) selection=(?P<selection>[a-z_]+)$")
RESOURCES = re.compile(r"^POOLEOS:KERNEL:SMP-IPI-RESOURCES PASS contract=(?P<contract>PKSMP3) physical_start=0x(?P<start>[0-9A-F]{16}) pages=(?P<pages>[0-9]+) sipi_vector=(?P<vector>[0-9]+) trampoline_bytes=(?P<trampoline>[0-9]+) allocation_sequence=(?P<sequence>[0-9]+) tables=(?P<tables>[0-9]+) mapped_pages=(?P<mapped>[0-9]+) guard_pages=(?P<guards>[0-9]+) apic_pt_offset=(?P<apic_pt>[0-9]+) apic_leaf=(?P<leaf>[a-z_]+) below_1mib=(?P<below>[0-9]+)$")
ONLINE = re.compile(r"^POOLEOS:KERNEL:SMP-IPI-ONLINE PASS contract=(?P<contract>PKSMP3) service_state=(?P<state>[0-9]+) if=(?P<iflag>[0-9]+) vectors=(?P<vectors>[0-9,]+) apic_mmio=0x(?P<apic>[0-9A-F]{16}) apic_table=(?P<table>[0-9]+)$")
ACCEPTED = re.compile(r"^POOLEOS:KERNEL:SMP-IPI-ACCEPTED PASS contract=(?P<contract>PKSMP3) operations=(?P<operations>[a-z_,]+) sequences=(?P<sequences>[0-9,]+) accepted=(?P<accepted>[0-9]+) reschedule=(?P<reschedule>[0-9]+) shootdown=(?P<shootdown>[0-9]+) call_function=(?P<call>[0-9]+) diagnostic=(?P<diagnostic>[0-9]+) panic=(?P<panic>[0-9]+) stop=(?P<stop>[0-9]+)$")
CONTROLS = re.compile(r"^POOLEOS:KERNEL:SMP-IPI-CONTROLS PASS contract=(?P<contract>PKSMP3) invalid_capability=(?P<capability>[0-9]+) vector_mismatch=(?P<vector>[0-9]+) stale_sequence=(?P<stale>[0-9]+) duplicate_sequence=(?P<duplicate>[0-9]+) denied=(?P<denied>[0-9]+) delivery_count=(?P<delivery>[0-9]+) eoi_count=(?P<eoi>[0-9]+) spurious=(?P<spurious>[0-9]+) apic_error=(?P<apic_error>[0-9]+)$")
TIMEOUT = re.compile(r"^POOLEOS:KERNEL:SMP-IPI-TIMEOUT PASS contract=(?P<contract>PKSMP3) target_apic_id=(?P<target>[0-9]+) attempt=(?P<attempt>[0-9]+) bounded=(?P<bounded>[0-9]+) offline_cpu=(?P<offline>[0-9]+) timeout_count=(?P<count>[0-9]+)$")
STOP = re.compile(r"^POOLEOS:KERNEL:SMP-IPI-STOP PASS contract=(?P<contract>PKSMP3) ack_attempt=(?P<attempt>[0-9]+) ack_sequence=(?P<sequence>[0-9]+) last_accepted_sequence=(?P<last>[0-9]+) service_state=(?P<service>[0-9]+) mailbox_state=(?P<mailbox>[0-9]+) runtime_state=(?P<runtime>[0-9]+) panic_latched=(?P<panic>[0-9]+) response_checksum=0x(?P<response>[0-9A-F]{16}) baseline_checksum=0x(?P<baseline>[0-9A-F]{16}) runtime_checksum=0x(?P<runtime_checksum>[0-9A-F]{16}) init_asserts=(?P<asserts>[0-9]+) init_deasserts=(?P<deasserts>[0-9]+) sipis=(?P<sipis>[0-9]+) tss_busy=(?P<tss>[0-9]+) idt_verified=(?P<idt>[0-9]+) xstate_verified=(?P<xstate>[0-9]+) apic_table_verified=(?P<apic_table>[0-9]+) final_init=(?P<final_init>[0-9]+) parked=(?P<parked>[0-9]+)$")
RELEASE = re.compile(r"^POOLEOS:KERNEL:SMP-IPI-RELEASE PASS contract=(?P<contract>PKSMP3) release_sequence=(?P<sequence>[0-9]+) zeroed_bytes=(?P<zeroed>[0-9]+) verified_bytes=(?P<verified>[0-9]+) resources_released=(?P<released>[0-9]+) capability_revoked=(?P<capability>[0-9]+) runtime_revoked=(?P<runtime>[0-9]+) mmio_revoked=(?P<mmio>[0-9]+) pic_restored=(?P<pic>[0-9]+) hpet_restored=(?P<hpet>[0-9]+) apic_base_restored=(?P<apic>[a-z_]+)$")
RESULT = re.compile(r"^POOLEOS:KERNEL:SMP-IPI-RESULT PASS contract=(?P<contract>PKSMP3) profile=(?P<profile>sandybridge_x87_sse_two_vcpu) capability_gate=(?P<gate>[a-z_]+) operation_classes=(?P<classes>[0-9]+) valid_deliveries=(?P<valid>[0-9]+) denied_deliveries=(?P<denied>[0-9]+) offline_timeouts=(?P<timeouts>[0-9]+) eois=(?P<eois>[0-9]+) panic_latched=(?P<panic>[0-9]+) stop_quiesced=(?P<quiesced>[0-9]+) ap_parked=(?P<parked>[0-9]+) resources_released=(?P<released>[0-9]+) rollback=(?P<rollback>[a-z_]+) shootdown_transport_only=(?P<shootdown>[0-9]+) tlb_invalidations=(?P<tlb>[0-9]+) call_allowlist_noop=(?P<call>[0-9]+) arbitrary_callback=(?P<callback>[0-9]+) scheduler=(?P<scheduler>[0-9]+) target=(?P<target>[0-9]+) signatures=(?P<signatures>[0-9]+) authority=(?P<authority>[0-9]+) actions=(?P<actions>[0-9]+) production=(?P<production>[0-9]+) terminal=(?P<terminal>[a-z_]+)$")


class KernelSmpIpiError(RuntimeError):
    """Raised when PKSMP3 data or evidence violates the frozen contract."""
...
def _match(pattern: re.Pattern[str], marker: str, label: str) -> re.Match[str]:
    ...
    _require(match is not None, f"PKSMP3 {label} marker violates its contract")
...
def resource_layout(start_page: int, page_count: int) -> dict[str, Any]:
    _require(start_page > 0 and page_count == RESOURCE_PAGE_COUNT, "PKSMP3 resource geometry changed")
    start = start_page * PAGE_BYTES
    end = start + page_count * PAGE_BYTES
    _require(end <= 0x10_0000 and start_page <= 0xFF, "PKSMP3 resources escaped SIPI memory")
    roles = {
        "trampoline": [0],
        "tables": [1, 2, 3, 4],
        "rsp0": [6, 7, 8, 9],
    ...
    _require(flattened == list(range(RESOURCE_PAGE_COUNT)), "PKSMP3 resource roles are incomplete")
...
def canonical_request(attempt: int, sequence: int, operation: int, target_apic_id: int) -> dict[str, int]:
    _require(operation in OPERATIONS, "PKSMP3 operation is not allowlisted")
    request = {
...
def validate_request(
    ...
    _require(request["status"] == 1, "PKSMP3 request is not armed")
    _require((request["capability_high"], request["capability_low"]) == (CAPABILITY_HIGH, CAPABILITY_LOW), "PKSMP3 capability mismatch")
    _require(request["attempt"] == prior_attempt + 1, "PKSMP3 attempt replay control failed")
    _require(request["operation"] == handler_operation and handler_operation in OPERATIONS, "PKSMP3 operation mismatch")
    _require(request["vector"] == OPERATIONS[handler_operation]["vector"], "PKSMP3 vector mismatch")
    _require(request["target_apic_id"] == target_apic_id, "PKSMP3 target mismatch")
    _require(request["checksum"] == request_checksum(request), "PKSMP3 request checksum mismatch")
    _require(request["sequence"] == last_sequence + 1, "PKSMP3 stale or duplicate sequence")
    _require(request["payload"] == OPERATIONS[handler_operation]["payload"], "PKSMP3 payload mismatch")
...
def validate_markers(markers: list[str]) -> dict[str, Any]:
    _require(len(markers) == MARKER_COUNT, f"expected {MARKER_COUNT} PKSMP3 markers")
    arm = native_kernel_transfer.TRANSFER_ARM.fullmatch(markers[23])
    _require(arm is not None and int(arm.group(10), 10) == SELECTOR, "PKSMP3 transfer selector changed")
    ...
    stop = _match(STOP, markers[36], "stop")
    release = _match(RELEASE, markers[37], "release")
    result = _match(RESULT, markers[38], "result")

    _require(tuple(_dec(early, name) for name in ("selector", "bsp", "iflag")) == (14, 1, 0), "PKSMP3 early state changed")
    _require(tuple(_dec(topology, name) for name in ("processors", "enabled", "bsp", "target")) == (2, 2, 0, 1), "PKSMP3 topology changed")
    _require((_hex(topology, "apic"), topology.group("selection")) == (APIC_PHYSICAL_ADDRESS, "lowest_enabled_non_bsp"), "PKSMP3 topology policy changed")

    layout = resource_layout(_hex(resources, "start") // PAGE_BYTES, _dec(resources, "pages"))
    _require(_hex(resources, "start") == layout["start"] == 0x1000, "PKSMP3 low-memory start changed")
    _require(tuple(_dec(resources, name) for name in ("vector", "trampoline", "sequence", "tables", "mapped", "guards", "apic_pt", "below")) == (1, TRAMPOLINE_BYTES, 2, 5, 13, 14, 31, 1), "PKSMP3 resource receipt changed")
    _require(resources.group("leaf") == "pwt_pcd_rw_nx", "PKSMP3 APIC leaf policy changed")
    _require((APIC_PHYSICAL_ADDRESS >> 30) & 0x1FF == APIC_PDPT_INDEX and (APIC_PHYSICAL_ADDRESS >> 21) & 0x1FF == APIC_PAGE_DIRECTORY_INDEX, "PKSMP3 APIC hierarchy model changed")

    _require((_dec(online, "state"), _dec(online, "iflag"), online.group("vectors"), _hex(online, "apic"), _dec(online, "table")) == (2, 1, "224,225,226,227,228,229", APIC_PHYSICAL_ADDRESS, 1), "PKSMP3 online IPI service changed")
    expected_operations = ",".join(str(OPERATIONS[index]["name"]) for index in sorted(OPERATIONS))
    _require(accepted.group("operations") == expected_operations and accepted.group("sequences") == "1,2,3,4,5,6", "PKSMP3 operation order changed")
    _require(tuple(_dec(accepted, name) for name in ("accepted", "reschedule", "shootdown", "call", "diagnostic", "panic", "stop")) == (6, 1, 1, 1, 1, 1, 1), "PKSMP3 accepted-operation receipt changed")
    _require(tuple(_dec(controls, name) for name in ("capability", "vector", "stale", "duplicate", "denied", "delivery", "eoi", "spurious", "apic_error")) == (1, 1, 1, 1, 4, 10, 10, 0, 0), "PKSMP3 denial or EOI accounting changed")
    _require(tuple(_dec(timeout, name) for name in ("target", "attempt", "bounded", "offline", "count")) == (2, 10, 1, 1, 1), "PKSMP3 offline-target timeout changed")
    ...
    _require(tuple(stop_values.values()) == (10, 6, 6, 4, 3, 5, 1, 1, 1, 2, 1, 1, 1, 1, 1, 1), "PKSMP3 stop lifecycle changed")
    ...
    _require(_hex(stop, "response") == expected_response, "PKSMP3 response checksum changed")
    baseline_checksum = _hex(stop, "baseline")
    runtime_checksum = _hex(stop, "runtime_checksum")
    _require(baseline_checksum != 0 and runtime_checksum != 0 and baseline_checksum != runtime_checksum, "PKSMP3 inherited runtime checksums are invalid")

    _require(tuple(_dec(release, name) for name in ("sequence", "zeroed", "verified", "released", "capability", "runtime", "mmio", "pic", "hpet")) == (3, 131072, 131072, 32, 1, 1, 1, 1, 1), "PKSMP3 release receipt changed")
    _require(release.group("apic") == "unchanged", "PKSMP3 APIC base changed")
    _require((result.group("gate"), result.group("rollback"), result.group("terminal")) == ("development_fixed_token", "host_verified", "halt"), "PKSMP3 bounded profile changed")
    _require(tuple(_dec(result, name) for name in ("classes", "valid", "denied", "timeouts", "eois", "panic", "quiesced", "parked", "released", "shootdown", "tlb", "call", "callback", "scheduler", "target", "signatures", "authority", "actions", "production")) == (6, 6, 4, 1, 10, 1, 1, 1, 32, 1, 0, 1, 0, 0, 0, 0, 0, 0, 0), "PKSMP3 claim boundary changed")
    ...
    return {
        ...
        "operations": {"accepted": 6, "denied": 4, "deliveries": 10, "eois": 10, "timeout_count": 1},
        ...
        "result": {"shootdown_transport_only": 1, "tlb_invalidations": 0, "call_allowlist_noop": 1, "arbitrary_callback": 0, "scheduler": 0, "production": 0},
    }
...
def normalize_dynamic_markers(markers: list[str]) -> list[str]:
    ...
    for field in ("baseline_checksum", "runtime_checksum"):
        normalized[36] = re.sub(rf"{field}=0x[0-9A-F]{{16}}", f"{field}=<validated-dynamic>", normalized[36], count=1)
    return normalized
