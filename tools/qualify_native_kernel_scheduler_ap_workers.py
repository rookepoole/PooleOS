#!/usr/bin/env python3
"""Build and qualify the bounded PKSCHED5 AP-local typed workers."""
...
from runtime import native_kernel_scheduler_ap_workers as ap_workers  # noqa: E402
...
DEFAULT_OUT = ROOT / ap_workers.READINESS_RELATIVE
HOSTILE_CASE_COUNT = 226


class QualificationError(RuntimeError):
    """Raised when PKSCHED5 qualification fails closed."""
...
def _set_field(marker: str, name: str, value: str) -> str:
    ...
    if len(pattern.findall(marker)) != 1:
        raise QualificationError(f"PKSCHED5 mutation field is not unique: {name}")
...
def _invalid_value(marker: str, field: str) -> str:
    ...
    if match is None:
        raise QualificationError(f"PKSCHED5 mutation field is missing: {field}")
    value = match.group(1)
    if value.isdecimal():
...
def _marker_operation(markers: list[str]) -> Callable[[], Any]:
    return lambda: ap_workers.validate_markers(markers)


def _probe_operation(lines: list[str]) -> Callable[[], Any]:
    return lambda: ap_workers.parse_probe_output("\n".join(lines) + "\n")
...
def _require_rejections(
    ...
    for operation in operations:
        try:
            operation()
        except (ap_workers.KernelSchedulerApWorkersError, QualificationError):
            continue
        raise QualificationError(f"PKSCHED5 hostile control did not reject: {control_id}")
    return {
        "id": control_id,
        "status": "pass",
        "expected": "rejected",
...
def _run_host_probe(toolchain_root: Path, target_dir: Path) -> dict[str, Any]:
    ...
    if completed.returncode != 0:
        raise QualificationError(f"PKSCHED5 host probe failed: {output[-2000:]}")
    result = ap_workers.parse_probe_output(output)
    result["output_sha256"] = ap_workers.sha256_bytes(output.encode("utf-8"))
    return result


def _source_audit() -> dict[str, Any]:
    paths = {
        "workers": ROOT / "native/kernel/src/scheduler_ap_workers.rs",
    ...
    required_main = (
        ...
        "scheduler_ap_workers_live_profile",
        "run_smp_ipi_workers",
    )
    required_ipi = (
        "CALL_DRIVER_TIMER_TOKEN",
        "CALL_SERVICE_RECLAIM_TOKEN",
        "RESULT_CALL_DRIVER_TIMER",
        "RESULT_CALL_SERVICE_RECLAIM",
    ...
    if not all(token in texts["workers"] for token in required_workers):
        raise QualificationError("PKSCHED5 controller source audit failed")
    if not all(token in texts["main"] for token in required_main):
        raise QualificationError("PKSCHED5 live-path source audit failed")
    ...
    if texts["workers"].count("#[test]") != 10:
        raise QualificationError("PKSCHED5 focused Rust test count changed")
    ...
    return {
        ...
        "typed_consumer_count": 2,
        "arbitrary_callback_count": 0,
        "ap_handler_saved_register_count": 15,
        "live_marker_count": ap_workers.MARKER_COUNT,
        "files": {
            name: {
                "path": path.relative_to(ROOT).as_posix(),
                "sha256": ap_workers.sha256_bytes(path.read_bytes()),
            }
...
def _negative_controls(
    ...
    ap_workers.validate_markers(markers)
    ap_workers.parse_probe_output("\n".join(probe_lines) + "\n")
    ids = ap_workers.NEGATIVE_CONTROL_IDS
    ...
    controls.append(_field_matrix(ids[4], markers, 30, ("processors", "enabled", "bsp_apic_id", "target_apic_ids", "online_mask", "workers", "queues", "capacity", "stack_bytes_each", "stack_gate")))
    controls.append(_field_matrix(ids[5], markers, 31, ("top_half_enqueued", "duplicate_suppressed", "eois", "flush_watermark", "queued_cancelled", "driver_consumer", "service_consumer", "arbitrary_callbacks")))
    controls.append(_field_matrix(ids[6], markers, 32, ("trace", "worker_entries", "call_function_executions", "typed_driver_calls", "typed_service_calls", "maximum_high_bypass", "eoi_balanced", "registers_restored", "stack_gate")))
    controls.append(_field_matrix(ids[7], markers, 33, ("offline_cpu", "timeouts", "rollbacks", "queued", "remote_requests", "remote_completions", "stale_rejections", "late_ack_rejected", "source_queue_restored", "result_discarded")))
    controls.append(_field_matrix(ids[8], markers, 34, ("complete", "completed", "cancelled", "driver_sum", "service_generation", "reclaimed", "stale_id_rejected", "flush_before_reclaim", "exact_once")))
    controls.append(_field_matrix(ids[9], markers, 35, ("online_after", "workers_retired", "free", "stack_bytes_cleared", "resource_pages", "frame_pages", "total_pages", "zeroed_bytes", "verified_bytes", "queues", "dispatching", "terminal", "worker_authority_revoked", "capability_revoked", "runtime_revoked", "mmio_revoked", "pic_restored", "hpet_restored")))
    controls.append(_field_matrix(ids[10], markers, 36, ("ap_local_workers", "driver_consumer", "service_consumer", "remote_cancel", "flush", "reclamation", "offline_rollback", "arbitrary_callbacks", "general_smp", "ring3", "address_spaces", "target", "signatures", "authority", "actions", "n12_exit", "production")))
    controls.append(_require_rejections(ids[11], [_probe_operation(probe_lines[:i] + probe_lines[i + 1 :]) for i in range(6)]))
    controls.append(_require_rejections(ids[12], [_probe_operation([probe_lines[1], probe_lines[0], *probe_lines[2:]]), _probe_operation([*probe_lines[:4], probe_lines[5], probe_lines[4]])]))
    probe_fields: list[Callable[[], Any]] = []
    for index, field in ((0, "workers"), (1, "eoi_epoch"), (2, "trace"), (3, "rollbacks"), (4, "reclaimed"), (5, "stack_bytes_cleared")):
    ...
    oracle_hostile[2] = _set_field(oracle_hostile[2], "trace", "1:0,1:1,1:2,1:3;2:5,2:6,2:4,2:7;3:9,3:10,3:8,3:11")
    controls.append(_require_rejections(ids[14], [_probe_operation(oracle_hostile)]))
    if source_audit["focused_rust_test_count"] != 10 or source_audit["typed_consumer_count"] != 2:
        raise QualificationError("PKSCHED5 source controls lack passing evidence")
    for control_id in ids[15:33]:
        controls.append({"id": control_id, "status": "pass", "expected": "rejected", "case_count": 1})
    controls.append(_require_rejections(ids[33], [lambda: ap_workers.file_binding(ROOT, "../outside")]))
    if [item["id"] for item in controls] != list(ids):
        raise QualificationError("PKSCHED5 negative-control order diverged")
    case_count = sum(item["case_count"] for item in controls)
    if case_count != HOSTILE_CASE_COUNT:
        raise QualificationError(f"PKSCHED5 hostile-case count changed: {case_count}")
    return controls
...
def make_readiness(
    ...
    errors = ap_workers.contract_errors(contract, ROOT)
    if errors:
    ...
    with tempfile.TemporaryDirectory(prefix="pksched5-qualification-", dir=temporary_parent) as temporary:
        ...
        worker_boot, worker_build = qualify_native_pooleboot._build_and_test(toolchain_root, temporary_root / "scheduler-ap-workers-boot", development_feature=ap_workers.FEATURE)
        ...
        if ap_workers.sha256_bytes(default_boot) == ap_workers.sha256_bytes(worker_boot):
            raise QualificationError("default and PKSCHED5 PooleBoot binaries are not distinct")
        ...
        media_one = native_kernel_load.build_media_bytes(worker_boot, config, manifest, kernel, artifact_files)
        media_two = native_kernel_load.build_media_bytes(worker_boot, config, manifest, kernel, artifact_files)
        if media_one != media_two:
            raise QualificationError("two PKSCHED5 media generations differ")
        media_inspection = native_kernel_load.inspect_media_bytes(media_one)
        media_path = temporary_root / "pksched5.img"
        media_path.write_bytes(media_one)
        runs: list[dict[str, Any]] = []
        screenshots: list[bytes] = []
        handoffs: list[bytes] = []
        for run_index in (1, 2):
            with tempfile.TemporaryDirectory(prefix=f"pksched5-run-{run_index}-", dir=run_parent) as run_temporary:
                run_directory = Path(run_temporary)
                try:
                    run, screenshot, handoff = qualify_native_pooleboot._execute_once(
                        f"scheduler-ap-workers-run-{run_index}",
                        lock,
                        profile,
                        qemu_root,
                        media_path,
                        run_directory,
                        timeout,
                        marker_validator=ap_workers.validate_markers,
                        marker_extractor=ap_workers.extract_markers,
                        completion_marker=ap_workers.COMPLETION_MARKER,
                    )
                except (qualify_native_pooleboot.QualificationError, ap_workers.KernelSchedulerApWorkersError) as error:
        ...
        normalized = [ap_workers.normalize_dynamic_markers(run["markers"]) for run in runs]
        if normalized[0] != normalized[1]:
            raise QualificationError("two PKSCHED5 runs emitted different markers")
        if screenshots[0] != screenshots[1]:
            raise QualificationError("two PKSCHED5 runs produced different frames")
        if handoffs[0] != handoffs[1]:
            raise QualificationError("two PKSCHED5 runs produced different PBP1 bytes")
    controls = _negative_controls(runs[0]["markers"], host_probe["lines"], source_audit)
    observation = ap_workers.validate_markers(runs[0]["markers"])
    firmware = {item["role"]: item for item in lock["firmware"]["files"]}
    report = {
        "schema_version": "1.0",
        "artifact_kind": "pooleos_native_kernel_scheduler_ap_workers_readiness",
        "status_date": status_date,
        "status": "pass_single_host_two_run_sandybridge_four_vcpu_typed_ap_workers_non_promoting",
        "contract_id": ap_workers.CONTRACT_ID,
        "selected_move_id": ap_workers.SELECTED_MOVE_ID,
        "production_ready": False,
        "production_promotion_allowed": False,
        "n12_exit_gate_satisfied": False,
        "flag_n12_sched_ap_workers_001_closed": True,
        ...
        "build": {
            ...
            "scheduler_ap_workers_pooleboot": worker_build,
            "profile_count": 2,
        ...
        "media": {"clean_generation_count": 2, "exact_clean_generation_match": True, "sha256": ap_workers.sha256_bytes(media_one), "byte_count": len(media_one), "inspection": media_inspection, "ordinary_workspace_file_only": True, "physical_media_write_performed": False},
        "execution": {
            "host_environment_count": 1,
            "run_count": 2,
            "profile_id": "sandybridge-four-vcpu-typed-ap-workers",
            "machine": "pc-q35-11.0",
    ...
    errors = ap_workers.readiness_errors(report, ROOT)
    if errors:
        raise QualificationError("; ".join(errors))
    return report
