#!/usr/bin/env python3
"""Verify a kernel PGVM2 loader transcript and emit loader output."""
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Verify a kernel PGVM2 loader transcript.")
    ...
    if errors:
        for error in errors:
            print(f"FAIL kernel PGVM2 loader transcript {error.path}: {error.message}")
        return 1
    kernel_pgvm2_loader_output.write_output(output, args.out)
    print(args.out)
    return 0 if output["status"] in {"blocked", "pass"} else 1
