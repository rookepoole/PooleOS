#!/usr/bin/env python3
"""Emit the additive PDC-GOLDEN-0.2 boundary and metamorphic corpus."""
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Emit the PDC-GOLDEN-0.2 published corpus.")
    ...
    parser.add_argument("--out", type=Path, default=ROOT / "runs" / "pdc_golden_metamorphic_corpus.json")
    args = parser.parse_args(argv)
    try:
        artifact = pdc_golden_metamorphic.make_golden_metamorphic_corpus(
            workspace=ROOT,
        ...
        print(f"FAIL pdc_golden_metamorphic_corpus: {exc}")
        return 1
    schema = json.loads((ROOT / "specs" / "pdc-golden-metamorphic-corpus.schema.json").read_text(encoding="utf-8"))
    errors = validate_json(artifact, schema)
    if errors:
        for error in errors:
            print(f"FAIL pdc_golden_metamorphic_corpus {error.path}: {error.message}")
        return 1
