#!/usr/bin/env python3
"""Build and qualify the bounded PKPMM1 physical-memory foundation."""
...
class QualificationError(RuntimeError):
    """Raised when live PKPMM1 qualification fails closed."""


def _set_field(marker: str, name: str, value: str) -> str:
    pattern = re.compile(rf"(\b{re.escape(name)}=)([^ ]+)")
    if len(pattern.findall(marker)) != 1:
        raise QualificationError(f"PKPMM1 mutation field is not unique: {name}")
...
def _require_marker_rejection(
    ...
    raise QualificationError(f"PKPMM1 hostile marker control did not reject: {control_id}")
...
def _require_pbp1_rejection(
    ...
    try:
        ...
        return {"id": control_id, "status": "pass", "expected": "rejected"}
    raise QualificationError(f"PKPMM1 hostile PBP1 control did not reject: {control_id}")
...
def _negative_controls(
    ...
    mutations = (
        ...
        (26, 38, "allocations", "3"),
        (27, 38, "frees", "1"),
        (28, 38, "dma_start", "0x0000000001000000"),
        (29, 38, "dma32_start", "0x0000000000001000"),
        (30, 38, "double_free_rejected", "0"),
        (31, 38, "quota_rejected", "0"),
        (32, 38, "unavailable_rejected", "0"),
        (33, 38, "metadata_poison", "0"),
        (34, 38, "coalesces", "0"),
        (35, 39, "managed_pages", str(observation["result"]["managed_pages"] + 1)),
        (36, 39, "allocated_pages", "1"),
        (37, 39, "physical_writes", "1"),
        (38, 39, "mappings", "1"),
        (39, 39, "reclaim", "1"),
        (40, 39, "concurrency", "1"),
        (41, 39, "signatures", "1"),
    ...
    controls.append(_require_pbp1_rejection(physical_memory.NEGATIVE_CONTROL_IDS[45], observation, overlap))
    source_kind = copy.deepcopy(transcript)
    source_kind["memory_entries"][0]["source_type"] = 1
    controls.append(_require_pbp1_rejection(physical_memory.NEGATIVE_CONTROL_IDS[46], observation, source_kind))
    ownership = copy.deepcopy(transcript)
    ownership["core"]["kernel_physical_base"] = ownership["memory_entries"][0]["physical_start"]
    controls.append(_require_pbp1_rejection(physical_memory.NEGATIVE_CONTROL_IDS[47], observation, ownership))
    if [item["id"] for item in controls] != list(physical_memory.NEGATIVE_CONTROL_IDS):
        raise QualificationError("PKPMM1 hostile-control order changed")
    return controls
...
def make_readiness(
    ...
    with tempfile.TemporaryDirectory(prefix="pkpmm1-qualification-", dir=temporary_parent) as temporary:
        ...
        if physical_memory.sha256_bytes(default_boot) == physical_memory.sha256_bytes(pmm_boot):
            raise QualificationError("default and PKPMM1 PooleBoot profiles are not distinct")
        ...
        if media_one != media_two:
            raise QualificationError("two PKPMM1 media generations differ")
        ...
        if media_inspection["files"][3]["sha256"] != kernel_readiness["product"]["canonical_sha256"]:
            raise QualificationError("PKPMM1 media kernel differs from PKENTRY1")
        media_path = temporary_root / "pkpmm1.img"
        ...
        for run_index in (1, 2):
            with tempfile.TemporaryDirectory(prefix=f"pkpmm1-run-{run_index}-", dir=run_parent) as run_temporary:
        ...
        if runs[0]["markers"] != runs[1]["markers"]:
            raise QualificationError("two PKPMM1 runs emitted different markers")
        if screenshots[0] != screenshots[1]:
            raise QualificationError("two PKPMM1 runs produced different frames")
        if handoffs[0] != handoffs[1]:
            raise QualificationError("two PKPMM1 runs produced different PBP1 bytes")
    ...
    report = {
        ...
        "status": "pass_single_host_two_run_qemu64_bounded_live_foundation_non_promoting",
        ...
        "execution": {
            ...
            "runs": runs,
            "observation": {
                key: observation[key] for key in ("map", "zones", "ownership", "exercise", "result")
            },
        ...
        "summary": {
            ...
            "allocator_operations": observation["exercise"]["allocations"] + observation["exercise"]["frees"],
        ...
        "open_items": [
            "Implement a mapped bootstrap metadata arena and physical-byte zero/poison/scrub policy without aliasing retained ranges.",
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        print(f"PKPMM1 qualification failed: {error}", file=sys.stderr)
        return 1
    print(
        "PKPMM1 qualification passed: "
