...
PHASE_EVIDENCE = {
    ...
    "N6": [
        ...
        "runs/native_kernel_entry_readiness.json: 74/74 host tests, 2/2 exact clean linked and canonical builds, 43/43 hostile controls, exact independent Rust/Python loaded bytes, and a current bound canonical SHA-256",
    ...
    "N9": [
        "specs/native-kernel-physical-memory-contract.json, specs/native-kernel-physical-memory-contract.schema.json, and docs/native-kernel-physical-memory.md: PKPMM3 freezes exact PBP1 intake, UEFI source-kind validation, usable-only initial ownership, held reclaim classes, page-zero exclusion, DMA/DMA32/Normal zones, generation-safe scrubbed transactions, and a five-page guarded generation-owned metadata arena with validated handoff, integrity seal, rollback, and release exclusion without reclaim, scalable growth, concurrency, or production claims",
        "native/kernel/src/physical_memory.rs, native/kernel/src/main.rs, runtime/native_kernel_physical_memory.py, tools/qualify_native_kernel_physical_memory.py, and tests/test_native_kernel_physical_memory.py: allocation-free no_std kernel allocator, complete-manager bootstrap migration, persistent supervisor RW/NX mappings with absent guards, corruption and mapping-failure rollback, full-page zero/readback, immutable receipts, independent transcript/oracle validation, source audit, and PBP1 substitution controls",
        "runs/native-kernel-physical-memory-readiness.json: 2/2 exact qemu64 QEMU/OVMF runs, 41 ordered markers, 87/87 hostile controls, 74/74 kernel host tests, 97 PBP1 memory entries, 117,919 usable source pages, 117,918 managed pages after page-zero and five-page metadata reservation, 11,250 boot-reclaimable pages held, 825 retained loader pages protected, 13 scrubbed and verified pages, 53,248 scrubbed and verified bytes, four lifecycle receipts, one metadata receipt, 7,680 physical word writes, 8,704 physical word reads, 43 temporary PTE writes and invalidations, five retained metadata mappings plus two absent guards, exact handoff/final integrity, release rejection, and zero reclaim, signatures, authority, or actions",
...
PHASE_GAPS = {
    ...
    "N9": ["PKPMM3 consumes the live PBP1 map and provides bounded generation-safe physical ownership, full-page scrub-before-allocation and scrub-before-reuse, and a retained guarded five-page metadata arena with complete-manager handoff, integrity seal, corruption detection, rollback, and release exclusion. PKVM1 supplies inactive page-table transactions. PKVM2 adds a kernel-complete one-BSP candidate root, a bounded nine-page direct map, exact CR3 activation/restoration, architectural Accessed/Dirty handling, and three active local invalidation receipts. Held-class reclaim, scalable metadata growth, a complete generation-owned direct map, SMP TLB shootdown and deferred reclaim, huge pages, PCID, COW, user faults, pager IPC, heaps/object caches, MMIO/PAT/MTRR qualification, concurrent allocation, pressure/OOM policy, target hardware, and the N9 exit gate remain open"],
...
PROGRAM_GAPS = [
    ...
    "A real reproducible 70-page PooleKernel image, PKENTRY1 intake, allocation-free PKREVAL1 verifier, bounded early diagnostics, opt-in QEMU-only live entry, BSP-only PKTRAP1 descriptor/exception containment, bounded BSP PKXSTATE1 x87/SSE ownership, PKPMM3 scrubbed lifecycle and guarded metadata-arena transactions, and PKVM2 active-root evidence exist, but authenticated boot trust, measured boot, production transfer, complete per-CPU runtime state, retained crash evidence, target execution, and N6/N7/N9 exit remain open",
    "PKERR1 freezes a pure exact-target CPU/errata rejection policy, PKXSTATE1 proves bounded x87/SSE standard-XSAVE ownership, PKXEXC1 proves deliberate #MF/#XM recovery plus terminal test-only #NM rejection with a linked scope audit under WHPX, and PKMSR1 proves only a read-only qemu64 BSP system-linkage/global-MCA/unsupported-PMU observation. PKPMM3 supplies bounded physical ownership, scrubbed lifecycle transactions, and a guarded five-page metadata arena with validated handoff, integrity seal, rollback, and release exclusion; PKVM1 supplies inactive page-table transactions; and PKVM2 proves one-BSP candidate-root activation/restoration with a nine-page direct map and three local invalidation receipts. No target-qualified complete native CPU policy, applicable Model 40h-4Fh errata authority, direct numeric client microcode floor or ratified replacement, target-specific privileged-MSR semantics, syscall/per-CPU/MCE/PMU activation, AVX/extended state, user-task exception delivery, scheduler/AP/migration integration, interrupts, time, SMP, held-class reclaim, scalable allocator metadata, complete kernel/user address spaces, SMP TLB shootdown, heap, MMIO/cache-alias qualification, concurrent allocator, pressure, or OOM implementation exists",
    ...
    "PKPMM3 supplies bounded one-BSP physical-page ownership, scrub-before-allocation and scrub-before-reuse with full readback and fault rollback, and a retained guarded five-page metadata arena with validated handoff, integrity seal, corruption detection, rollback, and release exclusion. PKVM1 supplies inactive four-level 4 KiB transactions, and PKVM2 supplies one bounded active root, nine-page direct map, exact CR3 restoration, and three local invalidation receipts. Held-class reclaim, scalable metadata growth, a complete generation-owned direct map, SMP TLB shootdown and deferred reclaim, huge pages, PCID, COW, user faults, pager IPC, cache/MMIO qualification, heaps/object caches, concurrency/SMP, pressure and OOM policy, scheduler, task, syscall, capability, IPC, isolation, and asynchronous I/O remain open",
