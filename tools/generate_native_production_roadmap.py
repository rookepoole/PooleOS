...
PHASE_EVIDENCE = {
    ...
    "N5": [
        ...
        "runs/native-kernel-revalidation-readiness.json: 173/173 kernel Rust tests, 8/8 Python tests, both no_std target builds, nine exact retained files and parsers, 36/36 hostile controls, 32,768/32,768 deterministic mutation rejects, exact unsigned-policy denial, zero authority grants/actions/state writes, and no standalone live-entry claim",
    ...
    "N6": [
        ...
        "runs/native_kernel_entry_readiness.json: 173/173 host tests, 2/2 exact clean linked and canonical builds, 43/43 hostile controls, 1,181 relative relocations, exact independent Rust/Python loaded bytes, and canonical SHA-256 9C23236E85A6D2C7AEEFDA12F3CEC202DC3BF34B89D9CEAEEBB7037A079DA168",
    ...
    "N7": [
        ...
        "runs/native-kernel-cpu-policy-readiness.json: 173/173 kernel host tests, 2 exact fresh-vars qemu64 QEMU/OVMF runs, 35 ordered markers, 41/41 hostile controls, exact Rust/Python agreement, AuthenticAMD family 15 model 107 stepping 1 observation, 5 MSR reads, zero MSR writes, and zero authority or actions",
    ...
    "N8": [
        ...
        "runs/native-kernel-interrupt-time-readiness.json: 173/173 kernel host tests, two exact fresh-vars qemu64 QEMU/OVMF runs, 36 ordered markers, 58/58 hostile controls, eight timer deliveries, eight EOIs, zero APIC-error or spurious deliveries, three absent MMIO guards, exact LAPIC/HPET mapping revocation, zero AP starts, and zero signatures, authority, actions, or production claims",
        ...
        "runs/native-kernel-smp-first-ap-readiness.json: 173/173 kernel host tests, two exact fresh-vars two-vCPU qemu64 QEMU/OVMF runs, 38 ordered markers, 72/72 hostile controls, exactly one AP started/online/quiesced/parked, fourteen pages and 57,344 bytes scrubbed/verified/released, and zero signatures, authority, target, N8-exit, or production claims",
    ...
    "N9": [
        ...
        "runs/native-kernel-virtual-memory-readiness.json: 2/2 exact qemu64 QEMU/OVMF runs, 40 ordered markers, 46/46 hostile controls, 117,830 admitted pages in eleven sparse ranges with 12,935 hole pages, 237 direct leaf tables, one direct directory and 243 total table pages, coverage checksum 0x0F19BDCE2F42383C, 367,417 physical table writes, 950,626 temporary-PTE writes and invalidations, two CR3 writes, three active local invalidation receipts, one exact generation-retirement receipt, zero remote shootdowns, and zero signatures, authority, or actions",
    ...
    "N12": [
        ...
        "runs/native-kernel-scheduler-readiness.json: 14/14 scheduler Rust tests within 173/173 kernel host tests, four exact Rust/Python host-probe receipts over 4,096 steps, 1,761 dispatches and 2,334 migrations, two exact 17-marker qemu64 BSP runs, eight live dispatches, sixteen context transitions, 32,768 stack bytes cleared, and 28/28 hostile-control categories covering 115 rejected cases with zero live AP dispatch, ring-3/address-space switch, authority, N12-exit, or production claims",
        ...
        "runs/native-kernel-scheduler-smp-readiness.json: 8/8 focused SMP scheduler tests within 173/173 kernel host tests, five exact Rust/Python host-probe receipts, two exact 37-marker four-vCPU selector-18 runs, one cross-CPU wake, two migrations, three transaction acknowledgements, six AP dispatches, one APIC-4 timeout rollback, two stale acknowledgement rejections, eight task retirements, and 32/32 hostile-control categories covering 209 rejected cases; 102 pages or 417,792 bytes are scrubbed, verified, and released",
...
def make_roadmap(test_count: int, status_date: str) -> dict:
    ...
    for flag_id, flag_class, phase_id, closure in FLAGS:
        ...
        if flag_id in {"FLAG-N12-SCHED-SMP-001", "FLAG-N12-SCHED-AP-WORKERS-001"}:
            evidence.extend(
                [
...
def main() -> int:
    ...
    parser.add_argument("--test-count", type=int, default=857)
