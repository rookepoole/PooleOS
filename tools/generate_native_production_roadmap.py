...
PHASE_EVIDENCE = {
    ...
    "N6": [
        ...
        "runs/native_kernel_entry_readiness.json: 70/70 host tests, 2/2 exact clean linked and canonical builds, 43/43 hostile controls, exact independent Rust/Python loaded bytes, and a current bound canonical SHA-256",
    ...
    "N9": [
        "specs/native-kernel-physical-memory-contract.json, specs/native-kernel-physical-memory-contract.schema.json, and docs/native-kernel-physical-memory.md: PKPMM2 freezes exact PBP1 intake, UEFI source-kind validation, usable-only initial ownership, held reclaim classes, page-zero exclusion, DMA/DMA32/Normal zones, generation-safe handles, quotas, poisoning, coalescing, and scrub-before-allocation and scrub-before-reuse transactions without reclaim, concurrency, or production claims",
        "native/kernel/src/physical_memory.rs, runtime/native_kernel_physical_memory.py, tools/qualify_native_kernel_physical_memory.py, and tests/test_native_kernel_physical_memory.py: allocation-free no_std kernel allocator, volatile one-page physical-access adapter, full-page zero/readback, immutable scrub receipts, ownership-preserving fault rollback, independent transcript/oracle validation, source audit, and PBP1 substitution controls",
        "runs/native-kernel-physical-memory-readiness.json: 2/2 exact qemu64 QEMU/OVMF runs, 40 ordered markers, 63/63 hostile controls, 70/70 kernel host tests, 97 PBP1 memory entries, 117,923 usable source pages, 117,922 managed pages after page-zero exclusion, 11,250 boot-reclaimable pages held, 821 retained loader pages protected, eight scrubbed and verified pages, 32,768 scrubbed and verified bytes, four receipts, 5,120 physical word writes, 6,144 physical word reads, 28 temporary PTE writes and invalidations, exact alias revocation, and zero reclaim, signatures, authority, or actions",
...
PHASE_GAPS = {
    ...
    "N9": ["PKPMM2 consumes the live PBP1 map and provides bounded generation-safe physical ownership plus full-page scrub-before-allocation and scrub-before-reuse with readback and ownership-preserving fault rollback. PKVM1 supplies inactive page-table transactions. PKVM2 adds a kernel-complete one-BSP candidate root, a bounded nine-page direct map, exact CR3 activation/restoration, architectural Accessed/Dirty handling, and three active local invalidation receipts. A durable mapped allocator-metadata arena, held-class reclaim, a complete generation-owned direct map, SMP TLB shootdown and deferred reclaim, huge pages, PCID, COW, user faults, pager IPC, heaps/object caches, MMIO/PAT/MTRR qualification, concurrent allocation, pressure/OOM policy, target hardware, and the N9 exit gate remain open"],
...
PROGRAM_GAPS = [
    ...
    "A real reproducible 66-page PooleKernel image, PKENTRY1 intake, allocation-free PKREVAL1 verifier, bounded early diagnostics, opt-in QEMU-only live entry, BSP-only PKTRAP1 descriptor/exception containment, bounded BSP PKXSTATE1 x87/SSE ownership, PKPMM2 scrubbed lifecycle transactions, and PKVM2 active-root evidence exist, but authenticated boot trust, measured boot, production transfer, complete per-CPU runtime state, retained crash evidence, target execution, and N6/N7/N9 exit remain open",
    "PKERR1 freezes a pure exact-target CPU/errata rejection policy, PKXSTATE1 proves bounded x87/SSE standard-XSAVE ownership, PKXEXC1 proves deliberate #MF/#XM recovery plus terminal test-only #NM rejection with a linked scope audit under WHPX, and PKMSR1 proves only a read-only qemu64 BSP system-linkage/global-MCA/unsupported-PMU observation. PKPMM2 supplies bounded physical ownership and scrubbed lifecycle transactions, PKVM1 supplies inactive page-table transactions, and PKVM2 proves one-BSP candidate-root activation/restoration with a nine-page direct map and three local invalidation receipts; no target-qualified complete native CPU policy, applicable Model 40h-4Fh errata authority, direct numeric client microcode floor or ratified replacement, target-specific privileged-MSR semantics, syscall/per-CPU/MCE/PMU activation, AVX/extended state, user-task exception delivery, scheduler/AP/migration integration, interrupts, time, SMP, durable mapped allocator-metadata arena, reclaim transition, complete kernel/user address spaces, SMP TLB shootdown, heap, MMIO/cache-alias qualification, concurrent allocator, pressure, or OOM implementation exists",
    ...
    "PKPMM2 supplies bounded one-BSP physical-page ownership plus scrub-before-allocation and scrub-before-reuse with full readback and fault rollback, PKVM1 supplies inactive four-level 4 KiB transactions, and PKVM2 supplies one bounded active root, nine-page direct map, exact CR3 restoration, and three local invalidation receipts; a durable mapped allocator-metadata arena, reclaim transitions, a complete generation-owned direct map, SMP TLB shootdown and deferred reclaim, huge pages, PCID, COW, user faults, pager IPC, cache/MMIO qualification, heaps/object caches, concurrency/SMP, pressure and OOM policy, scheduler, task, syscall, capability, IPC, isolation, and asynchronous I/O remain open",
