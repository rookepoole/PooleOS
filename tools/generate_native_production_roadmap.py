...
PHASE_EVIDENCE = {
    ...
    "N5": [
        ...
        "runtime/native_kernel_load.py and tools/qualify_native_kernel_load.py: deterministic 64 MiB protective-MBR/GPT/FAT32 ordinary-file media with exact fallback EFI, PBC1 config, PSM1 system manifest, PKELF1 PooleKernel, six PBART1 artifacts, exact retained-page inner-set reconstruction, and no physical-media output mode",
        "runs/native_pooleboot_readiness.json: 8/8 host tests, 2/2 exact 192,512-byte PE builds, 2/2 exact ten-file media generations, 2/2 exact QEMU/OVMF runs, twenty-four ordered markers, 2/2 serial/debugcon matches, 2/2 exact GOP frames, and 139/139 integrated hostile controls with exact F3154B354C77D0567207994EFDDA4FE2D203611CA21D60B63872BC9FFC73C675 retained-set binding",
        ...
        "specs/native-kernel-load-contract.json and docs/native-kernel-load.md: PKLOAD6 freezes live UEFI intake, PSM1/PBART1 digest binding, exact PBASET1 loading and retention, six-format retained-page parsing and denial, retained PKMAP2 storage, final PBLIVE3 production, bounded PBEXIT1 retry, successful ExitBootServices, stop-before-transfer, trust, semantics, and nonclaim boundaries",
        "native/bootload, native/inner, native/boot/src/kload.rs, native/boot/src/kmap.rs, native/boot/src/exit.rs, native/bootexit, runtime/native_inner_live.py, and runtime/native_kernel_load.py: dependency-free contracts, reviewed raw UEFI adapters, and independent retained-set/media/map/exit oracle implementation",
        ...
        "runs/native_kernel_load_readiness.json: 81/81 Rust host tests, 2/2 exact PooleBoot builds, 2/2 exact PooleKernel builds, 2/2 exact ten-file media generations, 2/2 QEMU/OVMF runs, 24 ordered markers, 139/139 integrated hostile controls, exact retained PINIT1, PREC1, PSYM1, PMCU1, PFWM1, and PPOL1 parsing with payload/route cross-binding and mandatory development denial, two exact post-exit PBP1 reconstructions, exact 48-page kernel plus six artifact/table/stack/handoff retained-range agreement, successful ExitBootServices, zero later firmware calls, and exact guest/oracle agreement",
    ],
    "N6": [
...
PROGRAM_GAPS = [
    ...
    "A reproducible unsigned PooleBoot proof application boots twice with deterministic ten-file GPT/FAT32 media, exact GOP frames, retained PKMAP2 kernel/six-artifact/table/guarded-stack/handoff storage, independently reconstructed final PBLIVE3 bytes, bounded PBEXIT1 retry, successful ExitBootServices, zero later firmware calls, and permanent stop before transfer. It reparses PINIT1, PREC1, PSYM1, synthetic-only PMCU1, synthetic-only PFWM1, and qualification-only PPOL1 directly from the retained firmware pages; cross-binds PPOL1's five payload digests and eleven PINIT1 routes; requires all six development gates to deny at the missing outer signature; and emits a domain-separated exact-set digest with zero authority, action, state-write, and hardware-observation counts. Artifact authentication, monotonic persistent trust state, independent PooleKernel retained-byte revalidation, capability creation, activation or update application, policy application, recovery execution or symbol consumption, licensed real vendor payload intake and validation, live FMP/ESRT/PLDM inventory, privileged per-processor revision observation, trusted signature selection, initial-system execution, final framebuffer cache policy, kernel-entry profile, retained CR3/RSP installation, kernel consumption, target-firmware and physical-media qualification, and N5 exit remain open",
...
def make_roadmap(test_count: int, status_date: str) -> dict:
    ...
    for flag_id, flag_class, phase_id, closure in FLAGS:
        ...
        if flag_id in {"FLAG-N5-INNER-PARSE-001", "FLAG-N5-INNER-TRUST-STATE-001"}:
            evidence.extend(
                [
                    "native/inner/src/lib.rs",
                    "native/inner/src/bin/pinner1_probe.rs",
        ...
        implementation_flags.append(
            {
                ...
                or flag_id in {"FLAG-N0-RATIFICATION-SCOPE-001", "FLAG-N2-CPUID-001", "FLAG-N4-PROFILE-001", "FLAG-N4-IPC-MODEL-001", "FLAG-N4-SCHEDULER-MODEL-001", "FLAG-N4-POOLEFS-MODEL-001", "FLAG-N5-POOLEBOOT-PROOF-001", "FLAG-N5-BOOTPROTO-001", "FLAG-N5-BOOTCFG-001", "FLAG-N5-ELF-001", "FLAG-N5-KLOAD-001", "FLAG-N5-MANIFEST-001", "FLAG-N5-PBP1-LIVE-001", "FLAG-N5-KMAP-001", "FLAG-N5-HANDOFF-EXIT-001", "FLAG-N5-INIT-SYSTEM-001", "FLAG-N5-INIT-BUNDLE-001", "FLAG-N5-RECOVERY-BUNDLE-001", "FLAG-N5-SYMBOL-BUNDLE-001", "FLAG-N5-MICROCODE-BUNDLE-001", "FLAG-N5-FIRMWARE-BUNDLE-001", "FLAG-N5-POLICY-BUNDLE-001", "FLAG-N5-INIT-SEMANTICS-001", "FLAG-N5-INNER-PARSE-001", "FLAG-N6-KENTRY-001"}
                else "open",
                "phase_id": phase_id,
...
def main() -> int:
    ...
    parser.add_argument("--test-count", type=int, default=696)
