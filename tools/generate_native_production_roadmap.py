...
PHASE_EVIDENCE = {
    ...
    "N5": [
        ...
        "runs/native_pooleboot_readiness.json: 8/8 host tests, 2/2 exact 113,664-byte PE builds, 2/2 exact ten-file media generations, 2/2 exact QEMU/OVMF runs, twenty-three ordered markers, 2/2 serial/debugcon matches, 2/2 exact GOP frames, and 112/112 hostile controls",
...
PHASE_GAPS = {
    ...
    "N5": ["A reproducible unsigned PooleBoot, PBP1, PBC1, PSM1, PKELF1, PBART1, PKMAP2, PBEXIT1, and real PooleKernel path now loads and retains the exact seven-artifact development profile plus table/guarded-stack/handoff storage, binds final post-exit PBP1 bytes to the successful memory map, calls ExitBootServices, proves zero later firmware calls, and stops before transfer; however, signature-backed trusted selection, persistent rollback enforcement, typed payload semantics, initial-system execution, microcode application, final framebuffer cache policy, menu/rollback policy, transferable kernel-entry PBP1, retained CR3/RSP installation, kernel consumption, second host, target firmware, physical media, and N5 exit remain open"],
...
PROGRAM_GAPS = [
    ...
    "A reproducible unsigned PooleBoot proof application boots twice with deterministic ten-file GPT/FAT32 media, exact GOP frames, retained PKMAP2 kernel/six-artifact/table/guarded-stack/handoff storage, independently reconstructed final PBLIVE3 bytes, bounded PBEXIT1 retry, successful ExitBootServices, zero later firmware calls, and permanent stop before transfer; trusted signature selection, rollback persistence, typed artifact semantics, initial-system execution, microcode application, final framebuffer cache policy, kernel-entry profile, retained CR3/RSP installation, kernel consumption, target-firmware and physical-media qualification, and N5 exit remain open",
...
def main() -> int:
    ...
    parser.add_argument("--test-count", type=int, default=608)
