...
PHASE_EVIDENCE = {
    ...
    "N5": [
        "Cycle 162: docs/checkpoints/cycle162-active-root-retention.md records the 146-page image and PKMAP2 retained-window repair under N5.5/N5.8. Rust14/Python15 map tests and the independent retained-map fingerprint agree. Other N5 changed-image dependencies require replay; earlier receipts below retain historical scope",
    ...
    "N9": [
        "Cycle 162 PKVM3: mandatory active table/data retention, six copied-handle free rejections in each of two exact headless boots, 40 markers and 48 controls, three local invalidation receipts and one exact generation retirement. PMM cleanup preserves ownership on failed insertion, metadata or wrong-manager paths. Current VM/source checks pass; older PMM and SMP evidence below needs changed-image replay",
        "Cycle 161 completes all fourteen downstream profile receipts: 28 final successful headless boots, 658 control groups, 2,118 rejected cases and 219 kernel host tests per qualifier. All 26 selected native checks and the 105-check/708-Doctor/917-test pre-closeout canonical suite pass. Two overlapping scheduler boots are excluded and replaced by a frozen-source rerun. No native Rust or frozen-demo bytes change; N0 custody, N12.3 active-root/execution-stack/CPU-retirement ownership and the broader N36 schema/evidence audit remain open. Exact-final qualification, publication and review checks are required before a main merge; no production promotion is claimed.",
    ...
    return {
        ...
        "baseline": {
            ...
            "native_consistency_release_gate": {
                ...
                "qualification_status": "changed_image_dependency_replay_required",
                "passed_check_count_scope": "cycle162_pre_closeout_runtime_excluded_failed_audit",
                "last_fully_qualified_cycle": 161,
        ...
        "claim_boundaries": [
            "Cycle 162 qualifies mandatory active PKVM3 table/data ownership and owner-authorized PMM cleanup on the exact 146-page D0AA3295 kernel: 228 kernel tests, 16 retention cases, two fresh headless boots, six retained-free rejections per run and 48 marker controls. The selected projection passes 4/27 checks; 23 changed-image dependencies still require replay beginning at N5-SYMBOLS-SEMANTICS-001. Main remains the fully qualified Cycle 161 tree merged by PR74. All earlier cycle claims below are historical. No execution-stack/general CPU-retirement completion, phase/flag closure, target qualification, demo rebase, release or production promotion follows",
            "Cycle 161 completes all fourteen downstream profile receipts: 28 final successful headless boots, 658 control groups, 2,118 rejected cases and 219 kernel host tests per qualifier. All 26 selected native checks and the 105-check/708-Doctor/917-test pre-closeout canonical suite pass. Two overlapping scheduler boots are excluded and replaced by a frozen-source rerun. No native Rust or frozen-demo bytes change; N0 custody, N12.3 active-root/execution-stack/CPU-retirement ownership and the broader N36 schema/evidence audit remain open. Exact-final qualification, publication and review checks are required before a main merge; no production promotion is claimed.",
...
def main() -> int:
    ...
    parser.add_argument("--test-count", type=int, default=918)
    parser.add_argument("--status-date", default="2026-09-07")
