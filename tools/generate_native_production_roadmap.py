...
PHASE_EVIDENCE = {
    ...
    "N5": [
        ...
        "runs/native-kernel-revalidation-readiness.json: 189/189 kernel Rust tests, 8/8 Python tests, both no_std target builds, nine exact retained files and parsers, 36/36 hostile controls, 32,768/32,768 deterministic mutation rejects, exact unsigned-policy denial, zero authority grants/actions/state writes, and no standalone live-entry claim",
    ...
    "N6": [
        ...
        "runs/native_kernel_entry_readiness.json: 189/189 host tests, 2/2 exact clean linked and canonical builds, 43/43 hostile controls, 1,264 relative relocations, exact independent Rust/Python loaded bytes, and canonical SHA-256 FCE5C1F2478651D010A2F2781B80494FD0D9721880D33CE8C66A499B35C8DAB6",
    ...
    "N7": [
        ...
        "runs/native-kernel-cpu-policy-readiness.json: 189/189 kernel host tests, 2 exact fresh-vars qemu64 QEMU/OVMF runs, 35 ordered markers, 41/41 hostile controls, exact Rust/Python agreement, AuthenticAMD family 15 model 107 stepping 1 observation, 5 MSR reads, zero MSR writes, and zero authority or actions",
    ...
    "N8": [
        ...
        "runs/native-kernel-interrupt-time-readiness.json: 189/189 kernel host tests, two exact fresh-vars qemu64 QEMU/OVMF runs, 36 ordered markers, 58/58 hostile controls, eight timer deliveries, eight EOIs, zero APIC-error or spurious deliveries, three absent MMIO guards, exact LAPIC/HPET mapping revocation, zero AP starts, and zero signatures, authority, actions, or production claims",
        ...
        "runs/native-kernel-smp-first-ap-readiness.json: 189/189 kernel host tests, two exact fresh-vars two-vCPU qemu64 QEMU/OVMF runs, 38 ordered markers, 72/72 hostile controls, exactly one AP started/online/quiesced/parked, fourteen pages and 57,344 bytes scrubbed/verified/released, and zero signatures, authority, target, N8-exit, or production claims",
    ...
    "N12": [
        ...
        "runs/native-kernel-scheduler-readiness.json: 14/14 scheduler Rust tests within 189/189 kernel host tests, four exact Rust/Python host-probe receipts over 4,096 steps, 1,761 dispatches and 2,334 migrations, two exact 17-marker qemu64 BSP runs, eight live dispatches, sixteen context transitions, 32,768 stack bytes cleared, and 28/28 hostile-control categories covering 115 rejected cases with zero live AP dispatch, ring-3/address-space switch, authority, N12-exit, or production claims",
        ...
        "runs/native-kernel-scheduler-smp-readiness.json: 8/8 focused SMP scheduler tests within 189/189 kernel host tests, five exact Rust/Python host-probe receipts, two exact 37-marker four-vCPU selector-18 runs, one cross-CPU wake, two migrations, three transaction acknowledgements, six AP dispatches, one APIC-4 timeout rollback, two stale acknowledgement rejections, eight task retirements, and 32/32 hostile-control categories covering 209 rejected cases; 102 pages or 417,792 bytes are scrubbed, verified, and released",
        ...
        "runs/native-kernel-scheduler-ap-workers-readiness.json: 10/10 focused AP-worker tests within 189/189 kernel host tests, six exact Rust/Python host-probe receipts, two exact 37-marker four-vCPU selector-19 runs, thirteen enqueues, twelve typed AP calls, eleven completions, two cancellations, one offline timeout rollback, thirteen reclaimed slots, and 34/34 hostile-control categories covering 226 rejected cases; three workers retire and all 102 pages or 417,792 bytes are scrubbed, verified, and released",
...
def make_roadmap(test_count: int, status_date: str) -> dict:
    ...
    return {
        ...
        "claim_boundaries": [
            ...
            "The Cycle 147 N12-SCHED-SMP-PREEMPT-001 receipt adds selector 20 PKSCHED6 and closes only FLAG-N12-SCHED-SMP-PREEMPT-001 for one exact BSP-0/AP-1,2,3 SandyBridge-minus-AVX development topology. Four allocation-free timer/event/frame/run-queue lanes apply deterministic cancel, wake, and migration ordering; eight live reschedule IPIs and five modeled exact acknowledgements gate ownership; three two-tick quantum switches complete; one APIC-4 timeout restores source ownership and rejects its late acknowledgement; maximum bypass and watchdog age remain two. All eight tasks retire, all timer/frame owners are revoked, all three APs park, and 102 pages or 417,792 bytes are scrubbed, verified, and released. Five focused tests within 189 kernel host tests, seven exact Rust/Python host receipts, two exact 38-marker four-vCPU boots, and 34 hostile-control categories covering 232 rejected cases pass. The canonical kernel is 513,672 bytes in a 585,728-byte, 143-page image with entry 0xA000, text end 0x70000, RELRO end and data start 0x7D000, 1,264 relocations, and SHA-256 FCE5C1F2478651D010A2F2781B80494FD0D9721880D33CE8C66A499B35C8DAB6. AP-local timer/frame lanes remain bounded semantic inputs; this proves no AP-local timer interrupt delivery, general topology/hotplug/x2APIC scheduling, general SMP preemption, ring-3 or address-space switch, full per-task architectural ownership, target hardware, N12 exit, release, or production readiness. ADD-N12-CONCURRENCY-ATOMICS-001 and FLAG-N12-CONCURRENCY-ATOMICS-001 preserve the next bounded dependency.",
...
def main() -> int:
    ...
    parser.add_argument("--test-count", type=int, default=874)
