...
PHASE_EVIDENCE = {
    ...
    "N6": [
        ...
        "runs/native_kernel_entry_readiness.json: 88/88 host tests, 2/2 exact clean linked and canonical builds, 43/43 hostile controls, 693 relative relocations, exact independent Rust/Python loaded bytes, and a current bound canonical SHA-256",
    ...
    "N9": [
        ...
        "runs/native-kernel-physical-memory-readiness.json: 2/2 exact qemu64 QEMU/OVMF runs, 45 ordered markers, 191/191 hostile controls, 88/88 kernel host tests, 97 PBP1 entries, 117,888 usable source pages, and 129,148 final managed pages. PKACPI1 validates one RSDP/XSDT and exactly one APIC, FACP, HPET, and MCFG table, copies 600 table bytes into a retained one-page 616-byte snapshot, and binds source checksum 078583AEEFDD6581 plus snapshot checksum 4089A5CFEC81CB41. Boot reclaim admits 11,250 pages under receipt 5DEA9A3BC9E10C18; only after opaque validation/copy/readback evidence does ACPI reclaim admit 11 pages under receipt 60DAA52A8A05ABD6. The stable manager remains five guarded pages; checked pressure reaches a 29-page generation, retires three predecessors totaling 15 pages, records 119 checks, seven triggers, 59 successful cycles, three soft fallbacks, and one hard pre-effect rejection, and binds growth checksum 0xCC30B722527718AE. The runs protect 856 loader pages, scrub and verify 11,473 pages and 46,993,408 bytes, perform 5,875,277 physical writes, 5,879,957 reads, and 23,172 temporary PTE writes and invalidations with zero signatures, authority, actions, AML execution, SMP, target, or production claims",
...
PROGRAM_GAPS = [
    ...
    "PKPMM7 supplies bounded one-BSP physical-page ownership, scrub-before-allocation and scrub-before-reuse with full readback and fault rollback, a stable guarded five-page manager, external generation-owned guarded ledgers, checked automatic repeated growth, verified retirement, bounded-window fallback and pre-effect rejection, exact post-ExitBootServices Boot Services reclaim, and PKACPI1-gated ACPI reclaim after required-table validation plus retained snapshot copy/readback. PKVM1 supplies inactive four-level 4 KiB transactions, and PKVM2 supplies one bounded active root, nine-page direct map, exact CR3 restoration, and three local invalidation receipts. A complete generation-owned sparse direct map, SMP TLB shootdown and deferred reclaim, huge pages, PCID, COW, user faults, pager IPC, cache/MMIO qualification, heaps/object caches, interrupt-context allocation, concurrency/SMP, general pressure and OOM policy, scheduler, task, syscall, capability, IPC, isolation, and asynchronous I/O remain open",
...
def main() -> int:
    ...
    parser.add_argument("--test-count", type=int, default=788)
