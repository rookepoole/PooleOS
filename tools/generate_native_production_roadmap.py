...
PHASE_EVIDENCE = {
    ...
    "N9": [
        "specs/native-kernel-physical-memory-contract.json, specs/native-kernel-physical-memory-contract.schema.json, and docs/native-kernel-physical-memory.md: PKPMM4 freezes exact PBP1 intake, UEFI source-kind validation, usable-only initial ownership, page-zero exclusion, DMA/DMA32/Normal zones, generation-safe scrubbed transactions, a five-page guarded generation-owned metadata arena, and lifecycle-gated boot-services versus ACPI reclaim without scalable growth, concurrency, or production claims",
        "native/kernel/src/physical_memory.rs, native/kernel/src/main.rs, runtime/native_kernel_physical_memory.py, tools/qualify_native_kernel_physical_memory.py, and tests/test_native_kernel_physical_memory.py: allocation-free no_std kernel allocator, complete-manager bootstrap migration, persistent supervisor RW/NX mappings with absent guards, streamed three-pass reclaim preflight/scrub/commit, retained-range exclusion, idempotence, content-fault ownership preservation, independent transcript/oracle validation, source audit, and PBP1 substitution controls",
        "runs/native-kernel-physical-memory-readiness.json: 2/2 exact qemu64 QEMU/OVMF runs, 42 ordered markers, 109/109 hostile controls, 79/79 kernel host tests, 97 PBP1 memory entries, 117,919 usable source pages, 129,168 final managed pages, 70 boot-services source records coalesced into 12 ranges and 11,250 admitted pages, 11 ACPI reclaimable pages still held, 825 retained loader pages protected, 11,263 scrubbed and verified pages, 46,133,248 scrubbed and verified bytes, one immutable reclaim receipt, exact idempotence, 5,767,680 physical word writes, 5,768,704 reads, 22,543 temporary PTE writes and invalidations, five retained metadata mappings plus two absent guards, range checksum 0x5A485D4A5725EED8, receipt checksum 0x4D3EBF743B7F2CCC, exact handoff/final integrity, and zero signatures, authority, or actions",
...
PHASE_GAPS = {
    ...
    "N9": ["PKPMM4 consumes the live PBP1 map and provides bounded generation-safe physical ownership, full-page scrub-before-allocation and scrub-before-reuse, a retained guarded five-page metadata arena, and exact post-ExitBootServices admission of boot-services pages through streamed preflight/scrub/commit with retained-range exclusion, immutable receipts, idempotence, and ownership-preserving content faults. ACPI reclaim remains held until explicit table release. PKVM1 supplies inactive page-table transactions. PKVM2 adds a kernel-complete one-BSP candidate root, a bounded nine-page direct map, exact CR3 activation/restoration, architectural Accessed/Dirty handling, and three active local invalidation receipts. Scalable metadata growth, complete ACPI lifecycle integration, a complete generation-owned direct map, SMP TLB shootdown and deferred reclaim, huge pages, PCID, COW, user faults, pager IPC, heaps/object caches, MMIO/PAT/MTRR qualification, concurrent allocation, pressure/OOM policy, target hardware, and the N9 exit gate remain open"],
...
PROGRAM_GAPS = [
    ...
    "A real reproducible 70-page PooleKernel image, PKENTRY1 intake, allocation-free PKREVAL1 verifier, bounded early diagnostics, opt-in QEMU-only live entry, BSP-only PKTRAP1 descriptor/exception containment, bounded BSP PKXSTATE1 x87/SSE ownership, PKPMM4 scrubbed lifecycle, guarded metadata-arena, and boot-services reclaim transactions, and PKVM2 active-root evidence exist, but authenticated boot trust, measured boot, production transfer, complete per-CPU runtime state, retained crash evidence, target execution, and N6/N7/N9 exit remain open",
    "PKERR1 freezes a pure exact-target CPU/errata rejection policy, PKXSTATE1 proves bounded x87/SSE standard-XSAVE ownership, PKXEXC1 proves deliberate #MF/#XM recovery plus terminal test-only #NM rejection with a linked scope audit under WHPX, and PKMSR1 proves only a read-only qemu64 BSP system-linkage/global-MCA/unsupported-PMU observation. PKPMM4 supplies bounded physical ownership, scrubbed lifecycle transactions, a guarded five-page metadata arena, and streamed lifecycle-gated boot-services reclaim while holding ACPI pages; PKVM1 supplies inactive page-table transactions; and PKVM2 proves one-BSP candidate-root activation/restoration with a nine-page direct map and three local invalidation receipts. No target-qualified complete native CPU policy, applicable Model 40h-4Fh errata authority, direct numeric client microcode floor or ratified replacement, target-specific privileged-MSR semantics, syscall/per-CPU/MCE/PMU activation, AVX/extended state, user-task exception delivery, scheduler/AP/migration integration, interrupts, time, SMP, scalable allocator metadata, complete ACPI lifecycle integration, complete kernel/user address spaces, SMP TLB shootdown, heap, MMIO/cache-alias qualification, concurrent allocator, pressure, or OOM implementation exists",
    ...
    "PKPMM4 supplies bounded one-BSP physical-page ownership, scrub-before-allocation and scrub-before-reuse with full readback and fault rollback, a retained guarded five-page metadata arena, and exact post-ExitBootServices boot-services reclaim with retained-range exclusion, complete preflight, scrub-before-admission, idempotence, immutable receipts, and ownership-preserving content faults while ACPI remains held. PKVM1 supplies inactive four-level 4 KiB transactions, and PKVM2 supplies one bounded active root, nine-page direct map, exact CR3 restoration, and three local invalidation receipts. Scalable metadata growth, complete ACPI lifecycle integration, a complete generation-owned direct map, SMP TLB shootdown and deferred reclaim, huge pages, PCID, COW, user faults, pager IPC, cache/MMIO qualification, heaps/object caches, concurrency/SMP, pressure and OOM policy, scheduler, task, syscall, capability, IPC, isolation, and asynchronous I/O remain open",
