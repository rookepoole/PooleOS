...
PHASE_EVIDENCE = {
    ...
    "N5": [
        "Cycle 160 confirms the six Cycle 159 N5 receipts still pass current-source checks without changing native executable bytes. N7 replay now passes too; fourteen memory-through-lock checks and the full candidate suite remain pending. Earlier cycle entries retain their historical scopes",
        ...
        "specs/native-kernel-map-contract.json, native/kmap, runtime/native_kernel_map.py, and docs/native-kernel-map.md: current PKMAP2 exact 144-page 4 KiB supervisor kernel mapping, two retained leaf tables, guarded stack starting at page 145, read-only handoff starting at page 182, W^X/WP/NX, active-root audit, framebuffer preservation, ten-role retained allocation coverage, and nonclaim contract",
    ...
    "N7": [
        ...
        "runs/native-kernel-cpu-policy-readiness.json: 219/219 kernel host tests, 2 exact fresh-vars qemu64 QEMU/OVMF runs, 35 ordered markers, 41/41 hostile controls, exact Rust/Python agreement, AuthenticAMD family 15 model 107 stepping 1 observation, 5 MSR reads, zero MSR writes, and zero authority or actions",
    ...
    "N9": [
        ...
        "runs/native-kernel-virtual-memory-readiness.json: Cycle 156 passes 2/2 exact qemu64 QEMU/OVMF runs, 40 ordered markers, 46/46 hostile controls and 214/214 kernel host tests; 117,821 admitted pages in eleven ranges with 12,944 hole pages, 237 direct leaf tables, one direct directory and 243 table pages, checksum 0x64E09067B6BFDCB3, 367,408 physical table writes, 950,682 temporary-PTE writes and invalidations, two CR3 writes, three local invalidation receipts, one exact generation-retirement receipt, zero remote shootdowns, signatures, authority or actions",
    ...
    "N12": [
        ...
        "ADD-N12-CONCURRENCY-RECLAMATION-001: replay the Cycle 158 changed-image prerequisites, then mandatory active-root and execution-stack ownership, acknowledged cross-CPU quiescence, live timeout/offline/death/pressure/shutdown rollback, an independent oracle and exact-topology live evidence before closing N12.3",
    ],
    ...
    return {
        ...
        "baseline": {
            ...
            "native_consistency_release_gate": {
                ...
                "production_ready": False,
                "native_promotion_role": "changed_candidate_non_promoting",
                "qualification_status": "partial_current_source_dependency_replay_full_gate_pending",
                "passed_check_count_scope": "last_full_audit_cycle158_not_current_projection",
...
def main() -> int:
    ...
    parser.add_argument("--status-date", default="2026-09-06")
