...
PHASE_EVIDENCE = {
    ...
    "N6": [
        ...
        "runs/native_kernel_entry_readiness.json: 99/99 host tests, 2/2 exact clean linked and canonical builds, 43/43 hostile controls, 812 relative relocations, exact independent Rust/Python loaded bytes, and canonical SHA-256 2ACD4A5EF30CA1A4A22711FD31E2A259A5C87D97BCE7FB1BF49A3488B3FC02B2",
    ...
    "N7": [
        ...
        "runs/native-kernel-privilege-msr-policy-readiness.json: two exact fresh-vars TCG QEMU/OVMF runs, 35 markers, 47/47 hostile controls, 11 support-gated MSR reads, ten observed MCA banks, zero bank reads, zero runtime writes, and a linked-image scope audit isolating both WRMSR sites to the separately selected PKIRQ1 branch with zero authority and explicit emulator/target/production nonclaims",
    ],
    "N8": [
        ...
        "runs/native-kernel-interrupt-time-readiness.json: 99/99 kernel host tests, two exact fresh-vars qemu64 QEMU/OVMF runs, 36 ordered markers, 58/58 hostile controls, eight timer deliveries, eight EOIs, zero APIC-error or spurious deliveries, three absent MMIO guards, exact LAPIC/HPET mapping revocation, zero AP starts, and zero signatures, authority, actions, or production claims",
    ],
    "N9": [
        ...
        "runs/native-kernel-physical-memory-readiness.json: 2/2 exact qemu64 QEMU/OVMF runs, 45 ordered markers, 191/191 hostile controls, 99/99 kernel host tests, 97 PBP1 entries, 117,882 usable source pages, and 129,142 final managed pages. PKACPI1 validates one RSDP/XSDT and exactly one APIC, FACP, HPET, and MCFG table, copies 600 table bytes into a retained one-page 616-byte snapshot, and binds source checksum 078583AEEFDD6581 plus snapshot checksum 4089A5CFEC81CB41. Boot reclaim admits 11,250 pages under receipt 5DEA9A3BC9E10C18; only after opaque validation/copy/readback evidence does ACPI reclaim admit 11 pages under receipt 60DAA52A8A05ABD6. The stable manager remains five guarded pages; checked pressure reaches a 29-page generation, retires three predecessors totaling 15 pages, records 119 checks, seven triggers, 59 successful cycles, three soft fallbacks, and one hard pre-effect rejection, and binds growth checksum 0xCC30B722527718AE. The runs protect 862 loader pages, scrub and verify 11,473 pages and 46,993,408 bytes, perform 5,875,277 physical writes, 5,879,957 reads, and 23,172 temporary PTE writes and invalidations with zero signatures, authority, actions, AML execution, SMP, target, or production claims",
        ...
        "runs/native-kernel-virtual-memory-readiness.json: 2/2 exact qemu64 QEMU/OVMF runs, 40 ordered markers, 46/46 hostile controls, 117,881 admitted pages in eleven sparse ranges with 12,884 hole pages, 237 direct leaf tables, one direct directory and 243 total table pages, coverage checksum 0x23C9E575300AC1CE, 367,468 physical table writes, 950,282 temporary-PTE writes and invalidations, two CR3 writes, three active local invalidation receipts, one exact generation-retirement receipt, zero remote shootdowns, and zero signatures, authority, or actions",
...
PHASE_GAPS = {
    ...
    "N6": ["A reproducible real 89-page PooleKernel PKELF1 image, PKENTRY1 intake, allocation-free PKREVAL1 verifier, bounded diagnostics, an opt-in QEMU-only live transfer, and BSP-only descriptor/exception, CPU, xstate, MSR, physical-memory, ACPI-container, virtual-memory, and interrupt/time development profiles now exist on one host; authenticated boot trust, measured boot, production transfer, final framebuffer remap/revocation, complete per-CPU runtime state, retained crash paths, target execution, and N6 exit remain open"],
    ...
    "N8": ["PKIRQ1 proves a bounded one-BSP qemu64 local xAPIC and HPET substrate with retained-table discovery, guarded uncacheable mappings, vector ownership, calibrated one-shot timer delivery, exact EOI accounting, legacy-PIC masking, and normal-path rollback. I/O APIC routing, MSI/MSI-X, invariant-TSC calibration, watchdog and timer queues, public time domains, first-AP startup and per-CPU state, IPIs, SMP shootdown, panic-path transactional recovery, target hardware, and the N8 exit gate remain open"],
...
PROGRAM_GAPS = [
    ...
    "A real reproducible 83-page PooleKernel image, PKENTRY1 intake, allocation-free PKREVAL1 verifier, bounded early diagnostics, opt-in QEMU-only live entry, BSP-only PKTRAP1 descriptor/exception containment, bounded BSP PKXSTATE1 x87/SSE ownership, PKPMM7 scrubbed lifecycle and checked repeated ledger growth, guarded stable-manager and generation-owned active-ledger transactions, PKACPI1 required-table snapshot/reclaim evidence, and PKVM3 sparse PMM-owned direct-map evidence exist, but authenticated boot trust, measured boot, production transfer, complete per-CPU runtime state, retained crash evidence, target execution, and N6/N7/N9 exit remain open",
    "PKERR1 freezes a pure exact-target CPU/errata rejection policy, PKXSTATE1 proves bounded x87/SSE standard-XSAVE ownership, PKXEXC1 proves deliberate #MF/#XM recovery plus terminal test-only #NM rejection with a linked scope audit under WHPX, and PKMSR1 proves only a read-only qemu64 BSP system-linkage/global-MCA/unsupported-PMU observation. PKPMM7 supplies bounded physical ownership, scrubbed lifecycle transactions, a stable guarded five-page manager, external generation-owned active ledgers, checked automatic growth with retirement, bounded-window fallback and pre-effect rejection, streamed lifecycle-gated Boot Services reclaim, and PKACPI1-gated ACPI reclaim after required-table validation and retained copy/readback; PKVM1 supplies inactive page-table transactions; and PKVM3 proves one-BSP activation/restoration of a complete-profile PMM-owned sparse direct map with cache-alias rejection and exact generation-retirement receipts. No target-qualified complete native CPU policy, applicable Model 40h-4Fh errata authority, direct numeric client microcode floor or ratified replacement, target-specific privileged-MSR semantics, syscall/per-CPU/MCE/PMU activation, AVX/extended state, user-task exception delivery, scheduler/AP/migration integration, interrupts, time, SMP shootdown, AML or complete ACPI resource-graph execution, complete kernel/user address spaces, heap, MMIO/PAT/MTRR qualification, interrupt-context or concurrent allocator, general pressure, or OOM implementation exists",
...
def main() -> int:
    ...
    parser.add_argument("--test-count", type=int, default=796)
