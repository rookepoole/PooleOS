...
PHASE_EVIDENCE = {
    ...
    "N6": [
        ...
        "runs/native_kernel_entry_readiness.json: 38/38 host tests, 2/2 exact clean linked and canonical builds, 43/43 hostile controls, 180,224 canonical bytes in a 262,144-byte image, entry offset 0x8000, 352 relative relocations, exact independent Rust/Python loaded bytes, and canonical SHA-256 36582C1CBEB4CCCB9E872C432CA095EC0896FB8E5507FA10305AA406C13D37F7",
...
PROGRAM_GAPS = [
    ...
    "PKERR1 freezes a pure exact-target CPU/errata rejection policy, PKXSTATE1 proves bounded x87/SSE standard-XSAVE ownership, and PKXEXC1 proves deliberate #MF/#XM recovery plus terminal test-only #NM rejection with a linked scope audit under WHPX on one BSP; no target-qualified complete native CPU policy, applicable Model 40h-4Fh errata authority, direct numeric client microcode floor or ratified replacement, AVX/extended state, user-task exception delivery, scheduler/AP/migration integration, syscall/GS/TSC_AUX/MCE/performance-MSR policy, interrupts, time, SMP, physical memory, virtual memory, or reclaim implementation exists",
...
def main() -> int:
    ...
    parser.add_argument("--test-count", type=int, default=767)
