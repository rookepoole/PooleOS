...
PHASE_EVIDENCE = {
    ...
    "N5": [
        ...
        "runs/native-kernel-revalidation-readiness.json: 130/130 kernel Rust tests, 8/8 Python tests, both no_std target builds, nine exact retained files and parsers, 36/36 hostile controls, 32,768/32,768 deterministic mutation rejects, exact unsigned-policy denial, zero authority grants/actions/state writes, and no standalone live-entry claim",
    ...
    "N6": [
        ...
        "runs/native_kernel_entry_readiness.json: 130/130 host tests, 2/2 exact clean linked and canonical builds, 43/43 hostile controls, 959 relative relocations, exact independent Rust/Python loaded bytes, and canonical SHA-256 6B8A9C2C3EAC559E1D9CB5965800A1671DB8F487F149861300D4EDCB279B3A11",
    ...
    "N7": [
        ...
        "runs/native-kernel-cpu-policy-readiness.json: 130/130 kernel host tests, 2 exact fresh-vars qemu64 QEMU/OVMF runs, 35 ordered markers, 41/41 hostile controls, exact Rust/Python agreement, AuthenticAMD family 15 model 107 stepping 1 observation, 5 MSR reads, zero MSR writes, and zero authority or actions",
        ...
        "runs/native-kernel-privilege-msr-policy-readiness.json: two exact fresh-vars TCG QEMU/OVMF runs, 35 markers, 47/47 hostile controls, 11 support-gated MSR reads, ten observed MCA banks, zero bank reads, zero runtime writes, and an exact linked-image audit of 25 RDMSR and three WRMSR sites that isolates PKSMP1, PKSMP2, PKSMP3, and PKIRQ1 accesses from the read-only PKMSR1 profile, with zero authority and explicit emulator/target/production nonclaims",
    ],
    "N8": [
        ...
        "runs/native-kernel-interrupt-time-readiness.json: 130/130 kernel host tests, two exact fresh-vars qemu64 QEMU/OVMF runs, 36 ordered markers, 58/58 hostile controls, eight timer deliveries, eight EOIs, zero APIC-error or spurious deliveries, three absent MMIO guards, exact LAPIC/HPET mapping revocation, zero AP starts, and zero signatures, authority, actions, or production claims",
        ...
        "runs/native-kernel-smp-first-ap-readiness.json: 130/130 kernel host tests, two exact fresh-vars two-vCPU qemu64 QEMU/OVMF runs, 38 ordered markers, 72/72 hostile controls, exactly one AP started/online/quiesced/parked, fourteen pages and 57,344 bytes scrubbed/verified/released, and zero signatures, authority, target, N8-exit, or production claims",
        ...
        "runs/native-kernel-smp-percpu-runtime-readiness.json: 130/130 kernel host tests, two exact fresh-vars two-vCPU SandyBridge-minus-AVX TCG QEMU/OVMF runs, 42 ordered markers, 19/19 hostile-control categories covering 159 rejected cases, exactly one AP-local descriptor set, three guarded stack classes, 27 gates, one xstate round trip, and 32 pages or 131,072 bytes scrubbed/verified/released with zero signatures, authority, N8-exit, or production claims",
        "specs/native-kernel-smp-ipi-contract.json, native/kernel/src/smp_ipi.rs, runtime/native_kernel_smp_ipi.py, and docs/native-kernel-smp-ipi.md: PKSMP3 freezes one development-only capability-gated IPI transport with six fixed operation classes, checksum-bound request and response state, replay controls, bounded timeout, stop/panic lifecycle, exact release, and explicit no-real-shootdown/no-callback/no-scheduler/no-target/nonproduction boundaries",
        "runs/native-kernel-smp-ipi-readiness.json: 130/130 kernel host tests, two exact fresh-vars two-vCPU SandyBridge-minus-AVX TCG QEMU/OVMF runs, 39 ordered markers, 18/18 hostile-control categories covering 120 rejected cases, six accepted and four denied live deliveries, ten EOIs, one offline-target timeout, and 32 pages or 131,072 bytes scrubbed/verified/released with zero TLB invalidations, signatures, authority, N8-exit, or production claims",
    ],
    "N9": [
...
def make_roadmap(test_count: int, status_date: str) -> dict:
    ...
    return {
        ...
        "execution_protocol": {
            ...
            "last_updated_cycle": 139,
            "selected_move_id": "N8-SMP-IPI-001",
            "immediate_next_move_id": "N0-HW-KEY-ACQUIRE-001",
            "owner_independent_next_move_id": "N9-SMP-SHOOTDOWN-001",
            "required_records": [
...
def main() -> int:
    ...
    parser.add_argument("--test-count", type=int, default=821)
