...
PHASE_EVIDENCE = {
    ...
    "N5": [
        "specs/native-pooleboot-proof.json: POOLEOS-N5-POOLEBOOT-2 bounded unsigned aggregate contract across N5.1-N5.5 and N5.7",
        "native/boot: Poole-authored no_std PE32+ UEFI application with reviewed firmware bindings, independent serial/debugcon diagnostics, live bounded filesystem/config/kernel intake, GOP identity, memory-map observation, and EFI_SUCCESS return",
        "runtime/native_kernel_load.py and tools/qualify_native_kernel_load.py: deterministic 64 MiB protective-MBR/GPT/FAT32 ordinary-file media with exact fallback EFI, PBC1 config, and PKELF1 PooleKernel inspection and no physical-media output mode",
        "runs/native_pooleboot_readiness.json: 8/8 host tests, 2/2 exact PE builds, 2/2 exact media generations, 2/2 exact QEMU/OVMF runs, seventeen ordered markers, 2/2 serial/debugcon matches, 2/2 exact GOP frames, and 30/30 hostile controls",
        ...
        "runs/native_boot_handoff_readiness.json: 8/8 Rust tests, 2/2 no_std target builds, twelve layout assertions, 3/3 golden vectors, 32/32 hostile controls, and 16,384 Rust/Python differential cases with zero mismatches",
        ...
        "specs/native-kernel-load-contract.json and docs/native-kernel-load.md: PKLOAD1 freezes the live UEFI file, allocation, load, mapping-plan, cleanup, marker, trust, and nonclaim boundary",
        "native/bootload, native/boot/src/kload.rs, and runtime/native_kernel_load.py: dependency-free load contract, reviewed raw UEFI adapter, and independent media/oracle implementation",
        "runs/native_kernel_load_readiness.json: 33/33 Rust host tests, 2/2 exact PooleBoot builds, 2/2 exact PooleKernel builds, 2/2 exact media generations, 2/2 QEMU/OVMF runs, 17 ordered markers, 30/30 hostile controls, and exact guest/oracle agreement",
    ],
    "N6": [
        ...
        "runs/native_kernel_entry_readiness.json: 13/13 host tests, 2/2 exact clean linked and canonical builds, 43/43 hostile controls, exact independent Rust/Python loaded bytes, and canonical SHA-256 8D79F687B92EE4C3592F45BB8B2C33E805D3D246319D54AB7EC354458490CA5B",
...
PHASE_GAPS = {
    ...
    "N5": ["A reproducible unsigned PooleBoot, PBP1, PBC1, PKELF1, real PooleKernel image, and bounded live config/kernel read-allocation-load-release path exist; however, manifest-driven authenticated selection, retained allocations, page-table installation, menu/rollback policy, live handoff population and consumption, ExitBootServices transfer, second host, target firmware, physical media, and N5 exit remain open"],
...
PROGRAM_GAPS = [
    ...
    "A reproducible unsigned PooleBoot proof application boots twice with deterministic GPT/FAT32 media, ordered dual-channel markers, and exact GOP frames, while canonical PBP1, PBC1, bounded PKELF1, and a separately qualified real PooleKernel product pass their declared gates; live file I/O, trusted selection, signed-artifact authentication, firmware allocation, page-table installation, live handoff producer/consumer, ExitBootServices transfer, target firmware, physical-media qualification, and N5 exit remain open",
        ...
        implementation_flags.append(
            {
                ...
                or flag_id in {"FLAG-N0-RATIFICATION-SCOPE-001", "FLAG-N2-CPUID-001", "FLAG-N4-PROFILE-001", "FLAG-N4-IPC-MODEL-001", "FLAG-N4-SCHEDULER-MODEL-001", "FLAG-N4-POOLEFS-MODEL-001", "FLAG-N5-POOLEBOOT-PROOF-001", "FLAG-N5-BOOTPROTO-001", "FLAG-N5-BOOTCFG-001", "FLAG-N5-ELF-001", "FLAG-N5-KLOAD-001", "FLAG-N6-KENTRY-001"}
                else "open",
                "phase_id": phase_id,
...
def main() -> int:
    ...
    parser.add_argument("--test-count", type=int, default=575)
