...
PHASE_EVIDENCE = {
    ...
    "N9": [
        "specs/native-kernel-physical-memory-contract.json, specs/native-kernel-physical-memory-contract.schema.json, and docs/native-kernel-physical-memory.md: PKPMM5 freezes exact PBP1 intake, UEFI source-kind validation, usable-only initial ownership, page-zero exclusion, DMA/DMA32/Normal zones, generation-safe scrubbed transactions, a stable five-page guarded manager, external generation-owned guarded active ledgers, one explicit growth transaction, and lifecycle-gated boot-services versus ACPI reclaim without automatic pressure triggering, concurrency, or production claims",
        "native/kernel/src/physical_memory.rs, native/kernel/src/main.rs, runtime/native_kernel_physical_memory.py, tools/qualify_native_kernel_physical_memory.py, and tests/test_native_kernel_physical_memory.py: allocation-free no_std kernel allocator, stable-manager bootstrap migration, two alternate 32-page guarded ledger windows, transactional preflight/map/copy/seal/switch/revoke/scrub-retire growth, retryable postcommit retirement, streamed three-pass reclaim, independent transcript/oracle validation, source audit, PBP1 substitution controls, and host mapping/content/retirement fault injection",
        "runs/native-kernel-physical-memory-readiness.json: 2/2 exact qemu64 QEMU/OVMF runs, 43 ordered markers, 137/137 hostile controls, 82/82 kernel host tests, 97 PBP1 entries, 117,913 usable source pages, and 129,162 final managed pages. The stable manager occupies five guarded pages; generation 2 starts in four guarded active-ledger pages at capacities 256/32/256/16/2 and atomically grows to generation 3 in eight pages at capacities 512/64/512/32/4. The old four-page generation is revoked, scrubbed, and retired with checksum 0xFA339A347E3A3CAF. Reclaim then admits 11,250 Boot Services pages while keeping 11 ACPI pages held. The runs protect 831 loader pages, scrub and verify 11,279 pages and 46,198,784 bytes, perform 5,775,872 physical writes, 5,776,896 reads, and 22,591 temporary PTE writes and invalidations with zero signatures, authority, or actions",
...
PHASE_GAPS = {
    ...
    "N9": ["PKPMM5 consumes the live PBP1 map and provides bounded generation-safe physical ownership, scrub-before-allocation/reuse, a stable guarded five-page manager, external generation-owned guarded active ledgers, one atomic 4-to-8-page ledger growth with verified old-generation retirement, and exact post-ExitBootServices Boot Services admission while ACPI remains held. PKVM1 supplies inactive page-table transactions. PKVM2 adds a kernel-complete one-BSP candidate root, a bounded nine-page direct map, exact CR3 activation/restoration, architectural Accessed/Dirty handling, and three active local invalidation receipts. Automatic pressure-triggered and multi-generation growth, ledger-window exhaustion/fallback, complete ACPI lifecycle integration, a complete generation-owned direct map, SMP TLB shootdown and deferred reclaim, huge pages, PCID, COW, user faults, pager IPC, heaps/object caches, MMIO/PAT/MTRR qualification, concurrent allocation, pressure/OOM policy, target hardware, and the N9 exit gate remain open"],
...
PROGRAM_GAPS = [
    ...
    "A real reproducible 76-page PooleKernel image, PKENTRY1 intake, allocation-free PKREVAL1 verifier, bounded early diagnostics, opt-in QEMU-only live entry, BSP-only PKTRAP1 descriptor/exception containment, bounded BSP PKXSTATE1 x87/SSE ownership, PKPMM5 scrubbed lifecycle, guarded stable-manager and generation-owned active-ledger growth transactions, and PKVM2 active-root evidence exist, but authenticated boot trust, measured boot, production transfer, complete per-CPU runtime state, retained crash evidence, target execution, and N6/N7/N9 exit remain open",
    "PKERR1 freezes a pure exact-target CPU/errata rejection policy, PKXSTATE1 proves bounded x87/SSE standard-XSAVE ownership, PKXEXC1 proves deliberate #MF/#XM recovery plus terminal test-only #NM rejection with a linked scope audit under WHPX, and PKMSR1 proves only a read-only qemu64 BSP system-linkage/global-MCA/unsupported-PMU observation. PKPMM5 supplies bounded physical ownership, scrubbed lifecycle transactions, a stable guarded five-page manager, external generation-owned active ledgers, one atomic 4-to-8-page growth/retirement transaction, and streamed lifecycle-gated Boot Services reclaim while holding ACPI pages; PKVM1 supplies inactive page-table transactions; and PKVM2 proves one-BSP candidate-root activation/restoration with a nine-page direct map and three local invalidation receipts. No target-qualified complete native CPU policy, applicable Model 40h-4Fh errata authority, direct numeric client microcode floor or ratified replacement, target-specific privileged-MSR semantics, syscall/per-CPU/MCE/PMU activation, AVX/extended state, user-task exception delivery, scheduler/AP/migration integration, interrupts, time, SMP, automatic or repeated allocator-ledger growth, complete ACPI lifecycle integration, complete kernel/user address spaces, SMP TLB shootdown, heap, MMIO/cache-alias qualification, concurrent allocator, pressure, or OOM implementation exists",
    ...
    "PKPMM5 supplies bounded one-BSP physical-page ownership, scrub-before-allocation and scrub-before-reuse with full readback and fault rollback, a stable guarded five-page manager, external generation-owned guarded ledgers, one explicit atomic 4-to-8-page growth with retirement, and exact post-ExitBootServices Boot Services reclaim while ACPI remains held. PKVM1 supplies inactive four-level 4 KiB transactions, and PKVM2 supplies one bounded active root, nine-page direct map, exact CR3 restoration, and three local invalidation receipts. Automatic pressure-triggered and repeated growth, both ledger-window exhaustion paths, complete ACPI lifecycle integration, a complete generation-owned direct map, SMP TLB shootdown and deferred reclaim, huge pages, PCID, COW, user faults, pager IPC, cache/MMIO qualification, heaps/object caches, concurrency/SMP, pressure and OOM policy, scheduler, task, syscall, capability, IPC, isolation, and asynchronous I/O remain open",
...
def make_roadmap(test_count: int, status_date: str) -> dict:
    ...
    return {
        ...
        "execution_protocol": {
            ...
            "last_updated_cycle": 131,
            "selected_move_id": "N9-PMM-GROWTH-001",
            "immediate_next_move_id": "N0-HW-KEY-ACQUIRE-001",
            "owner_independent_next_move_id": "N9-PMM-GROWTH-AUTOMATION-001",
            "required_records": [
...
def main() -> int:
    ...
    parser.add_argument("--test-count", type=int, default=786)
