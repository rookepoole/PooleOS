...
PHASE_EVIDENCE = {
    ...
    "N5": [
        ...
        "specs/native-kernel-map-contract.json, native/kmap, runtime/native_kernel_map.py, and docs/native-kernel-map.md: PKMAP2 exact 136-page 4 KiB supervisor kernel mapping, guarded stack, read-only handoff, W^X/WP/NX, active-root audit, framebuffer preservation, ten-role retained allocation coverage, and nonclaim contract",
        ...
        "runs/native_kernel_load_readiness.json: exact 269/269 aggregate Rust host tests, 2/2 exact PooleBoot builds, 2/2 exact PooleKernel builds, 2/2 exact twelve-file media generations, 2/2 QEMU/OVMF runs, 25 ordered markers, 155 integrated hostile controls, exact retained PINIT1, PREC1, PSYM1, PMCU1, PFWM1, and PPOL1 parsing with payload/route cross-binding and mandatory development denial, exact retained PSM1/PBTP1/PBTS1 parsing with fourteen trust cross-bindings and unsigned-policy denial, two exact post-exit PBLIVE4 PBP1 reconstructions, exact 136-page kernel plus nine retained files/table/36-page guarded-stack/handoff/firmware-record agreement, successful ExitBootServices, zero later firmware calls, and exact guest/oracle agreement",
        ...
        "runs/native-kernel-revalidation-readiness.json: 184/184 kernel Rust tests, 8/8 Python tests, both no_std target builds, nine exact retained files and parsers, 36/36 hostile controls, 32,768/32,768 deterministic mutation rejects, exact unsigned-policy denial, zero authority grants/actions/state writes, and no standalone live-entry claim",
    ...
    "N6": [
        ...
        "runs/native_kernel_entry_readiness.json: 184/184 host tests, 2/2 exact clean linked and canonical builds, 43/43 hostile controls, 1,222 relative relocations, exact independent Rust/Python loaded bytes, and canonical SHA-256 D11591395FDD8CD7BEEFA0D847A5C99EB133ED15F8DCDDE3392BFA499DCEDC33",
    ...
    "N7": [
        ...
        "runs/native-kernel-cpu-policy-readiness.json: 184/184 kernel host tests, 2 exact fresh-vars qemu64 QEMU/OVMF runs, 35 ordered markers, 41/41 hostile controls, exact Rust/Python agreement, AuthenticAMD family 15 model 107 stepping 1 observation, 5 MSR reads, zero MSR writes, and zero authority or actions",
    ...
    "N8": [
        ...
        "runs/native-kernel-interrupt-time-readiness.json: 184/184 kernel host tests, two exact fresh-vars qemu64 QEMU/OVMF runs, 36 ordered markers, 58/58 hostile controls, eight timer deliveries, eight EOIs, zero APIC-error or spurious deliveries, three absent MMIO guards, exact LAPIC/HPET mapping revocation, zero AP starts, and zero signatures, authority, actions, or production claims",
        ...
        "runs/native-kernel-smp-first-ap-readiness.json: 184/184 kernel host tests, two exact fresh-vars two-vCPU qemu64 QEMU/OVMF runs, 38 ordered markers, 72/72 hostile controls, exactly one AP started/online/quiesced/parked, fourteen pages and 57,344 bytes scrubbed/verified/released, and zero signatures, authority, target, N8-exit, or production claims",
    ...
    "N9": [
        ...
        "runs/native-kernel-physical-memory-readiness.json: 2/2 exact qemu64 QEMU/OVMF runs, 45 ordered markers, 191/191 hostile controls, 184/184 kernel host tests, 98 PBP1 entries, 117,831 usable source pages, and 129,091 final managed pages. PKACPI1 validates one RSDP/XSDT and exactly one APIC, FACP, HPET, and MCFG table, copies 600 table bytes into a retained one-page 616-byte snapshot, and binds source checksum 078583AEEFDD6581 plus snapshot checksum 4089A5CFEC81CB41. Boot reclaim admits 11,250 pages under receipt 5DEA9A3BC9E10C18; only after opaque validation/copy/readback evidence does ACPI reclaim admit 11 pages under receipt 60DAA52A8A05ABD6. The stable manager remains five guarded pages; checked pressure reaches a 29-page generation, retires three predecessors totaling 15 pages, records 119 checks, seven triggers, 59 successful cycles, three soft fallbacks, and one hard pre-effect rejection, and binds growth checksum 0xF34B6B4EC24C701D. The runs protect 913 loader pages, scrub and verify 11,473 pages and 46,993,408 bytes, perform 5,875,277 physical writes, 5,879,957 reads, and 23,172 temporary PTE writes and invalidations with zero signatures, authority, actions, AML execution, concurrent/SMP allocation, target, or production claims",
        ...
        "runs/native-kernel-virtual-memory-readiness.json: 2/2 exact qemu64 QEMU/OVMF runs, 40 ordered markers, 46/46 hostile controls, 117,830 admitted pages in eleven sparse ranges with 12,935 hole pages, 237 direct leaf tables, one direct directory and 243 total table pages, coverage checksum 0xFAE801D1D2F69BE6, 367,417 physical table writes, 950,626 temporary-PTE writes and invalidations, two CR3 writes, three active local invalidation receipts, one exact generation-retirement receipt, zero remote shootdowns, and zero signatures, authority, or actions",
    ...
    "N12": [
        ...
        "runs/native-kernel-scheduler-readiness.json: 14/14 scheduler Rust tests within 184/184 kernel host tests, four exact Rust/Python host-probe receipts over 4,096 steps, 1,761 dispatches and 2,334 migrations, two exact 17-marker qemu64 BSP runs, eight live dispatches, sixteen context transitions, 32,768 stack bytes cleared, and 28/28 hostile-control categories covering 115 rejected cases with zero live AP dispatch, ring-3/address-space switch, authority, N12-exit, or production claims",
        ...
        "runs/native-kernel-scheduler-smp-readiness.json: 8/8 focused SMP scheduler tests within 184/184 kernel host tests, five exact Rust/Python host-probe receipts, two exact 37-marker four-vCPU selector-18 runs, one cross-CPU wake, two migrations, three transaction acknowledgements, six AP dispatches, one APIC-4 timeout rollback, two stale acknowledgement rejections, eight task retirements, and 32/32 hostile-control categories covering 209 rejected cases; 102 pages or 417,792 bytes are scrubbed, verified, and released",
        ...
        "runs/native-kernel-scheduler-ap-workers-readiness.json: 10/10 focused AP-worker tests within 184/184 kernel host tests, six exact Rust/Python host-probe receipts, two exact 37-marker four-vCPU selector-19 runs, thirteen enqueues, twelve typed AP calls, eleven completions, two cancellations, one offline timeout rollback, thirteen reclaimed slots, and 34/34 hostile-control categories covering 226 rejected cases; three workers retire and all 102 pages or 417,792 bytes are scrubbed, verified, and released",
    ...
    "N36": ["Cycle 146 host baseline: TEST_COUNT tests with one expected Windows symlink-permission skip", "native binary parser, reproduction, leakage, malformed, substitution, governance, hardware, Tier 0, bounded-model, deterministic boot-media, PBP1/PBC1/PSM1/PBART1, six inner-format, PBTRUST1/PBSTATE1, PKELF1, PKENTRY1, PKLOAD6/PBLIVE4/PKMAP2/PBEXIT1, PKREVAL1, PKXFER1, PKTRAP1, PKCPU1, PKERR1, PKXSTATE1, PKXEXC1, PKMSR1, PKPMM7/PKACPI1, PKVM1, PKVM3, PKIRQ1, PKSMP1, PKSMP2, PKSMP5, PKSCHED1, PKSCHED2, PKSCHED3, PKSCHED4, and PKSCHED5 exact-topology typed AP-worker/source/machine-code/zero-authority controls, canonical-LF readiness regression controls, PooleGlyph roadmap bindings, Doctor external-report nonmutation, and collector-smoke negatives"],
    "N37": ["Cycle 146 consistency release gate: RELEASE_CHECKS checks over RELEASE_ARTIFACTS artifacts", "content-addressed source, objectives and governance receipts, native toolchain, bounded hardware/Tier 0/model evidence, PBTRUST1/PBSTATE1, bounded PooleBoot, PBP1/PBC1/PSM1/PBART1 and six inner formats, PKELF1, PKENTRY1, PKLOAD6/PKMAP2/PBEXIT1, PKREVAL1, PKXFER1, PKTRAP1, PKCPU1, PKERR1, PKXSTATE1, PKXEXC1, PKMSR1, PKPMM7/PKACPI1, PKVM1, PKVM3, PKIRQ1, PKSMP1, PKSMP2, PKSMP5, PKSCHED1, PKSCHED2, PKSCHED3, PKSCHED4, PKSCHED5, PooleGlyph planning artifacts, and retained historical consistency artifacts"],
}
...
PHASE_GAPS = {
    ...
    "N6": ["A reproducible real 136-page PooleKernel PKELF1 image, PKENTRY1 intake, allocation-free PKREVAL1 verifier, bounded diagnostics, an opt-in QEMU-only live transfer, and bounded descriptor/exception, CPU, xstate, MSR, physical-memory, ACPI-container, virtual-memory, interrupt/time, first-AP, AP-local runtime, fixed-vector IPI, one-page remote-shootdown, cooperative scheduler, BSP timer/wakeup preemption, BSP deferred-work, exact-topology SMP scheduler, and typed AP-local worker profiles now exist on one host; authenticated boot trust, measured boot, production transfer, final framebuffer remap/revocation, production capability authority, retained crash paths, target execution, and N6 exit remain open"],
    ...
    "N12": ["PKSCHED1 supplies a bounded allocation-free scheduler core, neutral fixed-priority round-robin policy, selected lifecycle and synchronization primitives, and a live cooperative BSP two-task context switch. PKSCHED2 integrates that core with PKIRQ1 through exact interrupt frames and proves bounded BSP quantum and wakeup preemption. PKSCHED3 adds allocation-free interrupt-deferred work, two fixed BSP workers, generation-safe reclamation, cancellation, flush, rollback, shutdown, and complete cleanup. PKSCHED4 adds four AP/BSP-local run queues on one exact topology, acknowledgement-gated ownership, one generation-safe cross-CPU wake, two migrations, six live AP dispatches, topology balancing, per-CPU idle ownership, timeout rollback, stale-ack rejection, and exact teardown. PKSCHED5 adds three AP-local workers and fixed typed timer-driver and generation-reclaim consumers with EOI gating, remote cancellation, flush-before-reclaim, offline rollback, bounded starvation, and exact park/scrub/release. Arbitrary callbacks and a general driver/service framework, general topology/hotplug/x2APIC scheduling, general SMP timer preemption, complete atomics/lock/reclamation families, ring-3 and address-space switching, per-task FS/GS and full xstate/debug/PMU ownership, watchdog/latency targets, target-hardware evidence, and the N12 exit gate remain open"],
...
def make_roadmap(test_count: int, status_date: str) -> dict:
    ...
    return {
        ...
        "claim_boundaries": [
            ...
            "The Cycle 146 N12-SCHED-AP-WORKERS-001 receipt adds selector 19 PKSCHED5 and closes only FLAG-N12-SCHED-AP-WORKERS-001 for one exact BSP-0/AP-1,2,3 SandyBridge-minus-AVX development topology. Three allocation-free AP-local queues and workers consume thirteen fixed work items only after the matching timer EOI; twelve typed call-function deliveries execute nine timer-driver and three generation-reclaim operations on private guarded IST1 stacks. One queued cancellation and one remote in-flight cancellation complete with exact semantics; one APIC-4 timeout restores source ownership and rejects its late acknowledgement; flush reaches eleven completed and two cancelled items before all thirteen slots are reclaimed. All three workers retire, all three APs park, and 102 pages or 417,792 bytes are scrubbed, verified, and released. Ten focused tests within 184 kernel host tests, six exact Rust/Python host receipts, two exact 37-marker four-vCPU boots, and 34 hostile-control categories covering 226 rejected cases pass. The canonical kernel is 485,000 bytes in a 557,056-byte, 136-page image with entry 0xA000, text end 0x69000, RELRO end and data start 0x76000, 1,222 relocations, and SHA-256 D11591395FDD8CD7BEEFA0D847A5C99EB133ED15F8DCDDE3392BFA499DCEDC33. This proves no arbitrary callback API, general driver or service framework, general topology/hotplug/x2APIC scheduling, general SMP timer preemption, ring-3 or address-space switch, full per-task architectural ownership, target hardware, N12 exit, release, or production readiness. ADD-N12-SCHED-SMP-PREEMPT-001 and FLAG-N12-SCHED-SMP-PREEMPT-001 preserve the next bounded dependency.",
...
def main() -> int:
    ...
    parser.add_argument("--test-count", type=int, default=866)
