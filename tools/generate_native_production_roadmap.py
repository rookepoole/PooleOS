...
PHASE_EVIDENCE = {
    ...
    "N5": [
        "specs/native-pooleboot-proof.json: POOLEOS-N5-POOLEBOOT-1 bounded unsigned proof contract across N5.1, N5.2, N5.3, and N5.7",
        "native/boot: Poole-authored no_std PE32+ UEFI proof application with raw reviewed firmware bindings, independent serial/debugcon diagnostics, bounded table/map handling, GOP identity, and EFI_SUCCESS return",
        "runtime/native_pooleboot.py and tools/build_native_pooleboot_media.py: deterministic 64 MiB protective-MBR/GPT/FAT32 ordinary-file media with exact fallback EFI inspection and no physical-media output mode",
        "runs/native_pooleboot_readiness.json: 8/8 host tests, 2/2 exact PE builds, 2/2 exact media generations, 2/2 exact QEMU/OVMF runs, eleven ordered markers, 2/2 serial/debugcon matches, 2/2 exact GOP frames, and 15/15 hostile controls",
...
PHASE_GAPS = {
    ...
    "N5": ["A reproducible unsigned PooleBoot proof, PBP1, PBC1, PKELF1, and a separately qualified real PooleKernel product image exist; however, live configuration and kernel-file discovery/read, authenticated artifact selection, firmware page allocation, page-table installation, menu policy, live handoff population and consumption, ExitBootServices transfer, second host, target firmware, physical media, and N5 exit remain open"],
        ...
        implementation_flags.append(
            {
                ...
                or flag_id in {"FLAG-N0-RATIFICATION-SCOPE-001", "FLAG-N2-CPUID-001", "FLAG-N4-PROFILE-001", "FLAG-N4-IPC-MODEL-001", "FLAG-N4-SCHEDULER-MODEL-001", "FLAG-N4-POOLEFS-MODEL-001", "FLAG-N5-POOLEBOOT-PROOF-001", "FLAG-N5-BOOTPROTO-001", "FLAG-N5-BOOTCFG-001", "FLAG-N5-ELF-001", "FLAG-N6-KENTRY-001"}
                else "open",
                "phase_id": phase_id,
...
def main() -> int:
    ...
    parser.add_argument("--test-count", type=int, default=565)
