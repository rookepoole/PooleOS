...
PHASE_EVIDENCE = {
    ...
    "N5": [
        ...
        "runs/native_kernel_load_readiness.json: exact 217/217 aggregate Rust host tests, 2/2 exact PooleBoot builds, 2/2 exact PooleKernel builds, 2/2 exact twelve-file media generations, 2/2 QEMU/OVMF runs, 25 ordered markers, 155 integrated hostile controls, exact retained PINIT1, PREC1, PSYM1, PMCU1, PFWM1, and PPOL1 parsing with payload/route cross-binding and mandatory development denial, exact retained PSM1/PBTP1/PBTS1 parsing with fourteen trust cross-bindings and unsigned-policy denial, two exact post-exit PBLIVE4 PBP1 reconstructions, exact 112-page kernel plus nine retained files/table/32-page guarded-stack/handoff/firmware-record agreement, successful ExitBootServices, zero later firmware calls, and exact guest/oracle agreement",
        ...
        "runs/native-kernel-revalidation-readiness.json: 132/132 kernel Rust tests, 8/8 Python tests, both no_std target builds, nine exact retained files and parsers, 36/36 hostile controls, 32,768/32,768 deterministic mutation rejects, exact unsigned-policy denial, zero authority grants/actions/state writes, and no standalone live-entry claim",
    ...
    "N6": [
        ...
        "runs/native_kernel_entry_readiness.json: 132/132 host tests, 2/2 exact clean linked and canonical builds, 43/43 hostile controls, 969 relative relocations, exact independent Rust/Python loaded bytes, and canonical SHA-256 95DDA27784DA944A9C0F5B04029255EDE4DE1BB0684A8EA10DCFC07E686B59A2",
    ...
    "N7": [
        ...
        "runs/native-kernel-cpu-policy-readiness.json: 132/132 kernel host tests, 2 exact fresh-vars qemu64 QEMU/OVMF runs, 35 ordered markers, 41/41 hostile controls, exact Rust/Python agreement, AuthenticAMD family 15 model 107 stepping 1 observation, 5 MSR reads, zero MSR writes, and zero authority or actions",
        ...
        "runs/native-kernel-privilege-msr-policy-readiness.json: two exact fresh-vars TCG QEMU/OVMF runs, 35 markers, 47/47 hostile controls, 11 support-gated MSR reads, ten observed MCA banks, zero bank reads, zero runtime writes, and an exact linked-image audit of 25 RDMSR and three WRMSR sites that isolates PKSMP1, PKSMP2, PKSMP4, and PKIRQ1 accesses from the read-only PKMSR1 profile, with zero authority and explicit emulator/target/production nonclaims",
    ],
    "N8": [
        ...
        "runs/native-kernel-interrupt-time-readiness.json: 132/132 kernel host tests, two exact fresh-vars qemu64 QEMU/OVMF runs, 36 ordered markers, 58/58 hostile controls, eight timer deliveries, eight EOIs, zero APIC-error or spurious deliveries, three absent MMIO guards, exact LAPIC/HPET mapping revocation, zero AP starts, and zero signatures, authority, actions, or production claims",
        ...
        "runs/native-kernel-smp-first-ap-readiness.json: 132/132 kernel host tests, two exact fresh-vars two-vCPU qemu64 QEMU/OVMF runs, 38 ordered markers, 72/72 hostile controls, exactly one AP started/online/quiesced/parked, fourteen pages and 57,344 bytes scrubbed/verified/released, and zero signatures, authority, target, N8-exit, or production claims",
        ...
        "runs/native-kernel-smp-percpu-runtime-readiness.json: 132/132 kernel host tests, two exact fresh-vars two-vCPU SandyBridge-minus-AVX TCG QEMU/OVMF runs, 42 ordered markers, 19/19 hostile-control categories covering 159 rejected cases, exactly one AP-local descriptor set, three guarded stack classes, 27 gates, one xstate round trip, and 32 pages or 131,072 bytes scrubbed/verified/released with zero signatures, authority, N8-exit, or production claims",
        "specs/native-kernel-smp-ipi-contract.json, native/kernel/src/smp_ipi.rs, runtime/native_kernel_smp_ipi.py, and docs/native-kernel-smp-ipi.md: PKSMP4 retains the six-class development IPI transport and freezes one generation-bound remote one-page TLB invalidation with target-mask and acknowledgement binding, stale-generation and duplicate-ack rejection, bounded offline timeout with same-attempt retry, deferred-reclaim ordering, exact no-reuse-before-retirement evidence, and explicit no-general-shootdown/no-scheduler/no-target/nonproduction boundaries",
        "runs/native-kernel-smp-ipi-readiness.json: 132/132 kernel host tests, two exact fresh-vars two-vCPU SandyBridge-minus-AVX TCG QEMU/OVMF runs, 40 ordered markers, 25/25 hostile-control categories covering 169 rejected cases, six accepted and four denied live deliveries, ten EOIs, one offline-target timeout, one exact AP-side INVLPG, one retired generation, one premature-reclaim rejection, and 34 pages or 139,264 bytes scrubbed/verified/released with zero signatures, authority, N8/N9-exit, or production claims",
    ],
    "N9": [
        ...
        "runs/native-kernel-physical-memory-readiness.json: 2/2 exact qemu64 QEMU/OVMF runs, 45 ordered markers, 191/191 hostile controls, 132/132 kernel host tests, 97 PBP1 entries, 117,859 usable source pages, and 129,119 final managed pages. PKACPI1 validates one RSDP/XSDT and exactly one APIC, FACP, HPET, and MCFG table, copies 600 table bytes into a retained one-page 616-byte snapshot, and binds source checksum 078583AEEFDD6581 plus snapshot checksum 4089A5CFEC81CB41. Boot reclaim admits 11,250 pages under receipt 5DEA9A3BC9E10C18; only after opaque validation/copy/readback evidence does ACPI reclaim admit 11 pages under receipt 60DAA52A8A05ABD6. The stable manager remains five guarded pages; checked pressure reaches a 29-page generation, retires three predecessors totaling 15 pages, records 119 checks, seven triggers, 59 successful cycles, three soft fallbacks, and one hard pre-effect rejection, and binds growth checksum 0xF6CB1CC42ACDAB03. The runs protect 885 loader pages, scrub and verify 11,473 pages and 46,993,408 bytes, perform 5,875,277 physical writes, 5,879,957 reads, and 23,172 temporary PTE writes and invalidations with zero signatures, authority, actions, AML execution, concurrent/SMP allocation, target, or production claims",
        ...
        "runs/native-kernel-virtual-memory-readiness.json: 2/2 exact qemu64 QEMU/OVMF runs, 40 ordered markers, 46/46 hostile controls, 117,858 admitted pages in eleven sparse ranges with 12,907 hole pages, 237 direct leaf tables, one direct directory and 243 total table pages, coverage checksum 0x6E870613F130BA60, 367,445 physical table writes, 950,466 temporary-PTE writes and invalidations, two CR3 writes, three active local invalidation receipts, one exact generation-retirement receipt, zero remote shootdowns, and zero signatures, authority, or actions",
        "runs/native-kernel-smp-ipi-readiness.json: PKSMP4 records two exact 40-marker two-vCPU runs, 25/25 hostile-control categories covering 169 rejects, one AP-owned root and page, one exact linked-image-audited AP-side INVLPG, target/ack mask 0x2, generation 1-to-2 retirement, one premature-reclaim rejection, one offline-target timeout with same-attempt retry, and exact scrub/release of 34 pages or 139,264 bytes without general SMP, target, authority, N8/N9-exit, or production claims",
    ...
    "N36": ["Cycle 140 host baseline: TEST_COUNT tests with one Windows symlink-permission skip", "native binary parser, reproduction, leakage, malformed, substitution, governance, hardware, Tier 0, bounded-model, deterministic boot-media, PBP1/PBC1/PSM1/PBART1, six inner-format, PBTRUST1/PBSTATE1, PKELF1, PKENTRY1, PKLOAD6/PBLIVE4/PKMAP2/PBEXIT1, PKREVAL1, PKXFER1, PKTRAP1, PKCPU1, PKERR1, PKXSTATE1, PKXEXC1, PKMSR1, PKPMM7/PKACPI1, PKVM1, PKVM3, PKIRQ1, PKSMP1, PKSMP2, and PKSMP4 transport/shootdown/source/machine-code/zero-authority controls, canonical-LF readiness regression controls, PooleGlyph roadmap bindings, Doctor external-report nonmutation, and collector-smoke negatives"],
    "N37": ["Cycle 140 consistency release gate: 97/97 checks over 92 artifacts", "content-addressed source, objectives and governance receipts, native toolchain, bounded hardware/Tier 0/model evidence, PBTRUST1/PBSTATE1, bounded PooleBoot, PBP1/PBC1/PSM1/PBART1 and six inner formats, PKELF1, PKENTRY1, PKLOAD6/PKMAP2/PBEXIT1, PKREVAL1, PKXFER1, PKTRAP1, PKCPU1, PKERR1, PKXSTATE1, PKXEXC1, PKMSR1, PKPMM7/PKACPI1, PKVM1, PKVM3, PKIRQ1, PKSMP1, PKSMP2, PKSMP4, PooleGlyph planning artifacts, and retained historical consistency artifacts"],
}
...
PROGRAM_GAPS = [
    ...
    "A real reproducible 112-page PooleKernel image, PKENTRY1 intake, allocation-free PKREVAL1 verifier, bounded early diagnostics, opt-in QEMU-only live entry, BSP-only PKTRAP1 descriptor/exception containment, bounded BSP PKXSTATE1 x87/SSE ownership, PKPMM7 scrubbed lifecycle and checked repeated ledger growth, guarded stable-manager and generation-owned active-ledger transactions, PKACPI1 required-table snapshot/reclaim evidence, PKVM3 sparse PMM-owned direct-map evidence, PKIRQ1 one-BSP timer evidence, PKSMP1 one-AP lifecycle evidence, PKSMP2 one-AP processor-local runtime evidence, and PKSMP4 fixed IPI plus one-page remote-invalidation evidence exist, but authenticated boot trust, measured boot, production transfer, production capability authority, general multi-AP shootdown, scheduler ownership, retained crash evidence, target execution, and N6/N7/N8/N9 exit remain open",
    "PKERR1 freezes a pure exact-target CPU/errata rejection policy, PKXSTATE1 proves bounded x87/SSE standard-XSAVE ownership, PKXEXC1 proves deliberate #MF/#XM recovery plus terminal test-only #NM rejection with a linked scope audit under WHPX, and PKMSR1 proves only a read-only qemu64 BSP system-linkage/global-MCA/unsupported-PMU observation. PKPMM7 supplies bounded physical ownership, scrubbed lifecycle transactions, a stable guarded five-page manager, external generation-owned active ledgers, checked automatic growth with retirement, bounded-window fallback and pre-effect rejection, streamed lifecycle-gated Boot Services reclaim, and PKACPI1-gated ACPI reclaim after required-table validation and retained copy/readback; PKVM1 supplies inactive page-table transactions; PKVM3 proves one-BSP activation/restoration of a complete-profile PMM-owned sparse direct map; PKIRQ1 proves one bounded local timer transaction; PKSMP1 proves one first-AP start/quiesce/park lifecycle; PKSMP2 proves one AP-local GDT/TSS/IDT, guarded-stack, x87/SSE-owner, and interrupt-vector runtime transaction; and PKSMP4 proves six fixed IPI classes plus one AP-side one-page INVLPG and one deferred generation retirement. No target-qualified complete native CPU policy, applicable Model 40h-4Fh errata authority, direct numeric client microcode floor or ratified replacement, target-specific privileged-MSR semantics, syscall/MCE/PMU activation, AVX/extended state, user-task exception delivery, scheduler or migration integration, general interrupt routing/time services, production capability authority, multi-AP operation, general SMP shootdown, AML or complete ACPI resource-graph execution, complete kernel/user address spaces, heap, MMIO/PAT/MTRR qualification, interrupt-context or concurrent allocator, general pressure, or OOM implementation exists",
...
def make_roadmap(test_count: int, status_date: str) -> dict:
    ...
    for flag_id, flag_class, phase_id, closure in FLAGS:
        ...
        if flag_id == "FLAG-N8-SMP-IPI-001":
            evidence.extend(
                [
...
def main() -> int:
    ...
    parser.add_argument("--test-count", type=int, default=823)
