...
PHASE_EVIDENCE = {
    ...
    "N5": [
        ...
        "runs/native-kernel-revalidation-readiness.json: 165/165 kernel Rust tests, 8/8 Python tests, both no_std target builds, nine exact retained files and parsers, 36/36 hostile controls, 32,768/32,768 deterministic mutation rejects, exact unsigned-policy denial, zero authority grants/actions/state writes, and no standalone live-entry claim",
    ...
    "N6": [
        ...
        "runs/native_kernel_entry_readiness.json: 165/165 host tests, 2/2 exact clean linked and canonical builds, 43/43 hostile controls, 1,125 relative relocations, exact independent Rust/Python loaded bytes, and canonical SHA-256 FC13CF79E94318FAE10AFF9E7198036B30C587CF2BFD10457A045ACC6EB7665E",
    ...
    "N7": [
        ...
        "runs/native-kernel-cpu-policy-readiness.json: 165/165 kernel host tests, 2 exact fresh-vars qemu64 QEMU/OVMF runs, 35 ordered markers, 41/41 hostile controls, exact Rust/Python agreement, AuthenticAMD family 15 model 107 stepping 1 observation, 5 MSR reads, zero MSR writes, and zero authority or actions",
        ...
        "runs/native-kernel-privilege-msr-policy-readiness.json: two exact fresh-vars TCG QEMU/OVMF runs, 35 markers, 47/47 hostile controls, 11 support-gated MSR reads, ten observed MCA banks, zero bank reads, zero runtime writes, and an exact linked-image audit of 43 RDMSR and three WRMSR sites that isolates PKSMP1, PKSMP2, PKSMP5, PKSCHED1, PKSCHED2, PKSCHED3, and PKIRQ1 accesses from the read-only PKMSR1 profile, with zero authority and explicit emulator/target/production nonclaims",
    ],
    "N8": [
        ...
        "runs/native-kernel-interrupt-time-readiness.json: 165/165 kernel host tests, two exact fresh-vars qemu64 QEMU/OVMF runs, 36 ordered markers, 58/58 hostile controls, eight timer deliveries, eight EOIs, zero APIC-error or spurious deliveries, three absent MMIO guards, exact LAPIC/HPET mapping revocation, zero AP starts, and zero signatures, authority, actions, or production claims",
        ...
        "runs/native-kernel-smp-first-ap-readiness.json: 165/165 kernel host tests, two exact fresh-vars two-vCPU qemu64 QEMU/OVMF runs, 38 ordered markers, 72/72 hostile controls, exactly one AP started/online/quiesced/parked, fourteen pages and 57,344 bytes scrubbed/verified/released, and zero signatures, authority, target, N8-exit, or production claims",
    ...
    "N9": [
        ...
        "runs/native-kernel-physical-memory-readiness.json: 2/2 exact qemu64 QEMU/OVMF runs, 45 ordered markers, 191/191 hostile controls, 165/165 kernel host tests, 97 PBP1 entries, 117,835 usable source pages, and 129,095 final managed pages. PKACPI1 validates one RSDP/XSDT and exactly one APIC, FACP, HPET, and MCFG table, copies 600 table bytes into a retained one-page 616-byte snapshot, and binds source checksum 078583AEEFDD6581 plus snapshot checksum 4089A5CFEC81CB41. Boot reclaim admits 11,250 pages under receipt 5DEA9A3BC9E10C18; only after opaque validation/copy/readback evidence does ACPI reclaim admit 11 pages under receipt 60DAA52A8A05ABD6. The stable manager remains five guarded pages; checked pressure reaches a 29-page generation, retires three predecessors totaling 15 pages, records 119 checks, seven triggers, 59 successful cycles, three soft fallbacks, and one hard pre-effect rejection, and binds growth checksum 0x17C9628B9F0CAAE8. The runs protect 909 loader pages, scrub and verify 11,473 pages and 46,993,408 bytes, perform 5,875,277 physical writes, 5,879,957 reads, and 23,172 temporary PTE writes and invalidations with zero signatures, authority, actions, AML execution, concurrent/SMP allocation, target, or production claims",
        ...
        "runs/native-kernel-virtual-memory-readiness.json: 2/2 exact qemu64 QEMU/OVMF runs, 40 ordered markers, 46/46 hostile controls, 117,834 admitted pages in eleven sparse ranges with 12,931 hole pages, 237 direct leaf tables, one direct directory and 243 total table pages, coverage checksum 0xBEFD9B199E2AA16C, 367,421 physical table writes, 950,658 temporary-PTE writes and invalidations, two CR3 writes, three active local invalidation receipts, one exact generation-retirement receipt, zero remote shootdowns, and zero signatures, authority, or actions",
    ...
    "N12": [
        ...
        "runs/native-kernel-scheduler-readiness.json: 14/14 scheduler Rust tests within 165/165 kernel host tests, four exact Rust/Python host-probe receipts over 4,096 steps, 1,761 dispatches and 2,334 migrations, two exact 17-marker qemu64 BSP runs, eight live dispatches, sixteen context transitions, 32,768 stack bytes cleared, and 28/28 hostile-control categories covering 115 rejected cases with zero live AP dispatch, ring-3/address-space switch, authority, N12-exit, or production claims",
        ...
        "ADD-N12-SCHED-SMP-001: integrate AP-local run queues and workers, remote reschedule IPI ownership, generation-safe cross-CPU wake/migration, offline and timeout rollback, topology-aware balancing, per-CPU idle ownership, and exact park/scrub/release teardown before SMP scheduling can be promoted",
    ],
    ...
    "N36": ["Cycle 144 host baseline: TEST_COUNT tests with one Windows symlink-permission skip", "native binary parser, reproduction, leakage, malformed, substitution, governance, hardware, Tier 0, bounded-model, deterministic boot-media, PBP1/PBC1/PSM1/PBART1, six inner-format, PBTRUST1/PBSTATE1, PKELF1, PKENTRY1, PKLOAD6/PBLIVE4/PKMAP2/PBEXIT1, PKREVAL1, PKXFER1, PKTRAP1, PKCPU1, PKERR1, PKXSTATE1, PKXEXC1, PKMSR1, PKPMM7/PKACPI1, PKVM1, PKVM3, PKIRQ1, PKSMP1, PKSMP2, PKSMP5, PKSCHED1, PKSCHED2, and PKSCHED3 deferred-work/source/machine-code/zero-authority controls, canonical-LF readiness regression controls, PooleGlyph roadmap bindings, Doctor external-report nonmutation, and collector-smoke negatives"],
    "N37": ["Cycle 144 consistency release gate: 100/100 checks over 95 artifacts", "content-addressed source, objectives and governance receipts, native toolchain, bounded hardware/Tier 0/model evidence, PBTRUST1/PBSTATE1, bounded PooleBoot, PBP1/PBC1/PSM1/PBART1 and six inner formats, PKELF1, PKENTRY1, PKLOAD6/PKMAP2/PBEXIT1, PKREVAL1, PKXFER1, PKTRAP1, PKCPU1, PKERR1, PKXSTATE1, PKXEXC1, PKMSR1, PKPMM7/PKACPI1, PKVM1, PKVM3, PKIRQ1, PKSMP1, PKSMP2, PKSMP5, PKSCHED1, PKSCHED2, PKSCHED3, PooleGlyph planning artifacts, and retained historical consistency artifacts"],
}
...
PHASE_GAPS = {
    ...
    "N8": ["PKIRQ1 proves a bounded one-BSP qemu64 local xAPIC and HPET substrate; PKSMP1 and PKSMP2 prove one-AP startup and one AP-local runtime; and PKSMP5 scales that path to an exact four-vCPU topology with three private AP runtimes, dynamic local masks, aggregate target/ack mask 0xE, partial-start rollback, fresh retry, fixed development IPI delivery, final park, and exact cleanup. I/O APIC routing, MSI/MSI-X, invariant-TSC calibration, watchdog and timer queues, public time domains, general topology and x2APIC, production capability authority, general SMP shootdown, additional failure interleavings, panic-path transactional recovery, scheduler CPU ownership, target hardware, and the N8 exit gate remain open"],
...
FLAGS = [
    ...
    ("FLAG-N12-SCHED-SMP-001", "REQUIRED", "N12", "Integrate and qualify AP-local scheduler queues and deferred workers with explicit CPU ownership transfer, remote reschedule IPI delivery and acknowledgement, generation-safe cross-CPU wake and migration, offline and timeout rollback, topology-aware balancing, per-CPU idle ownership, exact teardown, independent oracle agreement, and no ring-3, address-space, target, N12-exit, or production overclaim"),
...
def make_roadmap(test_count: int, status_date: str) -> dict:
    ...
    for flag_id, flag_class, phase_id, closure in FLAGS:
        ...
        if flag_id in {"FLAG-N12-SCHED-DEFERRED-001", "FLAG-N12-SCHED-SMP-001"}:
            evidence.extend(
                [
...
def main() -> int:
    ...
    parser.add_argument("--test-count", type=int, default=844)
