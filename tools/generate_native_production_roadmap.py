...
PHASE_EVIDENCE = {
    ...
    "N5": [
        ...
        "runs/native-kernel-revalidation-readiness.json: 70/70 Rust tests, 8/8 Python tests, both no_std target builds, nine exact retained files and parsers, 36/36 hostile controls, 32,768/32,768 deterministic mutation rejects, exact unsigned-policy denial, zero authority grants/actions/state writes, and no standalone live-entry claim",
    ...
    "N6": [
        ...
        "runs/native_kernel_entry_readiness.json: 79/79 host tests, 2/2 exact clean linked and canonical builds, 43/43 hostile controls, 593 relative relocations, exact independent Rust/Python loaded bytes, and a current bound canonical SHA-256",
    ...
    "N9": [
        "specs/native-kernel-physical-memory-contract.json, specs/native-kernel-physical-memory-contract.schema.json, and docs/native-kernel-physical-memory.md: PKPMM6 freezes exact PBP1 intake, UEFI source-kind validation, usable-only initial ownership, page-zero exclusion, DMA/DMA32/Normal zones, generation-safe scrubbed transactions, a stable five-page guarded manager, external generation-owned guarded active ledgers, checked pressure-triggered repeated growth, bounded-window fallback/rejection, and lifecycle-gated boot-services versus ACPI reclaim without concurrency, interrupt-context, SMP, or production claims",
        "native/kernel/src/physical_memory.rs, native/kernel/src/main.rs, runtime/native_kernel_physical_memory.py, tools/qualify_native_kernel_physical_memory.py, and tests/test_native_kernel_physical_memory.py: allocation-free no_std kernel allocator, stable-manager bootstrap migration, two alternate 32-page guarded ledger windows, exact-demand pressure checks with rollback/retry headroom, repeated transactional preflight/map/copy/seal/switch/revoke/scrub-retire growth, retryable postcommit retirement, soft bounded-window fallback, hard pre-effect rejection, streamed three-pass reclaim, independent transcript/oracle validation, source audit, PBP1 substitution controls, and host mapping/content/pressure/retirement fault injection",
        "runs/native-kernel-physical-memory-readiness.json: 2/2 exact qemu64 QEMU/OVMF runs, 43 ordered markers, 147/147 hostile controls, 84/84 kernel host tests, 97 PBP1 entries, 117,911 usable source pages, and 129,160 final managed pages. The stable manager occupies five guarded pages; checked pressure grows active ledgers across 4/8/15/29-page generations with final capacities 2048/256/2048/128/16, three automatic growths, four soft fallbacks, and one host-proved hard pre-effect rejection of the 58-page next layout. Three predecessors totaling 27 pages are revoked, scrubbed, and retired with checksum 0xF7AD111CA266071D. Reclaim admits 11,250 Boot Services pages while keeping 11 ACPI pages held. The runs protect 833 loader pages, scrub and verify 11,462 pages and 46,948,352 bytes, perform 5,869,568 physical writes, 5,870,592 reads, and 22,798 temporary PTE writes and invalidations with zero signatures, authority, or actions",
        ...
        "runs/native-kernel-virtual-memory-readiness.json: 2/2 exact qemu64 QEMU/OVMF runs, 40 ordered markers, 46/46 hostile controls, eight table pages and one data page, 8,720 physical table writes, 5,560 bootstrap temporary-PTE writes and invalidations after the Cycle 132 retained-layout rebind, two CR3 writes, three active local invalidation receipts, exact original-root restoration and allocation release, zero shootdowns, and zero signatures, authority, or actions",
...
PHASE_GAPS = {
    ...
    "N6": ["A reproducible real 78-page PooleKernel PKELF1 image, PKENTRY1 intake, allocation-free PKREVAL1 verifier, bounded diagnostics, an opt-in QEMU-only live transfer, and BSP-only descriptor/exception, CPU, xstate, MSR, physical-memory, and virtual-memory development profiles now exist on one host; authenticated boot trust, measured boot, production transfer, final framebuffer remap/revocation, complete per-CPU runtime state, retained crash paths, target execution, and N6 exit remain open"],
    ...
    "N9": ["PKPMM6 consumes the live PBP1 map and provides bounded generation-safe physical ownership, scrub-before-allocation/reuse, a stable guarded five-page manager, external generation-owned guarded active ledgers, checked automatic growth across four generations, verified predecessor retirement, bounded-window soft fallback and pre-effect hard rejection, and exact post-ExitBootServices Boot Services admission while ACPI remains held. PKVM1 supplies inactive page-table transactions. PKVM2 adds a kernel-complete one-BSP candidate root, a bounded nine-page direct map, exact CR3 activation/restoration, architectural Accessed/Dirty handling, and three active local invalidation receipts. Complete ACPI consumer integration, a complete generation-owned direct map, SMP TLB shootdown and deferred reclaim, huge pages, PCID, COW, user faults, pager IPC, heaps/object caches, MMIO/PAT/MTRR qualification, interrupt-context and concurrent allocation, pressure/OOM policy beyond bounded metadata pressure, target hardware, and the N9 exit gate remain open"],
...
PROGRAM_GAPS = [
    ...
    "A real reproducible 78-page PooleKernel image, PKENTRY1 intake, allocation-free PKREVAL1 verifier, bounded early diagnostics, opt-in QEMU-only live entry, BSP-only PKTRAP1 descriptor/exception containment, bounded BSP PKXSTATE1 x87/SSE ownership, PKPMM6 scrubbed lifecycle and checked repeated ledger growth, guarded stable-manager and generation-owned active-ledger transactions, and PKVM2 active-root evidence exist, but authenticated boot trust, measured boot, production transfer, complete per-CPU runtime state, retained crash evidence, target execution, and N6/N7/N9 exit remain open",
    "PKERR1 freezes a pure exact-target CPU/errata rejection policy, PKXSTATE1 proves bounded x87/SSE standard-XSAVE ownership, PKXEXC1 proves deliberate #MF/#XM recovery plus terminal test-only #NM rejection with a linked scope audit under WHPX, and PKMSR1 proves only a read-only qemu64 BSP system-linkage/global-MCA/unsupported-PMU observation. PKPMM6 supplies bounded physical ownership, scrubbed lifecycle transactions, a stable guarded five-page manager, external generation-owned active ledgers, checked automatic four/eight/fifteen/twenty-nine-page growth with retirement, bounded-window fallback and pre-effect rejection, and streamed lifecycle-gated Boot Services reclaim while holding ACPI pages; PKVM1 supplies inactive page-table transactions; and PKVM2 proves one-BSP candidate-root activation/restoration with a nine-page direct map and three local invalidation receipts. No target-qualified complete native CPU policy, applicable Model 40h-4Fh errata authority, direct numeric client microcode floor or ratified replacement, target-specific privileged-MSR semantics, syscall/per-CPU/MCE/PMU activation, AVX/extended state, user-task exception delivery, scheduler/AP/migration integration, interrupts, time, SMP, complete ACPI consumer integration, complete kernel/user address spaces, SMP TLB shootdown, heap, MMIO/cache-alias qualification, interrupt-context or concurrent allocator, general pressure, or OOM implementation exists",
    ...
    "PKPMM6 supplies bounded one-BSP physical-page ownership, scrub-before-allocation and scrub-before-reuse with full readback and fault rollback, a stable guarded five-page manager, external generation-owned guarded ledgers, checked automatic repeated growth, verified retirement, bounded-window fallback and pre-effect rejection, and exact post-ExitBootServices Boot Services reclaim while ACPI remains held. PKVM1 supplies inactive four-level 4 KiB transactions, and PKVM2 supplies one bounded active root, nine-page direct map, exact CR3 restoration, and three local invalidation receipts. Complete ACPI consumer integration, a complete generation-owned direct map, SMP TLB shootdown and deferred reclaim, huge pages, PCID, COW, user faults, pager IPC, cache/MMIO qualification, heaps/object caches, interrupt-context allocation, concurrency/SMP, general pressure and OOM policy, scheduler, task, syscall, capability, IPC, isolation, and asynchronous I/O remain open",
...
def main() -> int:
    ...
    parser.add_argument("--test-count", type=int, default=787)
    parser.add_argument("--status-date", default="2026-07-24")
