...
PHASE_EVIDENCE = {
    ...
    "N5": [
        ...
        "native/boot: Poole-authored no_std PE32+ UEFI application with reviewed firmware bindings, live bounded filesystem/config/kernel/PBART1 intake, exact retained-page inner parsing, GOP identity, retained PKMAP2 kernel/artifact/table/guarded-stack/handoff storage, final PBLIVE3 production, bounded PBEXIT1 retry, successful ExitBootServices, direct post-exit serial/debugcon diagnostics, and permanent stop before transfer",
        ...
        "specs/native-boot-trust-contract.json and docs/native-boot-trust.md: PBTRUST1 separates immutable PBTP1 trust policy, mutable PBTS1 acceptance state, and PREC1 boot-attempt state; fixes exact record, signer, revocation, role, rollback-floor, generation, copy, previous-state, external-evidence, denial-order, and no-authority boundaries",
        "native/trust and runtime/native_boot_trust.py: allocation-free no_std Rust parser/authorization model plus independent Python encoder, parser, authorization oracle, and deterministic differential harness",
        "runs/native_boot_trust_readiness.json: 4/4 Rust tests, both no_std targets, one PooleBoot UEFI integration build, 88/88 controls, 24,576 Rust/Python differential cases, fourteen exact bindings, mandatory unsigned-policy denial, zero signature verification, authority grants, and state writes, and explicit rejection of the ESP state candidate as persistent authority",
        ...
        "runs/native_pooleboot_readiness.json: 8/8 host tests, 2/2 exact 202,752-byte PE builds, 2/2 exact twelve-file media generations, 2/2 exact QEMU/OVMF runs, twenty-five ordered markers, 2/2 serial/debugcon matches, 2/2 exact GOP frames, and 148/148 integrated hostile controls with exact retained-set and PBTRUST1 denial binding",
        ...
        "specs/native-kernel-load-contract.json and docs/native-kernel-load.md: PKLOAD6 freezes live UEFI intake, PSM1/PBART1 digest binding, exact PBASET1 loading and retention, six-format retained-page parsing and denial, PBTRUST1 policy/state candidate parsing, fourteen cross-bindings and exact unsigned-policy denial, retained PKMAP2 storage, final PBLIVE3 production, bounded PBEXIT1 retry, successful ExitBootServices, stop-before-transfer, trust, semantics, and nonclaim boundaries",
        ...
        "runs/native_kernel_load_readiness.json: 85/85 Rust host tests, 2/2 exact PooleBoot builds, 2/2 exact PooleKernel builds, 2/2 exact twelve-file media generations, 2/2 QEMU/OVMF runs, 25 ordered markers, 148/148 integrated hostile controls, exact retained PINIT1, PREC1, PSYM1, PMCU1, PFWM1, and PPOL1 parsing with payload/route cross-binding and mandatory development denial, exact PBTP1/PBTS1 candidate parsing with fourteen cross-bindings and unsigned-policy denial, two exact post-exit PBP1 reconstructions, exact 48-page kernel plus six artifact/table/stack/handoff retained-range agreement, successful ExitBootServices, zero later firmware calls, and exact guest/oracle agreement",
    ],
    "N6": [
...
PHASE_GAPS = {
    ...
    "N5": ["A reproducible unsigned PooleBoot, PBP1, PBC1, PSM1, PKELF1, PBART1, PINIT1, PREC1, PSYM1, PMCU1, PFWM1, PPOL1, PKMAP2, PBEXIT1, and real PooleKernel path now loads and retains the exact seven-artifact development profile plus table/guarded-stack/handoff storage; reparses all six exact retained inner files, cross-binds PPOL1 payload digests and PINIT1 capability routes, requires all six development gates to deny at the missing outer signature, and reports zero authority, actions, state writes, or hardware observations; binds final post-exit PBP1 bytes to the successful memory map; calls ExitBootServices; proves zero later firmware calls; and stops before transfer. Artifact authentication and authenticated rollback state, PooleKernel retained-byte reparsing and capability creation or activation, recovery execution, symbol consumption, policy application, real vendor-container validation and licensed payload intake, live FMP/ESRT/PLDM inventory, privileged per-processor revision observation, signature-backed trusted selection, initial-system execution, final framebuffer cache policy, live menu/rollback policy, transferable kernel-entry PBP1, retained CR3/RSP installation, kernel consumption, second host, target firmware, physical media, and N5 exit remain open"],
...
FLAGS = [
    ...
    ("FLAG-N5-INNER-TRUST-STATE-001", "BLOCKER", "N5", "Define and enforce the authenticated artifact-trust and monotonic persistent-state boundary over the exact retained bytes before any PooleBoot authority decision, while preserving the no-key, no-signing, no-mutation authorization boundary and requiring independent PooleKernel revalidation before capability creation"),
...
PROGRAM_GAPS = [
    ...
    "A reproducible unsigned PooleBoot proof application boots twice with deterministic twelve-file GPT/FAT32 media, exact GOP frames, retained PKMAP2 kernel/six-artifact/table/guarded-stack/handoff storage, independently reconstructed final PBLIVE3 bytes, bounded PBEXIT1 retry, successful ExitBootServices, zero later firmware calls, and permanent stop before transfer. It reparses PINIT1, PREC1, PSYM1, synthetic-only PMCU1, synthetic-only PFWM1, and qualification-only PPOL1 directly from retained firmware pages; cross-binds PPOL1's five payload digests and eleven PINIT1 routes; parses separate PBTRUST1 immutable-policy and mutable acceptance-state candidates; binds exact manifest, kernel, retained set, revocation set, role mask, policy version, secure-version floor, trust epoch, and state generation; and denies at the unsigned-policy gate with zero authority or writes. Policy signature verification, authenticated revocation, a redundant authenticated monotonic writable state backend, deterministic backend selection/repair/migration under power loss, Secure Boot-state verification, independent PooleKernel retained-byte and selected-state revalidation, capability creation, activation or update application, policy application, recovery execution or symbol consumption, licensed real vendor payload intake and validation, live FMP/ESRT/PLDM inventory, privileged per-processor revision observation, initial-system execution, final framebuffer cache policy, kernel-entry profile, retained CR3/RSP installation, kernel consumption, target-firmware and physical-media qualification, and N5 exit remain open",
...
def make_roadmap(test_count: int, status_date: str) -> dict:
        ...
        implementation_flags.append(
            {
                ...
                or flag_id in {"FLAG-N0-RATIFICATION-SCOPE-001", "FLAG-N2-CPUID-001", "FLAG-N4-PROFILE-001", "FLAG-N4-IPC-MODEL-001", "FLAG-N4-SCHEDULER-MODEL-001", "FLAG-N4-POOLEFS-MODEL-001", "FLAG-N5-POOLEBOOT-PROOF-001", "FLAG-N5-BOOTPROTO-001", "FLAG-N5-BOOTCFG-001", "FLAG-N5-ELF-001", "FLAG-N5-KLOAD-001", "FLAG-N5-MANIFEST-001", "FLAG-N5-PBP1-LIVE-001", "FLAG-N5-KMAP-001", "FLAG-N5-HANDOFF-EXIT-001", "FLAG-N5-INIT-SYSTEM-001", "FLAG-N5-INIT-BUNDLE-001", "FLAG-N5-RECOVERY-BUNDLE-001", "FLAG-N5-SYMBOL-BUNDLE-001", "FLAG-N5-MICROCODE-BUNDLE-001", "FLAG-N5-FIRMWARE-BUNDLE-001", "FLAG-N5-POLICY-BUNDLE-001", "FLAG-N5-INIT-SEMANTICS-001", "FLAG-N5-INNER-PARSE-001", "FLAG-N5-INNER-TRUST-CONTRACT-001", "FLAG-N6-KENTRY-001"}
                else "open",
                "phase_id": phase_id,
    ...
    return {
        ...
        "execution_protocol": {
            ...
            "last_updated_cycle": 115,
            "selected_move_id": "N5-INNER-TRUST-CONTRACT-001",
            "immediate_next_move_id": "N0-HW-KEY-ACQUIRE-001",
            "owner_independent_next_move_id": "N5-INNER-TRUST-BACKEND-001",
            "required_records": [
...
def main() -> int:
    ...
    parser.add_argument("--test-count", type=int, default=709)
