...
PHASE_EVIDENCE = {
    ...
    "N5": [
        ...
        "native/boot: Poole-authored no_std PE32+ UEFI application with reviewed firmware bindings, live bounded filesystem/config/kernel/PBART1/PBTP1/PBTS1 intake, exact retained-page inner parsing, GOP identity, retained PKMAP2 kernel/six-artifact/PSM1/trust-policy/trust-state/table/guarded-stack/handoff storage, final PBLIVE3 production, bounded PBEXIT1 retry, successful ExitBootServices, direct post-exit serial/debugcon diagnostics, and permanent stop before transfer",
        ...
        "runs/native-kernel-revalidation-readiness.json: 13/13 Rust tests, 8/8 Python tests, both no_std target builds, nine exact retained files and parsers, 36/36 hostile controls, 32,768/32,768 deterministic mutation rejects, exact unsigned-policy denial, zero authority grants/actions/state writes, and no live kernel-entry claim",
    ],
    "N6": [
        ...
        "runs/native_kernel_entry_readiness.json: 13/13 host tests, 2/2 exact clean linked and canonical builds, 43/43 hostile controls, 180,224 canonical bytes in a 262,144-byte image, entry offset 0x8000, 302 relative relocations, exact independent Rust/Python loaded bytes, and canonical SHA-256 5CBB39B4BFF9A23E8D65E3115FE536D4CDB13EAA372E8DAA5071F1530210132E",
    ...
    "N36": ["Cycle 117 host baseline: 724 tests with one Windows symlink-permission skip", "native binary parser, reproduction, leakage, malformed, substitution, objectives, ADR-signing, ratification-scope, frozen owner-packet and completed owner-response omission/staleness/placeholder/custody/private-material/authorization controls, hardware privacy, malformed-CPUID, Tier 0 profile/provenance/path/overclaim, bounded-model multi-case mutation/trace/claim controls including six independent PooleFS mutants, deterministic GPT/FAT32 inspection, media path-policy rejection, PE mutation, marker/frame, PBP1, PBC1, PSM1, PBART1, PINIT1 parser/graph/activation, PREC1 policy/state/transition/receipt/activation, PSYM1 identity/address/privacy/parser/lookup/debug-correspondence/activation, PMCU1 identity/layout/digest/revision/selection/rollback/apply-plan/post-verify/activation, PFWM1 identity/layout/dependency/version-floor/authority/dry-run/post-reset, PPOL1 mode/precedence/attenuation/cross-binding/receipt/activation, retained-page six-format parsing/digest/denial controls, PBTRUST1 policy/state/authorization/binding/rollback/external-evidence and PBSTATE1 anchor/selection/repair/migration/power-loss controls, PKELF1, PKENTRY1, PKLOAD6/PBLIVE3/PKMAP2/PBEXIT1 producer controls, and PKREVAL1 role/locator/size/digest/binding/substitution/mutation/denial controls, PooleGlyph roadmap bindings, Doctor external-report nonmutation, and collector-smoke negatives"],
    "N37": ["Cycle 117 consistency release gate: 84/84 checks over 79 artifacts", "content-addressed source, objectives-readiness, scope-hardened ADR-readiness, frozen N0 owner packet, deterministic owner-response receipt, native-toolchain, bounded hardware-readiness, native Tier 0 readiness, native model-readiness, PBTRUST1/PBSTATE1 readiness, bounded PooleBoot readiness, PBP1 readiness, PBC1 readiness, PSM1 readiness, PBART1 readiness, PINIT1 readiness, PREC1 readiness, PSYM1 readiness, PMCU1 readiness, PFWM1 readiness, PPOL1 readiness, retained inner-set parsing, PKELF1 readiness, PKENTRY1 readiness, PKLOAD6/PKMAP2/PBEXIT1 readiness, PKREVAL1 readiness, and PooleGlyph planning artifacts"],
}


PHASE_GAPS = {
    "N0": [
        "The completed response records exact owner direction for ADR-0003, ADR-0004, and all 38 objective definitions, but the selected hardware-backed governance key is unavailable and no public key, signature, tag, release publication, cryptographic ratification, or production promotion is authorized; Standing Authority Amendment V2 permits only fully gated clean source PR merges",
    ...
    "N5": ["A reproducible unsigned PooleBoot, PBP1, PBC1, PSM1, PKELF1, PBART1, PINIT1, PREC1, PSYM1, PMCU1, PFWM1, PPOL1, PBTRUST1/PBSTATE1, PKMAP2, PBEXIT1, PKREVAL1, and real PooleKernel path now loads and retains the exact six-artifact development profile plus exact PSM1/PBTP1/PBTS1 files and table/guarded-stack/handoff storage; reparses all six exact retained inner files in PooleBoot; independently reparses all nine retained files in allocation-free PooleKernel code under host qualification; cross-binds policy, routes, and trust candidates; requires exact unsigned-policy denial; and reports zero authority, actions, state writes, or hardware observations. PooleBoot binds final post-exit PBP1 bytes to the successful memory map, calls ExitBootServices, proves zero later firmware calls, and stops before transfer. Artifact authentication and authenticated rollback state, a real cryptographic monotonic writable provider, live PooleKernel entry and PKREVAL1 execution, capability creation or activation, recovery execution, symbol consumption, policy application, real vendor-container validation and licensed payload intake, live FMP/ESRT/PLDM inventory, privileged per-processor revision observation, signature-backed trusted selection, initial-system execution, final framebuffer cache policy, live menu/rollback policy, retained CR3/RSP installation, second host, target firmware, physical media, and N5 exit remain open"],
    "N6": ["A reproducible real 64-page PooleKernel PKELF1 image, candidate PKENTRY1 intake, allocation-free PKREVAL1 verifier, bounded ring/COM1/framebuffer diagnostics, and panic classes exist only as single-host product and host-executed evidence; boot trust, measured boot, final CR3/RSP installation, live transfer and verifier execution, framebuffer mapping proof, GDT/IDT/TSS, exception and retained-crash paths, kernel runtime, target execution, and N6 exit remain open"],
...
FLAGS = [
    ...
    ("FLAG-N5-INNER-TRUST-STATE-001", "BLOCKER", "N5", "Implement and enforce a real cryptographic monotonic writable provider, authenticated redundant PBTS1 persistence, repair and migration execution, rollback floors, revocation, and Secure Boot evidence before any PooleBoot authority decision, while preserving the no-key, no-signing, no-mutation authorization boundary"),
...
PROGRAM_GAPS = [
    ...
    "A reproducible unsigned PooleBoot proof application boots twice with deterministic twelve-file GPT/FAT32 media, exact GOP frames, retained PKMAP2 kernel/PSM1/six-artifact/PBTP1/PBTS1/table/guarded-stack/handoff storage, independently reconstructed ten-descriptor final PBLIVE3 bytes, bounded PBEXIT1 retry, successful ExitBootServices, zero later firmware calls, and permanent stop before transfer. It reparses PINIT1, PREC1, PSYM1, synthetic-only PMCU1, synthetic-only PFWM1, qualification-only PPOL1, PBTP1, and PBTS1 directly from retained firmware pages; cross-binds manifest, payload, route, trust, rollback, and state facts; and denies at the unsigned-policy gate with zero authority or writes. PBSTATE1 separately models authenticated monotonic-anchor validation, deterministic redundant-copy selection, rollback/future rejection, repair/migration planning, and nine interrupted-transition recovery boundaries with no performed effects. PKREVAL1 independently repeats all nine retained-file parses and binding reconstruction in host-executed allocation-free PooleKernel code and rejects 32,768 mutations with zero authority, actions, or writes. Policy signature verification, authenticated revocation, a real cryptographic monotonic writable state provider, persistent backend I/O and executed repair/migration, Secure Boot-state verification, live PooleKernel entry and revalidation, capability creation, activation or update application, policy application, recovery execution or symbol consumption, licensed real vendor payload intake and validation, live FMP/ESRT/PLDM inventory, privileged per-processor revision observation, initial-system execution, final framebuffer cache policy, retained CR3/RSP installation, target-firmware and physical-media qualification, and N5 exit remain open",
    "A real reproducible 64-page PooleKernel image, PKENTRY1 intake, allocation-free PKREVAL1 verifier, bounded early ring/COM1/framebuffer paths, and panic classes exist, but boot trust, measured boot, final CR3/RSP installation, live transfer and verifier execution, descriptor/exception setup, retained crash evidence, kernel runtime, target execution, and N6 exit remain open",
        ...
        implementation_flags.append(
            {
                ...
                or flag_id in {"FLAG-N0-RATIFICATION-SCOPE-001", "FLAG-N2-CPUID-001", "FLAG-N4-PROFILE-001", "FLAG-N4-IPC-MODEL-001", "FLAG-N4-SCHEDULER-MODEL-001", "FLAG-N4-POOLEFS-MODEL-001", "FLAG-N5-POOLEBOOT-PROOF-001", "FLAG-N5-BOOTPROTO-001", "FLAG-N5-BOOTCFG-001", "FLAG-N5-ELF-001", "FLAG-N5-KLOAD-001", "FLAG-N5-MANIFEST-001", "FLAG-N5-PBP1-LIVE-001", "FLAG-N5-KMAP-001", "FLAG-N5-HANDOFF-EXIT-001", "FLAG-N5-INIT-SYSTEM-001", "FLAG-N5-INIT-BUNDLE-001", "FLAG-N5-RECOVERY-BUNDLE-001", "FLAG-N5-SYMBOL-BUNDLE-001", "FLAG-N5-MICROCODE-BUNDLE-001", "FLAG-N5-FIRMWARE-BUNDLE-001", "FLAG-N5-POLICY-BUNDLE-001", "FLAG-N5-INIT-SEMANTICS-001", "FLAG-N5-INNER-PARSE-001", "FLAG-N5-INNER-TRUST-CONTRACT-001", "FLAG-N5-INNER-TRUST-BACKEND-MODEL-001", "FLAG-N5-INNER-KERNEL-REVALIDATE-001", "FLAG-N6-KENTRY-001"}
                else "open",
                "phase_id": phase_id,
...
def main() -> int:
    ...
    parser.add_argument("--test-count", type=int, default=724)
