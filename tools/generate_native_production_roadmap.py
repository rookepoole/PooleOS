...
PHASE_EVIDENCE = {
    ...
    "N5": [
        ...
        "runs/native-kernel-revalidation-readiness.json: 60/60 Rust tests, 8/8 Python tests, both no_std target builds, nine exact retained files and parsers, 36/36 hostile controls, 32,768/32,768 deterministic mutation rejects, exact unsigned-policy denial, zero authority grants/actions/state writes, and no standalone live-entry claim",
    ...
    "N6": [
        ...
        "runs/native_kernel_entry_readiness.json: 60/60 host tests, 2/2 exact clean linked and canonical builds, 43/43 hostile controls, 204,800 canonical bytes in a 262,144-byte image, entry offset 0x8000, 472 relative relocations, exact independent Rust/Python loaded bytes, and canonical SHA-256 8DE68D5A5F9D71DBA57843B0AA55C9A42207C4026D69B6383B3C751DC5E434FA",
    ...
    "N9": [
        ...
        "specs/native-kernel-virtual-memory-contract.json and docs/native-kernel-virtual-memory.md: PKVM1 freezes the initial 48-bit address bands, one exact PKMAP2 bootstrap temporary leaf, PMM generation ownership, four-level 4 KiB table policy, bounded map/protect/unmap transactions, W^X and cache-alias rejection, rollback, and inactive-root reclamation receipts",
        "native/kernel/src/virtual_memory.rs, native/kernel/src/main.rs, runtime/native_kernel_virtual_memory.py, tools/qualify_native_kernel_virtual_memory.py, and tests/test_native_kernel_virtual_memory.py: fixed-capacity no_std transaction core, live CPUID width validation, privileged volatile bootstrap adapter, independent PBP1 first-fit oracle, source audit, host rollback/fault tests, and bounded selector-9 evidence",
        "runs/native-kernel-virtual-memory-readiness.json: 2/2 exact qemu64 QEMU/OVMF runs, 40 ordered markers, 39/39 hostile controls, four materialized inactive table pages, 4,104 inactive-table writes, 40 bootstrap-leaf writes, 40 local INVLPG operations, two inactive receipts, zero CR3 writes or shootdowns, exact allocation release, and zero signatures, authority, or actions",
...
PHASE_GAPS = {
    ...
    "N9": ["PKPMM1 consumes the live PBP1 map and provides bounded generation-safe physical ownership. PKVM1 adds a fixed-capacity inactive four-level 4 KiB hierarchy, frozen address bands, bounded map/protect/unmap with rollback, W^X and cache-alias rejection, inactive-root receipts, and one exactly revoked PKMAP2 bootstrap temporary leaf. Page scrubbing, held-class reclaim, kernel-complete active roots, direct-map policy, active-address-space TLB receipts and SMP shootdown, huge pages, PCID, COW, user faults, pager IPC, heaps/object caches, MMIO/PAT/MTRR qualification, concurrent allocation, pressure/OOM policy, target hardware, and the N9 exit gate remain open"],
...
PROGRAM_GAPS = [
    ...
    "PKERR1 freezes a pure exact-target CPU/errata rejection policy, PKXSTATE1 proves bounded x87/SSE standard-XSAVE ownership, PKXEXC1 proves deliberate #MF/#XM recovery plus terminal test-only #NM rejection with a linked scope audit under WHPX, and PKMSR1 proves only a read-only qemu64 BSP system-linkage/global-MCA/unsupported-PMU observation. PKPMM1 supplies bounded physical ownership and PKVM1 supplies bounded inactive page-table transactions plus one revoked bootstrap alias; no target-qualified complete native CPU policy, applicable Model 40h-4Fh errata authority, direct numeric client microcode floor or ratified replacement, target-specific privileged-MSR semantics, syscall/per-CPU/MCE/PMU activation, AVX/extended state, user-task exception delivery, scheduler/AP/migration integration, interrupts, time, SMP, page-content scrubbing, reclaim transition, active kernel/user address spaces, SMP TLB shootdown, heap, MMIO/cache-alias qualification, concurrent allocator, pressure, or OOM implementation exists",
    ...
    "PKPMM1 supplies bounded one-BSP physical-page ownership and PKVM1 supplies bounded inactive four-level 4 KiB tables, map/protect/unmap rollback, and one revoked PKMAP2 bootstrap alias; page-content scrubbing, reclaim transitions, kernel-complete active roots, direct-map and SMP TLB policy, huge pages, PCID, COW, user faults, pager IPC, cache/MMIO qualification, heaps/object caches, concurrency/SMP, pressure and OOM policy, scheduler, task, syscall, capability, IPC, isolation, and asynchronous I/O remain open",
