...
def _host_checks(cargo: Path, env: dict[str, str], temporary_root: Path) -> dict[str, Any]:
    ...
    if match is None or int(match.group(1)) != 88:
        raise QualificationError("expected exactly eighty-eight PooleKernel host tests")
...
def _validate_manifest(contract: dict[str, Any]) -> dict[str, str]:
    expected = {
        ...
        "entry_offset": f"0x{contract['product']['entry_offset']:08X}",
