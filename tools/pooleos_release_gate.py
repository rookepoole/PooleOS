...
from runtime import native_kernel_scheduler  # noqa: E402
from runtime import native_kernel_scheduler_preempt  # noqa: E402
from runtime import native_kernel_scheduler_deferred  # noqa: E402
from runtime import native_kernel_scheduler_smp  # noqa: E402
from runtime import native_kernel_scheduler_ap_workers  # noqa: E402
from runtime import native_kernel_scheduler_smp_preempt  # noqa: E402
from runtime import native_kernel_atomics  # noqa: E402
from runtime import native_kernel_locks  # noqa: E402
...
NATIVE_KERNEL_SCHEDULER_READINESS = (
...
NATIVE_KERNEL_SCHEDULER_SMP_READINESS = (
    ROOT / "runs" / "native-kernel-scheduler-smp-readiness.json"
)
NATIVE_KERNEL_SCHEDULER_AP_WORKERS_READINESS = (
    ROOT / "runs" / "native-kernel-scheduler-ap-workers-readiness.json"
)
NATIVE_KERNEL_SCHEDULER_SMP_PREEMPT_READINESS = (
    ROOT / "runs" / "native-kernel-scheduler-smp-preempt-readiness.json"
)
NATIVE_KERNEL_ATOMICS_READINESS = (
    ROOT / "runs" / "native-kernel-atomics-readiness.json"
)
NATIVE_KERNEL_LOCKS_READINESS = ROOT / "runs" / "native-kernel-locks-readiness.json"
...
DEFAULT_GAPS = [
    "The completed owner response records both ADR dispositions and all 38 objective definitions while accepting zero measurements. The primary hardware-backed governance key is enrolled and registered. Enrollment signature verification, independent recovery custody, detached architecture signatures, the signed baseline tag, immutable release refs, and retained CI review evidence remain open. No alternate recovery-key profile is accepted or provisioned.",
...
def check_native_pooleboot_readiness(path: Path = NATIVE_POOLEBOOT_READINESS) -> dict:
    ...
    expected_summary = {
        ...
        "inner_set_retained_set_sha256": "7B68DDA305B46690E02CA7597633F16ADBDAAF7607759253F95EB919F2493B9F",
        ...
        "trust_policy_sha256": "D81F916D655315273E1945A92C6668C021220F74CCD30A19FB75B2CCB472DAF9",
        "trust_state_sha256": "DF907AE5AFD573E9C4514E50A65A271889904BB3DF57260CBCF751EDEDE84215",
    ...
    detail = (
        ...
        "retained_files=9; inner=6/6; inner_sha256=7B68DDA305B4; authority=0; actions=0; state=0; hardware=0; "
...
def check_native_kernel_load_readiness(path: Path = NATIVE_KERNEL_LOAD_READINESS) -> dict:
    ...
    if summary.get("rust_host_tests_passed") != 304 or summary.get("rust_host_tests_total") != 304:
    ...
    detail = (
        "contract=PKLOAD6; rust_tests=304/304; boot_builds=2/2; kernel_builds=2/2; "
        "media=2/2; guest_runs=2/2; markers=25; retained_files=9; inner=6/6; "
        "inner_sha256=7B68DDA305B4; "
        "trust=unsigned-deny; trust_bindings=14; trust_authority=0; trust_writes=0; "
...
def check_native_kernel_revalidation_readiness(
    ...
    build = artifact.get("build", {})
    if not isinstance(build, dict) or build.get("host_test_count") != 219:
    ...
    detail = (
        "contract=PKREVAL1; kernel_tests=219/219; python_tests=8/8; targets=2/2; "
...
def check_native_kernel_interrupt_time_readiness(
    ...
    expected_summary = {
        "kernel_host_tests_passed": 214,
        "kernel_host_tests_total": 214,
        "qemu_runs_passed": 2,
        "qemu_runs_total": 2,
        "markers_per_run": 36,
...
def check_native_kernel_smp_ipi_readiness(
    ...
    expected_summary = {
        ...
        "accepted_deliveries": 9,
        "denied_deliveries": 3,
        "offline_timeouts": 1,
        "partial_start_rollbacks": 1,
        "fresh_retries": 1,
        "eois": 12,
        "remote_tlb_invalidations": 3,
        "retired_generations": 1,
        "premature_reclaim_rejections": 2,
    ...
    if artifact.get("summary") != expected_summary:
        errors.append("PKSMP5 readiness summary changed")
    execution = artifact.get("execution", {})
    runs = execution.get("runs", []) if isinstance(execution, dict) else []
    if (
        len(runs) != 2
        or execution.get("run_count") != 2
        or execution.get("virtual_cpu_count") != 4
        or execution.get("application_processor_count") != 3
        or execution.get("cpu_model") != "SandyBridge,-avx"
        or execution.get("acceleration") != "tcg_multi_thread"
        or execution.get("deterministic_instruction_clock") is not False
        or execution.get("static_markers_exact_match") is not True
        ...
        errors.append("PKSMP5 live run evidence is incomplete")
    controls = artifact.get("negative_controls", [])
    if (
        not isinstance(controls, list)
        or len(controls) != 30
    ...
    if (
        ...
        or observation.get("topology", {}).get("bsp_apic_id") != 0
        or observation.get("topology", {}).get("target_apic_ids") != [1, 2, 3]
        or observation.get("topology", {}).get("target_mask") != 14
        or observation.get("partial_rollback", {}).get("started_mask") != 6
        or observation.get("partial_rollback", {}).get("offline_apic_id") != 4
        or observation.get("partial_rollback", {}).get("timeout_count") != 1
        or observation.get("partial_rollback", {}).get("parked_mask") != 6
        or observation.get("partial_rollback", {}).get("released_mask") != 14
        or observation.get("retry", {}).get("retry_count") != 1
        or observation.get("retry", {}).get("simultaneous_online") is not True
        or len(observation.get("aps", [])) != 3
        or observation.get("shootdown", {}).get("target_mask") != 14
        or observation.get("shootdown", {}).get("ack_mask") != 14
        or observation.get("shootdown", {}).get("invalidation_count") != 3
        or observation.get("shootdown", {}).get("premature_reclaim_rejections") != 2
        or observation.get("shootdown", {}).get("stage") != "released"
        or observation.get("result", {}).get("application_processors_online") != 3
        or observation.get("result", {}).get("scheduler") != 0
        or observation.get("result", {}).get("general_broadcast") != 0
        or observation.get("result", {}).get("target_hardware") != 0
        or observation.get("release", {}).get("resource_pages") != 96
        or observation.get("release", {}).get("frame_pages") != 6
        or observation.get("release", {}).get("total_pages") != 102
        or observation.get("release", {}).get("verified_bytes") != 417_792
    ):
        errors.append("PKSMP5 topology, rollback, shootdown, release, or nonclaim boundary changed")
    contract = native_kernel_smp_ipi.read_json(ROOT / native_kernel_smp_ipi.CONTRACT_RELATIVE)
    if artifact.get("claims") != contract.get("claims"):
        errors.append("PKSMP5 claim boundary changed")
    if (
        artifact.get("flag_n9_smp_shootdown_001_closed") is not True
        or artifact.get("flag_n8_smp_multi_ap_001_closed") is not True
        or artifact.get("n8_exit_gate_satisfied") is not False
        or artifact.get("n9_exit_gate_satisfied") is not False
        or artifact.get("production_ready") is not False
    ):
...
def check_native_kernel_scheduler_readiness(
    path: Path = NATIVE_KERNEL_SCHEDULER_READINESS,
) -> dict:
    artifact, artifact_schema_errors = _load_schema_artifact(
        path, "native-kernel-scheduler-readiness.schema.json"
    )
    errors = [
        f"native kernel scheduler readiness {error.path}: {error.message}"
        for error in artifact_schema_errors[:8]
    ]
    if not isinstance(artifact, dict):
        return readiness.make_check(
    ...
    expected_summary = {
        ...
        "stack_bytes_cleared": 32_768,
        "negative_controls_total": 28,
        "hostile_cases_total": 115,
        "production_claim_count": 0,
    }
    if artifact.get("summary") != expected_summary:
        errors.append("PKSCHED1 readiness summary changed")
    build = artifact.get("build", {})
    host_probe = build.get("host_probe", {}) if isinstance(build, dict) else {}
    linked = build.get("linked_switch_audit", {}) if isinstance(build, dict) else {}
    if (
        host_probe.get("receipt_count") != 4
        or host_probe.get("rust_python_exact_agreement") is not True
        or host_probe.get("stress", {}).get("dispatches") != 1761
        or host_probe.get("stress", {}).get("migrations") != 2334
        or host_probe.get("stress", {}).get("checksum") != 0x23B7_6E2F_80E2_B747
        or linked.get("scope")
        != "poole_scheduler_context_switch..poole_scheduler_context_switch_end"
        or linked.get("instruction_count") != 18
        or linked.get("scope_byte_count") != 36
        or linked.get("forbidden_instruction_count") != 0
        or linked.get("end_symbol_verified") is not True
        or linked.get("status") != "pass"
    ):
        errors.append("PKSCHED1 host oracle or linked switch audit changed")
    execution = artifact.get("execution", {})
    runs = execution.get("runs", []) if isinstance(execution, dict) else []
    if (
        len(runs) != 2
        or execution.get("run_count") != 2
        or execution.get("virtual_cpu_count") != 1
        or execution.get("bsp_only") is not True
        or execution.get("cpu_model") != "qemu64"
        or execution.get("acceleration") != "tcg_single_thread"
        or execution.get("deterministic_instruction_clock") is not True
        or execution.get("static_markers_exact_match") is not True
        or execution.get("dynamic_fields_revalidated") is not True
        or execution.get("exact_screenshot_match") is not True
        or execution.get("exact_pbp1_match") is not True
        ...
        errors.append("PKSCHED1 live run evidence is incomplete")
    controls = artifact.get("negative_controls", [])
    if (
        not isinstance(controls, list)
        or len(controls) != 28
        ...
        errors.append("PKSCHED1 hostile-control evidence is incomplete")
    observation = execution.get("observation", {}) if isinstance(execution, dict) else {}
    if (
        observation.get("core", {}).get("cpu_capacity") != 4
        or observation.get("core", {}).get("task_capacity") != 8
        or observation.get("core", {}).get("trace") != [0, 1, 0, 1, 0, 1, 0, 1]
        or observation.get("switch", {}).get("transitions") != 16
        or observation.get("switch", {}).get("task_runs") != [4, 4]
        or observation.get("cleanup", {}).get("stack_bytes_cleared") != 32_768
        or observation.get("cleanup", {}).get("task_contexts_retired") != 2
        or observation.get("result", {}).get("live_smp_dispatch") != 0
        or observation.get("result", {}).get("production") != 0
    ):
        errors.append("PKSCHED1 live core, switch, cleanup, or nonclaim boundary changed")
    contract = native_kernel_scheduler.read_json(
        ROOT / native_kernel_scheduler.CONTRACT_RELATIVE
    )
    if artifact.get("claims") != contract.get("claims"):
        errors.append("PKSCHED1 claim boundary changed")
    if (
        artifact.get("n12_exit_gate_satisfied") is not False
        or artifact.get("production_ready") is not False
        or artifact.get("production_promotion_allowed") is not False
    ):
    ...
    detail = (
        ...
        "scheduler_tests=14; kernel_tests=214; trace_steps=4096; dispatches=1761; "
...
def check_native_kernel_scheduler_preemption_readiness(
    path: Path = NATIVE_KERNEL_SCHEDULER_PREEMPTION_READINESS,
) -> dict:
    artifact, artifact_schema_errors = _load_schema_artifact(
        path, "native-kernel-scheduler-preemption-readiness.schema.json"
    )
    errors = [
        f"native kernel scheduler preemption readiness {error.path}: {error.message}"
        for error in artifact_schema_errors[:8]
    ]
    if not isinstance(artifact, dict):
        return readiness.make_check(
    ...
    if artifact.get("summary") != expected_summary:
        errors.append("PKSCHED2 readiness summary changed")
    build = artifact.get("build", {})
    host_probe = build.get("host_probe", {}) if isinstance(build, dict) else {}
    source_audit = build.get("source_audit", {}) if isinstance(build, dict) else {}
    linked = build.get("linked_switch_audit", {}) if isinstance(build, dict) else {}
    if (
        host_probe.get("receipt_count") != 3
        or host_probe.get("rust_python_exact_agreement") is not True
        or host_probe.get("trace", {}).get("next") != [0, 1, 2, 0, 3, 3]
        or host_probe.get("trace", {}).get("causes")
        != ["none", "quantum", "wake", "block", "wake", "none"]
        or host_probe.get("frame_hostile_rejections") != 8
        or source_audit.get("focused_rust_test_count") != 7
        or source_audit.get("fixed_deferred_capacity") != 8
        or source_audit.get("task_stack_count") != 4
        or source_audit.get("task_stack_bytes_each") != 16_384
        ...
        errors.append("PKSCHED2 host oracle, source, or linked switch audit changed")
    execution = artifact.get("execution", {})
    runs = execution.get("runs", []) if isinstance(execution, dict) else []
    if (
        len(runs) != 2
        ...
        errors.append("PKSCHED2 live run evidence is incomplete")
    observation = execution.get("observation", {}) if isinstance(execution, dict) else {}
    if (
        observation.get("timer", {}).get("deliveries") != 6
        or observation.get("trace", {}).get("ticks") != 6
        or observation.get("trace", {}).get("switches") != 4
        or observation.get("trace", {}).get("pending") != 0
        or observation.get("frames", {}).get("saved") != 6
        or observation.get("frames", {}).get("restored") != 4
        or observation.get("frames", {}).get("eois") != 6
        or observation.get("frames", {}).get("task_entries") != [1, 1, 1, 1]
        or observation.get("cleanup", {}).get("stack_bytes_cleared") != 65_536
        or observation.get("cleanup", {}).get("tasks_dead") != 4
        or observation.get("cleanup", {}).get("mmio_revoked") != 1
        or observation.get("result", {}).get("timer_and_wakeup_preemption") != 1
        or observation.get("result", {}).get("live_ap_dispatch") != 0
        or observation.get("result", {}).get("production") != 0
    ):
        errors.append("PKSCHED2 timer, frame, cleanup, or nonclaim evidence changed")
    controls = artifact.get("negative_controls", [])
    if (
        not isinstance(controls, list)
        or len(controls) != 25
    ...
    if artifact.get("claims") != native_kernel_scheduler_preempt.expected_claims():
        errors.append("PKSCHED2 claim boundary changed")
    if (
        artifact.get("n12_exit_gate_satisfied") is not False
        or artifact.get("production_ready") is not False
        or artifact.get("production_promotion_allowed") is not False
    ...
    detail = (
        ...
        "preemption_tests=7; kernel_tests=214; ticks=6; eois=6; tasks=4; "
...
def check_native_kernel_scheduler_deferred_readiness(
    path: Path = NATIVE_KERNEL_SCHEDULER_DEFERRED_READINESS,
) -> dict:
    artifact, artifact_schema_errors = _load_schema_artifact(
        path, "native-kernel-scheduler-deferred-readiness.schema.json"
    )
    errors = [
        f"native kernel scheduler deferred readiness {error.path}: {error.message}"
        for error in artifact_schema_errors[:8]
    ]
    if not isinstance(artifact, dict):
        return readiness.make_check(
    ...
    if (
        kernel_summary.get("rust_host_tests_total") != 214
        or host_probe.get("receipt_count") != 5
        or host_probe.get("rust_python_exact_agreement") is not True
        or trace.get("slots") != [0, 2, 4, 1, 5, 6]
        or trace.get("workers") != [0, 1, 0, 1, 0, 1]
        or trace.get("states")
        != ["completed", "completed", "completed", "completed", "completed", "cancelled"]
        or trace.get("max_bypass") != 3
        or trace.get("sum") != 120
        or trace.get("xor") != 90
        or trace.get("fence") != 0
        or trace.get("remaining_slots") != [7]
        or source_audit.get("focused_rust_test_count") != 7
        or source_audit.get("fixed_work_capacity") != 8
        or source_audit.get("worker_count") != 2
        or source_audit.get("worker_stack_bytes_each") != 16_384
        or source_audit.get("allocation_free_controller") is not True
        ...
        or linked.get("scope_byte_count") != 36
        or linked.get("forbidden_instruction_count") != 0
        or linked.get("relocation_count") != 1304
    ...
    if (
        ...
        errors.append("PKSCHED3 live run evidence is incomplete")
    observation = artifact.get("observation", {})
    workers = observation.get("workers", {}) if isinstance(observation, dict) else {}
    worker_trace = workers.get("trace", {}) if isinstance(workers, dict) else {}
    if (
        observation.get("timer", {}).get("deliveries") != 1
        or observation.get("queue", {}).get("enqueued") != 8
        or observation.get("queue", {}).get("duplicate_suppressed") != 1
        or observation.get("queue", {}).get("queued_cancelled") != 1
        or workers.get("dispatches") != 6
        or workers.get("entries") != [3, 3]
        or workers.get("transitions") != 12
        or worker_trace.get("slots") != [0, 2, 4, 1, 5, 6]
        or worker_trace.get("workers") != [0, 1, 0, 1, 0, 1]
        or observation.get("flush", {}).get("receipts") != 8
        or observation.get("flush", {}).get("sum_lane") != 120
        or observation.get("flush", {}).get("xor_lane") != 90
        or observation.get("flush", {}).get("fence_lane") != 0
        or observation.get("faults", {}).get("rollbacks") != 5
        or observation.get("faults", {}).get("leaked_slots") != 0
        or observation.get("cleanup", {}).get("stack_bytes_cleared") != 32_768
        or observation.get("cleanup", {}).get("slots_free") != 8
        or observation.get("cleanup", {}).get("mmio_revoked") != 1
        or observation.get("result", {}).get("fixed_bsp_workers") != 1
        or observation.get("result", {}).get("live_ap_dispatch") != 0
        or observation.get("result", {}).get("production") != 0
    ):
        errors.append("PKSCHED3 queue, worker, reclamation, or nonclaim evidence changed")
    controls = artifact.get("negative_controls", [])
    if (
        not isinstance(controls, list)
        or len(controls) != 30
    ...
    if artifact.get("claims") != native_kernel_scheduler_deferred.expected_claims():
        errors.append("PKSCHED3 claim boundary changed")
    if (
        artifact.get("n12_exit_gate_satisfied") is not False
        or artifact.get("production_ready") is not False
        or artifact.get("production_promotion_allowed") is not False
    ...
    detail = (
        ...
        "deferred_tests=7; kernel_tests=214; enqueued=8; completed=5; cancelled=3; "
...
def check_native_kernel_scheduler_smp_readiness(
    path: Path = NATIVE_KERNEL_SCHEDULER_SMP_READINESS,
) -> dict:
    artifact, artifact_schema_errors = _load_schema_artifact(
        path, "native-kernel-scheduler-smp-readiness.schema.json"
    )
    errors = [
        f"native kernel scheduler SMP readiness {error.path}: {error.message}"
        for error in artifact_schema_errors[:8]
    ]
    if not isinstance(artifact, dict):
        return readiness.make_check(
    ...
    host_probe = build.get("host_probe", {}) if isinstance(build, dict) else {}
    source_audit = build.get("source_audit", {}) if isinstance(build, dict) else {}
    linked = build.get("linked_invlpg_audit", {}) if isinstance(build, dict) else {}
    trace = host_probe.get("trace", {}) if isinstance(host_probe, dict) else {}
    ...
    if (
        ...
        or trace.get("queues_after_transfer") != [2, 2, 2]
        or trace.get("bsp_trace") != [0, 7]
        or trace.get("ap_trace") != expected_ap_trace
    ...
    if (
        ...
        or any(
            ...
            or run.get("serial_debugcon_exact_match") is not True
            or run.get("pbp1_serial_debugcon_exact_match") is not True
            or run.get("independent_kernel_revalidation", {}).get(
                "guest_host_exact_match"
            )
            is not True
            for run in runs
        )
    ):
        errors.append("PKSCHED4 live four-vCPU evidence is incomplete")
    observation = artifact.get("observation", {})
    if (
        observation.get("topology", {}).get("processor_count") != 4
        or observation.get("topology", {}).get("target_apic_ids") != [1, 2, 3]
        or observation.get("topology", {}).get("online_mask") != 15
        or observation.get("transfers", {}).get("remote_wakes") != 1
        or observation.get("transfers", {}).get("migrations") != 2
        or observation.get("transfers", {}).get("transaction_acks") != 3
        or observation.get("transfers", {}).get("remote_acks") != 9
        or observation.get("dispatch", {}).get("bsp") != 2
        or observation.get("dispatch", {}).get("aps") != 6
        or observation.get("dispatch", {}).get("trace", {}).get("ap_trace")
        != expected_ap_trace
        or observation.get("dispatch", {}).get("call_function_executions") != 9
        or observation.get("rollback", {}).get("offline_cpu") != 4
        or observation.get("rollback", {}).get("timeouts") != 1
        or observation.get("rollback", {}).get("rollbacks") != 1
        or observation.get("rollback", {}).get("stale_rejections") != 2
        or observation.get("cleanup", {}).get("tasks_dead") != 8
        or observation.get("cleanup", {}).get("idle_owners") != 4
        or observation.get("cleanup", {}).get("pages_released") != 102
        or observation.get("cleanup", {}).get("bytes_verified") != 417_792
        or observation.get("result", {}).get("bounded_live_ap_scheduler") != 1
        or observation.get("result", {}).get("general_smp") != 0
        or observation.get("result", {}).get("production") != 0
    ):
        errors.append("PKSCHED4 scheduler, rollback, cleanup, or nonclaim evidence changed")
    controls = artifact.get("negative_controls", [])
    if (
        not isinstance(controls, list)
        or len(controls) != 32
    ...
    if artifact.get("claims") != native_kernel_scheduler_smp.expected_claims():
        errors.append("PKSCHED4 claim boundary changed")
    if (
        artifact.get("flag_n12_sched_smp_001_closed") is not True
        or artifact.get("n12_exit_gate_satisfied") is not False
        or artifact.get("production_ready") is not False
        or artifact.get("production_promotion_allowed") is not False
    ):
    ...
    detail = (
        ...
        "scheduler_tests=8; kernel_tests=214; wake=1; migrations=2; transfer_acks=3; "
...
def check_native_kernel_scheduler_ap_workers_readiness(
    path: Path = NATIVE_KERNEL_SCHEDULER_AP_WORKERS_READINESS,
) -> dict:
    artifact, artifact_schema_errors = _load_schema_artifact(
        path, "native-kernel-scheduler-ap-workers-readiness.schema.json"
    )
    errors = [
        f"native kernel scheduler AP-worker readiness {error.path}: {error.message}"
        for error in artifact_schema_errors[:8]
    ]
    if not isinstance(artifact, dict):
        return readiness.make_check(
    ...
    if (
        ...
        or trace.get("worker_entries") != [4, 4, 4]
        or trace.get("driver_calls") != 9
        or trace.get("service_calls") != 3
        or trace.get("driver_sum") != 177
        or trace.get("service_generation") != 4
        or trace.get("maximum_high_bypass") != 2
        or source_audit.get("focused_rust_test_count") != 10
        or source_audit.get("fixed_worker_count") != 3
        or source_audit.get("fixed_work_capacity") != 15
        or source_audit.get("allocation_free_controller") is not True
        or source_audit.get("typed_consumer_count") != 2
        or source_audit.get("arbitrary_callback_count") != 0
        or source_audit.get("ap_handler_saved_register_count") != 15
        or source_audit.get("live_marker_count") != 37
        or linked.get("invlpg_instruction_count") != 2
    ...
    if (
        ...
        or execution.get("virtual_cpu_count") != 4
        or execution.get("application_processor_count") != 3
        or execution.get("worker_count") != 3
        or execution.get("cpu_model") != "SandyBridge,-avx"
        or execution.get("acceleration") != "tcg_multi_thread"
        or execution.get("static_markers_exact_match") is not True
        or execution.get("dynamic_fields_revalidated") is not True
        ...
        errors.append("PKSCHED5 live four-vCPU evidence is incomplete")
    observation = artifact.get("observation", {})
    if (
        observation.get("topology", {}).get("processor_count") != 4
        or observation.get("topology", {}).get("target_apic_ids") != [1, 2, 3]
        or observation.get("topology", {}).get("worker_stack_bytes_each") != 8192
        or observation.get("queue", {}).get("enqueued") != 13
        or observation.get("queue", {}).get("queued_cancelled") != 1
        or observation.get("queue", {}).get("flush_watermark") != 13
        or observation.get("dispatch", {}).get("trace", {}).get("trace")
        != expected_trace
        or observation.get("dispatch", {}).get("call_function_executions") != 12
        or observation.get("dispatch", {}).get("driver_calls") != 9
        or observation.get("dispatch", {}).get("service_calls") != 3
        or observation.get("cancellation", {}).get("queued") != 1
        or observation.get("cancellation", {}).get("remote") != 1
        or observation.get("cancellation", {}).get("offline_timeouts") != 1
        or observation.get("cancellation", {}).get("rollbacks") != 1
        or observation.get("flush", {}).get("completed") != 11
        or observation.get("flush", {}).get("cancelled") != 2
        or observation.get("flush", {}).get("reclaimed") != 13
        or observation.get("flush", {}).get("driver_sum") != 177
        or observation.get("flush", {}).get("service_generation") != 4
        or observation.get("cleanup", {}).get("workers_retired") != 3
        or observation.get("cleanup", {}).get("stack_bytes_cleared") != 24_576
        or observation.get("cleanup", {}).get("pages_released") != 102
        or observation.get("cleanup", {}).get("bytes_verified") != 417_792
        or observation.get("result", {}).get("bounded_ap_local_workers") != 1
        or observation.get("result", {}).get("general_smp") != 0
        or observation.get("result", {}).get("production") != 0
    ):
        errors.append("PKSCHED5 worker, cancellation, cleanup, or nonclaim evidence changed")
    controls = artifact.get("negative_controls", [])
    if (
        not isinstance(controls, list)
        or len(controls) != 34
    ...
    if artifact.get("claims") != native_kernel_scheduler_ap_workers.expected_claims():
        errors.append("PKSCHED5 claim boundary changed")
    if (
        artifact.get("flag_n12_sched_ap_workers_001_closed") is not True
        or artifact.get("n12_exit_gate_satisfied") is not False
        or artifact.get("production_ready") is not False
        or artifact.get("production_promotion_allowed") is not False
    ):
    ...
    detail = (
        ...
        "markers=74/74; worker_tests=10; kernel_tests=214; typed_calls=12; "
...
def check_native_kernel_scheduler_smp_preempt_readiness(
    path: Path = NATIVE_KERNEL_SCHEDULER_SMP_PREEMPT_READINESS,
) -> dict:
    artifact, artifact_schema_errors = _load_schema_artifact(
        path, "native-kernel-scheduler-smp-preempt-readiness.schema.json"
    )
    errors = [
        f"native kernel scheduler SMP-preemption readiness {error.path}: {error.message}"
        for error in artifact_schema_errors[:8]
    ]
    if not isinstance(artifact, dict):
        return readiness.make_check(
    ...
    if (
        ...
        or source_audit.get("cpu_lane_count") != 4
        or source_audit.get("event_capacity") != 16
        or source_audit.get("allocation_free_controller") is not True
        or source_audit.get("live_reschedule_count") != 8
        or source_audit.get("ap_timer_interrupt_delivery") is not False
        or source_audit.get("live_marker_count") != 38
        or linked.get("invlpg_instruction_count") != 2
        or linked.get("remote_shootdown_invlpg_instruction_count") != 1
        or linked.get("successor_profile_invlpg_instruction_count") != 1
        or linked.get("successor_profile_executed") is not False
    ...
    if (
        ...
        errors.append("PKSCHED6 live four-vCPU evidence is incomplete")
    observation = artifact.get("observation", {})
    if (
        observation.get("topology", {}).get("processor_count") != 4
        or observation.get("topology", {}).get("target_apic_ids") != [1, 2, 3]
        or observation.get("events", {}).get("order") != "cancel,wake,migrate"
        or observation.get("events", {}).get("pending") != 0
        or observation.get("reschedule", {}).get("live_ipis") != 8
        or observation.get("reschedule", {}).get("model_acks") != 5
        or observation.get("reschedule", {}).get("quantum_preemptions") != 3
        or observation.get("ownership", {}).get("frame_epochs") != [2, 2, 2, 4]
        or observation.get("ownership", {}).get("timer_ticks") != [2, 2, 2, 4]
        or observation.get("ownership", {}).get("trace") != "1:2>1;3:6>7>5"
        or observation.get("bounds", {}).get("event_latency") != 0
        or observation.get("bounds", {}).get("watchdog_age") != 2
        or observation.get("bounds", {}).get("maximum_bypass") != 2
        or observation.get("cleanup", {}).get("dead") != 8
        or observation.get("cleanup", {}).get("owners_revoked") != 3
        or observation.get("cleanup", {}).get("pages_released") != 102
        or observation.get("cleanup", {}).get("bytes_verified") != 417_792
        or observation.get("result", {}).get("bounded_model") != 1
        or observation.get("result", {}).get("ap_timer_interrupts") != 0
        or observation.get("result", {}).get("general_smp") != 0
        or observation.get("result", {}).get("production") != 0
    ):
        errors.append("PKSCHED6 ownership, bounds, cleanup, or nonclaim evidence changed")
    controls = artifact.get("negative_controls", [])
    if (
        not isinstance(controls, list)
        or len(controls) != 34
    ...
    if artifact.get("claims") != native_kernel_scheduler_smp_preempt.expected_claims():
        errors.append("PKSCHED6 claim boundary changed")
    if (
        artifact.get("flag_n12_sched_smp_preempt_001_closed") is not True
        or artifact.get("n12_exit_gate_satisfied") is not False
        or artifact.get("production_ready") is not False
        or artifact.get("production_promotion_allowed") is not False
    ):
    ...
    detail = (
        ...
        "markers=76/76; scheduler_tests=5; kernel_tests=214; live_ipis=8; "
...
def check_native_kernel_atomics_readiness(
    path: Path = NATIVE_KERNEL_ATOMICS_READINESS,
) -> dict:
    artifact, artifact_schema_errors = _load_schema_artifact(
        path, "native-kernel-atomics-readiness.schema.json"
    )
    errors = [
        f"native kernel atomics readiness {error.path}: {error.message}"
        for error in artifact_schema_errors[:8]
    ]
    if not isinstance(artifact, dict):
        return readiness.make_check(
    ...
    if (
        kernel.get("host_tests_passed") != 214
        or kernel.get("host_tests_total") != 214
        or source.get("heap_api_token_count") != 0
        or source.get("typed_order_enum_count") != 4
        or source.get("atomic_wrapper_count") != 4
        or source.get("audit_symbol_count") != 7
        or source.get("unit_test_count") != 7
        or source.get("interrupt_retry_loop_count") != 0
        or source.get("atomics_selector") != 21
        or source.get("max_development_trap_scenario", 0) < 21
        or probe.get("receipt_count") != 8
        or probe.get("publication_rounds") != 4096
        or probe.get("contended_operation_count") != 20_480
        or probe.get("sequential_consistency_rounds") != 2048
        or probe.get("forbidden_observations") != 0
        or probe.get("rust_python_exact_agreement") is not True
    ):
        errors.append("PKATOM1 host, source, or litmus evidence changed")
    if (
        linked.get("target") != "x86_64-unknown-none"
        or linked.get("symbol_count") != 7
        or linked.get("all_instruction_rules_passed") is not True
        or linked.get("canonical_sha256") != kernel.get("canonical_sha256")
        or len(linked.get("symbols", [])) != 7
        or any(
            not isinstance(item, dict)
            or not item.get("required_instruction_classes")
            or len(item.get("body_sha256", "")) != 64
            for item in linked.get("symbols", [])
        )
    ):
        errors.append("PKATOM1 linked x86-64 instruction evidence changed")
    execution = artifact.get("execution", {})
    runs = execution.get("runs", [])
    observation = execution.get("observation", {})
    if (
        execution.get("run_count") != 2
        or execution.get("machine") != "pc-q35-11.0"
        or execution.get("cpu_model") != "qemu64"
        or execution.get("acceleration") != "tcg_single_thread"
        or execution.get("fresh_vars_each_run") is not True
        or execution.get("media_read_only") is not True
        or execution.get("physical_media_write_performed") is not False
        or execution.get("exact_marker_match") is not True
        or execution.get("exact_screenshot_match") is not True
        or execution.get("exact_pbp1_match") is not True
        or execution.get("markers_per_run") != 41
        or len(runs) != 2
        or any(
            not isinstance(run, dict)
            or len(run.get("markers", [])) != 41
            or run.get("serial_debugcon_exact_match") is not True
            or run.get("pbp1_serial_debugcon_exact_match") is not True
            or run.get("independent_kernel_revalidation", {}).get("guest_host_exact_match") is not True
            for run in runs
        )
        or observation.get("interrupt", {}).get("timer_deliveries") != 8
        or observation.get("interrupt", {}).get("atomic_updates") != 8
        or observation.get("interrupt", {}).get("observed_mask") != "0x000000FF"
        or observation.get("interrupt", {}).get("publication") != "0x00000000C0DEC0DE"
        or observation.get("interrupt", {}).get("cleanup") != 1
    ):
        errors.append("PKATOM1 live one-BSP interrupt evidence changed")
    controls = artifact.get("negative_controls", [])
    if (
        not isinstance(controls, list)
        or len(controls) != 29
        or sum(item.get("case_count", 0) for item in controls if isinstance(item, dict)) != 78
    ):
        errors.append("PKATOM1 hostile-control evidence is incomplete")
    if artifact.get("claims") != native_kernel_atomics.expected_claims():
        errors.append("PKATOM1 claim boundary changed")
    if (
        artifact.get("flag_n12_concurrency_atomics_001_closed") is not True
        or artifact.get("n12_exit_gate_satisfied") is not False
        or artifact.get("production_ready") is not False
        or artifact.get("production_promotion_allowed") is not False
    ):
    ...
    detail = (
        "contract=PKATOM1; kernel_tests=214/214; host_receipts=8; publication_rounds=4096; "
...
def check_native_kernel_locks_readiness(
    path: Path = NATIVE_KERNEL_LOCKS_READINESS,
) -> dict:
    artifact, artifact_schema_errors = _load_schema_artifact(
        path, "native-kernel-locks-readiness.schema.json"
    )
    errors = [
        f"native kernel locks readiness {error.path}: {error.message}"
        for error in artifact_schema_errors[:8]
    ]
    if not isinstance(artifact, dict):
        return readiness.make_check(
            "native_kernel_locks_readiness",
            False,
    ...
    if (
        host_tests.get("test_pass_count") != 214
        or host_tests.get("test_count") != 214
        or source.get("heap_api_token_count") != 0
        or source.get("lock_primitive_count") != 6
        or source.get("rank_class_count") != 5
        or source.get("focused_unit_test_count") != 10
        or source.get("live_locked_instruction_class_count") != 5
        or probe.get("receipt_count") != 9
        or probe.get("ticket_acquisitions") != 8192
        or probe.get("maximum_mutex_bypass") != 7
        or probe.get("reader_writer_rounds") != 1024
        or probe.get("sequence_rounds") != 2048
        or probe.get("rust_python_exact_agreement") is not True
        or build.get("all_profile_binaries_distinct") is not True
        or build.get("default_stop_marker_present") is not True
        or build.get("default_transfer_marker_absent") is not True
    ):
        errors.append("PKLOCK1 build, source, or host evidence changed")
    media = artifact.get("media", {})
    if (
        media.get("clean_generation_count") != 2
        or media.get("exact_clean_generation_match") is not True
        or media.get("ordinary_workspace_file_only") is not True
        or media.get("physical_media_write_performed") is not False
        or len(media.get("sha256", "")) != 64
        or not isinstance(media.get("byte_count"), int)
        or media.get("byte_count", 0) <= 0
    ):
        errors.append("PKLOCK1 deterministic media evidence changed")
    execution = artifact.get("execution", {})
    runs = execution.get("runs", [])
    observation = artifact.get("observation", {})
    ...
    if (
        ...
        or execution.get("fresh_vars_each_run") is not True
        or execution.get("media_read_only") is not True
        or execution.get("guest_network") is not False
        or execution.get("host_acceleration") is not False
        or execution.get("exact_marker_match") is not True
        or execution.get("exact_screenshot_match") is not True
        or execution.get("exact_pbp1_match") is not True
        or execution.get("markers_per_run") != 35
        or len(runs) != 2
        or any(
            not isinstance(run, dict)
            or len(run.get("markers", [])) != 35
        ...
        or live.get("virtual_cpu_count") != 4
        or live.get("application_processor_count") != 3
        or live.get("acquisitions") != 4
        or live.get("tickets") != [0, 1, 2, 3]
        or live.get("mappings_installed") != 3
        or live.get("mappings_revoked") != 3
    ):
        errors.append("PKLOCK1 live exact-topology evidence changed")
    controls = artifact.get("negative_controls", [])
    if (
        not isinstance(controls, list)
        or len(controls) != 30
        ...
        errors.append("PKLOCK1 hostile-control evidence is incomplete")
    if artifact.get("claims") != native_kernel_locks.expected_claims():
        errors.append("PKLOCK1 claim boundary changed")
    if (
        artifact.get("flag_n12_concurrency_locks_001_closed") is not True
        or artifact.get("phase_status") != {"N12": "partial", "N12.2": "complete"}
        or artifact.get("n12_exit_gate_satisfied") is not False
        or artifact.get("production_ready") is not False
        or artifact.get("production_promotion_allowed") is not False
    ):
    ...
    detail = (
        "contract=PKLOCK1; kernel_tests=214/214; host_receipts=9; ticket_acquisitions=8192; "
...
def check_native_kernel_entry_readiness(path: Path = NATIVE_KERNEL_ENTRY_READINESS) -> dict:
    ...
    expected_summary = {
        "rust_host_tests_passed": 219,
        "rust_host_tests_total": 219,
    ...
    if (
        ...
        or product.get("relocation_count") != 1305
    ...
    detail = (
        "contract=PKENTRY1; kernel_tests=219/219; clean_builds=2/2; negative=43/43; "
        "exact_loaded=2/2; bytes=517784; image_bytes=589824; entry=0xA000; "
        "relocations=1305; live_transfer=false; "
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--native-kernel-scheduler-readiness",
        type=Path,
        default=NATIVE_KERNEL_SCHEDULER_READINESS,
    )
    parser.add_argument(
        "--native-kernel-scheduler-preemption-readiness",
        type=Path,
        default=NATIVE_KERNEL_SCHEDULER_PREEMPTION_READINESS,
    )
    parser.add_argument(
        "--native-kernel-scheduler-deferred-readiness",
        type=Path,
        default=NATIVE_KERNEL_SCHEDULER_DEFERRED_READINESS,
    )
    parser.add_argument(
        "--native-kernel-scheduler-smp-readiness",
        type=Path,
        default=NATIVE_KERNEL_SCHEDULER_SMP_READINESS,
    )
    parser.add_argument(
        "--native-kernel-scheduler-ap-workers-readiness",
        type=Path,
        default=NATIVE_KERNEL_SCHEDULER_AP_WORKERS_READINESS,
    )
    parser.add_argument(
        "--native-kernel-scheduler-smp-preempt-readiness",
        type=Path,
        default=NATIVE_KERNEL_SCHEDULER_SMP_PREEMPT_READINESS,
    )
    parser.add_argument(
        "--native-kernel-atomics-readiness",
        type=Path,
        default=NATIVE_KERNEL_ATOMICS_READINESS,
    )
    parser.add_argument(
        "--native-kernel-locks-readiness",
        type=Path,
        default=NATIVE_KERNEL_LOCKS_READINESS,
    )
    parser.add_argument(
    ...
    checks = [
        ...
        check_native_kernel_scheduler_readiness(
            args.native_kernel_scheduler_readiness
        ),
        check_native_kernel_scheduler_preemption_readiness(
            args.native_kernel_scheduler_preemption_readiness
        ),
        check_native_kernel_scheduler_deferred_readiness(
            args.native_kernel_scheduler_deferred_readiness
        ),
        check_native_kernel_scheduler_smp_readiness(
            args.native_kernel_scheduler_smp_readiness
        ),
        check_native_kernel_scheduler_ap_workers_readiness(
            args.native_kernel_scheduler_ap_workers_readiness
        ),
        check_native_kernel_scheduler_smp_preempt_readiness(
            args.native_kernel_scheduler_smp_preempt_readiness
        ),
        check_native_kernel_atomics_readiness(args.native_kernel_atomics_readiness),
        check_native_kernel_locks_readiness(args.native_kernel_locks_readiness),
    ...
    artifacts = [
        ...
        for path in (
            ...
            args.native_kernel_scheduler_ap_workers_readiness,
            args.native_kernel_scheduler_smp_preempt_readiness,
            args.native_kernel_atomics_readiness,
            args.native_kernel_locks_readiness,
