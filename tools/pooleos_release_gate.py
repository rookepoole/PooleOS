...
DEFAULT_GAPS = [
    ...
    "A reproducible unsigned PooleBoot proof application boots twice under pinned non-promoting OVMF with deterministic twelve-file GPT/FAT32 media and twenty-five ordered serial/debugcon markers; PBP1, PBC1, PSM1, PKELF1, PBART1, PINIT1, PREC1, PSYM1, synthetic-only PMCU1, synthetic-only PFWM1, qualification-only PPOL1, PBTRUST1/PBSTATE1, PKMAP2, PBEXIT1, PKREVAL1, PKXFER1, PKTRAP1, PKCPU1, PKERR1, PKXSTATE1, and a separately qualified real PooleKernel image pass their bounded gates. PKLOAD6 proves exact retained storage and successful ExitBootServices with zero later firmware calls while the default path stops permanently. PKXFER1 separately proves one opt-in QEMU-only transfer and live nine-file PKREVAL1 denial. PKTRAP1 adds only three opt-in BSP scenarios, PKCPU1 adds only one opt-in BSP qemu64 read-only profile, PKERR1 adds only a pure exact-target policy that denies the current evidence with zero privileged reads or effects, and PKXSTATE1 adds only one opt-in BSP eager x87/SSE ownership profile with three allowlisted control writes and zero authority. Policy signatures, authenticated revocation, a real cryptographic monotonic writable provider, Secure Boot-state verification, activation, capability enforcement, applicable target errata and numeric microcode-floor authority, live firmware inventory, privileged per-processor revision observation, real vendor-container or payload validation, AVX/extended xstate, xstate exception delivery, scheduler/AP/migration integration, initial-system execution, final framebuffer policy, production transfer, second host, target firmware, and physical-media qualification remain open.",
    "A real reproducible PooleKernel image, PKENTRY1 intake, bounded early diagnostics, panic classes, PKXFER1 live entry, BSP-only PKTRAP1 descriptor/exception entry, BSP-only qemu64 PKCPU1 read-only CPU-policy observation, and bounded BSP PKXSTATE1 x87/SSE ownership exist. PKERR1 freezes an exact Ryzen 7 9800X3D policy and correctly denies the current evidence, but boot trust, measured boot, production transfer, direct target errata and numeric microcode-floor sources, AVX/extended state, deliberate xstate exceptions, scheduler/AP/migration integration, final machine-code SIMD audit, per-CPU descriptor state, guarded IST stacks, complete exception/NMI/machine-check handling, retained crash evidence, kernel runtime, target execution, and N6/N7 exit remain open.",
    "No target-qualified complete native CPU policy, applicable Model 40h-4Fh errata guide, direct numeric client microcode floor or ratified replacement, AVX/extended xstate, xstate exception delivery, scheduler/AP/migration integration, final machine-code SIMD audit, interrupt, time, SMP, physical-memory, virtual-memory, or reclaim implementation; PKCPU1 is a bounded qemu64 observation, PKERR1 is a pure fail-closed policy with zero privileged reads or effects, and PKXSTATE1 is bounded eager x87/SSE ownership on one QEMU BSP.",
...
def check_native_pooleboot_readiness(path: Path = NATIVE_POOLEBOOT_READINESS) -> dict:
    ...
    expected_summary = {
        ...
        "inner_set_retained_set_sha256": "872978D7FB5E1108D04736796A247E5E59D9EEC0C97AE991CFBAC83D819A15DC",
        ...
        "trust_policy_sha256": "714128872087FB09A8553E3D0B21713DADF58FD0929269A6F2DF5FFE8F28811B",
        "trust_state_sha256": "86672AD1384509D2053CB191C17210351BE77C7C939227152DE26668932EA4AC",
    ...
    detail = (
        ...
        "retained_files=9; inner=6/6; inner_sha256=872978D7FB5E1108; authority=0; actions=0; state=0; hardware=0; "
...
def check_native_kernel_load_readiness(path: Path = NATIVE_KERNEL_LOAD_READINESS) -> dict:
    ...
    if summary.get("rust_host_tests_passed") != 116 or summary.get("rust_host_tests_total") != 116:
    ...
    if summary.get("inner_retained_set_sha256") != (
        "872978D7FB5E1108D04736796A247E5E59D9EEC0C97AE991CFBAC83D819A15DC"
    ):
    ...
    detail = (
        "contract=PKLOAD6; rust_tests=116/116; boot_builds=2/2; kernel_builds=2/2; "
        "media=2/2; guest_runs=2/2; markers=25; retained_files=9; inner=6/6; "
        "inner_sha256=872978D7FB5E1108; "
        "trust=unsigned-deny; trust_bindings=14; trust_authority=0; trust_writes=0; "
...
def check_native_kernel_revalidation_readiness(
    ...
    build = artifact.get("build", {})
    if not isinstance(build, dict) or build.get("host_test_count") != 31:
    ...
    detail = (
        "contract=PKREVAL1; kernel_tests=31/31; python_tests=8/8; targets=2/2; "
...
def check_native_kernel_trap_readiness(
    ...
    if (
        ...
        != "7BFD0BFF2ED6F3F391AE09E57ED1559E3B53B14B4F5D4D969B2938FE8D30BAD6"
        or kernel_product.get("relocation_count") != 335
    ):
...
def check_native_kernel_entry_readiness(path: Path = NATIVE_KERNEL_ENTRY_READINESS) -> dict:
    ...
    expected_summary = {
        "rust_host_tests_passed": 31,
        "rust_host_tests_total": 31,
    ...
    if (
        ...
        or product.get("relocation_count") != 335
    ...
    detail = (
        "contract=PKENTRY1; kernel_tests=31/31; clean_builds=2/2; negative=43/43; "
        "exact_loaded=2/2; bytes=180224; image_bytes=262144; entry=0x8000; "
        "relocations=335; live_transfer=false; "
