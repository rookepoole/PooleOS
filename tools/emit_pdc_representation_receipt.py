#!/usr/bin/env python3
"""Emit exhaustive differential conversion evidence for PDC-REP-0.1."""
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Emit the PDC representation differential receipt.")
    parser.add_argument(
    ...
    parser.add_argument("--out", type=Path, default=ROOT / "runs" / "pdc_representation_receipt.json")
    args = parser.parse_args(argv)
    try:
        artifact = pdc_representation_evidence.make_representation_receipt(
            workspace=ROOT,
        ...
        print(f"FAIL pdc_representation_receipt: {exc}")
        return 1
    schema = json.loads((ROOT / "specs" / "pdc-representation-receipt.schema.json").read_text(encoding="utf-8"))
    errors = validate_json(artifact, schema)
    if errors:
        for error in errors:
            print(f"FAIL pdc_representation_receipt {error.path}: {error.message}")
        return 1
