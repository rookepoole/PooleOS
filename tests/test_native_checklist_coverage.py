...
class NativeChecklistCoverageTests(unittest.TestCase):
    ...
    def test_phase_and_added_requirement_inventories_are_complete(self) -> None:
        ...
        scheduler_smp_preempt = next(
            item for item in additions if item["id"] == "ADD-N12-SCHED-SMP-PREEMPT-001"
        )
        ...
        locks = next(
            item for item in additions if item["id"] == "ADD-N12-CONCURRENCY-LOCKS-001"
        )
        ...
        firmware = next(item for item in additions if item["id"] == "ADD-BOOT-009")
        self.assertEqual(firmware["phase_id"], "N5")
        self.assertIn("deterministic manifest", firmware["requirement"])
        policy = next(item for item in additions if item["id"] == "ADD-BOOT-010")
        self.assertEqual(policy["phase_id"], "N5")
        self.assertIn("compiled-in ceiling", policy["requirement"])
        inner_reparse = next(item for item in additions if item["id"] == "ADD-BOOT-011")
        self.assertEqual(inner_reparse["phase_id"], "N5")
        self.assertIn("independently reparse the exact retained PBART1 bytes", inner_reparse["requirement"])
        trust_state = next(item for item in additions if item["id"] == "ADD-BOOT-012")
        self.assertEqual(trust_state["phase_id"], "N5")
        self.assertIn("three separate contracts", trust_state["requirement"])
        self.assertIn("MUST NOT be accepted as persistent trust authority", trust_state["requirement"])
