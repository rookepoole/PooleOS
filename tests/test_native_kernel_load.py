...
def valid_markers() -> list[str]:
    # Fixed synthetic current-geometry markers, not measured boot evidence.
    return [
        "POOLEBOOT/0.1 ENTRY",
        ...
        "POOLEBOOT/0.1 KERNEL_BINDING PASS version=1 file_bytes=530072 image_bytes=602112 sha256_prefix=563ED1976CAB4DA7 path=manifest",
        "POOLEBOOT/0.1 KERNEL_FILE PASS bytes=530072 path=manifest_development",
        "POOLEBOOT/0.1 KERNEL_LOAD PASS image_bytes=602112 pages=147 entry_offset=40960 relocations=1325 files_closed=12 pools_freed=11 fnv1a64=EA020B94F75FE90A",
        "POOLEBOOT/0.1 ARTIFACT_SET PASS contract=PBART1 count=6 file_bytes=8761 pages=6 roles=2-7 fnv1a64=CE06D8FFA4E3303D retained=1 signatures=0 measured=0",
        "POOLEBOOT/0.1 INNER_SET PASS proof=N5-INNER-LIVE-PARSE-001 artifacts=6 parsers=6 bindings=6 denials=6 file_bytes=8761 payload_bytes=8185 sha256=A3078488088B2BF11B8D88F48862FA8D80957D610EB1F3FF4411AD5E4729FEAF retained=1 authority_grants=0 actions=0 state_writes=0 hardware_observations=0",
        "POOLEBOOT/0.1 TRUST_STATE DENY contract=PBTRUST1 policy_bytes=320 state_bytes=256 bindings=14 denials=1 denial=pbtrust_policy_unsigned policy_sha256=DDECB7E8BE1EEA8B491FDA3AA04AB56F81510BFBA69E1C0F30A93DC17C012803 state_sha256=C3C4C6412480A4C715C91F059A35EFBAA5E9D4A003916D8B8580C9C04A6B59BC source=esp_candidate auth=missing monotonic=missing signatures=0 authority_grants=0 state_writes=0",
        "POOLEBOOT/0.1 GOP PASS width=1280 height=800 stride=1280 mode=0 format=BGR",
        "POOLEBOOT/0.1 FRAME READY",
        "POOLEBOOT/0.1 KERNEL_MAP_PLAN PASS contract=PKMAP2 mappings=4 kernel_pages=147 ro=24 rx=105 rw=18 wx=0 pml4=511 pdpt=510 pd=0 pt=0 leaf_fnv1a64=2C93E3DFE8719D13",
        "POOLEBOOT/0.1 KERNEL_MAP_ACTIVE PASS table_pages=5 kernel_pages=147 physical_bits=40 mapped_fnv1a64=EA020B94F75FE90A framebuffer=preserved cache_signature=00 first_page_bytes=2097152 last_page_bytes=2097152",
        "POOLEBOOT/0.1 KERNEL_MAP_RETAIN PASS table_pages=5 stack_pages=36 handoff_pages=256 guards=2 total_pages=439 stack_pt=148 handoff_pt=185 kernel_phys=000000001DCEF000 root=000000001DDF9000 stack_phys=000000001DE37000 stack_top=FFFFFFFF800B8000 handoff_phys=000000001DA4D000 handoff_virt=FFFFFFFF800B9000 retained_fnv1a64=73396E416CE24E46 original_cr3=restored firmware_calls_while_active=0",
...
class NativeKernelLoadTests(unittest.TestCase):
    ...
    def test_readiness_date_is_calendar_valid_not_a_fixed_day(self) -> None:
        for value in ("2026-09-05", "2026-09-08", "2024-02-29"):
            ...
            with self.subTest(valid_date=value):
                errors = native_kernel_load.readiness_errors(candidate, ROOT)
        ...
        for value in invalid_dates:
            ...
            with self.subTest(invalid_date=value), patch.object(
                ...
                self.assertIn(expected, native_kernel_load.readiness_errors(candidate, ROOT))
    ...
    def test_marker_contract_captures_load_mapping_and_cleanup(self) -> None:
        ...
        self.assertEqual(147, summary["kernel"]["page_count"])
        self.assertEqual(0, summary["kernel_map"]["writable_executable_page_count"])
        self.assertEqual(147, summary["kernel_map"]["mapped_page_count"])
