...
def plan() -> dict[str, object]:
    return {
        ...
        "mappings": [
            ...
            {"virtual_offset": 0xA000, "memory_size": 0x68000, "permissions": "rx"},
            {"virtual_offset": 0x72000, "memory_size": 0xE000, "permissions": "r"},
            {"virtual_offset": 0x80000, "memory_size": 0x12000, "permissions": "rw"},
        ],
    }
...
class NativeKernelMapTests(unittest.TestCase):
    def test_retained_probe_tracks_current_linker_geometry(self) -> None:
        ...
        for exact in (
            "byte_count: 0x68000",
            "virtual_offset: 0x72000",
            "virtual_offset: 0x80000",
            "image_bytes: 0x92000",
            "page_count: 146",
        ):
            self.assertIn(exact, source)
        kernel = json.loads(
            (ROOT / "runs/native_kernel_entry_readiness.json").read_text(encoding="utf-8")
        )
        self.assertEqual(kernel["product"]["image_byte_count"], plan()["image_size"])
        main = (ROOT / "native/kernel/src/main.rs").read_text(encoding="utf-8")
        temporary = native_kernel_map.MIN_VIRTUAL_BASE + native_kernel_map.TEMPORARY_PAGE_INDEX * 4096
        self.assertIn(f"temp_start=0x{temporary:016X}", main)
        self.assertIn(f"temp_end=0x{temporary + 4096:016X}", main)
        vm_contract = json.loads((ROOT / "specs/native-kernel-virtual-memory-contract.json").read_text())
        self.assertEqual(int(vm_contract["layout"]["temporary_map_start"], 16), temporary)
        self.assertEqual(int(vm_contract["layout"]["temporary_map_end_exclusive"], 16), temporary + 4096)
    ...
    def test_exact_product_model_matches_frozen_summary(self) -> None:
        ...
        self.assertEqual(146, model["mapped_page_count"])
        self.assertEqual(24, model["read_only_page_count"])
        self.assertEqual(104, model["read_execute_page_count"])
        ...
        self.assertEqual("468CF57A8437F6E4", model["leaf_fingerprint"])
    ...
    def test_probe_parser_requires_exact_order_and_fingerprint(self) -> None:
        line = (
            "PKMAP1 PASS mappings=4 pages=146 ro=24 rx=104 rw=18 wx=0 "
            "pml4=511 pdpt=510 pd=0 pt=0 leaf_fnv1a64=468CF57A8437F6E4"
        )
    ...
    def test_retained_model_binds_guarded_stack_and_read_only_handoff(self) -> None:
        ...
        self.assertEqual([146, 183], model["guard_page_indices"])
        self.assertEqual(438, model["total_mapped_page_count"])
        self.assertEqual(440, native_kernel_map.TEMPORARY_PAGE_INDEX)
        self.assertEqual((441, 442, 5, 447), (
        ...
        self.assertEqual((448, 449, 32, 481), (
        ...
        self.assertEqual((482, 483, 32, 515), (
        ...
        self.assertEqual((516, 517, 518, 519, 520), (
    ...
    def test_retained_probe_and_lifecycle_are_exact(self) -> None:
        ...
        line = (
            "PKMAP2 PASS kernel_pages=146 stack_pages=36 handoff_pages=256 guards=2 "
            f"total_pages=438 stack_pt=147 handoff_pt=184 retained_fnv1a64={expected['retained_leaf_fingerprint']}"
        )
        self.assertEqual(
