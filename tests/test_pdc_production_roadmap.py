...
class PdcProductionRoadmapTests(unittest.TestCase):
    ...
    def test_production_boundary_and_next_move_are_explicit(self) -> None:
        self.assertFalse(self.roadmap["production_ready"])
        self.assertEqual(self.roadmap["baseline"]["pooleos_cycle"], 169)
        self.assertEqual(self.roadmap["baseline"]["pooleos_test_count"], 935)
        ...
        self.assertEqual(current["qualification_status"], "boot_chain_qualified_downstream_dependency_replay_pending")
        self.assertEqual(current["current_candidate_audit"]["cycle"], 169)
        ...
        self.assertEqual(current["passed_check_count_scope"], "historical_cycle165_exact_final_canonical_audit")
        ...
        self.assertEqual(current["last_fully_qualified_cycle"], 165)
    ...
    def test_dependency_replay_is_bound_to_fourteen_final_receipts(self) -> None:
        ...
        from runtime import native_kernel_smp_ipi
        self.assertEqual([], native_kernel_smp_ipi.readiness_errors(receipt))
        entry = receipt["build"]["kernel_entry"]
        self.assertEqual(entry["product"]["canonical_sha256"], qualification["kernel_sha256"])
        self.assertEqual(entry["host_tests"]["test_pass_count"], 243)
        ...
        for run in receipt["execution"]["runs"]:
            ...
            self.assertEqual(run["marker_summary"], native_kernel_smp_ipi.validate_markers(run["markers"]))
        self.assertEqual(len(receipt["negative_controls"]), 30)
        self.assertEqual(sum(c["case_count"] for c in receipt["negative_controls"]), 249)
        ...
        self.assertEqual(boot["cycle"], 169)
        self.assertEqual((boot["fresh_qemu_runs"], boot["kernel_entry_runs"], boot["focused_python_tests"]), (6, 2, 83))
        ...
        self.assertEqual(boot["inner_set_sha256"], "2DC54F8C02425C44DEB80A0F6285CAF4687A90537114902D39BB338C14BD7664")
