...
class PdcProductionRoadmapTests(unittest.TestCase):
    ...
    def test_master_checklist_binding_is_exact(self) -> None:
        ...
        self.assertEqual(checklist["added_requirement_count"], 48)
    ...
    def test_production_boundary_and_next_move_are_explicit(self) -> None:
        ...
        self.assertEqual(self.roadmap["baseline"]["pooleos_cycle"], 142)
        self.assertEqual(self.roadmap["baseline"]["pooleos_test_count"], 837)
        ...
        self.assertEqual(current["passed_checks"], 98)
        self.assertEqual(current["total_checks"], 98)
        self.assertEqual(current["artifact_count"], 93)
    ...
    def test_flags_and_gaps_are_native_and_traceable(self) -> None:
        ...
        self.assertEqual(len(flags), 86)
        self.assertEqual(len({flag["id"] for flag in flags}), 86)
        ...
        self.assertEqual(preemption_flag["status"], "open")
        self.assertIn(
            "runs/native-kernel-scheduler-readiness.json", preemption_flag["evidence"]
        )
        ...
        self.assertTrue(
            any("Interrupt-driven timer/wakeup preemption" in item for item in n12["current_gaps"])
        )
