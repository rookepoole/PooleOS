...
def valid_markers() -> list[str]:
    return [
        ...
        "POOLEBOOT/0.1 FIRMWARE_BOUNDARY PASS calls_after_exit=0 kernel_pages=136 artifact_pages=9 table_pages=4 stack_pages=36 handoff_pages=256",
        "POOLEBOOT/0.1 TRANSFER_ARM PASS contract=PKXFER1 mode=development emulator_only=1 entry=FFFFFFFF8000A000 handoff=FFFFFFFF800AE000 bytes=5160 stack_top=FFFFFFFF800AD000 root=000000001DDFA000 cr3=000000001DDFA000 trap_scenario=0 signatures=0 authority=0 actions=0 writes=0 firmware_calls_after_exit=0",
        native_kernel_transfer.TRANSFER_BOUNDARY,
        "POOLEOS:KERNEL:ENTRY PASS contract=PKENTRY1 transfer_contract=PKXFER1 build=PKBUILD1-CYCLE145-N12-SCHED-SMP-V0001-000001 entry_count=1 serial=present",
        "POOLEOS:KERNEL:STATE PASS handoff=0xFFFFFFFF800AE000 bytes=5160 entry=0xFFFFFFFF8000A000 stack_top=0xFFFFFFFF800AD000 root=0x000000001DDFA000 cr3=0x000000001DDFA000 rflags_if=0 rflags_df=0",
        "POOLEOS:KERNEL:PBP1 PASS profile=development records=5 artifacts=10 production_profile_valid=0",
        "POOLEOS:KERNEL:PKREVAL PASS contract=PKREVAL1 files=9 artifacts=6 parsers=9 manifest_bytes=2615 retained_bytes=11952 retained_set_sha256=27AE91CBAC981C48DBA99CE243F6AFF3835B0970850848AE656A2EE0C90A3943 policy_sha256=190382019CEC8877B417143A891568C6E1C2D5E6338AF55608EA0A199E08C10D state_sha256=4E2F619ACBB540653A9140A57FD3665482AF4510AC6080AC823F9A89C41123FB denial=pbtrust_policy_unsigned authority=0 actions=0 writes=0",
...
class NativeKernelTransferTests(unittest.TestCase):
    ...
    def test_transfer_address_and_cpu_state_mutations_reject(self) -> None:
        cases = ((23, "entry=FFFFFFFF8000A000", "entry=FFFFFFFF8000B000"), (26, "rflags_if=0", "rflags_if=1"), (26, "cr3=0x000000001DDFA000", "cr3=0x000000001DDFA001"))
    ...
    def test_transcript_binding_covers_transfer_critical_core(self) -> None:
        ...
        transcript = {
            ...
            "core": {
                ...
                "initial_stack_top_virtual": "FFFFFFFF800AD000",
                "page_table_root_physical": "000000001DDFA000",
                "handoff_virtual_base": "FFFFFFFF800AE000",
