...
This standalone crate receipt is protocol evidence, not by itself evidence that PooleBoot loaded PooleKernel, exited UEFI boot services, or transferred control. PKLOAD4/PBLIVE1 separately proves a temporary live pre-exit producer plus temporary PKMAP1 activation and rollback; it does not prove a retained read-only handoff or transfer. The authoritative contract and evidence are `specs/native-boot-handoff-contract.json`, `docs/native-boot-handoff.md`, and `runs/native_boot_handoff_readiness.json`.
