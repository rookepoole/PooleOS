...
`PKLOAD4` discovers the filesystem that loaded PooleBoot, opens and parses live PBC1 and its selected PSM1 manifest, binds slot/version/path/size/SHA-256/entry fields, reads and hashes the real PKELF1 PooleKernel product, materializes it in firmware pages, and keeps it live while PBLIVE1 produces an exact pre-exit PBP1 snapshot. PKMAP1 then builds, activates, audits, and exactly rolls back a temporary higher-half W^X map before all file, PBP1 pool, table-page, and kernel-page resources are released. Serial/debugcon transcripts are independently reconstructed and cross-bound. The unsigned manifest-driven selection remains development-only and untrusted.
